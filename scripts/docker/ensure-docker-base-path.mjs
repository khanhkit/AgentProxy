...
/**
 * Compares AGENTPROXY_BASE_PATH at container start against BUILD_AGENTPROXY_BASE_PATH
...
const SENTINEL = "BUILD_AGENTPROXY_BASE_PATH";
...
export function ensureDockerBasePath(opts = {}) {
  ...
  const runtime = normalizeBasePath(opts.env?.AGENTPROXY_BASE_PATH ?? process.env.AGENTPROXY_BASE_PATH);
  ...
  if (!runtime && baked) {
    throw new Error(
      `This AgentProxy image was built for subpath ${baked}, but AGENTPROXY_BASE_PATH is unset. ` +
        `Set AGENTPROXY_BASE_PATH=${baked} or rebuild without the build-arg.`
    );
  }
  ...
  try {
    ...
    console.error(
      "[ensure-docker-base-path] Rebuild the image with the same subpath, e.g.\n" +
        "  docker compose build --build-arg AGENTPROXY_BASE_PATH=/agentproxy"
    );
    process.exit(1);
  }
}
