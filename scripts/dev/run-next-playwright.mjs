...
const bootstrapMode = process.env.AGENTPROXY_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.AGENTPROXY_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "agentproxy-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_AGENTPROXY_E2E_MODE: process.env.NEXT_PUBLIC_AGENTPROXY_E2E_MODE || "1",
  AGENTPROXY_DISABLE_BACKGROUND_SERVICES:
    process.env.AGENTPROXY_DISABLE_BACKGROUND_SERVICES || "true",
  AGENTPROXY_DISABLE_TOKEN_HEALTHCHECK: process.env.AGENTPROXY_DISABLE_TOKEN_HEALTHCHECK || "true",
  AGENTPROXY_DISABLE_LOCAL_HEALTHCHECK: process.env.AGENTPROXY_DISABLE_LOCAL_HEALTHCHECK || "true",
  AGENTPROXY_HIDE_HEALTHCHECK_LOGS: process.env.AGENTPROXY_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        AGENTPROXY_E2E_PASSWORD: "",
        AGENTPROXY_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        AGENTPROXY_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.AGENTPROXY_USE_TURBOPACK
    ? {
        AGENTPROXY_USE_TURBOPACK: process.env.AGENTPROXY_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.AGENTPROXY_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.AGENTPROXY_PLAYWRIGHT_SKIP_BUILD === "1") return;
