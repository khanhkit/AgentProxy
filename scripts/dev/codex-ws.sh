#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um AgentProxy LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O AgentProxy expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O AgentProxy, no
...
#   AGENTPROXY_WS_BASE   base URL do AgentProxy local   (default abaixo)
#   AGENTPROXY_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   AGENTPROXY_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
AGENTPROXY_WS_BASE="${AGENTPROXY_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do AgentProxy local
AGENTPROXY_WS_MODEL="${AGENTPROXY_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export AGENTPROXY_LOCAL_KEY="${AGENTPROXY_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do AgentProxy.
model = "$AGENTPROXY_WS_MODEL"
model_provider = "agentproxy-local"

[model_providers.agentproxy-local]
name = "AgentProxy Local (WS)"
base_url = "$AGENTPROXY_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "AGENTPROXY_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$AGENTPROXY_WS_MODEL" -c model_provider="agentproxy-local")
