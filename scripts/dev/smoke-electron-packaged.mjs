...
export const LINUX_EXECUTABLE_NAMES = ["agentproxy-desktop", "agentproxy", "AgentProxy"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "AgentProxy.app", "Contents", "MacOS", "AgentProxy"),
    join(distDir, "mac-arm64", "AgentProxy.app", "Contents", "MacOS", "AgentProxy"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "AgentProxy.exe"),
    join(distDir, "win-x64-unpacked", "AgentProxy.exe"),
    join(distDir, "win-arm64-unpacked", "AgentProxy.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["AgentProxy.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing AgentProxy process before running this check.`
    );
  } catch (error) {
...
export async function ensureSmokeEnvDirs(smokeEnv, dataDir, { currentPlatform = platform() } = {}) {
  ...
  if (currentPlatform === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["agentproxy-desktop", "AgentProxy", "agentproxy"]) {
...
function assertExecutableExists(appExecutable) {
  ...
  throw new Error(
    `Packaged AgentProxy executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
  );
}
...
async function main() {
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "agentproxy-electron-smoke-")));
  const removeDataDir =
