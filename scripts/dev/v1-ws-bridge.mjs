...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://agentproxy.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://agentproxy.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-agentproxy-ws-session-id": this.sessionId,
      "x-agentproxy-ws-request-id": requestId,
    };
...
export function createAgentProxyWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createAgentProxyWsBridge requires a baseUrl");
  }
