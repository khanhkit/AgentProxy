...
//
// Pure, dependency-light helpers for AgentProxy's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `AGENTPROXY_TLS_CERT` and `AGENTPROXY_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.AGENTPROXY_TLS_CERT === "string" ? env.AGENTPROXY_TLS_CERT.trim() : "";
  const keyPath = typeof env?.AGENTPROXY_TLS_KEY === "string" ? env.AGENTPROXY_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[agentproxy][tls] HTTPS not enabled: both AGENTPROXY_TLS_CERT and AGENTPROXY_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[agentproxy][tls] HTTPS not enabled: could not read TLS cert/key ` +
