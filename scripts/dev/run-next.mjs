...
const rustCoreEnabled = isRustCoreEnabled(process.env);
if (rustCoreEnabled) {
  ensureRustCoreInternalToken(process.env);
}
    ...
    void (async () => {
      if (rustCoreEnabled) {
        try {
          ...
          console.log(
            `[Rust Core] ready on API port ${apiPort}; control plane dashboard=${dashboardPort}`
          );

          const child = rustCoreHandle?.child;
          if (child) {
            const onUnexpectedExit = (code, childSignal) => {
              if (shuttingDown) return;
              console.error(
                `[FATAL] Rust core exited unexpectedly (code=${code ?? "null"}, signal=${childSignal ?? "none"})`
              );
