#!/usr/bin/env node
/**
 * system-info.mjs — AgentProxy System Information Reporter (#280)
 *
...
lines.push("AgentProxy System Information Report");
...
const installedGlobal = run("npm list -g agentproxy --depth=0 2>/dev/null | grep agentproxy");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep agentproxy | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/khanhkit/AgentProxy/issues`
);
