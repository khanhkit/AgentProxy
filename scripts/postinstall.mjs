#!/usr/bin/env node
/**
 * Post-install warm-up for AgentProxy native runtimes.
 *
 * Runs after the main `scripts/build/postinstall.mjs` binary-copy step.
 * Tries to pre-resolve better-sqlite3 into ~/.agentproxy/runtime/ so that
...
if (
  process.env.AGENTPROXY_SKIP_POSTINSTALL === "1" ||
  process.env.CI === "true" ||
  process.env.CI === "1"
) {
  process.exit(0);
...
(async () => {
  try {
    ...
    console.warn(`[agentproxy] postinstall warm-up skipped: ${err?.message ?? err}`);
  }
})();
