...
set -euo pipefail
echo "[verify-published] npm i -g agentproxy@\${VERIFY_VERSION} (public registry)"
npm install -g "agentproxy@\${VERIFY_VERSION}"
export DATA_DIR=/tmp/agentproxy-data JWT_SECRET=verify-published-secret-with-sufficient-length API_KEY_SECRET=verify-published-api-key-secret DISABLE_SQLITE_AUTO_BACKUP=true AGENTPROXY_SKIP_SYSTEM_TRUST=1
mkdir -p "\$DATA_DIR"
agentproxy serve --port "\$VERIFY_PORT" &
node -e '
...
function main() {
  ...
  console.log(`[verify-published] clean-container verify of agentproxy@${version}…`);
  ...
  console.error(`[verify-published] ❌ FAILED (exit ${r.status}) — consider: npm deprecate agentproxy@${version} "<reason>"`);
  process.exit(1);
}
