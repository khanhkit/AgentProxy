...
whose source SHA-256 changed), runs the AgentProxy Cloud LLM through
AGENTPROXY_TRANSLATION_* env vars, and is wired into `npm run i18n:run`
...
def translate_block(text, target_language, api_url, api_key, model):
    ...
    prompt = (
        f"You are a professional technical translator working on the AgentProxy proxy project documentation.\n"
...
def main():
    parser = argparse.ArgumentParser(description="AgentProxy Auto-Translator for i18n Markdown")
    parser.add_argument("--api-url", default="http://localhost:20128/v1", help="Base URL of AgentProxy or target provider")
