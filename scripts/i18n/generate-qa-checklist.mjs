...
async function runAutomatedChecks() {
  ...
  for (const code of legacyCheckLocales) {
    ...
    try {
      ...
      if (content.includes("**[English](#-agentproxy--the-free-ai-gateway)**")) {
        anchorLineRemoved = false;
      }
      if (content.includes("## 🇧🇷 AgentProxy")) {
...
async function main() {
  ...
  automatedChecksLines.push(
    ...
    `- Apêndice "## 🇧🇷 AgentProxy" removido em ja/ar: **${automated.brAppendixRemoved ? "OK" : "PENDENTE"}**`,
