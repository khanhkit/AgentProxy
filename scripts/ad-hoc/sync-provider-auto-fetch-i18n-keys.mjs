 ...
 * scripts/i18n/sync-ui-keys.mjs (AGENTPROXY_TRANSLATION_API_URL/KEY/MODEL).
 *
...
async function main() {
  ...
  const limit = createLimiter(Number(process.env.AGENTPROXY_TRANSLATION_CONCURRENCY || 4));
  let filesChanged = 0;
  let keysAdded = 0;
