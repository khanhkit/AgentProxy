 ...
 *   AGENTPROXY_ENABLE_LIVE_WS — Set to "0" or "false" to disable
 */
...
const BOOTSTRAPPED_ENV = "AGENTPROXY_LIVE_WS_BOOTSTRAPPED";
...
async function main() {
  ...
  // child is re-spawned with AGENTPROXY_ENABLE_LIVE_WS="0" purely to stop liveServer.ts
  ...
  if (
    ...
    (process.env.AGENTPROXY_ENABLE_LIVE_WS === "0" ||
      process.env.AGENTPROXY_ENABLE_LIVE_WS?.toLowerCase() === "false")
  ) {
    console.log("[LiveWS] Disabled via AGENTPROXY_ENABLE_LIVE_WS");
    process.exit(0);
  }
