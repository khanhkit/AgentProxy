...
const uninstallAlreadyInProgress =
  process.env.AGENTPROXY_SKIP_UNINSTALL_HOOK === "1" ||
  process.env.npm_lifecycle_event === "uninstall";

console.log("🛑 AgentProxy Uninstaller");
console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

// 1. Stop PM2 process if it exists
try {
  console.log("Stopping and removing background PM2 processes...");
  execSync("pm2 delete agentproxy 2>/dev/null", { stdio: "ignore" });
} catch {
...
const dataDir = process.env.DATA_DIR || path.join(os.homedir(), ".agentproxy");
...
if (uninstallAlreadyInProgress) {
  ...
  try {
    execSync("npm uninstall -g agentproxy", {
      stdio: "inherit",
      env: {
        ...process.env,
        AGENTPROXY_SKIP_UNINSTALL_HOOK: "1",
      },
    });
    console.log("\n✅ AgentProxy has been successfully uninstalled from your system.");
    if (!fullUninstall) {
