#!/usr/bin/env node
/**
 * AgentProxy — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.agentproxy/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by AgentProxy bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/agentproxy.mjs`.
  //
  ...
  if (needsPersist) {
    merged.AGENTPROXY_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "agentproxy-field-encryption-v1", 32)
              const legacySalt = "agentproxy-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.agentproxy/server.env or ~/.agentproxy/.env,"
                );
                log(
                  "   or run: agentproxy reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
