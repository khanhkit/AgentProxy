#!/usr/bin/env node
/**
 * AgentProxy — Co-locate runtime workers into the raw Next standalone build.
 *
...
// STANDALONE defaults to the real build output; AGENTPROXY_STANDALONE_DIR overrides
// it so tests can drive the co-location logic against a synthetic tree without a
// full `next build`. Mirrors the AGENTPROXY_* override seams in the sibling build
// scripts (write-build-sha.mjs, write-build-base-path.mjs, optionalPackStaging.mjs).
const STANDALONE = process.env.AGENTPROXY_STANDALONE_DIR
  ? process.env.AGENTPROXY_STANDALONE_DIR
