...
const ENV_KEYS = [
  "AGENTPROXY_TRANSLATION_API_URL",
  "AGENTPROXY_TRANSLATION_API_KEY",
  "AGENTPROXY_TRANSLATION_MODEL",
  "AGENTPROXY_TRANSLATION_TIMEOUT_MS",
] as const;
...
test("backendConfig reads AGENTPROXY_TRANSLATION_* and strips a trailing slash from the URL", () => {
  withEnv(
    {
      AGENTPROXY_TRANSLATION_API_URL: "http://translation.test/v1/",
      AGENTPROXY_TRANSLATION_API_KEY: "  sk-test  ",
      AGENTPROXY_TRANSLATION_MODEL: "test-model",
    },
    () => {
  ...
  withEnv(
    {
      ...
      AGENTPROXY_TRANSLATION_API_KEY: "sk-test",
      AGENTPROXY_TRANSLATION_MODEL: "test-model",
      AGENTPROXY_TRANSLATION_TIMEOUT_MS: "1234",
    },
    () => {
...
test("backendConfig fails fast with the documented message when a var is missing", () => {
  withEnv(
    ...
    () => {
      assert.throws(
        () => backendConfig(),
        /Missing required env var: AGENTPROXY_TRANSLATION_API_URL\. Set it in \.env/
      );
    }
  );
  withEnv(
    ...
    () => {
      assert.throws(() => backendConfig(), /Missing required env var: AGENTPROXY_TRANSLATION_MODEL/);
    }
  );
});
