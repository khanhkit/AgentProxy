 ...
 * saw the REQUIRE_API_KEY posture and accepted every caller when AGENTPROXY_API_KEY
...
const ORIGINAL_A2A_KEY = process.env.AGENTPROXY_API_KEY;
...
test.after(() => {
  ...
  if (ORIGINAL_A2A_KEY === undefined) delete process.env.AGENTPROXY_API_KEY;
  else process.env.AGENTPROXY_API_KEY = ORIGINAL_A2A_KEY;
});
