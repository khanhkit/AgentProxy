...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "agentproxy-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@agentproxy\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@agentproxy\/open-sse\/services\/model\.ts/);
