...
// opt-in (Docker sets AGENTPROXY_MITM_STUB=1); a default production build must
...
describe("mitm manager stub alias (#6344)", () => {
  ...
  it("AGENTPROXY_MITM_STUB=1 opts into the stub (Docker graceful degradation, #3390)", () => {
    assert.equal(shouldStubMitmManager({ AGENTPROXY_MITM_STUB: "1" }), true);
    assert.deepEqual(mitmManagerAliasFor({ AGENTPROXY_MITM_STUB: "1" }), {
  ...
  it("the Dockerfile keeps Docker on the stub via AGENTPROXY_MITM_STUB=1", () => {
    const dockerfile = readFileSync(new URL("../../Dockerfile", import.meta.url), "utf8");
    assert.match(dockerfile, /^ENV AGENTPROXY_MITM_STUB=1$/m);
  });
});
