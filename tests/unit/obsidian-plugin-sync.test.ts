...
describe("Discovery Candidates", () => {
  ...
  test("generates MagicDNS candidate for dotted name", () => {
    ...
    assert.ok(result.some((candidate) => candidate === "http://my-macbook.local:27781"));
    assert.ok(result.some((candidate) => candidate === "http://my-macbook.local.ts.net:27781"));
  ...
  test("handles host with leading/trailing whitespace", () => {
    ...
    assert.ok(result.some((candidate) => candidate === "http://my-mac:27781"));
  });

  test("handles .local domain (dotted, not 100.x)", () => {
    const result = buildDiscoveryCandidates({ host: "desktop.local", port: 27781 });
    assert.ok(result.some((candidate) => candidate === "http://desktop.local:27781"));
    assert.ok(result.some((candidate) => candidate === "http://desktop.local.ts.net:27781"));
  });
});
