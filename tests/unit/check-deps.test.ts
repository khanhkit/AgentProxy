...
test("6A.8: discoverManifests finds root and workspace package.json files", () => {
  ...
  assert.ok(
    manifests.includes("@agentproxy/opencode-plugin/package.json"),
    "@agentproxy/opencode-plugin/package.json must be included"
  );
  assert.ok(
    manifests.includes("@agentproxy/opencode-provider/package.json"),
    "@agentproxy/opencode-provider/package.json must be included"
  );
});
