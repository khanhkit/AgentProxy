...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-health-autopilot-"));
...
const accountFallback = await import("@agentproxy/open-sse/services/accountFallback");
...
test("provider health autopilot action accepts LAN dashboard requests (#6277)", async () => {
  ...
  assert.equal(request.headers.get("x-agentproxy-peer-ip"), null);
