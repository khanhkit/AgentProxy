...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/agentproxy" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/agentproxy");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/agentproxy",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/agentproxy/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "agentproxy-base",
    ...
    assert.equal(config.repoDir, "/srv/agentproxy");
    assert.equal(config.composeFile, "/srv/agentproxy/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "agentproxy-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart agentproxy --update-env \|\| true/);
  });
});
