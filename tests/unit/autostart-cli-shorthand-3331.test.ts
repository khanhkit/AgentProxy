 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `agentproxy serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`agentproxy autostart on` / `... true`) via
...
test("status is the default action (bare `agentproxy autostart` is a safe read-only)", () => {
