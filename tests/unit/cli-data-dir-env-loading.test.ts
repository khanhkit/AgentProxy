...
test("CLI data-dir resolver preserves an existing legacy ~/.agentproxy before XDG/app data", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-cli-data-dir-"));
  const home = path.join(tmp, "home");
  const legacyDir = path.join(home, ".agentproxy");
...
test("CLI startup loads later non-conflicting .env files without overriding earlier values", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-cli-env-layers-"));
  const home = path.join(tmp, "home");
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      ["AGENTPROXY_BASE_URL=https://appdata.example/v1", "AGENTPROXY_HTTP_TIMEOUT_MS=1234", ""].join(
        "\n"
      ),
      "utf-8"
    );
    fs.writeFileSync(
      path.join(cwd, ".env"),
      ["AGENTPROXY_BASE_URL=https://cwd.example/v1", "PORT=34567", ""].join("\n"),
      "utf-8"
    );
    ...
    assert.equal(current.AGENTPROXY_BASE_URL, "https://data.example/v1");
    assert.equal(current.AGENTPROXY_HTTP_TIMEOUT_MS, "1234");
