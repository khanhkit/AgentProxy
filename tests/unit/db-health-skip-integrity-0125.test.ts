...
const runtime = fs.readFileSync(path.join(root, "src/lib/db/managedHealthRuntime.ts"), "utf8");

test("TC-OMNIDB-HEALTH-024A: managed DB health API can waive integrity scans", () => {
  assert.match(
    runtime,
    /export type ManagedHealthCheckOptions = \{[\s\S]{0,140}?autoRepair\?: boolean;[\s\S]{0,140}?skipIntegrityCheck\?: boolean;[\s\S]{0,80}?\};/
  );
  assert.match(core, /export const runManagedDbHealthCheck = managedHealth\.runDirect/);
  assert.match(
    runtime,
    /runDirect\(options\?: ManagedHealthCheckOptions\)[\s\S]{0,420}?skipIntegrityCheck:\s*options\?\.skipIntegrityCheck === true/
  );
});
