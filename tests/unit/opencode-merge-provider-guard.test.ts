...
//
//   provider: ["a", "b"]  ->  { "0": "a", "1": "b", agentproxy: {...} }
...
for (const [label, provider] of [
  ...
  test(`mergeOpenCodeConfig drops a provider block that is ${label}`, () => {
    ...
    assert.equal(merged.provider.agentproxy.options.baseURL, "http://localhost:20128/v1");
  });
}
...
test("mergeOpenCodeConfig still guards the root itself", () => {
  for (const existing of [["a"], "oops", 7, null, undefined]) {
    ...
    assert.deepEqual(Object.keys(merged.provider), ["agentproxy"]);
