...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-providers-validate-route-"));
...
test("providers validate route allows a local baseUrl by default (#5066 local-first)", async () => {
  await resetStorage();
  delete process.env.AGENTPROXY_ALLOW_PRIVATE_PROVIDER_URLS;
  delete process.env.AGENTPROXY_ALLOW_LOCAL_PROVIDER_URLS; // default ON
