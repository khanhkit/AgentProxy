...
} from "@agentproxy/open-sse/services/combo/resolveAutoStrategy.ts";
import { DEFAULT_WEIGHTS } from "@agentproxy/open-sse/services/autoCombo/scoring.ts";
...
// also supports a per-request `X-AgentProxy-Mode` override (relayOptions.mode) that can
...
test("per-request X-AgentProxy-Mode override changes the EFFECTIVE weights used for fallback ranking, not just selection", async () => {
