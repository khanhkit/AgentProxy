...
// forwards. On the stdio transport `agentproxy_ccr_retrieve` therefore fell back
...
// the registered agentproxy_ccr_retrieve handler with an `extra` carrying that
...
// `AGENTPROXY_API_KEY` no shell, `resolveMcpCallerApiKeyId()` resolve primeiro e mascara
...
test("static tool loops forward `extra` so stdio callers keep their scope/identity (#6178)", async () => {
  ...
  const retrieve = getRegisteredHandler(server, "agentproxy_ccr_retrieve");
