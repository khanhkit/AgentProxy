...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__agentproxyDb?.open) {
      (globalThis as any).__agentproxyDb.close();
    }
  } catch {}
