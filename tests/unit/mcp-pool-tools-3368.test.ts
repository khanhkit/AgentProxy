...
// imported or registered in open-sse/mcp-server/server.ts, so agentproxy_pool_*
...
test("read tools require read:health; lifecycle tools require write:resilience", () => {
  ...
  assert.deepEqual(tools.agentproxy_pool_status.scopes, ["read:health"]);
  assert.deepEqual(tools.agentproxy_pool_sessions.scopes, ["read:health"]);
  assert.deepEqual(tools.agentproxy_pool_health.scopes, ["read:health"]);
  assert.deepEqual(tools.agentproxy_browser_pool_status.scopes, ["read:health"]);
  assert.deepEqual(tools.agentproxy_pool_reset.scopes, ["write:resilience"]);
  assert.deepEqual(tools.agentproxy_pool_warm.scopes, ["write:resilience"]);
});
