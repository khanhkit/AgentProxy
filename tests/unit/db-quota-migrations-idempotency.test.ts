...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-quota-mig-idem-"));
...
test("running migration runner a second time produces zero errors and identical schema", async () => {
  ...
  // migrations are already recorded in _agentproxy_migrations — should be no-op.
  core.resetDbInstance();
