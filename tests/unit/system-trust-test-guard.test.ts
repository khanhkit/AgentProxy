...
//
// AGENTPROXY_SKIP_SYSTEM_TRUST=1 (set globally in tests/_setup/isolateDataDir.ts)
...
test("isolateDataDir setup exports the system-trust guard for every test process", () => {
  assert.equal(process.env.AGENTPROXY_SKIP_SYSTEM_TRUST, "1");
});
...
test("installCert under the guard skips the OS mutation but keeps input contracts", async () => {
  ...
  await assert.rejects(() => installCert("", "/nonexistent/agentproxy-guard-test.pem"));
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-trust-guard-"));
  const pem = path.join(dir, "agentproxy-guard-test.pem");
  fs.writeFileSync(
    pem,
