...
test("buildRooImport produces an openai-compatible provider profile (baseUrl /v1, model)", () => {
  ...
  const cfg = d.providerProfiles.apiConfigs.AgentProxy;
  ...
  assert.equal(d.providerProfiles.currentApiConfigName, "AgentProxy");
});
test("buildRooImport falls back to a placeholder key", () => {
  assert.equal(buildRooImport({ baseUrl: "http://x/v1", apiKey: "", model: "m" }).providerProfiles.apiConfigs.AgentProxy.openAiApiKey, "sk_agentproxy");
});
test("buildRooVscodeAutoImport sets the pointer, preserving other settings", () => {
  const s = buildRooVscodeAutoImport({ "editor.tabSize": 2 }, "/home/u/.agentproxy/roo-settings.json");
  assert.equal(s["editor.tabSize"], 2);
  assert.equal(s["roo-cline.autoImportSettingsPath"], "/home/u/.agentproxy/roo-settings.json");
});
