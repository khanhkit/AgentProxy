/**
 * Security regression: when AgentProxy itself runs behind an external reverse
...
test("classifyStampedPeerLocality: loopback socket WITH a proxy stamp is REMOTE (fail closed)", () => {
  // AgentProxy is behind nginx/Caddy/Cloudflare; the socket peer is the proxy.
...
test("classifyStampedPeerLocality: private-LAN socket WITH a proxy stamp is still REMOTE", () => {
  // Caddy/nginx running on a LAN box in front of AgentProxy. We do not know how
