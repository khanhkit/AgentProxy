...
describe("MCP description compression", () => {
  ...
  it("honors settings and environment kill switches", () => {
    const originalEnv = process.env.AGENTPROXY_MCP_DESCRIPTION_COMPRESSION;
    const originalAliasEnv = process.env.AGENTPROXY_MCP_COMPRESS_DESCRIPTIONS;
    try {
      delete process.env.AGENTPROXY_MCP_DESCRIPTION_COMPRESSION;
      delete process.env.AGENTPROXY_MCP_COMPRESS_DESCRIPTIONS;
      ...
      process.env.AGENTPROXY_MCP_COMPRESS_DESCRIPTIONS = "off";
      assert.equal(maybeCompressMcpDescription(input, { enabled: true }), input);
    } finally {
      if (originalEnv === undefined) {
        delete process.env.AGENTPROXY_MCP_DESCRIPTION_COMPRESSION;
      } else {
        process.env.AGENTPROXY_MCP_DESCRIPTION_COMPRESSION = originalEnv;
      }
      if (originalAliasEnv === undefined) {
        delete process.env.AGENTPROXY_MCP_COMPRESS_DESCRIPTIONS;
      } else {
        process.env.AGENTPROXY_MCP_COMPRESS_DESCRIPTIONS = originalAliasEnv;
      }
    }
  });
