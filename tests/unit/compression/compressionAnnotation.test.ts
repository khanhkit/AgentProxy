...
describe("formatCompressionAnnotation", () => {
  ...
  it("is ASCII-only so it survives HTTP header (X-AgentProxy-Compression) construction", () => {
    // Regression: the annotation is appended to the X-AgentProxy-Compression response
    ...
    assert.doesNotThrow(() => new Headers({ "X-AgentProxy-Compression": value }));
    const res = new Response(null, { headers: { "X-AgentProxy-Compression": value } });
    assert.equal(res.headers.get("X-AgentProxy-Compression"), value);
  });
  ...
  it("bounds high-cardinality rule telemetry before it reaches an HTTP response header", () => {
    ...
    assert.doesNotThrow(
      () => new Response(null, { headers: { "X-AgentProxy-Compression": annotation } })
    );
  });
