...
afterEach(() => {
  ...
  delete process.env.AGENTPROXY_RTK_TRUST_PROJECT_FILTERS;
...
describe("RTK custom filter lookup and trust", () => {
  it("skips untrusted project filters and reports a diagnostic", () => {
    const temp = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-rtk-project-"));
    process.chdir(temp);
  ...
  it("loads global filters before builtin fallback filters", () => {
    const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-rtk-project-"));
    const tempData = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-rtk-data-"));
