...
// firing, yet `agentproxy update` on the box said "already up to date". Part of the
// same known bug class as #4376 (already fixed in bin/cli/commands/update.mjs's own
// getLatestVersion()): `npm info agentproxy version --json` without `--prefer-online`
...
test("getLatestVersionFromNpmCli passes --prefer-online to bypass the stale npm cache (#11885, same class as #4376)", async () => {
  ...
  assert.ok((capturedArgs as string[]).includes("agentproxy"));
