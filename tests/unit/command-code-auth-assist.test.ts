...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-command-code-auth-"));
...
test("Command Code auth assist keeps auth URL callback on CLI localhost contract", async () => {
  process.env.AGENTPROXY_PUBLIC_BASE_URL = "https://agentproxy.example.com/base-path";
