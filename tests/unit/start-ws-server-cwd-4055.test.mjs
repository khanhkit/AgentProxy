...
test("#4055: resolvePackageRoot points at the dir above scripts/", () => {
  const scriptUrl = pathToFileURL(
    "/opt/homebrew/lib/node_modules/agentproxy/scripts/start-ws-server.mjs"
  ).href;
  assert.equal(
    mod.resolvePackageRoot(scriptUrl),
    "/opt/homebrew/lib/node_modules/agentproxy"
  );
});
...
test("#4055: the bootstrap spawn pins cwd to the package root so tsx + @/ aliases resolve", () => {
  ...
  assert.equal(spec.options.cwd, "/opt/homebrew/lib/node_modules/agentproxy");
  ...
  assert.equal(spec.options.env.AGENTPROXY_LIVE_WS_BOOTSTRAPPED, "1");
  assert.equal(spec.options.env.AGENTPROXY_ENABLE_LIVE_WS, "0");
