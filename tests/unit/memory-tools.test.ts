...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-memory-tools-"));
...
test.beforeEach(async () => {
  ...
  // within the memory subsystem (agentproxy_memory_search → retrieveMemories, which
...
test("memory clear deletes only older filtered entries and reports the deleted count", async () => {
  ...
  const result = await memoryTools.agentproxy_memory_clear.handler({
    apiKeyId: "key-clear",
    type: "factual",
