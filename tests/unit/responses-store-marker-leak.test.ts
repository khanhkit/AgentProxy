 ...
 * intent under the internal `_agentproxyResponsesStore` marker (see
 ...
 * it with `Unknown parameter: '_agentproxyResponsesStore'` -- confirmed live
 * against the real API.
 */
import test from "node:test";
import assert from "node:assert/strict";

test("translateRequest never leaks the internal _agentproxyResponsesStore marker into a Chat Completions destination", async () => {
  ...
  assert.equal("_agentproxyResponsesStore" in result, false);
