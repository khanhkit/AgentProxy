...
test("renameProcessTitle renames a bare 'next-server' title to 'agentproxy'", () => {
  assert.equal(renameProcessTitle("next-server"), "agentproxy");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "agentproxy (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "agentproxy (v16.2.9)");
  assert.equal(twice, "agentproxy (v16.2.9)");
});
