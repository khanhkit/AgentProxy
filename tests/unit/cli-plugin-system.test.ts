...
test("discoverPlugins retorna array vazio se nenhum plugin instalado (diretório não existe)", async () => {
  ...
  const orig = process.env.AGENTPROXY_PLUGIN_PATH;
  process.env.AGENTPROXY_PLUGIN_PATH = join(tmpdir(), `no-such-dir-${Date.now()}`);
  try {
    const plugins = await discoverPlugins();
    assert.ok(Array.isArray(plugins));
  } finally {
    if (orig === undefined) delete process.env.AGENTPROXY_PLUGIN_PATH;
    else process.env.AGENTPROXY_PLUGIN_PATH = orig;
  }
});

test("discoverPlugins descobre plugin com package.json válido", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `agentproxy-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "agentproxy-cmd-test-hello");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "agentproxy-cmd-test-hello",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
  ...
  const orig = process.env.AGENTPROXY_PLUGIN_PATH;
  process.env.AGENTPROXY_PLUGIN_PATH = pluginDir;
  try {
    const plugins = await discoverPlugins();
    assert.ok(
      plugins.some((p) => p.name === "agentproxy-cmd-test-hello"),
...
test("discoverPlugins ignora pacotes sem prefixo agentproxy-cmd-", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `agentproxy-plugins-test-${Date.now()}`);
...
test("loadPlugins não quebra CLI quando plugin tem erro de load (try/catch)", async () => {
  ...
  const pluginDir = join(tmpdir(), `agentproxy-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "agentproxy-cmd-broken");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "agentproxy-cmd-broken",
      version: "1.0.0",
      type: "module",
      main: "broken.mjs",
    })
...
test("loadPlugins carrega plugin válido e chama register()", async () => {
  const { loadPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `agentproxy-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "agentproxy-cmd-valid");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "agentproxy-cmd-valid",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
...
test("exemplo agentproxy-cmd-hello existe e tem register()", () => {
  const examplePath = join(ROOT, "examples", "agentproxy-cmd-hello", "index.mjs");
...
test("docs/frameworks/PLUGINS.md existe", () => {
  ...
  assert.ok(src.includes("agentproxy-cmd"), "deve mencionar agentproxy-cmd");
