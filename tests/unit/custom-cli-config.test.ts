...
test("buildAliasEnvVar sanitizes aliases for env variable export", () => {
  assert.equal(buildAliasEnvVar("review"), "AGENTPROXY_MODEL_REVIEW");
  assert.equal(buildAliasEnvVar("plan mode"), "AGENTPROXY_MODEL_PLAN_MODE");
...
test("custom CLI generators include default model and alias mappings", () => {
  ...
  assert.match(envScript, /export OPENAI_MODEL="agentproxy\/fast"/);
  assert.match(envScript, /export AGENTPROXY_MODEL_REVIEW="cc\/claude-sonnet-4-5-20250929"/);
