...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-qwen-web-retirement-"));
...
test("migration 167 retires every Qwen Web id fail-closed and preserves audit history", async () => {
  ...
  const applied = db
    .prepare("SELECT version FROM _agentproxy_migrations WHERE version = 167")
