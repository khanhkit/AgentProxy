...
test("npm + git repo → source (a source checkout self-updates via git)", () => {
  assert.equal(
    resolveAutoUpdateMode("npm", {
      isGitRepo: true,
      currentDir: "/home/me/agentproxy/dist/lib/system",
    }),
    "source"
  );
});
...
test("npm + global install under node_modules → npm", () => {
  assert.equal(
    resolveAutoUpdateMode("npm", {
      isGitRepo: false,
      currentDir: "/usr/lib/node_modules/agentproxy/dist/lib/system",
    }),
    "npm"
  );
});
...
test("npm + no git + not under node_modules → source (downloaded build/zip)", () => {
  assert.equal(
    resolveAutoUpdateMode("npm", {
      isGitRepo: false,
      currentDir: "/opt/agentproxy/dist/lib/system",
    }),
    "source"
  );
});
...
test("isUnderNodeModules matches real segments on both path separators", () => {
  assert.equal(isUnderNodeModules("/usr/lib/node_modules/agentproxy"), true);
  assert.equal(isUnderNodeModules("C:\\Users\\me\\node_modules\\agentproxy"), true);
