...
test("generateMitmCa produces a CA certificate (basicConstraints CA, key+cert PEM)", async () => {
  const ca = await generateMitmCa("AgentProxy MITM CA (test)");
  ...
  assert.match(x.subject, /AgentProxy MITM CA \(test\)/);
});
...
test("DynamicCertStore caches one SecureContext per hostname", async () => {
  const store = new DynamicCertStore("AgentProxy MITM CA (test)");
