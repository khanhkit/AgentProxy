...
test("machine-token derivation fails closed for a missing machine ID", () => {
  assert.equal(deriveMachineToken("", "agentproxy-cli-auth-v1"), "");
  assert.equal(deriveLegacyCliToken("", "agentproxy-cli-auth-v1"), "");
});
...
test("getMachineTokenSync respects AGENTPROXY_CLI_SALT env var", () => {
  const previous = process.env.AGENTPROXY_CLI_SALT;
  try {
    const before = getMachineTokenSync();
    process.env.AGENTPROXY_CLI_SALT = "__test_salt__";
    ...
    if (previous === undefined) delete process.env.AGENTPROXY_CLI_SALT;
    else process.env.AGENTPROXY_CLI_SALT = previous;
  }
});
