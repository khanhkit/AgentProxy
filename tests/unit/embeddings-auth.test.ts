...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.AGENTPROXY_API_KEY;

  await t.test("should return true if key matches AGENTPROXY_API_KEY", async () => {
    process.env.AGENTPROXY_API_KEY = "test-key";
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.AGENTPROXY_API_KEY = originalOmniKey;
});
