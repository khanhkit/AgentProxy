...
// #10524: SSE comment lines (`: x-agentproxy-*`) break strict SSE clients.
// Default should be disabled (opt-in via AGENTPROXY_SSE_COMMENTS=on).

test("#10524: sseCommentsEnabled defaults to false when env var is unset", () => {
  const original = process.env.AGENTPROXY_SSE_COMMENTS;
  delete process.env.AGENTPROXY_SSE_COMMENTS;

  assert.strictEqual(sseCommentsEnabled(), false, "SSE comments must be disabled by default");

  if (original !== undefined) process.env.AGENTPROXY_SSE_COMMENTS = original;
});
...
test("#10524: sseCommentsEnabled returns true when explicitly enabled", () => {
  ...
  for (const value of ["on", "true", "1", "yes", "ON", "TRUE"]) {
    process.env.AGENTPROXY_SSE_COMMENTS = value;
    assert.strictEqual(sseCommentsEnabled(), true, `SSE comments must be enabled for "${value}"`);
  }

  if (original !== undefined) process.env.AGENTPROXY_SSE_COMMENTS = original;
  else delete process.env.AGENTPROXY_SSE_COMMENTS;
});
