...
test("sensitive completion keeps omission markers when no response preview was supplied", () => {
  ...
  const completed = getCompletedDetails().get(id);
  assert.ok(completed);
  assert.deepEqual(completed.providerResponse, { _agentproxy_omitted: "video-transcript" });
  assert.deepEqual(completed.clientResponse, { _agentproxy_omitted: "video-transcript" });
});
