...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_agentproxy',
...
test("sentinel guard condition catches sk_agentproxy and null", () => {
  const SENTINEL = "sk_agentproxy";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_agentproxy')
  ...
  assert.equal(shouldReject("sk_agentproxy"), true, "sentinel must be rejected");
