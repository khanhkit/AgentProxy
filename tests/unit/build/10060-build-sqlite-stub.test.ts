 ...
 *
 * The reliable build signal is AGENTPROXY_BUILDING=1 (set by
 ...
 *
 *   1. build-next-isolated.mjs exports AGENTPROXY_BUILDING=1 into the build env.
...
describe("#10060 build env carries AGENTPROXY_BUILDING", () => {
  it("resolveNextBuildEnv sets AGENTPROXY_BUILDING=1", () => {
    const env = resolveNextBuildEnv({}, "linux");
    assert.equal(env.AGENTPROXY_BUILDING, "1");
  });
...
describe("#10060 getDbInstance stubs SQLite during build", () => {
  const savedBuilding = process.env.AGENTPROXY_BUILDING;
  const savedPhase = process.env.NEXT_PHASE;

  beforeEach(() => {
    delete process.env.NEXT_PHASE;
    process.env.AGENTPROXY_BUILDING = "1";
  });

  afterEach(() => {
    if (savedBuilding === undefined) delete process.env.AGENTPROXY_BUILDING;
    else process.env.AGENTPROXY_BUILDING = savedBuilding;
  ...
  it("returns a no-op stub (never the native better-sqlite3 driver) under the build signal", async () => {
    // Import fresh so isBuildPhase is evaluated with AGENTPROXY_BUILDING set.
