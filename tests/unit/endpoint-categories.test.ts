...
test("resolveEndpointCategory: canonicalizes equivalent client route forms before policy lookup", () => {
  const cases = [
    ["/api/v1/chat/completions", "chat"],
    ["/chat/completions", "chat"],
    ["/chat/completions/", "chat"],
    ["/v1/v1/chat/completions", "chat"],
    ["/api/v1/responses", "chat"],
    ["/responses", "chat"],
    ["/codex/example", "chat"],
  ...
  for (const [pathname, expected] of cases) {
    assert.equal(resolveEndpointCategory(pathname), expected, pathname);
  }
});
