 ...
 *   2. Import and use `buildErrorBody` from `@agentproxy/open-sse/utils/error`.
 *
...
for (const { rel, label } of PLUGIN_ROUTES) {
  ...
  test(`${label}: imports buildErrorBody from @agentproxy/open-sse/utils/error`, () => {
    const src = readRoute(rel);
    assert.match(
      src,
      /import \{[^}]*buildErrorBody[^}]*\} from ["']@agentproxy\/open-sse\/utils\/error["']/,
      `${rel}: must import buildErrorBody from @agentproxy/open-sse/utils/error`
    );
  });
