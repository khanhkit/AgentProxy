...
const AGENTPROXY_BIN = path.join(ROOT, "bin", "agentproxy.mjs");
...
// ~/.agentproxy so DATA_DIR is the only data directory in play.
function baseEnv(dataDir: string, isolatedHome: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DATA_DIR: dataDir,
    HOME: isolatedHome,
    // Give the CLI a key so bin/agentproxy.mjs never warns/provisions.
...
function mkHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-reset-home-"));
}
function mkDataDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-reset-data-"));
}

// #6261: `agentproxy reset-password` must be a real subcommand (not "unknown
...
test("agentproxy reset-password subcommand applies the reset over piped stdin (#6261, #6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
    const res = spawnSync("node", [AGENTPROXY_BIN, "reset-password"], {
...
test("agentproxy-reset-password applies the reset over piped two-line stdin (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
...
test("agentproxy-reset-password --password-stdin reads the whole stdin as the password (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
