...
test("source guard: combo target execution skips provider cooldown for per-model-quota on 500", () => {
  const src = fs.readFileSync(
  ...
  assert.ok(
    ...
    "combo target execution must skip provider cooldown recording for per-model-quota providers on 500"
  );
});
