...
test("wantsProgress detects X-AgentProxy-Progress header correctly", () => {
  const headersObj = { "x-agentproxy-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-agentproxy-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-agentproxy-progress": "true" };
