...
test("VPS compose publishes only the AgentProxy dashboard on loopback by default", () => {
  const { parsed } = readCompose();
  const services = parsed.services ?? {};

  assert.deepEqual(services.redis?.ports, undefined, "Redis must not publish a host port");
  assert.deepEqual(services.agentproxy?.ports, [
    "${AGENTPROXY_BIND_HOST:-127.0.0.1}:${AGENTPROXY_PORT:-20128}:20128",
  ]);
});

test("VPS compose requires an explicitly pinned image and production secrets", () => {
  const { raw, parsed } = readCompose();
  const agentproxy = parsed.services?.agentproxy;

  assert.equal(
    agentproxy?.image,
    "${AGENTPROXY_IMAGE:?Set AGENTPROXY_IMAGE to a versioned tag or digest}"
  );
  assert.equal(agentproxy?.environment?.REQUIRE_API_KEY, "${REQUIRE_API_KEY:-true}");
  ...
  assert.match(
    raw,
    /AGENTPROXY_WS_BRIDGE_SECRET: \$\{AGENTPROXY_WS_BRIDGE_SECRET:\?Set AGENTPROXY_WS_BRIDGE_SECRET in \.env\}/
  );
});
...
test("VPS environment example uses a versioned image rather than a floating channel", () => {
  ...
  assert.match(env, /^AGENTPROXY_IMAGE=[^\s:]+:\d+\.\d+\.\d+(?:-[a-z0-9.-]+)?$/m);
  assert.doesNotMatch(env, /^AGENTPROXY_IMAGE=.*:(?:latest|next)$/m);
