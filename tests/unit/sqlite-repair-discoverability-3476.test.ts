...
// ─── #3476: discoverability of `agentproxy runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, AgentProxy prints
...
// A self-heal command already exists (`agentproxy runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `agentproxy runtime repair` as a recovery hint"
  );
});
