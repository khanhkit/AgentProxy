...
// `AGENTPROXY_USE_TURBOPACK` is the operator's only control over the bundler:
...
test("resolveNextBuildBundlerFlag is decided by AGENTPROXY_USE_TURBOPACK alone, not by the runtime", async () => {
  ...
  try {
    for (const bun of [undefined, "1.1.20", "1.3.14"]) {
      ...
      assert.equal(
        buildIsolated.resolveNextBuildBundlerFlag({ AGENTPROXY_USE_TURBOPACK: "1" }),
        "--turbopack",
        where
      );
      assert.equal(
        buildIsolated.resolveNextBuildBundlerFlag({ AGENTPROXY_USE_TURBOPACK: "0" }),
        "--webpack",
        where
      );
    }
