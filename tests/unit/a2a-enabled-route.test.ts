...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-a2a-enabled-route-"));
...
test.after(() => {
  ...
  if (ORIGINAL_AGENTPROXY_API_KEY === undefined) {
    delete process.env.AGENTPROXY_API_KEY;
  } else {
    process.env.AGENTPROXY_API_KEY = ORIGINAL_AGENTPROXY_API_KEY;
  }
});
...
test("A2A JSON-RPC checks auth before returning disabled state", async () => {
  process.env.AGENTPROXY_API_KEY = "test-secret";
