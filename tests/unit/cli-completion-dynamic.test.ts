...
test("runCompletionCommand bash retorna 0 e string não-vazia", async () => {
  ...
  assert.ok(out.includes("agentproxy"), "bash script should mention agentproxy");
  assert.ok(out.includes("_agentproxy"), "bash script should define _agentproxy function");
});
...
test("runCompletionCommand fish retorna 0", async () => {
  ...
  const out = chunks.join("");
  assert.ok(out.includes("agentproxy"), "fish script should mention agentproxy");
});
...
test("completion scripts incluem combos/providers/models no cache dinamicamente", async () => {
  ...
  const out = chunks.join("");
  assert.ok(
    out.includes("completion-cache.json") || out.includes("agentproxy_get_cache"),
