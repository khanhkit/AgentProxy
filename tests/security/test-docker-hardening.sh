...
IMAGE_NAME="${1:-agentproxy-local-hardened}"
CONTAINER_NAME="${2:-agentproxy-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/agentproxy_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/agentproxy_hardening_container_id.txt
