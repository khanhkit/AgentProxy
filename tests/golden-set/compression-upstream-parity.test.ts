...
const upstreamAvailable = existsSync(upstreamFixtureDir) && existsSync(upstreamModulePath);
const upstream = upstreamAvailable
  ? (require(upstreamModulePath) as {
      compress(text: string): { compressed: string; before: number; after: number };
    })
  : null;

function upstreamCompress(text: string): string {
  assert.ok(upstream, "optional upstream Caveman reference fixture is unavailable");
...
describe(
  "upstream Caveman parity benchmark",
  { skip: upstreamAvailable ? false : "optional _references Caveman fixture is unavailable" },
  () => {
