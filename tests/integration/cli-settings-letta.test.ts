 ...
 *
 * Letta configures AgentProxy as its "lmstudio" provider (localModelDiscovery:
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-letta-settings-"));
...
// ── Test 6: POST refuses to overwrite an existing non-AgentProxy lmstudio config ──
...
// ── Test 7: DELETE → removes the AgentProxy lmstudio config ──────────────────
