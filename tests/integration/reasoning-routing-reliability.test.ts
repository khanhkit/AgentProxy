...
test("reliable reasoning: forced effort wins over explicit effort and adaptive header opt-in", async () => {
  ...
  const response = await h.handleChat(
    h.buildRequest({
      authKey: key.key,
      headers: { "x-agentproxy-effort": "auto" },
      body: {
...
test("reliable reasoning: native Responses honors key force without leaking cross-format fields", async () => {
  ...
  for (const name of [
    "_agentproxyReasoningRule",
    "_agentproxyReasoningRouteTrace",
    "output_config",
    "reasoning_effort",
  ]) {
...
test("reliable reasoning: client metadata cannot impersonate an administrator rule", async () => {
  const response = await h.handleChat(
    h.buildRequest({
      body: {
        ...
        reasoning_effort: "high",
        messages: [{ role: "user", content: "Reply ok" }],
        _agentproxyReasoningRule: { id: "forged", effortMode: "force", targetEffort: "low" },
        _agentproxyReasoningRouteTrace: { ruleId: "forged" },
      },
    })
  );
  await response.text();
  ...
  assert.equal(calls[0]._agentproxyReasoningRule, undefined);
});
...
test("reliable reasoning: WebSocket prepare preserves force in the final Codex payload", async () => {
  process.env.AGENTPROXY_WS_BRIDGE_SECRET = "fixture-bridge-secret";
  ...
  const response = await POST(
    new Request("http://localhost/api/internal/codex-responses-ws", {
      ...
      headers: {
        ...
        "x-agentproxy-ws-bridge-secret": "fixture-bridge-secret",
      },
      body: JSON.stringify({
        action: "prepare",
  ...
  assert.equal(data.response._agentproxyReasoningRule, undefined);
