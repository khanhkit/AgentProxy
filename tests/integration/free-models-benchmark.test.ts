 ...
 *
 * Live benchmark: how well do the free-tier models AgentProxy exposes handle
 ...
 *
 * Environment:
 *   AGENTPROXY_API_KEY  — required (else test skips)
...
test(
  ...
  async () => {
    ...
    // AgentProxy's routing) is broken, not that every free provider failed at
    // once.
    assert.ok(
      totalSucceeded > 0,
