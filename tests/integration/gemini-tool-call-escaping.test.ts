 ...
 * AgentProxy combo routing engine — both streaming and non-streaming.
 *
 * Gemma4 models emit literal 0x0A bytes in functionCall.args string values.
 * AgentProxy's translator must escape these into valid JSON \n sequences.
 *
 * Environment:
 *   AGENTPROXY_API_KEY  — required (else tests skip)
 *   AGENTPROXY_URL      — defaults to http://localhost:3000
...
// ── AgentProxy Combo — Chat Completions ────────────────────────────────────

test("agentproxy combo: tool call arguments are valid JSON", { skip }, async () => {
  const data = await sendToolCallChatRequest(MODEL, TOOL_CALL_PROMPT);
  const toolCalls = extractToolCalls(data);
  validateToolCallArguments(toolCalls);
  console.log(`  [OK] agentproxy combo: ${toolCalls.length} tool calls, model=${data.model}`);
});

test("agentproxy combo: streaming tool call produces valid JSON", { skip }, async () => {
  const data = await sendStreamingToolCallChatRequest(MODEL, TOOL_CALL_PROMPT);
  const toolCalls = extractToolCalls(data);
  if (toolCalls.length === 0) {
    console.log("  [skip] agentproxy combo streaming: model did not produce a tool call");
    return;
  }
  validateToolCallArguments(toolCalls);
  console.log(
    `  [OK] agentproxy combo streaming: ${toolCalls.length} tool calls, finish=${data.choices[0].finish_reason}`
  );
});

// ── AgentProxy Combo — Responses API ───────────────────────────────────────

test("agentproxy combo: responses tool call arguments are valid JSON", { skip }, async () => {
  const data = await sendToolCallResponsesRequest(MODEL, TOOL_CALL_PROMPT);
  const toolCalls = extractToolCallsFromResponses(data);
  if (toolCalls.length === 0) {
    console.log("  [skip] agentproxy combo responses: no tool call in response");
    return;
  }
  validateToolCallArguments(toolCalls);
  console.log(
    `  [OK] agentproxy combo responses: ${toolCalls.length} tool calls, model=${data.model}`
  );
});

test("agentproxy combo: streaming responses tool call produces valid JSON", { skip }, async () => {
  const toolCalls = await sendStreamingToolCallResponsesRequest(MODEL, TOOL_CALL_PROMPT);
  if (toolCalls.length === 0) {
    console.log("  [skip] agentproxy combo streaming responses: model did not produce a tool call");
    return;
  }
  validateToolCallArguments(toolCalls);
  console.log(`  [OK] agentproxy combo streaming responses: ${toolCalls.length} tool calls`);
});
