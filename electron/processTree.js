...
//
// The embedded server is spawned via process.execPath (= agentproxy.exe) with
// ELECTRON_RUN_AS_NODE=1, and it in turn spawns grandchildren (embedded services,
// MITM proxy, tunnels — several also agentproxy.exe-as-node). On Windows, Node's
...
// on agentproxy.exe, so the process "hangs in memory" after Exit and updates fail with
