 ...
 * routing; the ONLY credential AgentProxy presents is the capability token, sent as
...
export interface CodexAppServerConfig {
   ...
   * its own approval — the harness that consumes AgentProxy owns execution policy.
   */
...
function resolveTokenWithSource(
  ...
  const inlineEnv = firstString(process.env.AGENTPROXY_CODEX_APPSERVER_WS_TOKEN);
  ...
  const fileEnv = firstString(process.env.AGENTPROXY_CODEX_APPSERVER_WS_TOKEN_FILE);
  if (fileEnv) {
...
export function resolveAppServerConfig(psd: ProviderSpecificData): CodexAppServerConfig | null {
  const urlRes = firstStringWithSource(psd?.codexAppServerUrl, process.env.AGENTPROXY_CODEX_APPSERVER_WS);
  ...
  const cwd =
    firstString(psd?.codexAppServerCwd, process.env.AGENTPROXY_CODEX_APPSERVER_CWD) ?? "/tmp";

  const approvalPolicy =
    firstString(psd?.codexAppServerApprovalPolicy, process.env.AGENTPROXY_CODEX_APPSERVER_APPROVAL) ??
    undefined;
  const sandbox =
    firstString(psd?.codexAppServerSandbox, process.env.AGENTPROXY_CODEX_APPSERVER_SANDBOX) ??
    undefined;
 ...
 *   AGENTPROXY_CODEX_APPSERVER_SANDBOX). With "never" + a permissive sandbox,
 ...
 *   AGENTPROXY_CODEX_APPSERVER_AUTO_APPROVE. Harness tool calls are unaffected —
...
export function resolveThreadStartPolicy(
  ...
  const raw = firstString(
    ...
    process.env.AGENTPROXY_CODEX_APPSERVER_AUTO_APPROVE
  );
