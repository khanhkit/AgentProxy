/**
 * MaxAiExecutor — MaxAI web-app chat as an OpenAI-compatible AgentProxy provider.
 *
 ...
 * IPs). AgentProxy routes the executor's `fetch()` through the per-connection proxy
 ...
 * therefore minted/refreshed out-of-band by AgentProxy's own browser-mint flow
...
export class MaxAiExecutor extends BaseExecutor {
  ...
  override async execute(input: ExecuteInput): Promise<ExecutorExecuteResult> {
    ...
    if (upstream.status !== 200 || !upstream.body) {
      ...
      // AgentProxy's compression/overflow pipeline can shrink and retry instead of
