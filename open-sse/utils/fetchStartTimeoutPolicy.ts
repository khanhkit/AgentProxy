...
// never returns a response at all (not even headers), AgentProxy kept the
// connection open with nothing but keepalives, guaranteeing the client gave
// up first with an opaque 499 instead of AgentProxy detecting the stall and
...
// keepalives in flight) is ~120s. Keep the cap safely under that so AgentProxy's
