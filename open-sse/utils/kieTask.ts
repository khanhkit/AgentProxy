...
const FALLBACK_KIE_CALLBACK_URL = "https://agentproxy.local/api/kie/callback";
...
function getConfiguredKieCallbackUrl(): string {
  const explicit =
    process.env.KIE_CALLBACK_URL?.trim() || process.env.AGENTPROXY_KIE_CALLBACK_URL?.trim();
  if (explicit) return explicit;

  return (
    callbackUrlFromBaseUrl(process.env.AGENTPROXY_PUBLIC_URL) ||
