 ...
 * `stream`. Without this hint, AgentProxy previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // AgentProxy accumulates the provider stream and converts it to JSON for the client
  ...
  // AgentProxy fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve AgentProxy's streaming default unless
