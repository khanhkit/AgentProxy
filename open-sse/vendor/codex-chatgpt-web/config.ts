 ...
 */
export const CHATGPT_CONNECTOR_NAME = "AgentProxy Codex v2";
export const DEV_CHATGPT_CONNECTOR_NAME = `${CHATGPT_CONNECTOR_NAME} DEV`;
export const LEGACY_CHATGPT_CONNECTOR_NAMES = ["Codex Native", "AgentProxy Codex"] as const;
...
export function getConfigDir(): string {
  ...
  const dataDir = process.env.DATA_DIR?.trim() || process.env.AGENTPROXY_DATA_DIR?.trim();
  return resolve(expandUserPath(dataDir || join(homedir(), ".agentproxy")), "chatgpt-web-codex");
}
