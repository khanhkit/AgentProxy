 ...
 * `extra.authInfo` (AgentProxy authenticates with API keys, not OAuth client ids —
 ...
 *
 * On stdio transport (`agentproxy --mcp`) there is no HTTP context, so we fall
 * back to the AGENTPROXY_API_KEY / ROUTER_API_KEY environment variable.
 *
 ...
 *
 * Falls back to AGENTPROXY_API_KEY / ROUTER_API_KEY env vars for stdio transport
...
/**
 * Resolve the principal id from the AGENTPROXY_API_KEY (or ROUTER_API_KEY) env var.
...
async function resolvePrincipalFromEnv(): Promise<string | undefined> {
  const rawKey = process.env.AGENTPROXY_API_KEY || process.env.ROUTER_API_KEY;
  if (!rawKey) return undefined;
  try {
