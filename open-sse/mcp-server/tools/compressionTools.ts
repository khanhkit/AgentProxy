/**
 * AgentProxy MCP Compression Tools — Manage and monitor prompt compression.
 *
 * Tools:
 *   1. agentproxy_compression_status   — Get compression config, analytics, and cache stats
 *   2. agentproxy_compression_configure — Update compression settings
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("agentproxy_compression_status", args, result, duration, true);
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("agentproxy_compression_configure", args, result, duration, true);
...
export async function handleCcrStoreTool(
  ...
  await logToolCall("agentproxy_ccr_store", auditInput, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrRetrieveTool(
  ...
  if ((!args.mode || args.mode === "full") && metadata.bytes > MAX_CCR_MCP_FULL_BYTES) {
    ...
    await logToolCall("agentproxy_ccr_retrieve", args, output, Date.now() - start, true);
    return output;
  }
...
export async function handleCcrInspectTool(
  ...
  await logToolCall("agentproxy_ccr_inspect", args, output, Date.now() - start, Boolean(metadata));
  return output;
}
...
export async function handleCcrListTool(
  ...
  await logToolCall("agentproxy_ccr_list", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrDeleteTool(
  ...
  await logToolCall("agentproxy_ccr_delete", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrStatsTool(
  ...
  await logToolCall("agentproxy_ccr_stats", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleRtkDiscover(
  ...
  await logToolCall("agentproxy_rtk_discover", args, result, Date.now() - start, true);
  return result;
}
...
export async function handleRtkLearn(
  ...
  await logToolCall("agentproxy_rtk_learn", args, result, Date.now() - start, true);
  return result;
}
...
export const compressionTools = {
  ...
  agentproxy_ccr_inspect: {
    name: "agentproxy_ccr_inspect",
  ...
  agentproxy_ccr_delete: {
    name: "agentproxy_ccr_delete",
  ...
  agentproxy_ccr_stats: {
    name: "agentproxy_ccr_stats",
    description:
  ...
  agentproxy_rtk_discover: {
    name: "agentproxy_rtk_discover",
    description:
  ...
  agentproxy_rtk_learn: {
    name: "agentproxy_rtk_learn",
    description:
