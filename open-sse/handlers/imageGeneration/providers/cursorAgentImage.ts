 ...
 *
 * Why CLI (not AgentService/Run): AgentProxy's Cursor chat executor talks to
...
/** Strip AgentProxy `account::token` composites the same way CursorExecutor does. */
...
export async function handleCursorAgentImageGeneration({
  ...
  if (!peerLocality || !SPAWN_ALLOWED_LOCALITIES.has(peerLocality)) {
    return saveImageErrorResult({
      ...
      startTime,
      error:
        "Cursor Agent image generation spawns a local process and is only available from localhost or the private LAN AgentProxy runs on.",
    });
  }
