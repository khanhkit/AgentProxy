...
import { isModelLocked } from "@agentproxy/open-sse/services/accountFallback.ts";
import { parseModel } from "@agentproxy/open-sse/services/model.ts";
import type { StrictZeroCostExclusionReason } from "@agentproxy/open-sse/services/autoCombo/strictZeroCostFilter.ts";
...
export async function getAutoComboCandidates(
  ...
  if (channel === "auto") {
    ...
    const { createBuiltinAutoCombo } =
      await import("@agentproxy/open-sse/services/autoCombo/builtinCatalog.ts");
