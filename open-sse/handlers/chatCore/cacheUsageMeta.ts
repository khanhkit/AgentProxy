 ...
 * shapes), and attach an `_agentproxy` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _agentproxy: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._agentproxy &&
    typeof payload._agentproxy === "object" &&
    !Array.isArray(payload._agentproxy)
      ? payload._agentproxy
      : {};
  return {
    ...payload,
