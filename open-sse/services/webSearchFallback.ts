...
export const AGENTPROXY_WEB_SEARCH_FALLBACK_TOOL_NAME = "agentproxy_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = AGENTPROXY_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
// built-in web-search tool has to be converted to the agentproxy_web_search function
...
export function prepareWebSearchFallbackBody<T extends WebSearchFallbackBody>(
  ...
  if (!toolNames.has(AGENTPROXY_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: AGENTPROXY_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: AGENTPROXY_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
