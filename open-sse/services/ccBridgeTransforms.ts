 ...
 *
 * Goal: ensure the final request body AgentProxy sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - agentproxy-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "agentproxy-daystamp";
  /**
 ...
 * proven against the live AgentProxy deployment (call log
...
/**
 * Compute the `cc_version` suffix per the AgentProxy native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "agentproxy-daystamp"
