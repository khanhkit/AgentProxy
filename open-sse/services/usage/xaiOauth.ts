 ...
 * with the connection's OAuth access token, falling back to AgentProxy's own
...
export async function getXaiOauthUsage(
  ...
  try {
    ...
    const used =
      ...
      0;
    return {
      plan: "xAI OAuth (Grok) · AgentProxy-tracked",
      message: "Live weekly quota unavailable; showing AgentProxy-routed token totals only.",
      quotas: {
        monthly: {
          used,
          total: 0,
