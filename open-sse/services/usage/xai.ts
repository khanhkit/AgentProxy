 ...
 * self-track pattern — AgentProxy sums the tokens it itself routed to this
 ...
 * MiMo self-track pattern above — AgentProxy sums the tokens it itself routed
...
export async function getXaiUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "xAI / Grok (AgentProxy-tracked)",
      quotas: {
        monthly: {
          used,
          total: 0,
