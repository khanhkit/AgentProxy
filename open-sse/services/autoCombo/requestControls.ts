 ...
 *
 *   X-AgentProxy-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-AgentProxy-Budget:          <max USD per request>                             (#6023)
 *   X-AgentProxy-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-AgentProxy-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-AgentProxy-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-AgentProxy-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-agentproxy-mode")?.trim() || null;
  const budgetHeader = headers.get("x-agentproxy-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-agentproxy-budget-fallback")?.trim() || null;
