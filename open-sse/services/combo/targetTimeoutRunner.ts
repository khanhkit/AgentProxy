 ...
 * can fall back without treating AgentProxy's own deadline as a provider-connection failure.
...
/** Stable internal classification for AgentProxy's own combo per-target timer. */
  ...
  return async (
    ...
    const timeoutPromise = new Promise<Response>((resolve) => {
      timeoutId = setTimeout(() => {
        ...
        // HTTP 504 (not proprietary 524): this is AgentProxy's own per-target timer.
