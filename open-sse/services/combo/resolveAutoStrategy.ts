...
export interface ResolveAutoStrategyDeps {
  ...
  relayOptions?: {
    ...
    sessionId?: string | null;
    /** Per-request X-AgentProxy-Mode value (#6024/#6025). */
    mode?: string | null;
    /** Per-request X-AgentProxy-Budget value in USD (#6023). */
...
export async function resolveAutoStrategyOrder(
  ...
  // Per-request overrides (#6023 / #6024 / #6025 / #3470): X-AgentProxy-Budget,
  // X-AgentProxy-Budget-Fallback and X-AgentProxy-Mode headers (threaded via
  ...
  // per-request X-AgentProxy-Mode override — but `scoreAutoTargets()` (the fallback
