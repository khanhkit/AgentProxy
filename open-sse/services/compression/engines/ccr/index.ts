...
/**
 * Handler for the `agentproxy_ccr_retrieve` MCP tool.
 *
 ...
 * ENTIRE prompt for any caller that cannot resolve `agentproxy_ccr_retrieve`
...
function processMessages(
  ...
  const result = messages.map((msg) => {
    ...
    // AgentProxy is used as a chat-completion PROVIDER, the upstream LLM has no
    // way to call `agentproxy_ccr_retrieve` and expand markers — replacing tool
...
export const ccrEngine: CompressionEngine = {
  ...
  description:
    ...
    "retrievable via the `agentproxy_ccr_retrieve` MCP tool (H4). " +
  ...
  apply(body: Record<string, unknown>, options?: CompressionEngineApplyOptions): CompressionResult {
    ...
    // agentproxy_ccr_retrieve may have content replaced at all. For everyone
    ...
    // #8033: teach MCP-capable callers the marker → agentproxy_ccr_retrieve contract
