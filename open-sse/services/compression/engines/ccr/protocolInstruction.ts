 ...
 * content — it can only call `agentproxy_ccr_retrieve` if it (a) knows the tool
 ...
 * `agentproxy_ccr_retrieve` (an MCP-capable caller). Plain OpenAI-compatible
 ...
 * but only when:
 *  - the caller can actually reach `agentproxy_ccr_retrieve` (see
