 ...
 *
 * Opt-in only: gated behind `AGENTPROXY_BROWSER_POOL` / `WEB_COOKIE_USE_BROWSER`
 ...
 * (`WEB_COOKIE_USE_BROWSER` or `AGENTPROXY_BROWSER_POOL`). Off by default.
 */
export function shouldUseGrokBrowserBacked(): boolean {
  const flag = process.env.WEB_COOKIE_USE_BROWSER;
  if (flag === "1" || flag === "true" || flag === "on") return true;
  const poolFlag = process.env.AGENTPROXY_BROWSER_POOL;
