 ...
 *
 * AgentProxy's account-fallback engine (accountFallback.ts) historically rotated accounts using
 ...
 *   - master enable                            AGENTPROXY_ROTATION_ENABLED               (default true)
 *   - rate-limit reset/cooldown seconds        AGENTPROXY_ROTATION_RATE_LIMIT_RESET_SECONDS (0 => engine default)
 *   - per-status fallback enable               AGENTPROXY_ROTATE_ON_{429,500,502,400}    (429/500/502 default true, 400 default false)
 *   - per-status threshold (errors in window)  AGENTPROXY_ROTATE_{status}_THRESHOLD      (default 1 => immediate, current behavior)
 *   - per-status window seconds                AGENTPROXY_ROTATE_{status}_WINDOW_SECONDS (default 120)
 *
...
const GLOBAL_KEY = "__agentproxy_rotation_config__";
...
function buildFromEnv(): RotationConfig {
  return {
    enabled: envBool("AGENTPROXY_ROTATION_ENABLED", true),
    rateLimitResetMs: envInt("AGENTPROXY_ROTATION_RATE_LIMIT_RESET_SECONDS", 0, 0) * 1000,
    disableTagWithoutReset: envBool("AGENTPROXY_ROTATION_DISABLE_TAG_WITHOUT_RESET", true),
    rateLimit429: buildClass(
      "AGENTPROXY_ROTATE_ON_429",
      "AGENTPROXY_ROTATE_429_THRESHOLD",
      "AGENTPROXY_ROTATE_429_WINDOW_SECONDS",
      true
    ),
    serverError500: buildClass(
      "AGENTPROXY_ROTATE_ON_500",
      "AGENTPROXY_ROTATE_500_THRESHOLD",
      "AGENTPROXY_ROTATE_500_WINDOW_SECONDS",
      true
    ),
    badGateway502: buildClass(
      "AGENTPROXY_ROTATE_ON_502",
      "AGENTPROXY_ROTATE_502_THRESHOLD",
      "AGENTPROXY_ROTATE_502_WINDOW_SECONDS",
      true
    ),
    badRequest400: buildClass(
      "AGENTPROXY_ROTATE_ON_400",
      "AGENTPROXY_ROTATE_400_THRESHOLD",
      "AGENTPROXY_ROTATE_400_WINDOW_SECONDS",
      false
    ),
  };
}
...
const COUNTER_KEY = "__agentproxy_rotation_counters__";
