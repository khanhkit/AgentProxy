...
export function getQoderCliWorkspace(): string {
  const explicit = String(
    process.env.QODER_CLI_WORKSPACE || process.env.AGENTPROXY_QODER_WORKSPACE || ""
  ).trim();
...
/**
 * Isolated `--config-dir` for AgentProxy-driven qodercli runs. Keeping it separate
...
export function getQoderCliConfigDir(): string {
  ...
  const base = dataDir || path.join(os.homedir() || os.tmpdir(), ".agentproxy");
...
export async function runQoderCli(options: QoderCliRunOptions): Promise<QoderCliRunResult> {
  ...
  const args = [
    ...
    "--model",
    level,
    // Disable all built-in tools — AgentProxy only wants a plain LM reply, never
...
/**
 * Resolve an AgentProxy model id to the exact value to pass to `qodercli -m`.
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an AgentProxy OpenAI-compatible request through the Qoder CLI transport.",
...
export function buildCosyHeadersForValidation(bodyStr: string, token: string) {
  ...
  const uid = "agentproxy.user@qoder.sh";
  const userInfo = {
    uid: uid,
...
// AgentProxy mis-surfaced as "PAT may not be valid for the chat API". We mirror the
...
export async function validateQoderCliPat({
  ...
  if (run.timedOut) {
    return {
      valid: false,
      error:
        "qodercli timed out while validating the token. Check network/proxy access from the AgentProxy host.",
      unsupported: false,
    };
  }
