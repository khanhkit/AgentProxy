 ...
 * `AGENTPROXY_EMERGENCY_FALLBACK=false` (or `0`). Default remains enabled.
 */

import { isFeatureFlagEnabled } from "@/shared/utils/featureFlags";

const EMERGENCY_FALLBACK_FLAG_KEY = "AGENTPROXY_EMERGENCY_FALLBACK";
...
function isEmergencyFallbackRawEnvEnabled(): boolean {
  const raw = process.env.AGENTPROXY_EMERGENCY_FALLBACK;
...
export function shouldUseFallback(
  ...
  if (!isEmergencyFallbackEnvEnabled()) {
    return {
      shouldFallback: false,
      reason: "emergency fallback disabled via AGENTPROXY_EMERGENCY_FALLBACK",
    };
  }
