export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-AgentProxy-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.AGENTPROXY_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.AGENTPROXY_PUBLIC_PROTOCOL || "http";
