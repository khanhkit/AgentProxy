...
AgentProxy can open an isolated browser for provider connections that require an interactive web login. The operator signs in through the browser UI, then AgentProxy reads only the credential fields declared for that provider through the Chrome DevTools Protocol (CDP) and writes them to the selected `provider_connections` row.
...
```bash
docker build -t agentproxy-vnc-chromium:local docker/vnc-browser/chromium
```

`agentproxy-vnc-chromium:local` is the default image. Set `AGENTPROXY_VNC_IMAGE` only when using a compatible image that provides:
...
| --- | --- | --- |
| `AGENTPROXY_VNC_IMAGE` | `agentproxy-vnc-chromium:local` | Compatible browser image |
| `AGENTPROXY_VNC_CONTAINER_VNC_PORT` | `3000` | Browser UI port inside the container |
| `AGENTPROXY_VNC_CONTAINER_CDP_PORT` | `9223` | CDP bridge port inside the container |
| `AGENTPROXY_VNC_CONTAINER_PROFILE_DIR` | `/config` | Profile mount point inside the container |
| `AGENTPROXY_VNC_PROFILE_DIR` | `$HOME/.agentproxy/browser-login-profiles` | Host profile root |
| `AGENTPROXY_VNC_PERSIST_PROFILES` | `false` | Reuse a connection profile across sessions |
| `AGENTPROXY_VNC_IDLE_MS` | `600000` | Idle-session timeout in milliseconds |
| `AGENTPROXY_VNC_MAX_MS` | `1800000` | Maximum session lifetime in milliseconds |
| `AGENTPROXY_VNC_MAX_SESSIONS` | `4` | Maximum concurrent sessions |
| `AGENTPROXY_VNC_READY_MS` | `45000` | Browser/CDP startup timeout in milliseconds |
| `AGENTPROXY_VNC_HARVEST_MS` | `20000` | Credential-harvest timeout in milliseconds |
| `AGENTPROXY_VNC_CHROMIUM_ARGS` | see `manifest.ts` | Chromium command-line arguments |
| `AGENTPROXY_DOCKER_BIN` | `docker` | Docker-compatible CLI executable |
...
For an end-to-end check, build the image, start AgentProxy, create or select a supported web-provider connection, start a browser-login session through the management API, complete login through the returned UI URL, harvest credentials, and verify that the selected connection—not another account for the same provider—was updated.