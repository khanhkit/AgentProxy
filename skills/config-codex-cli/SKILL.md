...
description: Step-by-step agent workflow to configure the OpenAI Codex CLI on any machine (Linux, macOS, Windows) to use AgentProxy as an OpenAI-compatible backend. Detects OS and shell, writes config.toml and 7 named profiles, sets environment variables, and verifies the setup.
---
...
Two AgentProxy defaults silently break multi-hour Codex sessions. Document them whenever configuring Codex for overnight / multi-hour work. Full guide: `docs/guides/CODEX-CLI-CONFIGURATION.md` → **Long-running tasks**.
...
- Codex session keys (`x-codex-session-id`, `x-session-id`, `x-agentproxy-session`, body `prompt_cache_key` / `session_id`) are only used for pinning when TTL > 0.
...
3. Restart AgentProxy. Leave Codex `config.toml` as usual (`wire_api = "responses"`, correct `base_url`).
