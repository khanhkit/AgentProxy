...
<!-- skill:custom-start -->
<!-- Migrated from skills/agentproxy-routing/SKILL.md (preserved curated content) -->

# AgentProxy — Routing & Combos
...
```bash
curl $AGENTPROXY_URL/api/combos \
...
```bash
curl -X POST $AGENTPROXY_URL/api/combos \
  -H "Authorization: Bearer $AGENTPROXY_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-combo",
...
```bash
# Activate
curl -X PUT $AGENTPROXY_URL/api/combos/{id}/toggle \
  -H "Authorization: Bearer $AGENTPROXY_KEY" \
  -d '{ "enabled": true }'
```
...
```bash
curl $AGENTPROXY_URL/api/combos/{id}/metrics \
  -H "Authorization: Bearer $AGENTPROXY_KEY"
```
...
```bash
curl -X POST $AGENTPROXY_URL/api/routing/simulate \
  -H "Authorization: Bearer $AGENTPROXY_KEY" \
  -H "Content-Type: application/json" \
...
```
agentproxy_list_combos     → list all combos
agentproxy_switch_combo    → enable/disable a combo
agentproxy_set_routing_strategy → change strategy at runtime
agentproxy_simulate_route  → dry-run routing decision
agentproxy_best_combo_for_task → get recommendation by task type
```
