---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend AgentProxy's automation surface."
---
...
Read Crush CLI AgentProxy config

Local-only. Reads the AgentProxy provider block in Crush's config.
...
Write Crush CLI AgentProxy config

Local-only. Registers AgentProxy as an `openai-compat` provider in Crush's config.
...
Remove AgentProxy from Crush CLI config

Local-only. Removes the AgentProxy provider block from Crush's config.
...
Read CodeWhale CLI AgentProxy config

Local-only. Reads the AgentProxy config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI AgentProxy config

Local-only. Writes the AgentProxy config block in CodeWhale TOML format.
...
Remove AgentProxy from CodeWhale CLI config

Local-only. Removes the AgentProxy config block from CodeWhale's config.
