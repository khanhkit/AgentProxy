...
description: "Interact with AgentProxy via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities."
---
...
<!-- skill:custom-start -->
<!-- Migrated from skills/agentproxy-a2a/SKILL.md (preserved curated content) -->

# AgentProxy — A2A Protocol
...
AgentProxy publishes an Agent Card at `/.well-known/agent.json` and accepts
JSON-RPC 2.0 calls at `/a2a`.

## Discovery

```bash
curl $AGENTPROXY_URL/.well-known/agent.json
```
...
```bash
curl -X POST $AGENTPROXY_URL/a2a \
...
- `401` → missing/invalid `AGENTPROXY_KEY`

## Reference

Full docs: https://github.com/khanhkit/AgentProxy/blob/main/docs/frameworks/A2A-SERVER.md
<!-- skill:custom-end -->
