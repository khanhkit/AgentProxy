...
description: "Detect installed CLI coding tools (Claude Code, Codex, Cursor, Copilot, Cline and more), search GitHub for matching agent skills, and install them to the detected tools via AgentProxy's built-in APIs."
---
...
Discover and install agent skills for your coding CLI tools — all through AgentProxy's built-in APIs.
...
No separate Skill Collector app needed — AgentProxy's own CLI detection + GitHub search handles everything.

---

## Step 1 — Detect installed coding tools

Query AgentProxy's CLI tool detection to find which coding agents are installed:

```bash
curl -H "Authorization: Bearer $AGENTPROXY_API_KEY" http://localhost:20128/api/skills/collect/detect
```

This returns:

- Every CLI tool in AgentProxy's catalog (`CLI_TOOL_IDS`: claude, codex, cursor, copilot, opencode, cline, kilocode, hermes, hermes-agent, openclaw, droid, continue, qwen, windsurf, devin, antigravity, etc.)
...
```bash
curl -H "Authorization: Bearer $AGENTPROXY_API_KEY" \
...
```bash
AUTH_HEADER="Authorization: Bearer $AGENTPROXY_API_KEY"
...
- AgentProxy must be running locally on port 20128 (default) — see `docs/frameworks/SKILLS.md` for custom-port setups.
...
- This replaces the standalone Skill Collector Python app — all logic is now inside AgentProxy.
<!-- skill:custom-end -->
