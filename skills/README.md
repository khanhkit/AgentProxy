...
- **MCP tool**: `agentproxy_agent_skills_list` (scope `read:catalog`) — returns the full 45-entry catalog in one call.
...
`https://raw.githubusercontent.com/khanhkit/AgentProxy/main/skills/<id>/SKILL.md`
...
| `omni-providers`       | Providers                     | Manage provider connections, API keys, OAuth flows, and connection tests across AgentProxy's 327-provider catalog.                                                                              |
...
| `omni-api-keys`        | API Keys                      | Create, list, rotate, and revoke AgentProxy API keys. Control per-key scopes, spending limits, and expiration.                                                                                  |
...
| `omni-cli-tools`       | CLI Tools                     | Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend AgentProxy's automation surface.                                                     |
| `omni-tunnels`         | Tunnels                       | Create and manage secure tunnels (ngrok, Cloudflare Tunnel, custom) to expose AgentProxy to the internet or share access with remote agents and CI pipelines.                                   |
| `omni-sync-cloud`      | Cloud Sync                    | Synchronise AgentProxy configuration, provider connections, and settings to/from cloud storage. Manage cloud worker authentication and remote backup targets.                                   |
...
| `omni-mcp`             | MCP Server                    | Connect to the AgentProxy MCP server (110 tools, 3 transports: SSE/stdio/HTTP) across 33 permission scopes.                                                                                     |
| `omni-agents-a2a`      | Agents & A2A Protocol         | Interact with AgentProxy via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities. |
...
| ID                    | Name                          | Description                                                                                                                                                                                    |
| --------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cli-serve`           | CLI: Serve                    | Start, stop, and restart the AgentProxy server from the CLI. Manage daemon mode, port configuration, auto-recovery, system tray integration, and the dashboard open shortcut.                   |
...
| `cli-keys`            | CLI: API Keys                 | Create, list, rotate, and revoke AgentProxy API keys from the CLI. Manage OAuth flows for provider authentication and inspect key scopes and expiration.                                        |
...
| `cli-chat`            | CLI: Chat                     | Send chat completions, stream responses, and start an interactive REPL session from the CLI. Supports all AgentProxy providers, combo routing, and system prompt configuration.                 |
...
| `cli-a2a`             | CLI: A2A Protocol             | Interact with the AgentProxy A2A server from the CLI. Send tasks, inspect skill execution history, and test the JSON-RPC 2.0 agent-to-agent protocol interactively.                             |
...
| `cli-backup-sync`     | CLI: Backup & Sync            | Backup and restore AgentProxy data from the CLI. Trigger incremental snapshots, sync to cloud storage, manage backup schedules, and restore from archive files.                                 |
...
| ID                 | Name              | Description                                                           |
| ------------------ | ----------------- | --------------------------------------------------------------------- |
| `config-codex-cli` | Config: Codex CLI | Configure Codex CLI to use AgentProxy as an OpenAI-compatible backend. |
...
- API entry: `https://raw.githubusercontent.com/khanhkit/AgentProxy/main/skills/omni-auth/SKILL.md`
- CLI entry: `https://raw.githubusercontent.com/khanhkit/AgentProxy/main/skills/cli-serve/SKILL.md`
...
## What makes AgentProxy skills unique
