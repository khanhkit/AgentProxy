...
Some services were integrated into AgentProxy and later removed because the people who run
...
1. **A takedown request from a service operator is honored, not negotiated.** AgentProxy is
...
2. **"Removed" means every surface AgentProxy controls.** Executor, registry entry, provider
...
3. **Never reintroduce an entry on this page without written permission.** That includes
   ...
   documenting a manual way to reach it through AgentProxy. Close such PRs and issues with a
   link to this page.
...
2. Remove the integration following the checklist in policy item 2. Use
   [#12440](https://github.com/khanhkit/AgentProxy/pull/12440) as the reference for a
   keyless provider and [#10210](https://github.com/khanhkit/AgentProxy/pull/10210) for an
...
4. Reply to the operator once the PR is merged, listing what was removed and what AgentProxy
