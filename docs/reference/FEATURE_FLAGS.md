...
> Runtime toggles that change AgentProxy's behavior **without a redeploy**.
...
| `AGENTPROXY_CONTROL_PLANE_PROXY_DIRECT_FALLBACK` | boolean | `false` |         | Allow OAuth and provider validation flows to bypass a pinned proxy and connect directly when proxy reachability pre-checks fail. Off by default because this can change egress IP.            |
...
| `DISABLE_CONTEXT_WINDOW_CHECKS` | boolean | `false`    |         | Skip AgentProxy's local context-window / max-input-token check for direct single-model requests. Upstream limits still apply. |
...
| `AGENTPROXY_MCP_ENFORCE_SCOPES`              | boolean | `true`  |         | Enforce scope restrictions on MCP tool access.                                                                                                                                                                                                                                                                                                          |
| `AGENTPROXY_MCP_COMPRESS_DESCRIPTIONS`       | boolean | `false` |         | Compress MCP tool descriptions to reduce token usage.                                                                                                                                                                                                                                                                                                   |
| `AGENTPROXY_ENABLE_RUNTIME_BACKGROUND_TASKS` | boolean | `false` |         | Enable background task processing at runtime.                                                                                                                                                                                                                                                                                                           |
| `AGENTPROXY_DISABLE_BACKGROUND_SERVICES`     | boolean | `false` | ✓       | Disable all background services (quota refresh, sync, etc).                                                                                                                                                                                                                                                                                             |
| `AGENTPROXY_RTK_TRUST_PROJECT_FILTERS`       | boolean | `false` |         | Trust project-level RTK filters without validation.                                                                                                                                                                                                                                                                                                     |
| `AGENTPROXY_ENABLE_LIVE_WS`                  | boolean | `true`  | ✓       | Start the real-time dashboard WebSocket server on import (port 20129 by default).                                                                                                                                                                                                                                                                       |
| `AGENTPROXY_CODEX_WS_ENABLED`                | boolean | `true`  |         | Allow Codex to use the Responses-over-WebSocket transport. When off, Codex falls back to HTTP Responses.                                                                                                                                                                                                                                                |
| `AGENTPROXY_EMERGENCY_FALLBACK`              | boolean | `true`  |         | Route budget-exhausted requests to the emergency free fallback provider/model. (See [Emergency Budget Fallback](#emergency-budget-fallback) below.)                                                                                                                                                                                                     |
...
| Key                                   | Type    | Default | Description                                              |
| ------------------------------------- | ------- | ------- | -------------------------------------------------------- |
| `AGENTPROXY_DISABLE_LOCAL_HEALTHCHECK` | boolean | `false` | Disable the local instance health check endpoint.        |
| `AGENTPROXY_DISABLE_TOKEN_HEALTHCHECK` | boolean | `false` | Disable the token validation health check.               |
...
`AGENTPROXY_EMERGENCY_FALLBACK` (category `runtime`, default `true`) controls the
...
dashboard toggle, a DB override, or the `AGENTPROXY_EMERGENCY_FALLBACK`
