...
serverless backend so AgentProxy can egress from a stable region while keeping
the provider API key server-side. This document covers the two failure modes
that operators hit in production and the recovery paths AgentProxy ships for
each.
...
2. Relay rows whose auth is missing show a yellow `auth missing` badge and a
   **Repair** button.
3. Click **Repair**. AgentProxy calls
...
Check the relay awareness headers returned by AgentProxy's auto-test probe
