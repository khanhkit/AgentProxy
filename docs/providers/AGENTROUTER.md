...
For most users, **no special setup is required**. AgentProxy ships a built-in
...
AgentProxy also supports AgentRouter (and similar relays) through the **Claude Code
...
- AgentProxy running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting AgentProxy:
...
**`Invalid JSON response from provider (reset after Ns)` from the agentproxy logs** —
...
model actually resolves in the agentproxy logs (`ROUTING` tag shows
