...
Two ways to put Cursor behind AgentProxy without an IDE session:

1. **`cursor-api` provider** (card "Cursor API", alias `cua`): an API-key
   provider that holds a Cursor user API key (`crsr_…`, generated at
   `https://cursor.com/dashboard/api`). Any AgentProxy client then reaches
...
2. **Cursor CLI passthrough**: point the Cursor CLI (`agent`) at AgentProxy so
   every RPC the CLI makes is authenticated with an AgentProxy API key, forwarded
...
curl -sS http://localhost:20128/v1/chat/completions \
  -H "Authorization: Bearer <agentproxy-api-key>" \
  -H "Content-Type: application/json" \
...
| Path                                                                                                                         | Auth expected from the CLI   | What AgentProxy does                                                                                                                                                |
| ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /auth/exchange_user_api_key`                                                                                           | `Bearer <AgentProxy API key>` | Validates the key, mints a 1h HS256 JWT (signed with `JWT_SECRET`) and returns it                                                                                  |
...
that. A 401 from AgentProxy makes the CLI exchange again.

### Setup

1. Create an AgentProxy API key (Dashboard → API keys) and a `cursor-api`
   connection.
...
| ------------------------------------------------ | --------------------------------------------- |
| Unknown AgentProxy key and `REQUIRE_API_KEY=true` | 401 `unauthenticated` on exchange             |
...
| AgentProxy API key revoked after exchange         | 401 on the next RPC                           |
