...
When AgentProxy runs inside Docker, the standard "Import from Zed Keychain" flow fails
...
AgentProxy detects the Docker environment via two heuristics:
...
2. The **Manual Token Import** panel appears below the keychain import card. When
   AgentProxy detects Docker, this panel expands automatically after the first failed
