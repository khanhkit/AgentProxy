...
AgentProxy controls which **browser origins** may read cross-origin responses
...
taking seriously, but AgentProxy's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. AgentProxy's shared CORS path never
> does.
...
- If AgentProxy runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
## Example: reverse proxy in front of AgentProxy

CORS is enforced by AgentProxy itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let AgentProxy answer preflight:
...
Set the allowed browser origins in AgentProxy (`CORS_ALLOWED_ORIGINS` or the
