...
AgentProxy CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-agentproxy-cli-token`
request header.

This allows CLI subcommands (`agentproxy status`, `agentproxy providers`, etc.)
...
3. The CLI sends the token as `x-agentproxy-cli-token` only when the resolved
   ...
   and `agentproxy doctor` reports the failure instead of treating an empty token
   as valid.
...
Set `AGENTPROXY_CLI_SALT` to rotate the derived token without code changes.
...
export AGENTPROXY_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
agentproxy status
```

Default salt: `agentproxy-cli-auth-v1`
...
**Opt-out:** set `AGENTPROXY_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
