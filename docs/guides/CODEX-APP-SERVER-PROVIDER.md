...
AgentProxy exposes **two** ways to use OpenAI Codex:
...
| **`codex-app-server`** | Drives the **Codex CLI's own `codex app-server`** over JSON-RPC/WebSocket; the CLI owns and self-refreshes its OAuth (`~/.codex/auth.json`) exactly like an interactive `codex` session | **No** — AgentProxy never replays a token to the API |
...
```
┌─ AgentProxy app ─────────────────┐        ┌─ codex-app-server sidecar ─────────┐
...
- The Codex CLI is baked into `agentproxy:base`, so no codex install is needed on
...
`AGENTPROXY_CODEX_APPSERVER_WS_TOKEN_FILE`. No manual token wiring needed.
...
- **Bare-metal AgentProxy (no sidecar, host codex)** — point
  `AGENTPROXY_CODEX_APPSERVER_WS` at your own `codex app-server` and ensure the
