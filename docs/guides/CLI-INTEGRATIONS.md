...
AgentProxy ships a family of `setup-*` commands that configure a coding
CLI (Codex, Claude Code, OpenCode, Cline, …) to use AgentProxy as its backend — so
the tool talks to **one** endpoint and AgentProxy routes to the right provider with
auto-fallback. Each command reads the **live** model catalog from a running
AgentProxy (local or remote) and writes the tool's own config file on **your**
...
There is also a generic launcher — `agentproxy run <target>` — that spawns
...
`open-code`, `qwen-code`, `gemini-cli`), and `agentproxy completion` offers the
same manifest-derived target words. The legacy per-tool launchers —
`agentproxy launch` (Claude Code) and `agentproxy launch-codex` (Codex) — remain
available.
...
- [Remote Mode](./REMOTE-MODE.md) — drive a remote AgentProxy (VPS / Tailnet) from your laptop
...
Every command honours the **active context** (set with `agentproxy connect`, see
...
| -------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — one profile per compatible text model (`codex --profile <name>`)                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Both            |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — one profile per matched model (`CLAUDE_CONFIG_DIR`)                                                                | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Both            |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — `agentproxy` provider with every catalog model (`opencode -m agentproxy/<model>`)                                           | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Both            |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + prints VS Code extension settings                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Both            |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + merges `kilocode.*` into VS Code `settings.json` if present                                                            | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Both            |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` models, key via `${{ secrets.AGENTPROXY_API_KEY }}`                                                              | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Both            |
| `agentproxy setup-cursor`   | Cursor                       | Nothing — prints the in-app steps (Cursor config is opaque SQLite)                                                                                             | `--remote` `--api-key` `--only` `--port`                                                                                                   | Both            |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (import doc) + sets `roo-cline.autoImportSettingsPath` if a VS Code `settings.json` exists                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Both            |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` provider, key via `$AGENTPROXY_API_KEY`                                                                          | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Both            |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + prints env recipe                                                               | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Both            |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + prints env recipe                                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Both            |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` array + `AGENTPROXY_API_KEY` in `~/.qwen/.env`                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Both            |
| `agentproxy setup-5dive`    | 5dive (agent fleet)          | Nothing under `$HOME` — writes a 5dive **auth profile** (`/var/lib/5dive/auth-profiles/<name>/`) via `5dive agent auth set`; root-only, runs on the fleet host | `--remote` `--api-key` `--model` `--auth-profile` `--agent` `--byo-provider` `--fivedive-bin` `--no-sudo` `--yes` `--dry-run` `--port`     | Both            |
| `agentproxy run <target>`   | Runtime launch (generic)     | Nothing — spawn `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` with the right env and args; Qwen and Gemini use a temporary isolated home         | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Both            |
| `agentproxy launch`         | Claude Code                  | Nothing — spawns `claude` with `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injected                                                                            | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Both            |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | Nothing — spawns `codex` with the `agentproxy` provider injected via `-c` flags                                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Both            |

Notes on flags (verified in the command source):

- `--remote <url>` — fetch the catalog from a remote AgentProxy (overrides `--port`
  and the active context). `--api-key <key>` supplies the credential for that
  server (defaults to the `AGENTPROXY_API_KEY` env var, or the active context's token).
...
- `--model <id>` on `agentproxy run` follows the manifest's per-target wiring
  (`bin/cli/cli-manifest.mjs`): **aider** receives `--model openai/<id>` and
  **opencode** `--model agentproxy/<id>` (the prefix is added only when the id
  ...
  **codex** via `-c model_providers.agentproxy.*` args. **Qwen is the only run
  target that hard-requires `--model`** — `agentproxy run qwen` without it exits
  `2` with an explicit error.
- `--port <port>` — local AgentProxy port (default `20128`, ignored when `--remote`
  is set). Present on all `setup-*` and both launchers.
- `agentproxy run` exit codes: the child CLI's own exit code is propagated
...
> There is also a richer plugin integration — `agentproxy setup opencode` — which
> installs `@agentproxy/opencode-plugin`. They are different commands; the table
...
> `@agentproxy/opencode-plugin` for OpenCode v1 and
> `@agentproxy/opencode-plugin-v2` for OpenCode v2. The v2 package is new
...
> it by adding a `plugins` entry to `opencode.json`; `agentproxy setup opencode`
...
With AgentProxy running on `localhost:20128`, just run the setup command for your
...
Point any setup command at a remote AgentProxy with `--remote` + `--api-key`. The
...
nothing for `agentproxy run` to launch — `5dive` is a **configure-only** target.
...
profile then talks to AgentProxy. Three things are specific to this target:
...
Pointing the profile at an AgentProxy **combo** rather than a single model is what
gets the fleet provider failover: when the primary endpoint went hard-down mid-turn
in the run recorded on
[#11578](https://github.com/khanhkit/AgentProxy/issues/11578), the agent finished
...
AgentProxy exposes the OpenAI surface at `/v1`, the Anthropic surface at the root,
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | with `/v1`       | Yes                                         |
...
When you update with `agentproxy update` (after confirming, or with `--apply`),
AgentProxy runs the install with `--include=optional` baked in:

```bash
npm install -g agentproxy@latest --include=optional
```

This is **not** a flag you pass to `agentproxy update` — it is always applied by the
...
Other `agentproxy update` flags (verified in source): `--check` (exit 1 if
...
(and `:streamGenerateContent?alt=sse`) against it — exactly AgentProxy's native
Gemini surface (`/v1beta`). `agentproxy run gemini` wires that automatically:

- `GOOGLE_GEMINI_BASE_URL` → the active AgentProxy base URL (root, no `/v1`);
- `GEMINI_API_KEY` → the resolved AgentProxy credential (option/env/context);
- a **temporary isolated `GEMINI_CLI_HOME`** whose `.gemini/settings.json`
  ...
  never overrides the AgentProxy-directed launch — removed after exit;
...
AgentProxy server, an opt-in harness exists at
...
`AGENTPROXY_SMOKE_TIMEOUT_MS` overrides the 120s per-target timeout.
...
- [Codex CLI configuration](./CODEX-CLI-CONFIGURATION.md) — the one-time `[model_providers.agentproxy]` base setup
