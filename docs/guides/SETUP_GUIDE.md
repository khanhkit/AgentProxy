---
title: "📖 Setup Guide — AgentProxy"
version: 3.8.50
lastUpdated: 2026-08-18
---

# 📖 Setup Guide — AgentProxy

> Complete setup reference for AgentProxy. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/agentproxy-bin) installs AgentProxy and provides a systemd user service.
...
> **Windows note:** By default, AgentProxy uses `%APPDATA%\agentproxy` when the legacy `%USERPROFILE%\.agentproxy` directory is not present. Set `DATA_DIR` to choose a different data-directory location.
...
AgentProxy ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
Combined with env vars (`INITIAL_PASSWORD`, `AGENTPROXY_WS_BRIDGE_SECRET`, etc.), this lets you spin up an AgentProxy instance fully scriptable.
...
| `agentproxy setup`       | Guided CLI onboarding for password and first provider          |
| `agentproxy doctor`      | Run local health checks without starting the server            |
| `agentproxy providers`   | Discover, list, validate, and test providers from CLI          |
| `agentproxy config`      | CLI tool configuration — list, get, set, validate configs      |
| `agentproxy status`      | Offline status dashboard — version, DB, tools, config          |
| `agentproxy logs`        | Stream usage logs from the API (supports `--follow`)           |
| `agentproxy update`      | Check for or apply AgentProxy updates                           |
| `agentproxy provider`    | Manage provider connections — add, list, remove, test, default |
...
Instead of pasting the base URL and key by hand, let AgentProxy write each tool's
...
**remote** AgentProxy, plus `--dry-run` to preview. To launch a CLI with the right
env injected and no config written at all, use the generic launcher
`agentproxy run <target>` (claude, codex, aider, goose, opencode, qwen, gemini);
the legacy per-tool launchers `agentproxy launch` (Claude Code) and
`agentproxy launch-codex` (Codex) remain available.
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), AgentProxy derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, AgentProxy keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, AgentProxy will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run AgentProxy behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your AgentProxy stream/fetch timeouts.
