---
title: "Codex CLI — Configuration with AgentProxy"
version: 3.8.50
lastUpdated: 2026-08-18
---

# Codex CLI — Configuration with AgentProxy

Complete guide for using the Codex CLI pointed at AgentProxy as an OpenAI-compatible backend.
...
> API key stays in `AGENTPROXY_API_KEY` (never in the file), and a leftover legacy
...
### Local unauthenticated AgentProxy: placeholder key is enough

Codex validates that the environment variable named by `env_key` exists
**before** the first request leaves the CLI. If your **local** AgentProxy
instance does not require auth, any non-empty placeholder works:

```bash
export AGENTPROXY_API_KEY="${AGENTPROXY_API_KEY:-local}"
```

Use a real key instead when your AgentProxy server is protected or remote.
...
Many providers, including GLM and Kimi, still expose only a Chat Completions endpoint. DeepSeek V4 now exposes a native Responses API as well as an Anthropic-compatible endpoint; AgentProxy uses Responses by default and lets each DeepSeek connection select Anthropic compatibility.

**AgentProxy solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (AgentProxy)
    → AgentProxy selects the provider's native protocol and translates when needed
    → POST /responses (DeepSeek V4) or /chat/completions (Mistral / GLM / Kimi / others)
```

You never need a separate translation proxy when using AgentProxy. **All models use `wire_api = "responses"`** — AgentProxy handles the rest.
...
| Model                                | AgentProxy ID                         | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in AgentProxy use the `cx/` prefix:

| Codex CLI name          | AgentProxy model    |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the AgentProxy provider alias.
...
### AgentProxy Thinking Budget (server setting)

On the AgentProxy host, **Settings → AI → Thinking Budget** must be **`passthrough`** for Codex effort/summary to reach upstream. Mode **`auto` strips** all client `reasoning` / `reasoning_effort` fields and will empty thinking panels even when Codex is configured correctly.
...
## Generating profiles automatically with `agentproxy setup-codex`

If you run AgentProxy on a VPS, you can auto-generate profile files from the live model catalog:
...
AgentProxy can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `AGENTPROXY_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `agentproxy launch-codex`

Health-checks your AgentProxy instance before launching Codex:
...
# Launch codex with the agentproxy provider injected via -c flags (no config written)
agentproxy run codex
```
...
Two AgentProxy defaults can silently sabotage multi-hour Codex CLI sessions. Neither is a Codex CLI setting — both live on the AgentProxy side. Users migrating a config from upstream proxies that pin accounts and disable idle cutoffs often hit both and conclude AgentProxy “cannot sustain a long session.”
...
Related discussions: [#7126](https://github.com/khanhkit/AgentProxy/discussions/7126) (long task drops), [#5718](https://github.com/khanhkit/AgentProxy/discussions/5718) (why affinity defaults off). Tracking: [#7287](https://github.com/khanhkit/AgentProxy/issues/7287).
...
Critical detail: AgentProxy’s synthetic SSE **heartbeat does not reset** the idle clock. Only a real upstream body chunk updates `lastChunkTime`. A quiet model that is still “thinking” looks identical to a stalled upstream from the watchdog’s point of view.
...
In the AgentProxy process environment (`.env` / compose / systemd):
...
Restart AgentProxy after changing these env vars.
...
2. **Raise / disable idle cutoffs** in AgentProxy’s environment:
...
3. Keep the usual Codex `config.toml` (`wire_api = "responses"`, correct `base_url`, `AGENTPROXY_API_KEY`) — no Codex-side affinity/idle knobs exist for these two behaviors.
4. Restart AgentProxy, then start the long Codex task.
...
When the idle watchdog fires, AgentProxy logs a line shaped like:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grep for `Idle timeout: no data from` (or the code `stream_idle_timeout` / error name `StreamIdleTimeoutError`). The provider segment is whatever AgentProxy used for that request (`codex`, another provider id, or `provider` if unknown) — it is not always the literal string `codex`.
...
Verify the model exists in AgentProxy with the correct prefix. Use `agentproxy models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `AGENTPROXY_API_KEY` is exported: `echo $AGENTPROXY_API_KEY`.

**`ERROR: Missing environment variable: AGENTPROXY_API_KEY`**
...
`AGENTPROXY_API_KEY=local` when your **local** AgentProxy instance does not
require auth. Restart the shell if you added it to `~/.bashrc` or `~/.zshrc`.

**`Connection refused`**
Verify AgentProxy is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
...
See [Long-running tasks](#long-running-tasks). Enable session affinity (TTL above task length) and raise or disable `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grep AgentProxy logs for `Idle timeout: no data from`.
