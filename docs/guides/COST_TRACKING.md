...
How AgentProxy estimates, records, and reports the cost of every request — and why the
...
AgentProxy attributes a per-request USD cost to every completion by multiplying token
counts by a model's pricing rates. These numbers power the **Costs** dashboard, the
`agentproxy cost` / `agentproxy usage` CLI, CSV/JSON exports, and per-API-key budgets.

> **The dashboard "cost" is a savings tracker, not a bill.** AgentProxy never charges you
...
- It depends on the pricing table AgentProxy has for each model. A model with no pricing
...
3. **Hardcoded defaults** — shipped with AgentProxy.
...
- API-key spend is buffered and flushed in batches by the
  ...
  | Env var                             | Default | Purpose                            |
  | ----------------------------------- | ------- | ---------------------------------- |
  | `AGENTPROXY_SPEND_FLUSH_INTERVAL_MS` | `60000` | Flush interval in milliseconds.    |
  | `AGENTPROXY_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Max buffered entries before flush. |
...
AgentProxy's CLI exposes cost, usage, and pricing commands (registered in
...
- **All costs show $0 / "Legacy / Free".** The models in use have no pricing entry.
  Enable external sync (`PRICING_SYNC_ENABLED=true`) and run `agentproxy pricing sync`, or
...
- **Spend lags behind real time.** Per-key spend is batched; lower
  `AGENTPROXY_SPEND_FLUSH_INTERVAL_MS` if you need fresher numbers.
