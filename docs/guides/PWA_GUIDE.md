...
AgentProxy ships as a fully installable Progressive Web App. When you access the dashboard from any mobile browser — Android (Chrome) or iOS (Safari) — you can "Add to Home Screen" and get a native app-like experience with no app store required.

## What Is a PWA?

A Progressive Web App turns the AgentProxy web dashboard into something that looks and feels like a native mobile app. Once installed, it:
...
1. Open the AgentProxy dashboard in Chrome: `http://YOUR_IP:20128`
2. Chrome will show an **"Add AgentProxy to Home screen"** banner automatically, or:
   - Tap the **⋮** menu (three dots) → **"Add to Home screen"** or **"Install app"**
3. Confirm the prompt
4. AgentProxy appears on your home screen as a standalone app

### iOS (Safari)

1. Open the AgentProxy dashboard in Safari: `http://YOUR_IP:20128`
2. Tap the **Share** button (box with arrow)
3. Scroll down and tap **"Add to Home Screen"**
4. Name it (defaults to "AgentProxy") and tap **Add**
5. AgentProxy appears on your home screen with the app icon

### Desktop (Chrome / Edge)

1. Open the AgentProxy dashboard
2. Click the **install icon** in the address bar (or ⋮ → "Install AgentProxy...")
3. Confirm the prompt
4. AgentProxy opens as a standalone window — no tabs, no address bar
...
AgentProxy includes a service worker (`sw.js`) that provides intelligent caching:
...
AgentProxy provides icons optimized for each platform:
...
- **Cache versioning**: `agentproxy-pwa-v3` — bump this to force a fresh cache on update
...
When running AgentProxy on Android via Termux, the PWA works seamlessly:

1. Start AgentProxy in Termux: `npx agentproxy`
...
Install the PWA on any device that has browser access to your AgentProxy server:
