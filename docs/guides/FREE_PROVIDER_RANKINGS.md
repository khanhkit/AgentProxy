...
> **TL;DR**: AgentProxy ranks its **free** providers by model quality using **Arena AI
...
AgentProxy registers 329 providers, including 155 catalog entries marked **free/no-auth**
(no-auth,
...
Leaderboard categories map to AgentProxy task categories:

| Arena leaderboard | AgentProxy task categories                         |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, AgentProxy keeps
...
stripped and a small alias map expands canonical names into the variants AgentProxy uses
