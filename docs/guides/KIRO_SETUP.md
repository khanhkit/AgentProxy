...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to AgentProxy,
...
## How AgentProxy Solves This (v3.8.0+)

Starting with v3.8.0, AgentProxy calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each AgentProxy connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; AgentProxy reads credentials from the
...
4. AgentProxy validates the key and saves the connection.
...
(`clientSecretExpiresAt`). AgentProxy stores this timestamp in `providerSpecificData`
...
- Ensure AgentProxy can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise AgentProxy cannot
