...
AgentProxy supports **51 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
AgentProxy uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through AgentProxy
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `AGENTPROXY_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `AGENTPROXY_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `AGENTPROXY_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
```bash
# needs AGENTPROXY_TRANSLATION_API_URL / _API_KEY / _MODEL in .env
...
- **Chunked batching**: Joins multiple strings with `__AGENTPROXY_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including AgentProxy itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `agentproxy` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `agentproxy --lang de status`            |
| 2        | `AGENTPROXY_LANG` env var | `AGENTPROXY_LANG=ja agentproxy providers` |
...
The saved preference is written atomically to `~/.agentproxy/.env` and is loaded by the
...
(`AgentProxy`, `OAuth`, `MCP`, `A2A`, `DATA_DIR`, …) that must appear verbatim
