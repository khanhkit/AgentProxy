...
AgentProxy ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
| App version        | `3.8.0`                                                   |
| App id             | `online.agentproxy.desktop`                                |
| Product name       | `AgentProxy`                                               |
...
- Close button minimizes to tray; the tray menu has **Open AgentProxy**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
Uses `electron-updater` with the GitHub provider (`khanhkit/AgentProxy`).
...
Releases are published to GitHub Releases (`khanhkit/AgentProxy`), which is also where `electron-updater` checks for new versions.
