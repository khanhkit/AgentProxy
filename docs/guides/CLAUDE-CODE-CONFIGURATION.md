---
title: "Claude Code CLI — Configuration with AgentProxy"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — Configuration with AgentProxy

Point the **Claude Code** CLI (`claude`) at AgentProxy — local or a remote VPS —
...
| `ANTHROPIC_AUTH_TOKEN`                       | Sent as `Authorization: Bearer …` — use your AgentProxy access token / API key          |
...
`agentproxy launch` sets all of these for you: it resolves the base URL + token
from the active context (so `agentproxy connect <vps>` then `agentproxy launch`
...
`/model` picker normally shows **only** AgentProxy's Claude/Anthropic models — a
`kimi/kimi-k2.6` or `glm/glm-5.2` never appears, even though it routes fine.

AgentProxy can mirror any enabled model (and combo) under a `claude/…` id so it
...
When you pick one of these in Claude Code, AgentProxy strips the `claude/` wrapper
...
wins), so a plain AgentProxy never doubles its catalog for clients that don't use
Claude Code:
...
`agentproxy setup-claude` fetches the live `/v1/models` catalog and writes one
...
> `agentproxy launch --profile <name>` (it injects `ANTHROPIC_AUTH_TOKEN` from the
> active context), or export `ANTHROPIC_AUTH_TOKEN` yourself and run
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync after model discovery (opt-in).** AgentProxy can regenerate these same
...
auto-sync" → Claude Code), or set `AGENTPROXY_AUTO_SYNC_CLAUDE_PROFILES=true` (it also honors
...
Claude Code routes to capability tiers. Map each to an AgentProxy model via env /
...
Once you've run `agentproxy connect <host>` (see
[Remote Mode](./REMOTE-MODE.md)), `agentproxy launch` and `agentproxy setup-claude`
...
`/v1`** and restart `claude` (env is read once at startup). `agentproxy launch`
...
AgentProxy refuses to guess. Fix it either way: pin a prefixed id with
...
`AGENTPROXY_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (default off;
...
**Auth errors** — the profile holds no token. Use `agentproxy launch --profile`
