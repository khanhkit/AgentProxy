...
1. **Not in `FREE_MODEL_BUDGETS` at all** → excluded. This covers genuinely paid models and any
   provider/model AgentProxy hasn't classified yet — new candidates start excluded, not included.
...
catalog, with a usage adapter, `hardStopGuaranteed: true`) is usable the moment AgentProxy knows
