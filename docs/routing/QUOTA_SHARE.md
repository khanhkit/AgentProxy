...
**Problem it solves:** AgentProxy proxies many API keys against the same upstream
...
- All persistence is in the existing AgentProxy SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("agentproxy:quota-share:pools")` is non-empty.
