...
AgentProxy **aggregates** these free tiers into one endpoint. Instead of signing up for 10 different services, you connect them all to AgentProxy and use `model: "auto"` to automatically pick the best free option for each request.
...
The magic of AgentProxy is **stacking free tiers**. Instead of relying on one provider, you connect multiple free providers and let AgentProxy automatically pick the best one for each request.
...
Then use `model: "auto"` and AgentProxy will:
...
- `NOAUTH` means AgentProxy does not ask you for a provider credential; it does not
...
## How AgentProxy Makes Free Tiers Better

### 1. Automatic Fallback

If one free provider is busy or down, AgentProxy automatically tries the next one. You don't need to do anything.

### 2. Smart Routing

AgentProxy picks the **best free provider** for each request based on:
...
AgentProxy's compression pipeline can reduce eligible prompt and tool-output tokens. The
...
If provider terms permit multiple accounts or credentials, AgentProxy can treat each
...
account-policy risk. AgentProxy surfaces the available metadata; you choose what to enable.
