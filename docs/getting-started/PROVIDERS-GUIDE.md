---
title: "Providers Guide: Connect AI Models to AgentProxy"
version: 3.8.50
lastUpdated: 2026-08-06
---

# Providers Guide: Connect AI Models to AgentProxy

> **TL;DR**: A provider is a connection to an AI service (like OpenAI, Anthropic, Google). You need at least one provider to use AgentProxy.

---

## What Is a Provider?

Think of a provider like a **phone carrier**. Just as you need a phone carrier to make calls, you need an AI provider to use AI models. AgentProxy is like a phone that works with **all carriers** — you can switch between them automatically.
...
eligible list from AgentProxy's no-auth provider registry, then lets you review and deselect each
provider before confirming. AgentProxy shows the provider's caution notice and a link to its site
...
For Apple Silicon Macs with unified memory, AgentProxy supports connecting to local MLX models running via `mlx-lm.server` as regular OpenAI-compatible local providers.
...
3. **Connect in AgentProxy Dashboard**:
...
4. **Use with OpenCode**:
   ```bash
   # Configure OpenCode to use AgentProxy
   opencode config set api.base_url http://localhost:20128/v1
   opencode config set api.key <your-agentproxy-api-key>
...
AgentProxy does not automatically manage MLX server processes — it only routes requests to the OpenAI-compatible endpoints you configure.
...
| Tool calling fails | Ensure model supports tools; check AgentProxy logs for translation errors      |
...
AgentProxy works best with **multiple providers**. This gives you:

- **Automatic fallback** — If one provider fails, AgentProxy tries the next
- **Cost optimization** — AgentProxy picks the cheapest provider for each request
- **Speed optimization** — AgentProxy picks the fastest provider for each request
- **Quality optimization** — AgentProxy picks the best provider for each task
...
Then use `model: "auto"` and AgentProxy will automatically pick the best one for each request.
...
2. In AgentProxy: Providers → Add Provider → OpenAI
3. Paste API key → Connect

### Anthropic

1. Get API key: https://console.anthropic.com/
2. In AgentProxy: Providers → Add Provider → Anthropic
3. Paste API key → Connect

### Google (Gemini)

1. Get API key: https://aistudio.google.com/apikey
2. In AgentProxy: Providers → Add Provider → Gemini
3. Paste API key → Connect

### DeepSeek

1. Get API key: https://platform.deepseek.com/
2. In AgentProxy: Providers → Add Provider → DeepSeek
3. Paste API key → Connect

### Groq

1. Get API key: https://console.groq.com/
2. In AgentProxy: Providers → Add Provider → Groq
3. Paste API key → Connect
...
### "Do I need to pay to use AgentProxy?"

**No!** AgentProxy is free and open-source. You can use free providers (Kiro, OpenCode Free, Pollinations) without paying anything. You only pay if you choose to use paid providers.
...
**Yes!** That's the whole point of AgentProxy. Connect multiple providers and use `model: "auto"` to let AgentProxy pick the best one for each request.

### "What if a provider goes down?"

AgentProxy automatically skips failed providers and tries the next one. You don't need to do anything.
...
**Yes!** If you already have API keys for OpenAI, Anthropic, Google, etc., you can use them in AgentProxy. Just paste them when connecting the provider.

---

## What's Next?

- **[Auto-Combo Guide](./AUTO-COMBO-GUIDE.md)** — Let AgentProxy pick the best AI for you
