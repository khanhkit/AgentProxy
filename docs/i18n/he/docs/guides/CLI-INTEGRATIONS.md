...
AgentProxy מספקת משפחה של פקודות `setup-*` שמגדירות CLI קוד (Codex, Claude Code, OpenCode, Cline, …) להשתמש ב-AgentProxy כ-backend שלה — כך שהכלי מדבר עם **נקודת קצה אחת** ו-AgentProxy מנתבת לספק הנכון עם חזרה אוטומטית. כל פקודה קוראת את הקטלוג של המודל **החי** מ-AgentProxy פועל (מקומי או מרוחק) וכותבת את קובץ הקונפיגורציה של הכלי על **המחשב שלך**. מפתח ה-API מתייחס על ידי משתנה סביבה בכל מקום שהכלי תומך בו. פקודות ששומרות קובץ סביבה מקומי של הכלי מצוינות למטה.

יש גם מפעיל כללי — `agentproxy run <target>` — שמפעיל `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` או `gemini` עם הסביבה הנכונה מוזרקת, מבלי לכתוב שום קונפיגורציה בכלל. היעדים והכינויים שלהם מגיעים מהמניפסט הקנוני `bin/cli/cli-manifest.mjs`
(`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`,
`open-code`, `qwen-code`, `gemini-cli`), ו-`agentproxy completion` מציע את
אותן מילים נגזרות מהמניפסט. המפעילים הישנים לכל כלי —
`agentproxy launch` (Claude Code) ו-`agentproxy launch-codex` (Codex) — נשארים
זמינים.
...
- [מצב מרוחק](./REMOTE-MODE.md) — הפעל AgentProxy מרוחק (VPS / Tailnet) מהמחשב הנייד שלך
...
כל פקודה מכבדת את **ההקשר הפעיל** (מוגדר עם `agentproxy connect`, ראה
...
| -------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI         | `~/.codex/<name>.config.toml` — פרופיל אחד לכל מודל טקסט תואם (`codex --profile <name>`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | שניהם           |
| `agentproxy setup-claude`   | Claude Code              | `~/.claude/profiles/<name>/settings.json` — פרופיל אחד לכל מודל תואם (`CLAUDE_CONFIG_DIR`)                                                    | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | שניהם           |
| `agentproxy setup-opencode` | OpenCode (תואם ל-openai) | `~/.config/opencode/opencode.json` — ספק `agentproxy` עם כל מודל בקטלוג (`opencode -m agentproxy/<model>`)                                      | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | שניהם           |
| `agentproxy setup-cline`    | Cline                    | `~/.cline/data/{globalState,secrets}.json` (מצב CLI) + מדפיס הגדרות תוסף VS Code                                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | שניהם           |
| `agentproxy setup-kilo`     | Kilo Code                | `~/.local/share/kilo/auth.json` (CLI) + ממזג `kilocode.*` לתוך `settings.json` של VS Code אם קיים                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | שניהם           |
| `agentproxy setup-continue` | Continue / `cn` CLI      | `~/.continue/config.yaml` — מודלים `provider: openai`, מפתח דרך `${{ secrets.AGENTPROXY_API_KEY }}`                                            | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | שניהם           |
| `agentproxy setup-cursor`   | Cursor                   | כלום — מדפיס את הצעדים באפליקציה (הגדרת Cursor היא SQLite אטומה)                                                                              | `--remote` `--api-key` `--only` `--port`                                                                                                   | שניהם           |
| `agentproxy setup-roo`      | Roo Code                 | `~/.agentproxy/roo-settings.json` (מסמך ייבוא) + קובע `roo-cline.autoImportSettingsPath` אם קיים `settings.json` של VS Code                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | שניהם           |
| `agentproxy setup-crush`    | Crush                    | `~/.config/crush/crush.json` — ספק תואם ל-openai, מפתח דרך `$AGENTPROXY_API_KEY`                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | שניהם           |
| `agentproxy setup-goose`    | Goose                    | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + מדפיס מתכון סביבה                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | שניהם           |
| `agentproxy setup-aider`    | Aider                    | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + מדפיס מתכון סביבה                                                            | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | שניהם           |
| `agentproxy setup-qwen`     | Qwen Code                | `~/.qwen/settings.json` — מערך `modelProviders.openai` V4 + `AGENTPROXY_API_KEY` ב-`~/.qwen/.env`                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | שניהם           |
| `agentproxy run <target>`   | הפעלת זמן ריצה (כללית)   | כלום — מפעיל `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` עם הסביבה והארגומנטים הנכונים; Qwen ו-Gemini משתמשים בבית מבודד זמני | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | שניהם           |
| `agentproxy launch`         | Claude Code              | כלום — מפעיל `claude` עם `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` מוזרקים                                                                  | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | שניהם           |
| `agentproxy launch-codex`   | OpenAI Codex CLI         | כלום — מפעיל `codex` עם ספק `agentproxy` מוזרק דרך דגלי `-c`                                                                                   | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | שניהם           |

הערות על דגלים (מאומתים במקור הפקודה):

- `--remote <url>` — שולף את הקטלוג מ-AgentProxy מרוחק (מחליף את `--port`
  ואת ההקשר הפעיל). `--api-key <key>` מספק את האישור עבור השרת
  (ברירת מחדל היא משתנה הסביבה `AGENTPROXY_API_KEY`, או הטוקן של ההקשר הפעיל).
...
- `--model <id>` על `agentproxy run` עוקבת אחרי החיווט לפי המניפסט
  (`bin/cli/cli-manifest.mjs`): **aider** מקבלת `--model openai/<id>` ו
  **opencode** `--model agentproxy/<id>` (הקידומת מתווספת רק כאשר ה-id
  אינו נושא אותה כבר); **qwen** ו**gemini** מקבלות את ה-id כפי שהוא;
  **claude** מקבלת אותו דרך `ANTHROPIC_MODEL`, **goose** דרך `GOOSE_MODEL`, ו
  **codex** דרך `-c model_providers.agentproxy.*` args. **Qwen הוא היעד היחיד
  שדורש באופן מוחלט `--model`** — `agentproxy run qwen` בלעדיו יוצא
  `2` עם שגיאה מפורשת.
- `--port <port>` — פורט AgentProxy מקומי (ברירת מחדל `20128`, מתעלם כאשר `--remote`
  מוגדר). נוכח על כל `setup-*` ועל שני המפעילים.
- קודי יציאה של `agentproxy run`: קוד היציאה של ה-CLI הילד מועבר
...
> יש גם אינטגרציה עשירה יותר של תוסף — `agentproxy setup opencode` — שמתקינה
> `@agentproxy/opencode-plugin`. אלו פקודות שונות; הטבלה
...
עם AgentProxy פועל על `localhost:20128`, פשוט הרץ את פקודת ההגדרה עבור הכלי שלך. הקטלוג נמשך מהשרת המקומי.
...
export AGENTPROXY_API_KEY=sk-...  # מתייחס דרך {env:AGENTPROXY_API_KEY}, אף פעם לא על דיסק
...
```bash
agentproxy launch                 # Claude Code → AgentProxy המקומי
agentproxy launch-codex           # Codex CLI → AgentProxy המקומי
...
כוון כל פקודת הגדרה ל-AgentProxy מרחוק עם `--remote` + `--api-key`. הקטלוג נמשך מהמרחוק; הקונפיגורציה נכתבת במחשב המקומי שלך.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # ייצא קודם את AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # מייצר טוקן עם טווח, שומר את ההקשר
agentproxy setup-codex                 # ← עכשיו משתמש בקטלוג המרוחק
agentproxy setup-opencode              # ← אותו דבר
agentproxy launch                      # ← Claude Code נגד המרוחק
```
...
AgentProxy מציע את הממשק של OpenAI ב-`/v1`, את הממשק של Anthropic בשורש,
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | עם `/v1`        | כן                                        |
...
כאשר אתה מעדכן עם `agentproxy update` (לאחר אישור, או עם `--apply`),
AgentProxy מריץ את ההתקנה עם `--include=optional` כלול:

```bash
npm install -g agentproxy@latest --include=optional
```

זה **לא** דגל שאתה מעביר ל`agentproxy update` — הוא תמיד מוחל על ידי
...
דגלים אחרים של `agentproxy update` (מאומתים במקור): `--check` (יוצא 1 אם
מעודכן), `--apply` (מתקין מבלי לבקש אישור), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI דרך `agentproxy run gemini`
...
(וגם `:streamGenerateContent?alt=sse`) נגדו — בדיוק כמו הממשק המקומי של AgentProxy
(`/v1beta`). `agentproxy run gemini` מחבר את זה אוטומטית:

- `GOOGLE_GEMINI_BASE_URL` → ה-URL הבסיסי הפעיל של AgentProxy (שורש, ללא `/v1`);
- `GEMINI_API_KEY` → האישור שנפתר של AgentProxy (אפשרות/סביבה/הקשר);
- **`GEMINI_CLI_HOME` מבודד זמני** שבו `.gemini/settings.json`
  בוחר באימות `gemini-api-key`, כך שסשן OAuth של Google מאוחסן (Code Assist)
  לא יחליף את ההשקה המנוהלת על ידי AgentProxy — נמחק לאחר היציאה;
...
AgentProxy אמיתי, קיים מנגנון אופציה ב
...
אופציונלי: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` מגביל את הסוויפ;
`AGENTPROXY_SMOKE_TIMEOUT_MS` עוקף את מגבלת הזמן של 120 שניות לכל יעד.

## ראה גם

- [הגדרת קוד קלוד](./CLAUDE-CODE-CONFIGURATION.md) — המדריך העמוק יותר לקוד קלוד
- [הגדרת CLI של קודקס](./CODEX-CLI-CONFIGURATION.md) — ההגדרה הבסיסית של `[model_providers.agentproxy]` פעם אחת
