...
**AgentProxy** — einheitlicher KI-Proxy/Router. Ein Endpunkt, 329 LLM-Anbieter, automatischer Fallback.
...
AgentProxy hat drei verwandte, aber unterschiedliche Mechanismen für temporäre Fehler. Halten Sie ihren
...
- **Imports**: extern → intern (`@/`, `@agentproxy/open-sse`) → relativ
...
- **Pfad-Aliase**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + Dashboard am selben Port)
- **Datenverzeichnis**: `DATA_DIR` Umgebungsvariable, standardmäßig `~/.agentproxy/`
