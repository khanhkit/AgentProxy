...
AgentProxy एक परिवार के `setup-*` कमांड्स के साथ आता है जो एक कोडिंग
CLI (Codex, Claude Code, OpenCode, Cline, …) को AgentProxy को उसके बैकएंड के रूप में उपयोग करने के लिए कॉन्फ़िगर करता है — ताकि
उपकरण **एक** एंडपॉइंट से बात करे और AgentProxy सही प्रदाता की ओर रूट करता है
ऑटो-फॉलबैक के साथ। प्रत्येक कमांड एक चल रहे
AgentProxy (स्थानीय या दूरस्थ) से **लाइव** मॉडल कैटलॉग पढ़ता है और **आपके**
...
एक सामान्य लॉन्चर भी है — `agentproxy run <target>` — जो
...
`open-code`, `qwen-code`, `gemini-cli`) से प्राप्त किया जाता है, और `agentproxy completion` वही मैनिफेस्ट-व्युत्पन्न लक्ष्य शब्द प्रदान करता है। पुराने प्रति-उपकरण लॉन्चर —
`agentproxy launch` (Claude Code) और `agentproxy launch-codex` (Codex) — उपलब्ध रहते हैं।
...
- [दूरस्थ मोड](./REMOTE-MODE.md) — अपने लैपटॉप से एक दूरस्थ AgentProxy (VPS / Tailnet) चलाएं
...
हर कमांड **सक्रिय संदर्भ** का सम्मान करता है (जिसे `agentproxy connect` के साथ सेट किया गया है, देखें
...
| -------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — एक संगत पाठ मॉडल के लिए एक प्रोफ़ाइल (`codex --profile <name>`)                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | दोनों               |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — एक मेल खाने वाले मॉडल के लिए एक प्रोफ़ाइल (`CLAUDE_CONFIG_DIR`)                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | दोनों               |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — `agentproxy` प्रदाता के साथ हर कैटलॉग मॉडल (`opencode -m agentproxy/<model>`)                                                      | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | दोनों               |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI मोड) + VS Code एक्सटेंशन सेटिंग्स प्रिंट करता है                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | दोनों               |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + यदि मौजूद हो तो VS Code `settings.json` में `kilocode.*` को मर्ज करता है                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | दोनों               |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` मॉडल, कुंजी के माध्यम से `${{ secrets.AGENTPROXY_API_KEY }}`                                                            | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | दोनों               |
| `agentproxy setup-cursor`   | Cursor                       | कुछ नहीं — ऐप में चरणों को प्रिंट करता है (Cursor कॉन्फ़िगरेशन अपारदर्शी SQLite है)                                                                                   | `--remote` `--api-key` `--only` `--port`                                                                                                   | दोनों               |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (आयात दस्तावेज़) + यदि एक VS Code `settings.json` मौजूद है तो `roo-cline.autoImportSettingsPath` सेट करता है                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | दोनों               |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` प्रदाता, कुंजी के माध्यम से `$AGENTPROXY_API_KEY`                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | दोनों               |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + पर्यावरण नुस्खा प्रिंट करता है                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | दोनों               |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + पर्यावरण नुस्खा प्रिंट करता है                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | दोनों               |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` ऐरे + `AGENTPROXY_API_KEY` `~/.qwen/.env` में                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | दोनों               |
| `agentproxy run <target>`   | रनटाइम लॉन्च (सामान्य)       | कुछ नहीं — सही वातावरण और तर्कों के साथ `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` को स्पॉन करता है; Qwen और Gemini अस्थायी अलग घर का उपयोग करते हैं | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | दोनों               |
| `agentproxy launch`         | Claude Code                  | कुछ नहीं — `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` को इंजेक्ट करके `claude` को स्पॉन करता है                                                                      | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | दोनों               |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | कुछ नहीं — `-c` फ्लैग्स के माध्यम से `agentproxy` प्रदाता को इंजेक्ट करके `codex` को स्पॉन करता है                                                                     | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | दोनों               |

फ्लैग्स पर नोट्स (कमांड स्रोत में सत्यापित):

- `--remote <url>` — एक दूरस्थ AgentProxy से कैटलॉग लाता है (यह `--port`
  और सक्रिय संदर्भ को ओवरराइड करता है)। `--api-key <key>` उस
  सर्वर के लिए क्रेडेंशियल प्रदान करता है (डिफ़ॉल्ट रूप से `AGENTPROXY_API_KEY` पर्यावरण चर, या सक्रिय संदर्भ के टोकन पर)।
...
- `--model <id>` पर `agentproxy run` मैनिफेस्ट के प्रति-लक्ष्य वायरिंग का पालन करता है
  (`bin/cli/cli-manifest.mjs`): **aider** को `--model openai/<id>` प्राप्त होता है और
  **opencode** को `--model agentproxy/<id>` (प्रिफिक्स केवल तब जोड़ा जाता है जब आईडी
  ...
  **codex** को `-c model_providers.agentproxy.*` तर्कों के माध्यम से। **Qwen एकमात्र रन
  लक्ष्य है जिसे `--model` की सख्त आवश्यकता है** — `agentproxy run qwen` इसके बिना `2` के साथ
  स्पष्ट त्रुटि के साथ समाप्त होता है।
- `--port <port>` — स्थानीय AgentProxy पोर्ट (डिफ़ॉल्ट `20128`, जब `--remote`
  सेट होता है तो अनदेखा किया जाता है)। सभी `setup-*` और दोनों लॉन्चरों पर मौजूद है।
- `agentproxy run` निकासी कोड: बच्चे CLI का अपना निकासी कोड वर्बटिम प्रकट होता है;
...
> एक समृद्ध प्लगइन इंटीग्रेशन भी है — `agentproxy setup opencode` — जो
> `@agentproxy/opencode-plugin` स्थापित करता है। ये अलग-अलग कमांड हैं; ऊपर की तालिका
...
जब AgentProxy `localhost:20128` पर चल रहा हो, तो बस अपने टूल के लिए सेटअप कमांड चलाएँ। कैटलॉग स्थानीय सर्वर से लाया जाता है।
...
export AGENTPROXY_API_KEY=sk-...  # {env:AGENTPROXY_API_KEY} के माध्यम से संदर्भित, कभी भी डिस्क पर नहीं
...
```bash
agentproxy launch                 # Claude Code → स्थानीय AgentProxy
agentproxy launch-codex           # Codex CLI → स्थानीय AgentProxy
...
agentproxy run claude -- --print-system-prompt "इस अंतर की समीक्षा करें"
```
...
किसी भी सेटअप कमांड को `--remote` + `--api-key` के साथ एक दूरस्थ AgentProxy पर इंगित करें। कैटलॉग दूरस्थ से लाया जाता है; कॉन्फ़िगरेशन आपके स्थानीय मशीन पर लिखा जाता है।
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # पहले AGENTPROXY_API_KEY निर्यात करें
...
```bash
agentproxy connect 192.168.0.15        # एक स्कोप्ड टोकन बनाता है, संदर्भ को संग्रहीत करता है
agentproxy setup-codex                 # ← अब दूरस्थ कैटलॉग का उपयोग करता है
agentproxy setup-opencode              # ← वही
agentproxy launch                      # ← Claude Code दूरस्थ के खिलाफ
```
...
AgentProxy OpenAI सतह को `/v1` पर, एंथ्रोपिक सतह को रूट पर, और एक मूल जेमिनी सतह को `/v1beta` पर उजागर करता है। प्रत्येक एकीकरण उस रूप में वायर्ड है जिसकी इसकी टूल अपेक्षा करती है (कमांड स्रोत में सत्यापित):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | `/v1` के साथ     | हाँ                                             |
...
जब आप `agentproxy update` के साथ अपडेट करते हैं (पुष्टि करने के बाद, या `--apply` के साथ),
AgentProxy `--include=optional` के साथ इंस्टॉल चलाता है:

```bash
npm install -g agentproxy@latest --include=optional
```

यह **नहीं** है एक ध्वज जिसे आप `agentproxy update` को पास करते हैं — यह हमेशा
...
# [DRY RUN] चलाएगा: npm install -g agentproxy@latest --include=optional
```

अन्य `agentproxy update` ध्वज (स्रोत में सत्यापित): `--check` (यदि पुराना है तो
एक्ज़िट 1), `--apply` (बिना संकेत के इंस्टॉल करें), `--changelog`, `--no-backup`,
`--yes`।

---

## Google Gemini CLI के माध्यम से `agentproxy run gemini`
...
(और `:streamGenerateContent?alt=sse`) इसके खिलाफ जारी करता है — बिल्कुल AgentProxy का
स्थानीय Gemini सतह (`/v1beta`)। `agentproxy run gemini` इसे स्वचालित रूप से कनेक्ट करता है:

- `GOOGLE_GEMINI_BASE_URL` → सक्रिय AgentProxy बेस URL (रूट, कोई `/v1` नहीं);
- `GEMINI_API_KEY` → हल किया गया AgentProxy क्रेडेंशियल (विकल्प/पर्यावरण/संदर्भ);
- एक **अस्थायी अलग `GEMINI_CLI_HOME`** जिसका `.gemini/settings.json`
  ...
  (कोड सहायता) कभी भी AgentProxy-निर्देशित लॉन्च को ओवरराइड न करे — निकासी के बाद हटा दिया जाता है;
...
`tests/unit/cli/run-execution.test.ts`)। एक वास्तविक AgentProxy सर्वर के खिलाफ वास्तविक
...
वैकल्पिक: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` स्वेप को सीमित करता है;
`AGENTPROXY_SMOKE_TIMEOUT_MS` 120 सेकंड प्रति-लक्ष्य टाइमआउट को ओवरराइड करता है।
...
- [Codex CLI कॉन्फ़िगरेशन](./CODEX-CLI-CONFIGURATION.md) — एक बार का `[model_providers.agentproxy]` बेस सेटअप
