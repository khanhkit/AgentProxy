...
title: "Ενσωματώσεις CLI — συνδέστε οποιοδήποτε coding CLI με το AgentProxy"
version: 3.8.50
...
Το AgentProxy παρέχει μια οικογένεια εντολών `setup-*` που διαμορφώνουν ένα coding
CLI (Codex, Claude Code, OpenCode, Cline, …) ώστε να χρησιμοποιεί το AgentProxy ως backend — έτσι
το εργαλείο επικοινωνεί με **ένα** endpoint και το AgentProxy δρομολογεί στον κατάλληλο πάροχο με
αυτόματη εναλλαγή σε εφεδρικό. Κάθε εντολή διαβάζει τον **ενεργό** κατάλογο μοντέλων από ένα τρέχον
AgentProxy (τοπικό ή απομακρυσμένο) και γράφει το δικό του αρχείο ρυθμίσεων του εργαλείου στη **δική σας**
...
Υπάρχει επίσης ένας γενικός εκκινητής — `agentproxy run <target>` — που εκκινεί
...
`open-code`, `qwen-code`, `gemini-cli`), και το `agentproxy completion` προσφέρει τις
ίδιες λέξεις-στόχους που προέρχονται από το manifest. Οι παλαιοί εκκινητές ανά εργαλείο —
`agentproxy launch` (Claude Code) και `agentproxy launch-codex` (Codex) — παραμένουν
διαθέσιμοι.
...
- [Απομακρυσμένη λειτουργία](./REMOTE-MODE.md) — χειριστείτε ένα απομακρυσμένο AgentProxy (VPS / Tailnet) από το laptop σας
...
Κάθε εντολή λαμβάνει υπόψη το **ενεργό πλαίσιο** (ορίζεται με `agentproxy connect`, δείτε
...
| -------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI                   | `~/.codex/<name>.config.toml` — ένα προφίλ ανά συμβατό μοντέλο κειμένου (`codex --profile <name>`)                                                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Και τα δύο                   |
| `agentproxy setup-claude`   | Claude Code                        | `~/.claude/profiles/<name>/settings.json` — ένα προφίλ ανά αντιστοιχισμένο μοντέλο (`CLAUDE_CONFIG_DIR`)                                                                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Και τα δύο                   |
| `agentproxy setup-opencode` | OpenCode (openai-compatible)       | `~/.config/opencode/opencode.json` — πάροχος `agentproxy` με κάθε μοντέλο καταλόγου (`opencode -m agentproxy/<model>`)                                                                                     | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Και τα δύο                   |
| `agentproxy setup-cline`    | Cline                              | `~/.cline/data/{globalState,secrets}.json` (λειτουργία CLI) + εκτυπώνει ρυθμίσεις επέκτασης VS Code                                                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Και τα δύο                   |
| `agentproxy setup-kilo`     | Kilo Code                          | `~/.local/share/kilo/auth.json` (CLI) + συγχωνεύει το `kilocode.*` στο `settings.json` του VS Code αν υπάρχει                                                                                            | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Και τα δύο                   |
| `agentproxy setup-continue` | Continue / `cn` CLI                | `~/.continue/config.yaml` — μοντέλα `provider: openai`, κλειδί μέσω `${{ secrets.AGENTPROXY_API_KEY }}`                                                                                                   | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Και τα δύο                   |
| `agentproxy setup-cursor`   | Cursor                             | Τίποτα — εκτυπώνει τα βήματα εντός εφαρμογής (η ρύθμιση Cursor είναι αδιαφανές SQLite)                                                                                                                   | `--remote` `--api-key` `--only` `--port`                                                                                                   | Και τα δύο                   |
| `agentproxy setup-roo`      | Roo Code                           | `~/.agentproxy/roo-settings.json` (έγγραφο εισαγωγής) + ορίζει `roo-cline.autoImportSettingsPath` αν υπάρχει `settings.json` του VS Code                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Και τα δύο                   |
| `agentproxy setup-crush`    | Crush                              | `~/.config/crush/crush.json` — πάροχος `openai-compat`, κλειδί μέσω `$AGENTPROXY_API_KEY`                                                                                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Και τα δύο                   |
| `agentproxy setup-goose`    | Goose                              | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + εκτυπώνει συνταγή env                                                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Και τα δύο                   |
| `agentproxy setup-aider`    | Aider                              | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + εκτυπώνει συνταγή env                                                                                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Και τα δύο                   |
| `agentproxy setup-qwen`     | Qwen Code                          | `~/.qwen/settings.json` — πίνακας V4 `modelProviders.openai` + `AGENTPROXY_API_KEY` στο `~/.qwen/.env`                                                                                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Και τα δύο                   |
| `agentproxy setup-5dive`    | 5dive (στόλος πρακτόρων)           | Τίποτα κάτω από `$HOME` — γράφει ένα **προφίλ ελέγχου ταυτότητας** 5dive (`/var/lib/5dive/auth-profiles/<name>/`) μέσω `5dive agent auth set`· μόνο root, εκτελείται στον κεντρικό υπολογιστή του στόλου | `--remote` `--api-key` `--model` `--auth-profile` `--agent` `--byo-provider` `--fivedive-bin` `--no-sudo` `--yes` `--dry-run` `--port`     | Και τα δύο                   |
| `agentproxy run <target>`   | Εκκίνηση χρόνου εκτέλεσης (γενική) | Τίποτα — εκκινεί `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` με το κατάλληλο περιβάλλον και ορίσματα· Qwen και Gemini χρησιμοποιούν προσωρινό απομονωμένο home                           | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Και τα δύο                   |
| `agentproxy launch`         | Claude Code                        | Τίποτα — εκκινεί `claude` με `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` ενσωματωμένα                                                                                                                    | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Και τα δύο                   |
| `agentproxy launch-codex`   | OpenAI Codex CLI                   | Τίποτα — εκκινεί `codex` με τον πάροχο `agentproxy` ενσωματωμένο μέσω επισημάνσεων `-c`                                                                                                                   | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Και τα δύο                   |

Σημειώσεις για τις επισημάνσεις (επαληθευμένες στην πηγή εντολών):

- `--remote <url>` — ανάκτηση του καταλόγου από ένα απομακρυσμένο AgentProxy (παρακάμπτει `--port`
  και το ενεργό πλαίσιο). Το `--api-key <key>` παρέχει το διαπιστευτήριο για αυτόν τον
  διακομιστή (προεπιλογή στη μεταβλητή περιβάλλοντος `AGENTPROXY_API_KEY` ή στο token του ενεργού πλαισίου).
...
- Το `--model <id>` στο `agentproxy run` ακολουθεί την καλωδίωση ανά στόχο του manifest
  (`bin/cli/cli-manifest.mjs`): το **aider** λαμβάνει `--model openai/<id>` και
  το **opencode** `--model agentproxy/<id>` (το πρόθεμα προστίθεται μόνο όταν το id
  ...
  το **codex** μέσω ορισμάτων `-c model_providers.agentproxy.*`. **Το Qwen είναι ο μόνος στόχος εκτέλεσης
  που απαιτεί υποχρεωτικά `--model`** — το `agentproxy run qwen` χωρίς αυτό τερματίζει
  με `2` και ρητό μήνυμα σφάλματος.
- `--port <port>` — τοπική θύρα AgentProxy (προεπιλογή `20128`, αγνοείται όταν ορίζεται `--remote`).
  Υπάρχει σε όλες τις εντολές `setup-*` και σε αμφότερους τους εκκινητές.
- Κωδικοί εξόδου `agentproxy run`: ο δικός του κωδικός εξόδου του child CLI διαδίδεται
...
> Υπάρχει επίσης μια πλουσιότερη ενσωμάτωση plugin — `agentproxy setup opencode` — που
> εγκαθιστά το `@agentproxy/opencode-plugin`. Είναι διαφορετικές εντολές· ο πίνακας
...
Με το AgentProxy να τρέχει στο `localhost:20128`, απλώς εκτελέστε την εντολή ρύθμισης για το εργαλείο σας. Ο κατάλογος ανακτάται από τον τοπικό διακομιστή.
...
export AGENTPROXY_API_KEY=sk-...  # αναφέρεται μέσω {env:AGENTPROXY_API_KEY}, ποτέ σε δίσκο
...
```bash
agentproxy launch                 # Claude Code → τοπικό AgentProxy
agentproxy launch-codex           # Codex CLI → τοπικό AgentProxy
...
Κατευθύνετε οποιαδήποτε εντολή ρύθμισης σε ένα απομακρυσμένο AgentProxy με `--remote` + `--api-key`. Ο κατάλογος ανακτάται από το απομακρυσμένο σύστημα· η ρύθμιση παραμέτρων εγγράφεται στον τοπικό σας υπολογιστή.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # εκτελέστε πρώτα export AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # δημιουργεί διακριτό token, αποθηκεύει το πλαίσιο
agentproxy setup-codex                 # ← χρησιμοποιεί πλέον τον απομακρυσμένο κατάλογο
agentproxy setup-opencode              # ← το ίδιο
agentproxy launch                      # ← Claude Code έναντι του απομακρυσμένου
```
...
Το [5dive](https://5dive.ai) εκτελεί έναν στόλο από μακροχρόνιους πράκτορες κωδικοποίησης, ο καθένας σαν μονάδα systemd κάτω από τον δικό του χρήστη Unix. Δεν είναι το ίδιο CLI κωδικοποίησης, οπότε δεν υπάρχει τίποτα για να εκκινήσει το `agentproxy run` — το `5dive` είναι στόχος **μόνο ρύθμισης παραμέτρων**.
...
Και οι δύο μορφές γράφουν ένα **προφίλ ταυτοποίησης** 5dive, και κάθε θέση `claude` συνδεδεμένη με αυτό το προφίλ επικοινωνεί στη συνέχεια με το AgentProxy. Τρία πράγματα είναι ειδικά για αυτόν τον στόχο:
...
Η κατεύθυνση του προφίλ σε ένα **combo** AgentProxy αντί για ένα μοναδικό μοντέλο είναι αυτό που παρέχει την εφεδρεία παρόχου στον στόλο: όταν το κύριο endpoint κατέρρευσε πλήρως στη μέση μιας σειράς στην εκτέλεση που καταγράφηκε στο
[#11578](https://github.com/khanhkit/AgentProxy/issues/11578), ο πράκτορας ολοκλήρωσε τα υπόλοιπα βήματά του στο εφεδρικό σύστημα και δεν ανέδειξε ποτέ τη διακοπή.

## Συμβάσεις βασικού URL (ποια εργαλεία θέλουν `/v1`)

Το AgentProxy εκθέτει την επιφάνεια OpenAI στο `/v1`, την επιφάνεια Anthropic στη ρίζα,
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | με `/v1`                | Ναι                                               |
...
Όταν ενημερώνετε με `agentproxy update` (αφού επιβεβαιώσετε, ή με `--apply`),
το AgentProxy εκτελεί την εγκατάσταση με το `--include=optional` ενσωματωμένο:

```bash
npm install -g agentproxy@latest --include=optional
```

Αυτή **δεν** είναι μια σημαία που περνάτε στο `agentproxy update` — εφαρμόζεται πάντα από τον
...
# [DRY RUN] Θα εκτελούσε: npm install -g agentproxy@latest --include=optional
```

Άλλες σημαίες του `agentproxy update` (επαληθεύτηκαν στον πηγαίο κώδικα): `--check` (έξοδος με 1 αν
είναι παρωχημένο), `--apply` (εγκατάσταση χωρίς προτροπή), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI μέσω `agentproxy run gemini`
...
(και `:streamGenerateContent?alt=sse`) προς αυτήν — ακριβώς η εγγενής επιφάνεια Gemini του AgentProxy (`/v1beta`). Το `agentproxy run gemini` το συνδέει αυτόματα:

- `GOOGLE_GEMINI_BASE_URL` → το ενεργό βασικό URL του AgentProxy (ρίζα, χωρίς `/v1`);
- `GEMINI_API_KEY` → τα επιλυμένα διαπιστευτήρια AgentProxy (option/env/context);
- ένα **προσωρινό απομονωμένο `GEMINI_CLI_HOME`** του οποίου το `.gemini/settings.json`
  ...
  να μην παρακάμψει ποτέ την κατευθυνόμενη από το AgentProxy εκκίνηση — αφαιρείται μετά την έξοδο;
...
διακομιστή AgentProxy, υπάρχει ένα κατ' επιλογή harness στο
...
Προαιρετικά: το `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` περιορίζει το sweep·
το `AGENTPROXY_SMOKE_TIMEOUT_MS` παρακάμπτει το timeout των 120 δευτερολέπτων ανά στόχο.
...
- [Διαμόρφωση Codex CLI](./CODEX-CLI-CONFIGURATION.md) — η εφάπαξ βασική ρύθμιση `[model_providers.agentproxy]`
