...
AgentProxy มีคำสั่ง `setup-*` ที่ใช้ในการกำหนดค่า CLI สำหรับการเขียนโค้ด (Codex, Claude Code, OpenCode, Cline, …) เพื่อใช้ AgentProxy เป็น backend — ดังนั้นเครื่องมือจึงติดต่อกับ **หนึ่ง** endpoint และ AgentProxy จะทำการส่งต่อไปยังผู้ให้บริการที่ถูกต้องพร้อมการสำรองอัตโนมัติ คำสั่งแต่ละคำสั่งจะอ่านแคตตาล็อกโมเดล **สด** จาก AgentProxy ที่กำลังทำงาน (ท้องถิ่นหรือระยะไกล) และเขียนไฟล์การกำหนดค่าของเครื่องมือเองลงใน **เครื่องของคุณ** คีย์ API จะถูกอ้างอิงโดยตัวแปรสภาพแวดล้อมที่เครื่องมือรองรับ คำสั่งที่เก็บไฟล์สภาพแวดล้อมเฉพาะเครื่องมือจะถูกบันทึกไว้ด้านล่าง

นอกจากนี้ยังมีตัวเรียกใช้ทั่วไป — `agentproxy run <target>` — ที่สร้าง `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` หรือ `gemini` พร้อมกับ env ที่ถูกฉีดเข้าไป โดยไม่ต้องเขียนการกำหนดค่าใด ๆ เป้าหมายและชื่อเล่นของพวกเขามาจากเอกสารที่เป็นมาตรฐาน `bin/cli/cli-manifest.mjs` (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), และ `agentproxy completion` จะเสนอคำที่ได้จากเอกสารเดียวกัน คำสั่งเรียกใช้แบบเก่าต่อเครื่องมือ — `agentproxy launch` (Claude Code) และ `agentproxy launch-codex` (Codex) — ยังคงมีให้ใช้งาน
...
- [โหมดระยะไกล](./REMOTE-MODE.md) — ขับ AgentProxy ระยะไกล (VPS / Tailnet) จากแล็ปท็อปของคุณ
...
ทุกคำสั่งจะเคารพ **บริบทที่ใช้งานอยู่** (ตั้งค่าด้วย `agentproxy connect`, ดู [โหมดระยะไกล](./REMOTE-MODE.md)) หรือธง `--remote <url> --api-key <key>` ที่ชัดเจน "ท้องถิ่นกับระยะไกล" ด้านล่างหมายถึง: โดยไม่มีธงมันจะมุ่งเป้าไปที่ `http://localhost:20128`; ด้วย `--remote` (หรือบริบทระยะไกลที่ใช้งานอยู่) มันจะดึงแคตตาล็อกจากเซิร์ฟเวอร์นั้นและเขียนการกำหนดค่าในท้องถิ่น

| Command                    | Tool                         | What it writes                                                                                                                                       | Key flags                                                                                                                                  | Local vs remote |
| -------------------------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — โปรไฟล์หนึ่งต่อโมเดลข้อความที่เข้ากันได้ (`codex --profile <name>`)                                                  | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Both            |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — โปรไฟล์หนึ่งต่อโมเดลที่ตรงกัน (`CLAUDE_CONFIG_DIR`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Both            |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — ผู้ให้บริการ `agentproxy` พร้อมโมเดลทุกตัวในแคตตาล็อก (`opencode -m agentproxy/<model>`)                          | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Both            |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (โหมด CLI) + พิมพ์การตั้งค่าขยาย VS Code                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Both            |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + รวม `kilocode.*` ลงใน `settings.json` ของ VS Code หากมีอยู่                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Both            |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — โมเดล `provider: openai` คีย์ผ่าน `${{ secrets.AGENTPROXY_API_KEY }}`                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Both            |
| `agentproxy setup-cursor`   | Cursor                       | ไม่มีอะไร — พิมพ์ขั้นตอนในแอป (การกำหนดค่าของ Cursor เป็น SQLite ที่ไม่โปร่งใส)                                                                      | `--remote` `--api-key` `--only` `--port`                                                                                                   | Both            |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (เอกสารนำเข้า) + ตั้งค่า `roo-cline.autoImportSettingsPath` หากมี `settings.json` ของ VS Code                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Both            |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — ผู้ให้บริการ `openai-compat` คีย์ผ่าน `$AGENTPROXY_API_KEY`                                                            | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Both            |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + พิมพ์สูตร env                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Both            |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + พิมพ์สูตร env                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Both            |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — อาร์เรย์ `V4 modelProviders.openai` + `AGENTPROXY_API_KEY` ใน `~/.qwen/.env`                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Both            |
| `agentproxy run <target>`   | Runtime launch (generic)     | ไม่มีอะไร — สร้าง `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` พร้อม env และ args ที่ถูกต้อง; Qwen และ Gemini ใช้โฮมชั่วคราวที่แยกออก | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Both            |
| `agentproxy launch`         | Claude Code                  | ไม่มีอะไร — สร้าง `claude` พร้อมกับ `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` ที่ถูกฉีดเข้าไป                                                      | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Both            |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | ไม่มีอะไร — สร้าง `codex` พร้อมกับผู้ให้บริการ `agentproxy` ที่ถูกฉีดผ่านธง `-c`                                                                      | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Both            |

หมายเหตุเกี่ยวกับธง (ตรวจสอบในแหล่งที่มาของคำสั่ง):

- `--remote <url>` — ดึงแคตตาล็อกจาก AgentProxy ระยะไกล (เขียนทับ `--port` และบริบทที่ใช้งานอยู่) `--api-key <key>` จะจัดเตรียมข้อมูลประจำตัวสำหรับเซิร์ฟเวอร์นั้น (ค่าเริ่มต้นคือ `AGENTPROXY_API_KEY` env var หรือโทเค็นของบริบทที่ใช้งานอยู่)
...
- `--model <id>` บน `agentproxy run` จะปฏิบัติตามการเชื่อมต่อเฉพาะเป้าหมายในเอกสาร (`bin/cli/cli-manifest.mjs`): **aider** จะได้รับ `--model openai/<id>` และ **opencode** `--model agentproxy/<id>` (คำนำหน้าจะถูกเพิ่มเฉพาะเมื่อ id ไม่มีอยู่แล้ว); **qwen** และ **gemini** จะได้รับ id ตามตัวอักษร; **claude** จะได้รับผ่าน `ANTHROPIC_MODEL`, **goose** ผ่าน `GOOSE_MODEL`, และ **codex** ผ่าน `-c model_providers.agentproxy.*` args **Qwen เป็นเป้าหมายการทำงานเพียงอย่างเดียวที่ต้องการ `--model`** — `agentproxy run qwen` โดยไม่มีมันจะออก `2` พร้อมกับข้อผิดพลาดที่ชัดเจน
- `--port <port>` — พอร์ต AgentProxy ในท้องถิ่น (ค่าเริ่มต้น `20128`, จะถูกละเว้นเมื่อกำหนด `--remote`) ปรากฏในทุกคำสั่ง `setup-*` และทั้งสองตัวเรียกใช้
- รหัสออกจาก `agentproxy run`: รหัสออกของ CLI ลูกจะถูกส่งต่ออย่างตรงไปตรงมา; `2` = อาร์กิวเมนต์ไม่ถูกต้อง (เป้าหมายที่ไม่รองรับ, ขาด `--model` ที่จำเป็น, การป้องกันคอนเทนเนอร์); `127` = ไบนารีเป้าหมายไม่อยู่ใน `PATH`; `130`/`143`/`129` เมื่อการเรียกใช้สิ้นสุดโดย `SIGINT`/`SIGTERM`/`SIGHUP`; `1` = ความล้มเหลวในการเรียกใช้ในระหว่างเวลาอื่น
...
> นอกจากนี้ยังมีการรวมปลั๊กอินที่ร่ำรวยกว่า — `agentproxy setup opencode` — ซึ่ง
> ติดตั้ง `@agentproxy/opencode-plugin` พวกเขาเป็นคำสั่งที่แตกต่างกัน; ตาราง
...
เมื่อ AgentProxy ทำงานอยู่ที่ `localhost:20128` ให้รันคำสั่งตั้งค่าสำหรับเครื่องมือของคุณ คลังข้อมูลจะถูกดึงจากเซิร์ฟเวอร์ท้องถิ่น
...
export AGENTPROXY_API_KEY=sk-...  # อ้างอิงผ่าน {env:AGENTPROXY_API_KEY} ไม่เคยอยู่ในดิสก์
...
```bash
agentproxy launch                 # Claude Code → AgentProxy ท้องถิ่น
agentproxy launch-codex           # Codex CLI → AgentProxy ท้องถิ่น
...
ชี้คำสั่งตั้งค่าใด ๆ ไปที่ AgentProxy ระยะไกลด้วย `--remote` + `--api-key` คลังข้อมูลจะถูกดึงจากระยะไกล; การกำหนดค่าจะถูกเขียนลงในเครื่องของคุณ
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # ส่งออก AGENTPROXY_API_KEY ก่อน
...
```bash
agentproxy connect 192.168.0.15        # สร้างโทเค็นที่มีขอบเขต เก็บบริบท
agentproxy setup-codex                 # ← ตอนนี้ใช้คลังระยะไกล
agentproxy setup-opencode              # ← เช่นเดียวกัน
agentproxy launch                      # ← Claude Code กับระยะไกล
```
...
AgentProxy เปิดเผยพื้นผิว OpenAI ที่ `/v1` พื้นผิว Anthropic ที่ราก และพื้นผิว Gemini ดั้งเดิมที่ `/v1beta` การรวมแต่ละอย่างถูกเชื่อมต่อกับรูปแบบที่เครื่องมือของคุณคาดหวัง (ตรวจสอบในแหล่งที่มาของคำสั่ง):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | พร้อม `/v1`         | ใช่                                        |
...
เมื่อคุณอัปเดตด้วย `agentproxy update` (หลังจากยืนยัน หรือด้วย `--apply`),
AgentProxy จะรันการติดตั้งด้วย `--include=optional` ที่ฝังอยู่ในนั้น:

```bash
npm install -g agentproxy@latest --include=optional
```

นี่คือ **ไม่ใช่** ธงที่คุณส่งไปยัง `agentproxy update` — มันจะถูกนำไปใช้เสมอโดย
...
# [DRY RUN] จะรัน: npm install -g agentproxy@latest --include=optional
```

ธงอื่น ๆ ของ `agentproxy update` (ได้รับการตรวจสอบในซอร์ส): `--check` (ออก 1 หาก
ล้าสมัย), `--apply` (ติดตั้งโดยไม่ต้องถาม), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI ผ่าน `agentproxy run gemini`
...
(และ `:streamGenerateContent?alt=sse`) ต่อมัน — ตรงตามพื้นผิว Gemini ดั้งเดิมของ AgentProxy (`/v1beta`). `agentproxy run gemini` จะเชื่อมต่อสิ่งนั้นโดยอัตโนมัติ:

- `GOOGLE_GEMINI_BASE_URL` → URL พื้นฐาน AgentProxy ที่ใช้งานอยู่ (ราก, ไม่มี `/v1`);
- `GEMINI_API_KEY` → ข้อมูลรับรอง AgentProxy ที่แก้ไขแล้ว (ตัวเลือก/สภาพแวดล้อม/บริบท);
- **`GEMINI_CLI_HOME` ชั่วคราวที่แยกออก** ซึ่ง `.gemini/settings.json`
  ...
  จะไม่เขียนทับการเปิดตัวที่กำหนดโดย AgentProxy — จะถูกลบหลังจากออก;
...
`tests/unit/cli/run-execution.test.ts`). เพื่อยืนยันไบนารีจริงกับเซิร์ฟเวอร์ AgentProxy จริง
...
ตัวเลือก: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` จำกัดการตรวจสอบ;
`AGENTPROXY_SMOKE_TIMEOUT_MS` จะเขียนทับเวลา 120 วินาทีต่อเป้าหมาย.

## ดูเพิ่มเติม

- [การกำหนดค่าของ Claude Code](./CLAUDE-CODE-CONFIGURATION.md) — คู่มือ Claude Code ที่ลึกซึ้งยิ่งขึ้น
- [การกำหนดค่าของ Codex CLI](./CODEX-CLI-CONFIGURATION.md) — การตั้งค่าเบื้องต้น `[model_providers.agentproxy]` แบบครั้งเดียว
