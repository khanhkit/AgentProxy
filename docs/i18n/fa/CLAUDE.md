...
**AgentProxy** — پروکسی/روتر AI یکپارچه. یک نقطه انتهایی، 329 ارائه‌دهنده LLM، بازگشت خودکار.
...
AgentProxy سه مکانیزم موقت شکست مرتبط اما متمایز دارد. دامنه آن‌ها را هنگام اشکال‌زدایی رفتار مسیریابی جدا نگه دارید. برای یک نمای کلی، به
...
- **واردات**: خارجی → داخلی (`@/`, `@agentproxy/open-sse`) → نسبی
...
- **آلیاس‌های مسیر**: `@/*` → `src/`، `@agentproxy/open-sse` → `open-sse/`، `@agentproxy/open-sse/*` → `open-sse/*`
- **پورت پیش‌فرض**: 20128 (API + داشبورد در همان پورت)
- **دایرکتوری داده**: متغیر محیطی `DATA_DIR`، به طور پیش‌فرض به `~/.agentproxy/`
