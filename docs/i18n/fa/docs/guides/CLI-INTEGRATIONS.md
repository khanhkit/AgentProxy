...
AgentProxy یک خانواده از دستورات `setup-*` را ارائه می‌دهد که یک CLI کدنویسی (Codex، Claude Code، OpenCode، Cline و ...) را برای استفاده از AgentProxy به عنوان بک‌اند خود پیکربندی می‌کند — بنابراین ابزار با **یک** نقطه پایانی ارتباط برقرار می‌کند و AgentProxy به ارائه‌دهنده مناسب با بازگشت خودکار هدایت می‌کند. هر دستور فهرست مدل **زنده** را از یک AgentProxy در حال اجرا (محلی یا از راه دور) می‌خواند و فایل پیکربندی خود ابزار را بر روی **ماشین شما** می‌نویسد. کلید API توسط یک متغیر محیطی در هر جایی که ابزار از آن پشتیبانی می‌کند، ارجاع داده می‌شود. دستورات که یک فایل محیط محلی ابزار را حفظ می‌کنند، در زیر ذکر شده‌اند.

همچنین یک راه‌انداز عمومی وجود دارد — `agentproxy run <target>` — که `claude`، `codex`، `aider`، `goose`، `opencode`، `qwen` یا `gemini` را با محیط مناسب تزریق شده، بدون نوشتن هیچ پیکربندی، راه‌اندازی می‌کند. اهداف و نام‌های مستعار آن‌ها از فهرست رسمی `bin/cli/cli-manifest.mjs` می‌آیند (`claude-code|cc|anthropic`، `codex-cli|openai-codex|openai`، `goose-cli`، `open-code`، `qwen-code`، `gemini-cli`) و `agentproxy completion` همان کلمات هدف مشتق شده از فهرست را ارائه می‌دهد. راه‌اندازهای قدیمی برای هر ابزار — `agentproxy launch` (Claude Code) و `agentproxy launch-codex` (Codex) — همچنان در دسترس هستند.
...
- [حالت از راه دور](./REMOTE-MODE.md) — کنترل یک AgentProxy از راه دور (VPS / Tailnet) از لپ‌تاپ شما
...
هر دستور به **زمینه فعال** (تنظیم شده با `agentproxy connect`، به [حالت از راه دور](./REMOTE-MODE.md) مراجعه کنید) یا پرچم‌های صریح `--remote <url> --api-key <key>` احترام می‌گذارد. "محلی در مقابل از راه دور" در زیر به این معناست: بدون پرچم‌ها به `http://localhost:20128` هدف‌گذاری می‌کند؛ با `--remote` (یا یک زمینه از راه دور فعال) فهرست را از آن سرور دریافت کرده و پیکربندی را به صورت محلی می‌نویسد.

| دستور                      | ابزار                        | آنچه می‌نویسد                                                                                                                                                       | پرچم‌های کلیدی                                                                                                                             | محلی در مقابل از راه دور |
| -------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — یک پروفایل برای هر مدل متنی سازگار (`codex --profile <name>`)                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | هر دو                    |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — یک پروفایل برای هر مدل مطابقت یافته (`CLAUDE_CONFIG_DIR`)                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | هر دو                    |
| `agentproxy setup-opencode` | OpenCode (سازگار با openai)  | `~/.config/opencode/opencode.json` — ارائه‌دهنده `agentproxy` با هر مدل فهرست (`opencode -m agentproxy/<model>`)                                                      | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | هر دو                    |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (حالت CLI) + چاپ تنظیمات افزونه VS Code                                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | هر دو                    |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + ادغام `kilocode.*` به `settings.json` VS Code در صورت وجود                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | هر دو                    |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — مدل‌های `provider: openai`، کلید از طریق `${{ secrets.AGENTPROXY_API_KEY }}`                                                             | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | هر دو                    |
| `agentproxy setup-cursor`   | Cursor                       | هیچ چیز — چاپ مراحل درون‌برنامه (پیکربندی Cursor غیرشفاف SQLite)                                                                                                    | `--remote` `--api-key` `--only` `--port`                                                                                                   | هر دو                    |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (مدرک وارد شده) + تنظیم `roo-cline.autoImportSettingsPath` اگر یک `settings.json` VS Code وجود داشته باشد                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | هر دو                    |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — ارائه‌دهنده سازگار با `openai`، کلید از طریق `$AGENTPROXY_API_KEY`                                                                    | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | هر دو                    |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + چاپ دستورالعمل محیط                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | هر دو                    |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + چاپ دستورالعمل محیط                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | هر دو                    |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — آرایه `V4 modelProviders.openai` + `AGENTPROXY_API_KEY` در `~/.qwen/.env`                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | هر دو                    |
| `agentproxy run <target>`   | راه‌اندازی زمان اجرا (عمومی) | هیچ چیز — راه‌اندازی `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` با محیط و آرگومان‌های مناسب؛ Qwen و Gemini از یک خانه موقتی ایزوله استفاده می‌کنند | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | هر دو                    |
| `agentproxy launch`         | Claude Code                  | هیچ چیز — `claude` را با `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` تزریق شده راه‌اندازی می‌کند                                                                    | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | هر دو                    |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | هیچ چیز — `codex` را با ارائه‌دهنده `agentproxy` تزریق شده از طریق پرچم‌های `-c` راه‌اندازی می‌کند                                                                   | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | هر دو                    |

نکات مربوط به پرچم‌ها (تأیید شده در منبع دستور):

- `--remote <url>` — فهرست را از یک AgentProxy از راه دور دریافت کنید (پرچم‌های `--port` و زمینه فعال را نادیده می‌گیرد). `--api-key <key>` اعتبارنامه را برای آن سرور تأمین می‌کند (به طور پیش‌فرض به متغیر محیطی `AGENTPROXY_API_KEY` یا توکن زمینه فعال اشاره می‌کند).
...
- `--model <id>` در `agentproxy run` از اتصالات مشخص شده در فهرست استفاده می‌کند (`bin/cli/cli-manifest.mjs`): **aider** `--model openai/<id>` و **opencode** `--model agentproxy/<id>` (پیشوند فقط زمانی اضافه می‌شود که شناسه قبلاً آن را نداشته باشد)؛ **qwen** و **gemini** شناسه را به صورت عینی دریافت می‌کنند؛ **claude** آن را از طریق `ANTHROPIC_MODEL`، **goose** از طریق `GOOSE_MODEL` و **codex** از طریق آرگومان‌های `-c model_providers.agentproxy.*` دریافت می‌کند. **Qwen تنها هدف اجرایی است که به شدت نیاز به `--model` دارد** — `agentproxy run qwen` بدون آن با خطای صریح `2` خارج می‌شود.
- `--port <port>` — پورت محلی AgentProxy (پیش‌فرض `20128`، در صورت تنظیم `--remote` نادیده گرفته می‌شود). در تمام `setup-*` و هر دو راه‌انداز موجود است.
- کدهای خروجی `agentproxy run`: کد خروجی خود CLI فرزند به صورت عینی منتقل می‌شود؛ `2` = آرگومان‌های نامعتبر (هدف پشتیبانی نشده، `--model` مورد نیاز گم شده، نگهبان کانتینر)؛ `127` = باینری هدف در `PATH` نیست؛ `130`/`143`/`129` زمانی که راه‌اندازی با `SIGINT`/`SIGTERM`/`SIGHUP` پایان می‌یابد؛ `1` = سایر خطاهای راه‌اندازی زمان اجرا.
...
> همچنین یک ادغام پلاگین غنی‌تر وجود دارد — `agentproxy setup opencode` — که `@agentproxy/opencode-plugin` را نصب می‌کند. این‌ها دستورات متفاوتی هستند؛ جدول بالا `setup-opencode` را مستند می‌کند.

---

## استفاده محلی

با اجرای AgentProxy بر روی `localhost:20128`، فقط دستور راه‌اندازی را برای ابزار خود اجرا کنید. کاتالوگ از سرور محلی دریافت می‌شود.
...
export AGENTPROXY_API_KEY=sk-...  # ارجاع داده شده از طریق {env:AGENTPROXY_API_KEY}، هرگز بر روی دیسک
...
```bash
agentproxy launch                 # Claude Code → AgentProxy محلی
agentproxy launch-codex           # Codex CLI → AgentProxy محلی
...
agentproxy run claude -- --print-system-prompt "این تفاوت را بررسی کنید"
```
...
هر دستور راه‌اندازی را به یک AgentProxy از راه دور با `--remote` + `--api-key` اشاره کنید. کاتالوگ از راه دور دریافت می‌شود؛ پیکربندی بر روی ماشین محلی شما نوشته می‌شود.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # ابتدا AGENTPROXY_API_KEY را صادر کنید
...
```bash
agentproxy connect 192.168.0.15        # یک توکن محدوده‌ای ایجاد می‌کند، زمینه را ذخیره می‌کند
agentproxy setup-codex                 # ← حالا از کاتالوگ از راه دور استفاده می‌کند
agentproxy setup-opencode              # ← مشابه
agentproxy launch                      # ← Claude Code در برابر راه دور
```
...
AgentProxy سطح OpenAI را در `/v1`، سطح Anthropic را در ریشه و یک سطح بومی Gemini را در `/v1beta` ارائه می‌دهد. هر یکپارچگی به شکلی که ابزارش انتظار دارد متصل شده است (در منبع دستور تأیید شده):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | با `/v1`           | بله                                                  |
...
زمانی که با `agentproxy update` بروزرسانی می‌کنید (پس از تأیید یا با `--apply`)،
AgentProxy نصب را با `--include=optional` انجام می‌دهد:

```bash
npm install -g agentproxy@latest --include=optional
```

این **یک** پرچم نیست که به `agentproxy update` بدهید — همیشه توسط بروزرسان
...
سایر پرچم‌های `agentproxy update` (تأیید شده در منبع): `--check` (خروج 1 اگر
قدیمی باشد)، `--apply` (نصب بدون درخواست)، `--changelog`، `--no-backup`،
`--yes`.

---

## Google Gemini CLI از طریق `agentproxy run gemini`
...
Gemini AgentProxy (`/v1beta`). `agentproxy run gemini` این را به طور خودکار متصل می‌کند:

- `GOOGLE_GEMINI_BASE_URL` → URL پایه فعال AgentProxy (ریشه، بدون `/v1`)؛
- `GEMINI_API_KEY` → اعتبارنامه حل شده AgentProxy (گزینه/محیط/زمینه)؛
- یک **`GEMINI_CLI_HOME` موقت و ایزوله** که `.gemini/settings.json` آن
  ...
  ذخیره شده Google (Code Assist) هرگز راه‌اندازی هدایت شده AgentProxy را
...
در برابر یک سرور واقعی AgentProxy، یک ابزار اختیاری در
...
اختیاری: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` پاکسازی را محدود می‌کند؛
`AGENTPROXY_SMOKE_TIMEOUT_MS` زمان‌سنج 120 ثانیه‌ای برای هر هدف را نادیده می‌گیرد.
...
- [پیکربندی CLI کدکس](./CODEX-CLI-CONFIGURATION.md) — تنظیمات پایه یک‌باره `[model_providers.agentproxy]`
