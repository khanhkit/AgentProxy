...
O AgentProxy fornece uma família de comandos `setup-*` que configuram uma CLI de codificação (Codex, Claude Code, OpenCode, Cline, …) para usar o AgentProxy como seu backend — assim, a ferramenta comunica-se com **um** endpoint e o AgentProxy direciona para o provedor certo com fallback automático. Cada comando lê o catálogo de modelos **ao vivo** de um AgentProxy em execução (local ou remoto) e escreve o próprio arquivo de configuração da ferramenta na **sua** máquina. A chave da API é referenciada por uma variável de ambiente sempre que a ferramenta a suporta. Comandos que persistem um arquivo de ambiente local da ferramenta estão anotados abaixo.
...
| `agentproxy setup-qwen`     | Qwen Code                                  | `~/.qwen/settings.json` — array `modelProviders.openai` V4 + `AGENTPROXY_API_KEY` em `~/.qwen/.env`                                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Ambos           |
| `agentproxy run <target>`   | Lançamento em tempo de execução (genérico) | Nada — inicia `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` com o ambiente e argumentos corretos; Qwen e Gemini usam um diretório isolado temporário | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Ambos           |
...
- `--model <id>` em `agentproxy run` segue a fiação por alvo do manifesto
  ...
  **codex** via argumentos `-c model_providers.agentproxy.*`. **Qwen é o único alvo de execução
...
- Códigos de saída de `agentproxy run`: o próprio código de saída da CLI filha é propagado
...
Com o AgentProxy a correr em `localhost:20128`, basta executar o comando de configuração para a sua ferramenta. O catálogo é buscado no servidor local.
...
agentproxy run claude -- --print-system-prompt "revise este diff"
```
...
Aponte qualquer comando de configuração para um AgentProxy remoto com `--remote` + `--api-key`. O catálogo é buscado remotamente; a configuração é escrita na sua máquina local.
...
```bash
agentproxy connect 192.168.0.15        # gera um token com escopo, armazena o contexto
...
O AgentProxy expõe a superfície OpenAI em `/v1`, a superfície Anthropic na raiz, e uma superfície nativa Gemini em `/v1beta`. Cada integração está ligada à forma que a sua ferramenta espera (verificado na fonte do comando):
...
Quando você atualiza com `agentproxy update` (após confirmar, ou com `--apply`),
o AgentProxy executa a instalação com `--include=optional` incorporado:

```bash
npm install -g agentproxy@latest --include=optional
```

Este **não** é um parâmetro que você passa para `agentproxy update` — ele é sempre aplicado pelo
...
Outros parâmetros de `agentproxy update` (verificados no código-fonte): `--check` (sai com 1 se
...
Gemini do AgentProxy (`/v1beta`). `agentproxy run gemini` conecta isso automaticamente:
...
- um **`GEMINI_CLI_HOME` isolado temporário** cujo `.gemini/settings.json`
  ...
  nunca sobrescreva o lançamento direcionado pelo AgentProxy — removido após a saída;
...
do AgentProxy, existe um suporte opcional em
...
Opcional: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` restringe a varredura;
`AGENTPROXY_SMOKE_TIMEOUT_MS` substitui o tempo limite de 120s por alvo.
...
- [Configuração do Codex CLI](./CODEX-CLI-CONFIGURATION.md) — a configuração base única `[model_providers.agentproxy]`
