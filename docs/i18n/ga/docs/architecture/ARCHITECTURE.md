...
![Samhail athléimneachta 3 chiseal](../diagrams/exported/resilience-3layers.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Cliantanna Forbróra]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Cliantanna saincheaptha atá comhoiriúnach le OpenAI]
    ...
    subgraph Router[Próiseas Áitiúil AgentProxy]
        ...
        DB[(storage.sqlite)]
        UDB[(táblaí úsáide + sórtáil logaí)]
    end

    subgraph Upstreams[BunSoláthraithe Uachtaracha]
        P1[BunSoláthraithe OAuth\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[BunSoláthraithe Eochracha API\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Nóid Chomhoiriúnacha\natá comhoiriúnach le OpenAI / atá comhoiriúnach le Anthropic]
    end

    subgraph Cloud[Sioncronú Solais Roghnach]
        CLOUD[Céim Sioncronaithe Saobhréimse\nNEXT_PUBLIC_CLOUD_URL]
    end
...
```mermaid
flowchart TD
    A[Teaghrán múnla ag teacht isteach] --> B{An ainm Combo é?}
    B -- Sea --> C[Luchtaigh seicheamh múnlaí Combo]
    B -- Ní hea --> D[Cosán múnla aonair]

    C --> E[Bain triail as múnla N]
    E --> F[Réitigh soláthraí/múnla]
    D --> F

    F --> G[Roghnaigh dintiúir chuntais]
    G --> H{An bhfuil dintiúir ar fáil?}
    H -- Ní hea --> I[Cuir ar ais soláthraí neamh-infhaighte]
    H -- Sea --> J[Cuir iarratas i gcrích]

    J --> K{Ar éirigh leis?}
    K -- Sea --> L[Cuir ar ais freagra]
    K -- Ní hea --> M{Earráid incháilithe do thitim siar?}

    M -- Ní hea --> N[Cuir ar ais earráid]
    M -- Sea --> O[Marcáil cuntas neamh-infhaighte ar fhuarú]
    O --> P{An bhfuil cuntas eile don soláthraí?}
    P -- Sea --> G
    P -- Ní hea --> Q{An bhfuil sé i gCombo leis an gcéad mhúnla eile?}
    Q -- Sea --> E
    Q -- Ní hea --> R[Cuir ar ais gach rud neamh-infhaighte]
```
...
sequenceDiagram
    autonumber
    participant UI as Comhéadan Úsáideora Deais
    participant OAuth as /api/oauth/[provider]/[action]
    participant ProvAuth as Freastalaí Údaraithe Soláthraí
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Forghníomhaí Soláthraí

    UI->>OAuth: GET authorize nó device-code
    OAuth->>ProvAuth: cruthaigh sreabhadh údaraithe/gléis
    ProvAuth-->>OAuth: URL údaraithe nó pailléad cód gléis
    OAuth-->>UI: sonraí sreafa

    UI->>OAuth: POST exchange nó polláil
    OAuth->>ProvAuth: malartú comhartha/polláil
    ProvAuth-->>OAuth: comharthaí rochtana/athnuachana
    OAuth->>DB: createProviderConnection(oauth data)
    OAuth-->>UI: rath + id ceangail

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: bailíochtaigh dintiúir / athnuachan roghnach
    Exec-->>Test: faisnéis chomhartha bailí nó athnuachta
    Test->>DB: nuashonraigh stádas/comharthaí/earráidí
    Test-->>UI: toradh bailíochtaithe
```
...
sequenceDiagram
    autonumber
    participant UI as Comhéadan Úsáideora Leathanach Críochphointe
    participant Sync as /api/sync/cloud
    participant DB as localDb
    participant Cloud as Sioncronú Seachtrach Scamaill
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: set cloudEnabled=true
    Sync->>DB: cinntigh go bhfuil eochair API ann
    Sync->>Cloud: POST /sync/{machineId} (soláthraithe/ailias/Combo/eochracha)
    Cloud-->>Sync: toradh sioncronaithe
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: cumasaithe + stádas fíoraithe

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: sonraí cianda
    Sync->>DB: nuashonraigh comharthaí/stádas áitiúla níos nuaí
    Sync-->>UI: sioncronaithe

    UI->>Sync: POST action=disable
    Sync->>DB: set cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: athraigh ANTHROPIC_BASE_URL ar ais go háitiúil (más gá)
    Sync-->>UI: díchumasaithe
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Óstach Forbróra]
        CLI[Uirlisí CLI]
    ...
    subgraph ContainerOrProcess[Rith AgentProxy]
        ...
        UsageDB[(táblaí úsáide + míreanna logála)]
    end

    subgraph External[Snaidhmeanna Seachtracha]
        Providers[Soláthraithe AI]
        SyncCloud[Snaidhm Sioncronú Scála]
    end
