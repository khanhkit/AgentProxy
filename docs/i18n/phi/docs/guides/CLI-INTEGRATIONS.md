...
Ang AgentProxy ay nagdadala ng isang pamilya ng `setup-*` na mga utos na nagko-configure ng isang coding
CLI (Codex, Claude Code, OpenCode, Cline, …) upang gamitin ang AgentProxy bilang backend nito — kaya
ang tool ay nakikipag-usap sa **isang** endpoint at ang AgentProxy ay nagruruta sa tamang provider na may
auto-fallback. Bawat utos ay nagbabasa ng **live** na model catalog mula sa isang tumatakbong
AgentProxy (lokal o remote) at sumusulat ng sariling config file ng tool sa **iyong**
...
Mayroon ding isang generic na launcher — `agentproxy run <target>` — na naglalabas
...
`open-code`, `qwen-code`, `gemini-cli`), at ang `agentproxy completion` ay nag-aalok ng
parehong manifest-derived na mga target na salita. Ang legacy per-tool launchers —
`agentproxy launch` (Claude Code) at `agentproxy launch-codex` (Codex) — ay nananatiling
available.
...
- [Remote Mode](./REMOTE-MODE.md) — patakbuhin ang isang remote AgentProxy (VPS / Tailnet) mula sa iyong laptop
...
Bawat utos ay iginagalang ang **aktibong konteksto** (itinakda gamit ang `agentproxy connect`, tingnan ang
...
| -------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — isang profile bawat compatible na text model (`codex --profile <name>`)                                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Pareho          |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — isang profile bawat matched model (`CLAUDE_CONFIG_DIR`)                                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Pareho          |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — `agentproxy` provider na may bawat catalog model (`opencode -m agentproxy/<model>`)                                                            | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Pareho          |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + nagpi-print ng mga setting ng VS Code extension                                                                           | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Pareho          |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + pinagsasama ang `kilocode.*` sa VS Code `settings.json` kung ito ay naroroon                                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Pareho          |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` models, key via `${{ secrets.AGENTPROXY_API_KEY }}`                                                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Pareho          |
| `agentproxy setup-cursor`   | Cursor                       | Wala — nagpi-print ng mga hakbang sa app (ang Cursor config ay opaque SQLite)                                                                                                     | `--remote` `--api-key` `--only` `--port`                                                                                                   | Pareho          |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (import doc) + itinatakda ang `roo-cline.autoImportSettingsPath` kung ang isang VS Code `settings.json` ay umiiral                               | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Pareho          |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` provider, key via `$AGENTPROXY_API_KEY`                                                                                             | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Pareho          |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + nagpi-print ng env recipe                                                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Pareho          |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + nagpi-print ng env recipe                                                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Pareho          |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` array + `AGENTPROXY_API_KEY` sa `~/.qwen/.env`                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Pareho          |
| `agentproxy run <target>`   | Runtime launch (generic)     | Wala — naglalabas ng `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` na may tamang env at args; ang Qwen at Gemini ay gumagamit ng isang pansamantalang isolated home | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Pareho          |
| `agentproxy launch`         | Claude Code                  | Wala — naglalabas ng `claude` na may `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` na na-inject                                                                                     | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Pareho          |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | Wala — naglalabas ng `codex` na may `agentproxy` provider na na-inject sa pamamagitan ng `-c` na mga flag                                                                          | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Pareho          |

Mga tala sa mga flag (naka-verify sa source ng utos):

- `--remote <url>` — kunin ang catalog mula sa isang remote AgentProxy (pinapalitan ang `--port`
  at ang aktibong konteksto). Ang `--api-key <key>` ay nagbibigay ng credential para sa server na iyon
  (default sa `AGENTPROXY_API_KEY` env var, o ang token ng aktibong konteksto).
...
- `--model <id>` sa `agentproxy run` ay sumusunod sa wiring ng manifest per-target
  (`bin/cli/cli-manifest.mjs`): **aider** ay tumatanggap ng `--model openai/<id>` at
  **opencode** `--model agentproxy/<id>` (ang prefix ay idinadagdag lamang kapag ang id
  ...
  **codex** sa pamamagitan ng `-c model_providers.agentproxy.*` na mga args. **Qwen ang tanging run
  target na mahigpit na nangangailangan ng `--model`** — ang `agentproxy run qwen` nang walang ito ay lumalabas
  `2` na may tahasang error.
- `--port <port>` — lokal na AgentProxy port (default `20128`, hindi pinapansin kapag nakaset ang `--remote`).
  Present sa lahat ng `setup-*` at parehong launchers.
- Ang mga exit code ng `agentproxy run`: ang sariling exit code ng child CLI ay naipapasa
...
> Mayroon ding mas mayamang plugin integration — `agentproxy setup opencode` — na
> nag-i-install ng `@agentproxy/opencode-plugin`. Sila ay magkaibang mga utos; ang talahanayan
...
Sa AgentProxy na tumatakbo sa `localhost:20128`, patakbuhin lamang ang setup command para sa iyong tool. Ang katalogo ay kinukuha mula sa lokal na server.
...
export AGENTPROXY_API_KEY=sk-...  # tinutukoy sa pamamagitan ng {env:AGENTPROXY_API_KEY}, hindi kailanman sa disk
...
```bash
agentproxy launch                 # Claude Code → lokal na AgentProxy
agentproxy launch-codex           # Codex CLI → lokal na AgentProxy
...
Ituro ang anumang setup command sa isang malalayong AgentProxy gamit ang `--remote` + `--api-key`. Ang katalogo ay kinukuha mula sa malayo; ang config ay isinusulat sa iyong lokal na makina.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # i-export muna ang AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # nagmimina ng isang scoped token, nag-iimbak ng konteksto
agentproxy setup-codex                 # ← ngayon ay gumagamit ng malalayong katalogo
agentproxy setup-opencode              # ← pareho
agentproxy launch                      # ← Claude Code laban sa malayo
```
...
Ipinapakita ng AgentProxy ang OpenAI surface sa `/v1`, ang Anthropic surface sa root, at isang katutubong Gemini surface sa `/v1beta`. Ang bawat integrasyon ay naka-wire sa anyo na inaasahan ng kanyang tool (naka-verify sa pinagmulan ng command):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | kasama ang `/v1`     | Oo                                                        |
...
Kapag nag-update ka gamit ang `agentproxy update` (pagkatapos kumpirmahin, o gamit ang `--apply`),
ang AgentProxy ay nagpapatakbo ng install na may `--include=optional` na nakabake:

```bash
npm install -g agentproxy@latest --include=optional
```

Ito ay **hindi** isang flag na ipapasa mo sa `agentproxy update` — ito ay palaging inilalapat ng
...
# [DRY RUN] Gagawin: npm install -g agentproxy@latest --include=optional
```

Iba pang mga flag ng `agentproxy update` (naka-verify sa source): `--check` (exit 1 kung
luma na), `--apply` (install nang walang prompting), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI sa pamamagitan ng `agentproxy run gemini`
...
(at `:streamGenerateContent?alt=sse`) laban dito — eksaktong ibabaw ng Gemini ng AgentProxy
(`/v1beta`). Ang `agentproxy run gemini` ay awtomatikong kumokonekta dito:

- `GOOGLE_GEMINI_BASE_URL` → ang aktibong base URL ng AgentProxy (root, walang `/v1`);
- `GEMINI_API_KEY` → ang na-resolve na credential ng AgentProxy (option/env/context);
- isang **panandaliang nakahiwalay na `GEMINI_CLI_HOME`** na ang `.gemini/settings.json`
  ...
  ay hindi kailanman papalitan ang AgentProxy-directed launch — tinanggal pagkatapos ng exit;
...
na AgentProxy server, mayroong opt-in harness sa
...
Opsyonal: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` ay naglilimita sa sweep;
`AGENTPROXY_SMOKE_TIMEOUT_MS` ay nag-ooverride sa 120s per-target timeout.
...
- [Codex CLI configuration](./CODEX-CLI-CONFIGURATION.md) — ang isang beses na `[model_providers.agentproxy]` base setup
