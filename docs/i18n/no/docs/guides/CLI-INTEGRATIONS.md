...
AgentProxy leverer en familie av `setup-*` kommandoer som konfigurerer en kode-CLI (Codex, Claude Code, OpenCode, Cline, …) til å bruke AgentProxy som sin backend — slik at verktøyet snakker med **én** endepunkt og AgentProxy ruter til den riktige leverandøren med automatisk fallback. Hver kommando leser den **live** modellkatalogen fra en kjørende AgentProxy (lokal eller fjern) og skriver verktøyets egen konfigurasjonsfil på **din** maskin. API-nøkkelen refereres av en miljøvariabel der verktøyet støtter det. Kommandoer som vedvarer en verktøy-lokal miljøfil er notert nedenfor.

Det finnes også en generell launcher — `agentproxy run <target>` — som starter `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` eller `gemini` med riktig miljø injisert, uten å skrive noen konfigurasjon i det hele tatt. Mål og deres aliaser kommer fra den kanoniske manifestfilen `bin/cli/cli-manifest.mjs`
(`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`,
`open-code`, `qwen-code`, `gemini-cli`), og `agentproxy completion` tilbyr de samme manifest-avledede målordene. De eldre per-verktøy launcherene —
`agentproxy launch` (Claude Code) og `agentproxy launch-codex` (Codex) — forblir tilgjengelige.
...
- [Fjernmodus](./REMOTE-MODE.md) — kjør en fjern AgentProxy (VPS / Tailnet) fra din bærbare datamaskin
...
Hver kommando respekterer den **aktive konteksten** (satt med `agentproxy connect`, se
...
| -------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI               | `~/.codex/<name>.config.toml` — én profil per kompatibel tekstmodell (`codex --profile <name>`)                                                                 | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Begge          |
| `agentproxy setup-claude`   | Claude Code                    | `~/.claude/profiles/<name>/settings.json` — én profil per matchet modell (`CLAUDE_CONFIG_DIR`)                                                                  | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Begge          |
| `agentproxy setup-opencode` | OpenCode (openai-kompatibel)   | `~/.config/opencode/opencode.json` — `agentproxy` leverandør med hver katalogmodell (`opencode -m agentproxy/<model>`)                                            | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Begge          |
| `agentproxy setup-cline`    | Cline                          | `~/.cline/data/{globalState,secrets}.json` (CLI-modus) + skriver VS Code utvidelsesinnstillinger                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Begge          |
| `agentproxy setup-kilo`     | Kilo Code                      | `~/.local/share/kilo/auth.json` (CLI) + slår sammen `kilocode.*` inn i VS Code `settings.json` hvis tilstede                                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Begge          |
| `agentproxy setup-continue` | Continue / `cn` CLI            | `~/.continue/config.yaml` — `provider: openai` modeller, nøkkel via `${{ secrets.AGENTPROXY_API_KEY }}`                                                          | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Begge          |
| `agentproxy setup-cursor`   | Cursor                         | Ingenting — skriver trinnene i appen (Cursor-konfigurasjon er ugjennomsiktig SQLite)                                                                            | `--remote` `--api-key` `--only` `--port`                                                                                                   | Begge          |
| `agentproxy setup-roo`      | Roo Code                       | `~/.agentproxy/roo-settings.json` (importdokument) + setter `roo-cline.autoImportSettingsPath` hvis en VS Code `settings.json` eksisterer                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Begge          |
| `agentproxy setup-crush`    | Crush                          | `~/.config/crush/crush.json` — `openai-kompat` leverandør, nøkkel via `$AGENTPROXY_API_KEY`                                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Begge          |
| `agentproxy setup-goose`    | Goose                          | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + skriver miljøoppskrift                                                           | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Begge          |
| `agentproxy setup-aider`    | Aider                          | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + skriver miljøoppskrift                                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Begge          |
...
| `agentproxy run <target>`   | Kjøring av launcher (generisk) | Ingenting — start `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` med riktig miljø og argumenter; Qwen og Gemini bruker et midlertidig isolert hjem | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Begge          |
| `agentproxy launch`         | Claude Code                    | Ingenting — starter `claude` med `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injisert                                                                           | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Begge          |
| `agentproxy launch-codex`   | OpenAI Codex CLI               | Ingenting — starter `codex` med `agentproxy` leverandøren injisert via `-c` flagg                                                                                | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Begge          |

Notater om flagg (verifisert i kommandoens kilde):

- `--remote <url>` — hent katalogen fra en fjern AgentProxy (overstyrer `--port`
  og den aktive konteksten). `--api-key <key>` gir legitimasjonen for den
  serveren (standard til `AGENTPROXY_API_KEY` miljøvariabelen, eller den aktive kontekstens token).
...
- `--model <id>` på `agentproxy run` følger manifestets per-mål kabling
  (`bin/cli/cli-manifest.mjs`): **aider** mottar `--model openai/<id>` og
  **opencode** `--model agentproxy/<id>` (prefikset legges til kun når id-en
  ikke allerede bærer det); **qwen** og **gemini** mottar id-en ordrett;
  **claude** får det via `ANTHROPIC_MODEL`, **goose** via `GOOSE_MODEL`, og
  **codex** via `-c model_providers.agentproxy.*` argumenter. **Qwen er det eneste kjøre
  målet som hard-krever `--model`** — `agentproxy run qwen` uten det avslutter
  med `2` med en eksplisitt feil.
- `--port <port>` — lokal AgentProxy port (standard `20128`, ignorert når `--remote`
  er satt). Tilstede på alle `setup-*` og begge launcherene.
- `agentproxy run` exit-koder: barn-CLI-ens egen exit-kode blir videreformidlet
...
> Det finnes også en rikere plugin-integrasjon — `agentproxy setup opencode` — som
> installerer `@agentproxy/opencode-plugin`. De er forskjellige kommandoer; tabellen
...
Med AgentProxy som kjører på `localhost:20128`, kjør bare oppsettskommandoen for verktøyet ditt. Katalogen hentes fra den lokale serveren.
...
export AGENTPROXY_API_KEY=sk-...  # referert via {env:AGENTPROXY_API_KEY}, aldri på disk
...
Pek hvilken som helst oppsettskommando mot en fjern AgentProxy med `--remote` + `--api-key`. Katalogen hentes fra den fjerne; konfigurasjonen skrives på din lokale maskin.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # eksporter AGENTPROXY_API_KEY først
...
```bash
agentproxy connect 192.168.0.15        # lager en avgrenset token, lagrer konteksten
agentproxy setup-codex                 # ← bruker nå den fjerne katalogen
agentproxy setup-opencode              # ← samme
agentproxy launch                      # ← Claude Code mot den fjerne
```
...
AgentProxy eksponerer OpenAI-overflaten på `/v1`, Anthropic-overflaten på roten, og en native Gemini-overflate på `/v1beta`. Hver integrasjon er koblet til formen verktøyet forventer (verifisert i kommandokilden):
...
Når du oppdaterer med `agentproxy update` (etter å ha bekreftet, eller med `--apply`),
kjører AgentProxy installasjonen med `--include=optional` innebygd:

```bash
npm install -g agentproxy@latest --include=optional
```

Dette er **ikke** et flagg du sender til `agentproxy update` — det blir alltid brukt av
...
# [DRY RUN] Ville kjørt: npm install -g agentproxy@latest --include=optional
```

Andre `agentproxy update` flagg (verifisert i kildekoden): `--check` (exit 1 hvis
...
(og `:streamGenerateContent?alt=sse`) mot den — nøyaktig AgentProxy sin native
Gemini-overflate (`/v1beta`). `agentproxy run gemini` kobler dette automatisk:

- `GOOGLE_GEMINI_BASE_URL` → den aktive AgentProxy base-URL-en (rot, ingen `/v1`);
- `GEMINI_API_KEY` → den løste AgentProxy-legitimasjonen (alternativ/miljø/kontekst);
- en **midlertidig isolert `GEMINI_CLI_HOME`** hvis `.gemini/settings.json`
  ...
  aldri overskriver den AgentProxy-styrte lanseringen — fjernet etter avslutning;
...
AgentProxy-server, finnes det en opt-in ramme på
...
Valgfritt: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` begrenser feiingen;
`AGENTPROXY_SMOKE_TIMEOUT_MS` overstyrer 120s per-mål tidsgrense.
...
- [Codex CLI-konfigurasjon](./CODEX-CLI-CONFIGURATION.md) — den engangs `[model_providers.agentproxy]` grunnoppsettet
