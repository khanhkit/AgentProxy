...
**AgentProxy** — enhetlig AI proxy/ruter. Ett endepunkt, 329 LLM-leverandører, automatisk fallback.
...
AgentProxy har tre relaterte, men distinkte mekanismer for midlertidig feil. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@agentproxy/open-sse`) → relativ
...
- **Sti-aliaser**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Datakatalog**: `DATA_DIR` miljøvariabel, standard til `~/.agentproxy/`
