...
W przeciwieństwie do lokalnych silników kompresji AgentProxy (Caveman, RTK, LLMLingua, stacked
...
kontekstu. AgentProxy dołącza jedynie parametr w body (`context_management.edits[]`); faktyczne
czyszczenie wykonuje Claude względem własnego tokenizera.

Z natury jest to zdolność delegowana: inni providerzy odrzucają ten parametr, więc AgentProxy
...
AgentProxy wstrzykuje pojedynczą edycję do wychodzącego body Anthropic Messages:
...
Beta jest reklamowana nagłówkiem `anthropic-beta: context-management-2025-06-27`, który AgentProxy
...
AgentProxy stabilnie sortuje edycję `clear_thinking` na początek, bo Anthropic wymaga, by
...
Po odpowiedzi Claude AgentProxy zapisuje, ile kontekstu provider faktycznie usunął. To **nie** jest
...
| ----------------------- | ----------------------------------------------------- | -------------------------------------------- |
| Gdzie działa            | W AgentProxy, zanim żądanie opuści proxy               | U providera (Claude), po stronie serwera     |
...
Te dwa podejścia są komplementarne: lokalne silniki kompresują bajty wysyłane przez AgentProxy;
