...
AgentProxy dostarcza wieloplatformową aplikację desktopową (Windows / macOS / Linux) opartą na
...
| Id aplikacji       | `online.agentproxy.desktop` |
| Nazwa produktu     | `AgentProxy`                |
...
- Windows: `%APPDATA%\agentproxy`
- Linux: `$XDG_CONFIG_HOME/agentproxy` lub `~/.agentproxy`
- macOS: `~/.agentproxy`
...
- Przycisk zamknięcia minimalizuje do trayu; menu trayu ma **Open AgentProxy**, **Open Dashboard** (zewnętrzna przeglądarka), podmenu **Server Port**, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.agentproxy.online https://*.agentproxy.dev`
- Tryb dev dodaje `'unsafe-eval'` tylko do `script-src`

## Auto-aktualizacja

Używa `electron-updater` z providerem GitHub (`khanhkit/AgentProxy`).
...
- `AgentProxy.Setup.X.Y.Z.exe`, `AgentProxy X.Y.Z.exe` (Windows)
- `AgentProxy-X.Y.Z-mac.dmg`, `AgentProxy-X.Y.Z-arm64-mac.dmg` (macOS)
- `AgentProxy-X.Y.Z.AppImage`, `agentproxy-desktop_X.Y.Z_amd64.deb` (Linux)

Wydania są publikowane w GitHub Releases (`khanhkit/AgentProxy`), skąd `electron-updater` sprawdza też nowe wersje.
