---
title: "Tryb zdalny — steruj zdalnym AgentProxy z laptopa"
version: 3.8.40
...
Uruchom CLI `agentproxy` na laptopie, podczas gdy samo AgentProxy działa gdzie indziej
(VPS, serwer domowy, inna maszyna w Tailnet). Logujesz się raz przez
`agentproxy connect`, a odtąd **każde** polecenie CLI celuje w ten zdalny
serwer — te same komendy, ten sam wynik, tylko wykonane względem zdalnego hosta.

Nie ma drugiego narzędzia do instalacji: tryb zdalny to zwykłe CLI `agentproxy`
...
- **Konteksty** przechowują po jednym serwerze (`~/.agentproxy/config.json`, `chmod 600`).
  `agentproxy contexts use <name>` przełącza aktywny serwer; `default` to lokalny.
...
Wygeneruj token ze scope w dashboardzie (lub przez `agentproxy tokens create`) i
...
używającym dołączonego klienta desktop Antigravity, nie tylko w AgentProxy.)
...
Są dwa obsługiwane sposoby podłączenia Antigravity do zdalnego AgentProxy.
...
wklej blob `agentproxy-cred-v1.…` w pole **Step 2** (akceptuje albo
URL callbacku, albo blob poświadczeń). AgentProxy dekoduje go, uruchamia onboarding Cloud Code
...
`redirect_uri` w upstreamowej aplikacji OAuth. AgentProxy nie może go zmienić — provider
...
AgentProxy nasłuchuje na loopbacku **serwera**. Otwórz dashboard pod adresem LAN
...
- **`20128`** (port dashboardu) sprawia, że origin jest prawdziwym localhostem, co w ogóle
  powoduje, że AgentProxy uruchamia serwer callback PKCE — origin LAN nigdy
...
`agentproxy configure` czyta żywy katalog modeli **aktywnego serwera** i zapisuje
...
(`AGENTPROXY_API_KEY`) — sekret nigdy nie trafia na dysk. Jednorazową
bazową konfigurację Codex (blok `[model_providers.agentproxy]`) zobacz w
...
| ----------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | `agentproxy setup-codex`    | profile `~/.codex/<name>.config.toml` (per model)                                                                                                                         |
| Claude Code | `agentproxy setup-claude`   | `~/.claude/profiles/<name>/settings.json` (per model)                                                                                                                     |
| OpenCode    | `agentproxy setup-opencode` | `~/.config/opencode/opencode.json` — provider `agentproxy` openai-compatible z każdym modelem z katalogu (uruchom `opencode -m agentproxy/<model>`)                         |
| Cline       | `agentproxy setup-cline`    | `~/.cline/data/{globalState,secrets}.json` (tryb CLI) + wypisuje ustawienia rozszerzenia VS Code do wklejenia (OpenAI-compatible, Base URL **bez** `/v1`)                 |
| Kilo Code   | `agentproxy setup-kilo`     | `~/.local/share/kilo/auth.json` (CLI) + ustawienia VS Code `kilocode.*` — OpenAI-compatible, Base URL **z** `/v1`                                                         |
| Continue    | `agentproxy setup-continue` | `~/.continue/config.yaml` (VS Code/JetBrains + CLI `cn`) — `provider: openai`, `apiBase` **z** `/v1`, klucz przez `${{ secrets.AGENTPROXY_API_KEY }}`                      |
| Cursor      | `agentproxy setup-cursor`   | wypisuje kroki w aplikacji (Settings → Models → Override OpenAI Base URL **z** `/v1` + klucz + model). Konfiguracja Cursor to nieprzezroczyste SQLite — tylko panel czatu |
| Roo Code    | `agentproxy setup-roo`      | zapisuje JSON importu Roo (`~/.agentproxy/roo-settings.json`) + ustawia `roo-cline.autoImportSettingsPath` + wypisuje kroki UI (OpenAI-compatible, Base URL **z** `/v1`)   |
| Crush       | `agentproxy setup-crush`    | `~/.config/crush/crush.json` — provider `openai-compat`, `base_url` **z** `/v1`, klucz przez `$AGENTPROXY_API_KEY`                                                         |
| Goose       | `agentproxy setup-goose`    | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER=openai` + `OPENAI_HOST` **bez** `/v1` + `GOOSE_MODEL`) + przepis env                                                       |
| Aider       | `agentproxy setup-aider`    | `~/.aider.conf.yml` (`openai-api-base` **bez** `/v1` + `model: openai/<id>`) + przepis env (`aider --message --yes`)                                                      |
| Qwen Code   | `agentproxy setup-qwen`     | wpis V4 `modelProviders.openai` w `~/.qwen/settings.json` + `AGENTPROXY_API_KEY` w `~/.qwen/.env`                                                                          |
...
> OpenCode ma też bogatszą integrację **plugin**: `agentproxy setup opencode`
> (teraz świadome trybu zdalnego przez `--remote`) instaluje `@agentproxy/opencode-plugin`.
> `setup-opencode` to lekka alternatywa openai-compatible. Klucz API
> jest odwoływany przez `{env:AGENTPROXY_API_KEY}` — nigdy nie zapisywany na dysk.

---

## Zarządzanie kontekstami (przełączanie między serwerami)

**Kontekst** to zapisany serwer (baseUrl + poświadczenie + scope). `agentproxy connect`
tworzy jeden i czyni go aktywnym; odtąd każde polecenie go celuje. Zarządzaj i
przełączaj je przez `agentproxy contexts`:
...
> odwołaj token na serwerze przez `agentproxy tokens revoke <id>`, by faktycznie
> unieważnić dostęp.
...
- `agentproxy connect` korzysta z tej samej blokady brute-force logowania + logowania audytowego.
...
- Lokalny plik kontekstów to `~/.agentproxy/config.json` (`chmod 600`); tokeny
