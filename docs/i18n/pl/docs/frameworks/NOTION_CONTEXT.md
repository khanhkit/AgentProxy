...
AgentProxy może połączyć się z workspace'em **Notion** jako **źródłem kontekstu** — bazą
...
oraz dopisywać nowe bloki — wszystko proksowane przez AgentProxy (z retry, timeoutem
...
> strony/bazy, do których AgentProxy ma mieć dostęp (model uprawnień Notion opiera się
...
`POST` AgentProxy zapisuje token i od razu wykonuje testowe wyszukiwanie z 1 wynikiem;
...
`open-sse/mcp-server/server.ts` tylko gdy `AGENTPROXY_MCP_ENFORCE_SCOPES=true`;
dozwolone scope'y wywołującego pochodzą z `AGENTPROXY_MCP_SCOPES` (rozdzielone
