...
title: "Tích hợp CLI — chỉ định bất kỳ CLI lập trình nào vào AgentProxy"
version: 3.8.50
...
AgentProxy cung cấp một loạt các lệnh `setup-*` để cấu hình một CLI lập trình (Codex, Claude Code, OpenCode, Cline, …) sử dụng AgentProxy làm backend — vì vậy công cụ này giao tiếp với **một** điểm cuối và AgentProxy sẽ định tuyến đến nhà cung cấp đúng với chế độ tự động chuyển đổi. Mỗi lệnh đọc danh mục mô hình **trực tiếp** từ một AgentProxy đang chạy (cục bộ hoặc từ xa) và ghi tệp cấu hình của công cụ trên **máy của bạn**. Khóa API được tham chiếu bởi một biến môi trường ở bất kỳ đâu mà công cụ hỗ trợ. Các lệnh mà lưu trữ tệp môi trường cục bộ của công cụ được ghi chú bên dưới.

Cũng có một trình khởi động chung — `agentproxy run <target>` — tạo ra `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` hoặc `gemini` với môi trường đúng được tiêm vào, mà không ghi bất kỳ cấu hình nào. Các mục tiêu và bí danh của chúng đến từ bản khai báo chính thức `bin/cli/cli-manifest.mjs` (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), và `agentproxy completion` cung cấp các từ mục tiêu được lấy từ bản khai báo tương tự. Các trình khởi động theo công cụ cũ — `agentproxy launch` (Claude Code) và `agentproxy launch-codex` (Codex) — vẫn có sẵn.
...
- [Chế độ từ xa](./REMOTE-MODE.md) — điều khiển một AgentProxy từ xa (VPS / Tailnet) từ máy tính xách tay của bạn
...
Mỗi lệnh tôn trọng **ngữ cảnh hoạt động** (được thiết lập với `agentproxy connect`, xem [Chế độ từ xa](./REMOTE-MODE.md)) hoặc các cờ `--remote <url> --api-key <key>` rõ ràng. "Cục bộ so với từ xa" bên dưới có nghĩa là: không có cờ nào nó nhắm đến `http://localhost:20128`; với `--remote` (hoặc một ngữ cảnh từ xa đang hoạt động) nó lấy danh mục từ máy chủ đó và ghi cấu hình cục bộ.

| Lệnh                       | Công cụ                          | Nội dung ghi                                                                                                                                                            | Cờ chính                                                                                                                                   | Cục bộ so với từ xa |
| -------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI                 | `~/.codex/<name>.config.toml` — một hồ sơ cho mỗi mô hình văn bản tương thích (`codex --profile <name>`)                                                                | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Cả hai              |
| `agentproxy setup-claude`   | Claude Code                      | `~/.claude/profiles/<name>/settings.json` — một hồ sơ cho mỗi mô hình phù hợp (`CLAUDE_CONFIG_DIR`)                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Cả hai              |
| `agentproxy setup-opencode` | OpenCode (tương thích openai)    | `~/.config/opencode/opencode.json` — nhà cung cấp `agentproxy` với mọi mô hình trong danh mục (`opencode -m agentproxy/<model>`)                                          | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Cả hai              |
| `agentproxy setup-cline`    | Cline                            | `~/.cline/data/{globalState,secrets}.json` (chế độ CLI) + in cài đặt tiện ích mở rộng VS Code                                                                           | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Cả hai              |
| `agentproxy setup-kilo`     | Kilo Code                        | `~/.local/share/kilo/auth.json` (CLI) + hợp nhất `kilocode.*` vào `settings.json` của VS Code nếu có                                                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Cả hai              |
| `agentproxy setup-continue` | Continue / `cn` CLI              | `~/.continue/config.yaml` — mô hình `provider: openai`, khóa thông qua `${{ secrets.AGENTPROXY_API_KEY }}`                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Cả hai              |
| `agentproxy setup-cursor`   | Cursor                           | Không có gì — in các bước trong ứng dụng (cấu hình Cursor là SQLite không rõ ràng)                                                                                      | `--remote` `--api-key` `--only` `--port`                                                                                                   | Cả hai              |
| `agentproxy setup-roo`      | Roo Code                         | `~/.agentproxy/roo-settings.json` (tài liệu nhập) + thiết lập `roo-cline.autoImportSettingsPath` nếu có `settings.json` của VS Code                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Cả hai              |
| `agentproxy setup-crush`    | Crush                            | `~/.config/crush/crush.json` — nhà cung cấp `openai-compat`, khóa thông qua `$AGENTPROXY_API_KEY`                                                                        | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Cả hai              |
| `agentproxy setup-goose`    | Goose                            | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + in công thức môi trường                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Cả hai              |
| `agentproxy setup-aider`    | Aider                            | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + in công thức môi trường                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Cả hai              |
| `agentproxy setup-qwen`     | Qwen Code                        | `~/.qwen/settings.json` — mảng `modelProviders.openai` V4 + `AGENTPROXY_API_KEY` trong `~/.qwen/.env`                                                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Cả hai              |
| `agentproxy run <target>`   | Khởi động thời gian chạy (chung) | Không có gì — khởi động `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` với môi trường và tham số đúng; Qwen và Gemini sử dụng một thư mục tạm thời cách ly | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Cả hai              |
| `agentproxy launch`         | Claude Code                      | Không có gì — khởi động `claude` với `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` được tiêm vào                                                                          | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Cả hai              |
| `agentproxy launch-codex`   | OpenAI Codex CLI                 | Không có gì — khởi động `codex` với nhà cung cấp `agentproxy` được tiêm qua các cờ `-c`                                                                                  | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Cả hai              |

Ghi chú về các cờ (đã xác minh trong mã lệnh):

- `--remote <url>` — lấy danh mục từ một AgentProxy từ xa (ghi đè `--port` và ngữ cảnh hoạt động). `--api-key <key>` cung cấp thông tin xác thực cho máy chủ đó (mặc định là biến môi trường `AGENTPROXY_API_KEY`, hoặc mã thông báo của ngữ cảnh hoạt động).
...
- `--model <id>` trên `agentproxy run` theo cách kết nối theo từng mục tiêu trong bản khai báo (`bin/cli/cli-manifest.mjs`): **aider** nhận `--model openai/<id>` và **opencode** `--model agentproxy/<id>` (tiền tố chỉ được thêm vào khi ID không đã mang nó); **qwen** và **gemini** nhận ID nguyên văn; **claude** nhận nó qua `ANTHROPIC_MODEL`, **goose** qua `GOOSE_MODEL`, và **codex** qua các tham số `-c model_providers.agentproxy.*`. **Qwen là mục tiêu chạy duy nhất yêu cầu cứng `--model`** — `agentproxy run qwen` mà không có nó thoát `2` với một lỗi rõ ràng.
- `--port <port>` — cổng AgentProxy cục bộ (mặc định `20128`, bị bỏ qua khi `--remote` được thiết lập). Có mặt trên tất cả các lệnh `setup-*` và cả hai trình khởi động.
- Mã thoát của `agentproxy run`: mã thoát của CLI con được truyền đạt nguyên văn; `2` = tham số không hợp lệ (mục tiêu không được hỗ trợ, thiếu `--model` cần thiết, bảo vệ container); `127` = nhị phân mục tiêu không có trong `PATH`; `130`/`143`/`129` khi việc khởi động bị kết thúc bởi `SIGINT`/`SIGTERM`/`SIGHUP`; `1` = lỗi khởi động thời gian chạy khác.
...
> Cũng có một tích hợp plugin phong phú hơn — `agentproxy setup opencode` — mà
> cài đặt `@agentproxy/opencode-plugin`. Chúng là các lệnh khác nhau; bảng
...
Với AgentProxy chạy trên `localhost:20128`, chỉ cần chạy lệnh thiết lập cho công cụ của bạn. Danh mục được lấy từ máy chủ cục bộ.
...
export AGENTPROXY_API_KEY=sk-...  # được tham chiếu qua {env:AGENTPROXY_API_KEY}, không bao giờ trên đĩa
...
```bash
agentproxy launch                 # Claude Code → AgentProxy cục bộ
agentproxy launch-codex           # Codex CLI → AgentProxy cục bộ
...
Chỉ định bất kỳ lệnh thiết lập nào đến một AgentProxy từ xa với `--remote` + `--api-key`. Danh mục được lấy từ xa; cấu hình được ghi trên máy tính cục bộ của bạn.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # xuất AGENTPROXY_API_KEY trước
...
```bash
agentproxy connect 192.168.0.15        # tạo một mã thông báo có phạm vi, lưu ngữ cảnh
agentproxy setup-codex                 # ← bây giờ sử dụng danh mục từ xa
agentproxy setup-opencode              # ← giống nhau
agentproxy launch                      # ← Claude Code chống lại từ xa
```
...
AgentProxy cung cấp bề mặt OpenAI tại `/v1`, bề mặt Anthropic tại gốc, và một bề mặt Gemini gốc tại `/v1beta`. Mỗi tích hợp được kết nối với hình thức mà công cụ của nó mong đợi (được xác minh trong nguồn lệnh):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | với `/v1`          | Có                                          |
...
Khi bạn cập nhật với `agentproxy update` (sau khi xác nhận, hoặc với `--apply`),
AgentProxy sẽ chạy lệnh cài đặt với `--include=optional` được tích hợp sẵn:

```bash
npm install -g agentproxy@latest --include=optional
```

Đây **không** phải là một cờ bạn truyền cho `agentproxy update` — nó luôn được áp dụng bởi
...
Các cờ khác của `agentproxy update` (đã được xác minh trong mã nguồn): `--check` (thoát 1 nếu
có phiên bản cũ), `--apply` (cài đặt mà không cần nhắc), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI qua `agentproxy run gemini`
...
(và `:streamGenerateContent?alt=sse`) chống lại nó — chính xác là bề mặt Gemini gốc của AgentProxy
(`/v1beta`). `agentproxy run gemini` tự động kết nối điều đó:

- `GOOGLE_GEMINI_BASE_URL` → URL cơ sở AgentProxy đang hoạt động (gốc, không có `/v1`);
- `GEMINI_API_KEY` → thông tin xác thực AgentProxy đã được giải quyết (tùy chọn/env/ngữ cảnh);
- một **`GEMINI_CLI_HOME` tạm thời cách ly** mà `.gemini/settings.json`
  ...
  không bao giờ ghi đè lên việc khởi động theo hướng AgentProxy — sẽ bị xóa sau khi thoát;
...
AgentProxy THỰC sự, một khung tùy chọn tồn tại tại
...
Tùy chọn: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` giới hạn quét;
`AGENTPROXY_SMOKE_TIMEOUT_MS` ghi đè thời gian chờ 120 giây cho mỗi mục tiêu.
...
- [Cấu hình Codex CLI](./CODEX-CLI-CONFIGURATION.md) — thiết lập cơ bản một lần `[model_providers.agentproxy]`
