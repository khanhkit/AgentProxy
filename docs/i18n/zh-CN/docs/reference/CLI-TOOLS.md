...
AgentProxy 集成了三类 CLI 工具，分布在三个专用仪表板页面上：

| 页面         | 路由                    | 概念                                                           | 数量     |
| ------------ | ----------------------- | -------------------------------------------------------------- | -------- |
| **CLI 代码** | `/dashboard/cli-code`   | 指向 AgentProxy 的编码工具（客户端 → CLI → AgentProxy → 提供者） | 26       |
| **CLI 代理** | `/dashboard/cli-agents` | 指向 AgentProxy 的自主代理（相同流程，更广泛的范围）            | 8        |
| **ACP 代理** | `/dashboard/acp-agents` | AgentProxy 通过 stdio/ACP 作为后端生成的 CLI（反向流程）        | 见注册表 |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
    ...
    http://YOUR_SERVER:20128/v1
           │
           ▼  (AgentProxy 路由到正确的提供者)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP 代理（反向生成流程）：
    客户端请求 → AgentProxy → 通过 stdio/ACP 生成 CLI → 响应
```
...
您无需手动编写每个工具的配置。AgentProxy 为每个支持的 CLI 提供一个 `setup-*` 命令，该命令从正在运行的 AgentProxy（本地或远程）读取 **实时** 模型目录，并在您的机器上写入工具自己的配置：
...
每个命令接受 `--remote <url> --api-key <key>`（将本地工具配置为远程 AgentProxy），`--dry-run`（预览而不写入）和 `--port`。没有模型自动发现的工具（Cline、Kilo、Roo、Goose、Aider、Qwen）需要 `--model <id>`（并且 `--yes` 用于非交互式运行）。要启动一个 CLI，并注入正确的环境而不写入任何配置，请使用通用的 `agentproxy run <target>` 启动器（claude、codex、aider、goose、opencode、qwen、gemini — 目标和别名来自 `bin/cli/cli-manifest.mjs`）；遗留的每个工具启动器 `agentproxy launch`（Claude Code）和 `agentproxy launch-codex`（Codex）仍然可用。Gemini CLI 仅用于启动：它是一个 `agentproxy run` 目标，但没有 `setup-*`/`configure` 配方。

> **完整参考：** 主表 — 每个命令写入的内容、每个标志、本地与远程，以及哪些工具需要 `/v1` 后缀 — 位于 **[CLI 集成](../guides/CLI-INTEGRATIONS.md)**。

### 在容器内运行这些命令

在 AgentProxy 容器内执行的 `setup-*` 命令会写入容器自己的主目录，主机 CLI 无法读取，并且随着容器的消失而消失。AgentProxy 检测到这一点并以 `2` 退出，给出说明而不是写入。前进的两种支持方式 — 在主机上安装 CLI 并 `agentproxy connect` 到容器，或绑定挂载配置目录并设置 `CLI_CONFIG_HOME`（compose `host` 配置文件）。每个 `setup-*` 命令，以及 `agentproxy configure` 和 `agentproxy config set`，在配置容器自己的 CLI 时接受 `--allow-container-write`；`AGENTPROXY_ALLOW_CONTAINER_CONFIG_WRITE=true` 对服务器也有相同效果。请参见
[Docker 指南 → 配置主机 CLI 工具](../guides/DOCKER_GUIDE.md#configuring-host-cli-tools-when-agentproxy-runs-in-docker)。

仪表板的 **应用端点** (`POST /api/cli-tools/apply`) 强制执行相同的保护：在容器中，目标不是从主机绑定挂载的写入会返回 **`422`**，并带有 `containerEphemeralTarget: true`，安全错误文本，以及对于具有主机配方的工具（claude、codex、opencode、cline、kilo、continue） — 一个 `hostSetupCommand`（例如 `agentproxy setup-opencode`）以便在主机上运行；不会写入任何内容。`dryRun: true` 在容器模式下继续工作，并返回生成的内容 + 目标路径而不触及磁盘，因此您可以从仪表板预览并在主机上应用。此行为是故意的，并通过 `tests/unit/api/cli-tools/apply-container-guard.test.ts` 进行回归保护 — 永远不要通过移除保护来“修复” 422。
...
| **可配置** | 由 `agentproxy configure <cli>` 支持（存在设置配方）      | `bin/cli/cli-manifest.mjs` (`configure: true`)               |
| **可启动** | 由 `agentproxy run <target>` 支持（定义了 env/args 注入） | `bin/cli/cli-manifest.mjs` (`run: true`)                     |
...
此页面（从 `/dashboard/agents` 重命名）显示 AgentProxy 可以通过 stdio/ACP 协议 **生成** 的 CLI 作为后端执行引擎。目录在 `src/lib/acp/registry.ts` 中单独维护，并且与 `CLI_TOOLS` **不相同**。
...
# Google Gemini CLI (可通过 `agentproxy run gemini` 启动 → /v1beta surface)
...
密钥保留在 `AGENTPROXY_API_KEY` 环境变量中（`env_key`），而不是文件内：
...
或使用 AgentProxy 仪表板 → **CLI 工具 → Cline → 应用配置**。
...
或使用 AgentProxy 仪表板 → **CLI 工具 → KiloCode → 应用配置**。
...
当 VS Code Insiders 配置为自定义端点模型时使用此配置，且希望 AgentProxy 在没有自定义头字段的情况下工作。
...
- 将 `sk-your-agentproxy-key` 替换为在 AgentProxy 中创建的 API 密钥。
...
# CLI 使用自己的身份验证 — AgentProxy 不需要作为 Kiro CLI 本身的后端。
# 将 kiro-cli 与 AgentProxy 一起使用以支持其他工具。
kiro-cli status
```

对于 **Kiro IDE** 桌面应用，使用 AgentProxy 在 `/dashboard/cli-tools → Kiro` 下暴露的 MITM 端点。

---

## 10. 内部 AgentProxy CLI

`agentproxy` 二进制文件提供服务器生命周期、设置、诊断和提供者管理的命令。入口点：`bin/agentproxy.mjs`。

```bash
agentproxy                              # 启动服务器（默认端口 20128）
agentproxy setup                        # 交互式设置向导
agentproxy doctor                       # 检查配置、数据库、端口、运行时
agentproxy providers list               # 配置的提供者连接
agentproxy providers test-all           # 测试每个活动连接
agentproxy reset-password               # 重置管理员密码
agentproxy logs                         # 流式请求日志
agentproxy health                       # 详细健康状态（断路器、缓存、内存）
agentproxy --version                    # 打印版本
agentproxy --help                       # 显示所有命令
```
...
```bash
agentproxy setup                        # 交互式设置向导
agentproxy setup --non-interactive      # CI/自动化模式（读取环境变量 + 标志）
agentproxy setup --password '<value>'   # 直接设置管理员密码
...
| 变量                | 目的                                                          |
| ------------------- | ------------------------------------------------------------- |
| `AGENTPROXY_API_KEY` | 提供者 API 密钥（通过 Commander `.env()` 绑定到 `--api-key`） |
| `DATA_DIR`          | 覆盖 AgentProxy 数据目录                                       |

所有其他非交互式输入作为标志传递，而不是环境变量：
`--password`、`--provider`、`--provider-name`、`--provider-base-url`、`--default-model`
（请参见上面的 `agentproxy setup` 选项）。

### 诊断

```bash
agentproxy doctor                       # 检查配置、数据库、端口、运行时、内存、存活性
agentproxy doctor --json                # 机器可读的 JSON
agentproxy doctor --no-liveness         # 跳过 HTTP 健康探测
agentproxy doctor --host 0.0.0.0        # 覆盖存活性主机
agentproxy doctor --liveness-url <url>  # 完整健康端点 URL 覆盖
```
...
```bash
agentproxy providers available                       # AgentProxy 提供者目录
agentproxy providers available --search openai       # 按 id/名称/别名/类别过滤目录
agentproxy providers available --category api-key    # 按类别过滤（api-key、oauth、free 等）
agentproxy providers available --json                # 机器可读的 JSON

agentproxy providers list                            # 配置的提供者连接
agentproxy providers list --json

agentproxy providers test <id|name>                  # 测试一个配置的连接
agentproxy providers test-all                        # 测试每个活动连接
agentproxy providers validate                        # 仅限本地的结构验证
agentproxy providers add <provider> --credential-env PROVIDER_KEY
agentproxy providers import ./providers.json --dry-run --json
agentproxy providers auth <provider>                 # 现有的 OAuth 流程
...
已编辑的存在/形状。`providers available` 读取 AgentProxy 目录；
...
```bash
agentproxy reset-password                # 重置管理员密码（也可使用：agentproxy-reset-password）
agentproxy reset-encrypted-columns       # 显示警告 + 加密凭证重置的干运行
agentproxy reset-encrypted-columns --force  # 实际清空 SQLite 中的加密凭证
```

### 凭证导出 (⚠ 小心处理)

```bash
agentproxy auth export                                 # 显示警告 + 确认门 — 无数据库访问
agentproxy auth export --force                          # 将所有连接的解密凭证导出到 stdout 作为 JSON
agentproxy auth export --force --id <id>                 # 仅导出匹配的连接
agentproxy auth export --force --format env               # 输出 AGENTPROXY_<PROVIDER>_<FIELD>=<value> 行
agentproxy auth export --force --out creds.json           # 写入文件（以 0600 权限创建）
```
...
这些假定正在运行的 AgentProxy 服务器，除非另有说明：

```bash
agentproxy status                       # 综合运行时状态
agentproxy logs                         # 流式请求日志 (--json, --search, --follow)
agentproxy config show                  # 显示当前配置

agentproxy provider list                # 列出可用提供者（providers list 的别名）
agentproxy provider add                 # 将 AgentProxy 注册为工具上的提供者
agentproxy keys add | list | remove     # 管理 API 密钥
agentproxy models [provider]            # 列出模型 (--json, --search)
agentproxy combo list | switch | create | delete

agentproxy backup                       # 快照配置 + 数据库
agentproxy restore                      # 从先前的快照恢复

agentproxy health                       # 详细健康状态（断路器、缓存、内存）
agentproxy quota                        # 提供者配额使用情况
agentproxy cache                        # 缓存状态
agentproxy cache clear                  # 清除语义 + 签名缓存

agentproxy mcp status | restart         # MCP 服务器状态 / 重启
agentproxy a2a status | card            # A2A 服务器状态 / 代理卡

agentproxy tunnel list | create | stop  # 管理隧道（cloudflare/tailscale/ngrok）
agentproxy env show | get <k> | set <k> <v>  # 检查 / 设置环境变量（临时）

agentproxy test                         # 提供者连接性烟雾测试
agentproxy update                       # 检查更新
...
| 错误                            | 原因                   | 修复                                       |
| ------------------------------- | ---------------------- | ------------------------------------------ |
| `Connection refused`            | AgentProxy 未运行       | `agentproxy serve`                          |
