...
title: "CLI 集成 — 将任何编码 CLI 指向 AgentProxy"
version: 3.8.50
...
AgentProxy 提供了一系列 `setup-*` 命令，用于配置编码 CLI（Codex、Claude Code、OpenCode、Cline 等）以使用 AgentProxy 作为其后端——因此该工具只需与 **一个** 端点通信，AgentProxy 会自动路由到正确的提供者并进行自动回退。每个命令从运行中的 AgentProxy（本地或远程）读取 **实时** 模型目录，并在 **你的** 机器上写入工具自己的配置文件。API 密钥通过环境变量引用，工具支持的地方均如此。持久化工具本地环境文件的命令在下面注明。

还有一个通用启动器 — `agentproxy run <target>` — 它会启动 `claude`、`codex`、`aider`、`goose`、`opencode`、`qwen` 或 `gemini`，并注入正确的环境，而无需写入任何配置。目标及其别名来自规范清单 `bin/cli/cli-manifest.mjs`（`claude-code|cc|anthropic`、`codex-cli|openai-codex|openai`、`goose-cli`、`open-code`、`qwen-code`、`gemini-cli`），而 `agentproxy completion` 提供相同的基于清单的目标词。遗留的每个工具启动器 — `agentproxy launch`（Claude Code）和 `agentproxy launch-codex`（Codex） — 仍然可用。
...
- [远程模式](./REMOTE-MODE.md) — 从你的笔记本电脑驱动远程 AgentProxy（VPS / Tailnet）
...
每个命令都遵循 **活动上下文**（通过 `agentproxy connect` 设置，见 [远程模式](./REMOTE-MODE.md)）或显式的 `--remote <url> --api-key <key>` 标志。下面的“本地与远程”意味着：没有标志时，它的目标是 `http://localhost:20128`；使用 `--remote`（或活动的远程上下文）时，它从该服务器获取目录并在本地写入配置。

| 命令                       | 工具                    | 写入内容                                                                                                                           | 关键标志                                                                                                                                   | 本地与远程 |
| -------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI        | `~/.codex/<name>.config.toml` — 每个兼容文本模型一个配置文件（`codex --profile <name>`）                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | 两者       |
| `agentproxy setup-claude`   | Claude Code             | `~/.claude/profiles/<name>/settings.json` — 每个匹配模型一个配置文件（`CLAUDE_CONFIG_DIR`）                                        | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | 两者       |
| `agentproxy setup-opencode` | OpenCode（兼容 openai） | `~/.config/opencode/opencode.json` — 包含每个目录模型的 `agentproxy` 提供者（`opencode -m agentproxy/<model>`）                      | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | 两者       |
| `agentproxy setup-cline`    | Cline                   | `~/.cline/data/{globalState,secrets}.json`（CLI 模式） + 打印 VS Code 扩展设置                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | 两者       |
| `agentproxy setup-kilo`     | Kilo Code               | `~/.local/share/kilo/auth.json`（CLI） + 如果存在，则将 `kilocode.*` 合并到 VS Code `settings.json`                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | 两者       |
| `agentproxy setup-continue` | Continue / `cn` CLI     | `~/.continue/config.yaml` — `provider: openai` 模型，通过 `${{ secrets.AGENTPROXY_API_KEY }}` 提供密钥                              | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | 两者       |
| `agentproxy setup-cursor`   | Cursor                  | 无 — 打印应用内步骤（Cursor 配置是模糊的 SQLite）                                                                                  | `--remote` `--api-key` `--only` `--port`                                                                                                   | 两者       |
| `agentproxy setup-roo`      | Roo Code                | `~/.agentproxy/roo-settings.json`（导入文档） + 如果存在 VS Code `settings.json`，则设置 `roo-cline.autoImportSettingsPath`         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | 两者       |
| `agentproxy setup-crush`    | Crush                   | `~/.config/crush/crush.json` — `openai-compat` 提供者，通过 `$AGENTPROXY_API_KEY` 提供密钥                                          | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | 两者       |
| `agentproxy setup-goose`    | Goose                   | `~/.config/goose/config.yaml`（`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`） + 打印环境配方                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | 两者       |
| `agentproxy setup-aider`    | Aider                   | `~/.aider.conf.yml`（`openai-api-base` + `model: openai/<id>`） + 打印环境配方                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | 两者       |
| `agentproxy setup-qwen`     | Qwen Code               | `~/.qwen/settings.json` — V4 `modelProviders.openai` 数组 + `AGENTPROXY_API_KEY` 在 `~/.qwen/.env`                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | 两者       |
| `agentproxy run <target>`   | 运行时启动（通用）      | 无 — 启动 `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini`，并使用正确的环境和参数；Qwen 和 Gemini 使用临时隔离的主目录 | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | 两者       |
| `agentproxy launch`         | Claude Code             | 无 — 启动 `claude`，注入 `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`                                                               | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | 两者       |
| `agentproxy launch-codex`   | OpenAI Codex CLI        | 无 — 启动 `codex`，通过 `-c` 标志注入 `agentproxy` 提供者                                                                           | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | 两者       |

关于标志的说明（在命令源中验证）：

- `--remote <url>` — 从远程 AgentProxy 获取目录（覆盖 `--port` 和活动上下文）。`--api-key <key>` 为该服务器提供凭据（默认为 `AGENTPROXY_API_KEY` 环境变量，或活动上下文的令牌）。
...
- `--model <id>` 在 `agentproxy run` 上遵循清单的每个目标连接（`bin/cli/cli-manifest.mjs`）：**aider** 接收 `--model openai/<id>`，**opencode** 接收 `--model agentproxy/<id>`（前缀仅在 ID 不包含时添加）；**qwen** 和 **gemini** 直接接收 ID；**claude** 通过 `ANTHROPIC_MODEL` 获取，**goose** 通过 `GOOSE_MODEL`，**codex** 通过 `-c model_providers.agentproxy.*` 参数获取。**Qwen 是唯一一个强制要求 `--model` 的运行目标** — `agentproxy run qwen` 如果没有它将以明确错误退出 `2`。
- `--port <port>` — 本地 AgentProxy 端口（默认 `20128`，在设置 `--remote` 时被忽略）。在所有 `setup-*` 和两个启动器上均存在。
- `agentproxy run` 退出代码：子 CLI 的自身退出代码被逐字传播；`2` = 无效参数（不支持的目标，缺少必需的 `--model`，容器保护）；`127` = 目标二进制不在 `PATH` 中；`130`/`143`/`129` 当启动被 `SIGINT`/`SIGTERM`/`SIGHUP` 结束时；`1` = 其他运行时启动失败。
...
> 还有一个更丰富的插件集成 — `agentproxy setup opencode` — 它安装 `@agentproxy/opencode-plugin`。这两个命令不同；上表记录了 `setup-opencode`。

---

## 本地使用

在 `localhost:20128` 上运行 AgentProxy，只需为您的工具运行设置命令。目录从本地服务器获取。
...
export AGENTPROXY_API_KEY=sk-...  # 通过 {env:AGENTPROXY_API_KEY} 引用，绝不存储在磁盘上
...
将任何设置命令指向远程 AgentProxy，使用 `--remote` + `--api-key`。目录从远程获取；配置写入您的本地机器。

```bash
# OpenCode 针对远程 VPS，仅保留 glm/kimi 模型
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # 首先导出 AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # 生成一个作用域令牌，存储上下文
agentproxy setup-codex                 # ← 现在使用远程目录
agentproxy setup-opencode              # ← 同上
agentproxy launch                      # ← Claude Code 针对远程
```
...
AgentProxy 在 `/v1` 上暴露 OpenAI 接口，在根目录上暴露 Anthropic 接口，并在 `/v1beta` 上提供原生 Gemini 接口。每个集成都连接到其工具所期望的形式（在命令源中验证）：
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | 带 `/v1`       | 是                                       |
...
当你使用 `agentproxy update` 更新时（在确认后，或使用 `--apply`），
AgentProxy 会自动运行带有 `--include=optional` 的安装：

```bash
npm install -g agentproxy@latest --include=optional
```

这**不是**你传递给 `agentproxy update` 的标志——它始终由更新器应用。它保证 `optionalDependencies`（`better-sqlite3`、`keytar`、`tls-client`、LLMLingua SLM 堆栈）在更新后仍然存在，即使你的 npm 配置中设置了 `omit=optional`，否则会默默地丢弃本地 SQLite 驱动程序和操作系统密钥绑定。要预览确切的命令而不应用：

```bash
agentproxy update --dry-run
# [干运行] 将运行：npm install -g agentproxy@latest --include=optional
```

其他 `agentproxy update` 标志（在源代码中验证）：`--check`（如果过时则退出 1）、`--apply`（无提示安装）、`--changelog`、`--no-backup`、`--yes`。

---

## 通过 `agentproxy run gemini` 使用 Google Gemini CLI

与 `@google/gemini-cli` 0.50.0 验证的合同：CLI 尊重 `GOOGLE_GEMINI_BASE_URL` 并对其发出 `POST /v1beta/models/<model>:generateContent`
（和 `:streamGenerateContent?alt=sse`）——这正是 AgentProxy 的本地
Gemini 接口（`/v1beta`）。`agentproxy run gemini` 会自动连接这些：

- `GOOGLE_GEMINI_BASE_URL` → 活动的 AgentProxy 基础 URL（根，不带 `/v1`）；
- `GEMINI_API_KEY` → 解析后的 AgentProxy 凭证（选项/环境/上下文）；
- **临时隔离的 `GEMINI_CLI_HOME`**，其 `.gemini/settings.json`
  选择 `gemini-api-key` 认证，因此存储的 Google OAuth 会话（代码助手）
  永远不会覆盖 AgentProxy 指定的启动——退出后删除；
...
可选：`AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` 限制测试范围；
`AGENTPROXY_SMOKE_TIMEOUT_MS` 覆盖每个目标的 120 秒超时。
...
- [Codex CLI 配置](./CODEX-CLI-CONFIGURATION.md) — 一次性的 `[model_providers.agentproxy]` 基础设置
