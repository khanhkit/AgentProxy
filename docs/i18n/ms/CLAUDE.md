...
**AgentProxy** — proksi/router AI yang bersatu. Satu titik akhir, 329 penyedia LLM, auto-fallback.
...
AgentProxy mempunyai tiga mekanisme kegagalan sementara yang berkaitan tetapi berbeza. Pastikan skop mereka terpisah semasa menyahpepijat tingkah laku penghalaan. Lihat
...
- **Import**: luaran → dalaman (`@/`, `@agentproxy/open-sse`) → relatif
...
- **Alias laluan**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Port lalai**: 20128 (API + papan pemuka pada port yang sama)
- **Direktori data**: `DATA_DIR` env var, lalai kepada `~/.agentproxy/`
