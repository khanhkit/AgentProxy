...
**AgentProxy** — proxy/router AI unificat. Un endpoint, 329 furnizori LLM, fallback automat.
...
AgentProxy are trei mecanisme de eșec temporar, legate dar distincte. Mențineți domeniul lor separat atunci când depanați comportamentul rutării. Consultați diagrama [rezilienței cu 3 straturi](./docs/diagrams/exported/resilience-3layers.svg)
...
- **Importuri**: externe → interne (`@/`, `@agentproxy/open-sse`) → relative
...
- **Aliasuri de cale**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Port implicit**: 20128 (API + dashboard pe același port)
- **Director de date**: `DATA_DIR` variabilă de mediu, implicit `~/.agentproxy/`
