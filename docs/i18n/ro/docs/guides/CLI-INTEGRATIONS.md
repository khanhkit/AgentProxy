...
AgentProxy oferă o familie de comenzi `setup-*` care configurează un CLI de codare
(Codex, Claude Code, OpenCode, Cline, …) pentru a folosi AgentProxy ca backend — astfel
încât instrumentul comunică cu **un** endpoint și AgentProxy redirecționează către furnizorul corect cu
fallback automat. Fiecare comandă citește catalogul de modele **live** de la un AgentProxy
...
Există, de asemenea, un launcher generic — `agentproxy run <target>` — care lansează
...
`open-code`, `qwen-code`, `gemini-cli`), iar `agentproxy completion` oferă
aceleași cuvinte țintă derivate din manifest. Launcherele legate de fiecare instrument —
`agentproxy launch` (Claude Code) și `agentproxy launch-codex` (Codex) — rămân
disponibile.
...
- [Modul Remote](./REMOTE-MODE.md) — controlează un AgentProxy la distanță (VPS / Tailnet) de pe laptopul tău
...
Fiecare comandă respectă **contextul activ** (setat cu `agentproxy connect`, vezi
...
| -------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — un profil pentru fiecare model de text compatibil (`codex --profile <name>`)                                                   | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Ambele          |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — un profil pentru fiecare model potrivit (`CLAUDE_CONFIG_DIR`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Ambele          |
| `agentproxy setup-opencode` | OpenCode (compatibil openai) | `~/.config/opencode/opencode.json` — furnizor `agentproxy` cu fiecare model din catalog (`opencode -m agentproxy/<model>`)                                       | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Ambele          |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (mod CLI) + imprimă setările extensiei VS Code                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Ambele          |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + îmbină `kilocode.*` în `settings.json` al VS Code, dacă este prezent                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Ambele          |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — modele `provider: openai`, cheie prin `${{ secrets.AGENTPROXY_API_KEY }}`                                                           | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Ambele          |
| `agentproxy setup-cursor`   | Cursor                       | Nimic — imprimă pașii în aplicație (configurația Cursor este opacă SQLite)                                                                                     | `--remote` `--api-key` `--only` `--port`                                                                                                   | Ambele          |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (document de import) + setează `roo-cline.autoImportSettingsPath` dacă există un `settings.json` al VS Code                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Ambele          |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — furnizor `openai-compat`, cheie prin `$AGENTPROXY_API_KEY`                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Ambele          |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + imprimă rețeta de mediu                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Ambele          |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + imprimă rețeta de mediu                                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Ambele          |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — array `V4 modelProviders.openai` + `AGENTPROXY_API_KEY` în `~/.qwen/.env`                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Ambele          |
| `agentproxy run <target>`   | Lansare runtime (generic)    | Nimic — lansează `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` cu mediul și argumentele corecte; Qwen și Gemini folosesc un home izolat temporar | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Ambele          |
| `agentproxy launch`         | Claude Code                  | Nimic — lansează `claude` cu `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injectat                                                                              | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Ambele          |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | Nimic — lansează `codex` cu furnizorul `agentproxy` injectat prin flag-uri `-c`                                                                                 | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Ambele          |

Note despre flag-uri (verificate în sursa comenzii):

- `--remote <url>` — preia catalogul de la un AgentProxy la distanță (suprascrie `--port`
  și contextul activ). `--api-key <key>` furnizează acreditivul pentru acel
  server (se default-ează la variabila de mediu `AGENTPROXY_API_KEY`, sau token-ul contextului activ).
...
- `--model <id>` pe `agentproxy run` urmează conectarea per-țintă din manifest
  (`bin/cli/cli-manifest.mjs`): **aider** primește `--model openai/<id>` și
  **opencode** `--model agentproxy/<id>` (prefixul este adăugat doar când id-ul
  nu îl poartă deja); **qwen** și **gemini** primesc id-ul exact; **claude** îl primește prin `ANTHROPIC_MODEL`, **goose** prin `GOOSE_MODEL`, și
  **codex** prin argumente `-c model_providers.agentproxy.*`. **Qwen este singura țintă de execuție
  care necesită în mod strict `--model`** — `agentproxy run qwen` fără el iese
  `2` cu o eroare explicită.
- `--port <port>` — portul local AgentProxy (default `20128`, ignorat când `--remote`
  este setat). Prezent pe toate comenzile `setup-*` și pe ambele launchere.
- Codurile de ieșire ale `agentproxy run`: codul de ieșire al CLI-ului copil este propagat
...
> Există de asemenea o integrare mai bogată a plugin-ului — `agentproxy setup opencode` — care
> instalează `@agentproxy/opencode-plugin`. Acestea sunt comenzi diferite; tabelul
...
Cu AgentProxy rulând pe `localhost:20128`, pur și simplu rulează comanda de configurare pentru instrumentul tău. Catalogul este obținut de la serverul local.
...
export AGENTPROXY_API_KEY=sk-...  # referit prin {env:AGENTPROXY_API_KEY}, niciodată pe disc
...
agentproxy run claude -- --print-system-prompt "revizuiește acest diff"
```
...
Indică orice comandă de configurare către un AgentProxy la distanță cu `--remote` + `--api-key`. Catalogul este obținut de la distanță; configurația este scrisă pe mașina ta locală.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # exportă mai întâi AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # generează un token scoperit, stochează contextul
agentproxy setup-codex                 # ← acum folosește catalogul la distanță
agentproxy setup-opencode              # ← același lucru
agentproxy launch                      # ← Claude Code împotriva distanței
```
...
AgentProxy expune suprafața OpenAI la `/v1`, suprafața Anthropic la rădăcină, și o suprafață nativă Gemini la `/v1beta`. Fiecare integrare este conectată la forma pe care instrumentul său o așteaptă (verificat în sursa comenzii):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | cu `/v1`          | Da                                         |
...
Când actualizezi cu `agentproxy update` (după confirmare sau cu `--apply`),
AgentProxy rulează instalarea cu `--include=optional` inclus:

```bash
npm install -g agentproxy@latest --include=optional
```

Aceasta **nu** este o opțiune pe care o transmiți la `agentproxy update` — este întotdeauna aplicată de
...
# [DRY RUN] Ar rula: npm install -g agentproxy@latest --include=optional
```

Alte opțiuni `agentproxy update` (verificate în sursă): `--check` (iese cu 1 dacă
este depășit), `--apply` (instalează fără a solicita), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI prin `agentproxy run gemini`
...
Gemini a AgentProxy (`/v1beta`). `agentproxy run gemini` conectează asta automat:

- `GOOGLE_GEMINI_BASE_URL` → URL-ul de bază activ AgentProxy (rădăcină, fără `/v1`);
- `GEMINI_API_KEY` → acreditivul rezolvat AgentProxy (opțiune/env/context);
- un **`GEMINI_CLI_HOME`** temporar izolat al cărui `.gemini/settings.json`
  ...
  să nu suprascrie lansarea dirijată de AgentProxy — eliminată după ieșire;
...
AgentProxy, există un cadru opțional la
...
Opțional: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` restricționează verificarea;
`AGENTPROXY_SMOKE_TIMEOUT_MS` suprascrie timeout-ul de 120s pe țintă.
...
- [Configurarea Codex CLI](./CODEX-CLI-CONFIGURATION.md) — configurarea de bază `[model_providers.agentproxy]` unică
