...
AgentProxy는 세 가지 전용 대시보드 페이지에 걸쳐 세 가지 범주의 CLI 도구와 통합됩니다:

| 페이지           | 경로                    | 개념                                                                   | 수량            |
| ---------------- | ----------------------- | ---------------------------------------------------------------------- | --------------- |
| **CLI 코드**     | `/dashboard/cli-code`   | AgentProxy를 가리키는 코딩 도구 (클라이언트 → CLI → AgentProxy → 공급자) | 26              |
| **CLI 에이전트** | `/dashboard/cli-agents` | AgentProxy를 가리키는 자율 에이전트 (같은 흐름, 더 넓은 범위)           | 8               |
| **ACP 에이전트** | `/dashboard/acp-agents` | AgentProxy가 stdio/ACP를 통해 백엔드로 생성하는 CLI (역방향 흐름)       | 레지스트리 참조 |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
    ...
    http://YOUR_SERVER:20128/v1
           │
           ▼  (AgentProxy가 올바른 공급자로 라우팅)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP 에이전트 (역방향 생성 흐름):
    클라이언트 요청 → AgentProxy → stdio/ACP를 통해 CLI 생성 → 응답
```
...
각 도구의 구성을 수동으로 작성할 필요가 없습니다. AgentProxy는 실행 중인 AgentProxy(로컬 또는 원격)에서 **실시간** 모델 카탈로그를 읽고 도구의 자체 구성을 귀하의 머신에 작성하는 `setup-*` 명령을 제공합니다:
...
각 명령은 `--remote <url> --api-key <key>` (원격 AgentProxy에 대해 로컬 도구 구성), `--dry-run` (작성하지 않고 미리보기), 및 `--port`를 수용합니다. 모델 자동 검색이 없는 도구(Cline, Kilo, Roo, Goose, Aider, Qwen)는 `--model <id>` (비대화형 실행을 위한 `--yes`)를 사용합니다. 올바른 환경이 주입되고 전혀 구성되지 않은 CLI를 실행하려면 일반적인 `agentproxy run <target>` 실행기를 사용하십시오 (claude, codex, aider, goose, opencode, qwen, gemini — 대상 및 별칭은 `bin/cli/cli-manifest.mjs`에서 가져옵니다); 레거시 도구별 실행기 `agentproxy launch` (Claude Code) 및 `agentproxy launch-codex` (Codex)도 사용할 수 있습니다. Gemini CLI는 실행 전용입니다: `agentproxy run` 대상이지만 `setup-*`/`configure` 레시피가 없습니다.

> **전체 참조:** 마스터 테이블 — 각 명령이 작성하는 내용, 모든 플래그, 로컬 대 원격, `/v1` 접미사를 원하는 도구 — 는 **[CLI 통합](../guides/CLI-INTEGRATIONS.md)**에 있습니다.

### 컨테이너 내에서 실행하기

AgentProxy 컨테이너 내에서 실행된 `setup-*` 명령은 컨테이너의 자체 홈에 작성되며, 이는 호스트 CLI가 읽지 않으며 컨테이너와 함께 사라집니다. AgentProxy는 이를 감지하고 작성하는 대신 지침과 함께 `2`로 종료합니다. 두 가지 지원되는 방법 — 호스트에 CLI를 설치하고 `agentproxy connect`를 통해 컨테이너에 연결하거나, 구성 디렉토리를 바인드 마운트하고 `CLI_CONFIG_HOME`을 설정합니다 (compose `host` 프로필). 모든 `setup-*` 명령, plus `agentproxy configure` 및 `agentproxy config set`는 컨테이너의 자체 CLI를 구성하는 것이 실제로 의미하는 경우 `--allow-container-write`를 수용합니다; `AGENTPROXY_ALLOW_CONTAINER_CONFIG_WRITE=true`는 서버에 대해 동일한 작업을 수행합니다. [Docker 가이드 → 호스트 CLI 도구 구성](../guides/DOCKER_GUIDE.md#configuring-host-cli-tools-when-agentproxy-runs-in-docker)을 참조하십시오.

대시보드의 **적용 엔드포인트** (`POST /api/cli-tools/apply`)는 동일한 보호 장치를 시행합니다: 컨테이너 내에서 호스트에서 바인드 마운트되지 않은 대상을 대상으로 하는 쓰기는 **`422`**로 응답하며 `containerEphemeralTarget: true`, 안전한 오류 텍스트 및 — 호스트 레시피가 있는 도구(claude, codex, opencode, cline, kilo, continue)에 대해 — 호스트에서 실행할 `hostSetupCommand` (예: `agentproxy setup-opencode`)를 제공합니다; 아무것도 작성되지 않습니다. `dryRun: true`는 컨테이너 모드에서도 작동하며 디스크를 건드리지 않고 생성된 콘텐츠 + 대상 경로를 반환하므로 대시보드에서 미리 보고 호스트에서 적용할 수 있습니다. 이 동작은 의도적이며 `tests/unit/api/cli-tools/apply-container-guard.test.ts`에 의해 회귀 방지됩니다 — 422를 "수정"하여 보호 장치를 제거하지 마십시오.
...
| **구성 가능**  | `agentproxy configure <cli>`로 지원됨 (설정 레시피 존재)      | `bin/cli/cli-manifest.mjs` (`configure: true`)                    |
| **실행 가능**  | `agentproxy run <target>`로 지원됨 (env/args 주입 정의됨)     | `bin/cli/cli-manifest.mjs` (`run: true`)                          |
...
이 페이지는 `/dashboard/agents`에서 이름이 변경되었으며, AgentProxy가 stdio/ACP 프로토콜을 통해 **생성**할 수 있는 백엔드 실행 엔진으로서의 CLI를 보여줍니다. 카탈로그는 `src/lib/acp/registry.ts`에서 별도로 유지되며 `CLI_TOOLS`와는 **다릅니다**.
...
키는 `AGENTPROXY_API_KEY` 환경 변수 (`env_key`)에 남아 있으며,
...
> `opencode run "your prompt" --model agentproxy/claude-sonnet-4-5-thinking --variant high`를 사용하여
> 사고 변형을 전송하세요.
...
또는 AgentProxy 대시보드를 사용하세요 → **CLI 도구 → Cline → 구성 적용**.
...
또는 AgentProxy 대시보드를 사용하세요 → **CLI 도구 → KiloCode → 구성 적용**.
...
VS Code Insiders가 사용자 정의 엔드포인트 모델에 대해 구성되어 있고 AgentProxy가 사용자 정의 헤더 필드 없이 작동하도록 하려면 이 파일을 사용하세요.
...
- `sk-your-agentproxy-key`를 AgentProxy에서 생성한 API 키로 교체하세요.
...
# CLI는 자체 인증을 사용합니다 — Kiro CLI 자체에 AgentProxy가 필요하지 않습니다.
# 다른 도구와 함께 AgentProxy와 함께 kiro-cli를 사용하세요.
kiro-cli status
```

**Kiro IDE** 데스크탑 앱의 경우 AgentProxy에서 노출된 MITM 엔드포인트를 사용하세요
`/dashboard/cli-tools → Kiro` 아래에서.

---

## 10. 내부 AgentProxy CLI

`agentproxy` 바이너리는 서버 생명 주기, 설정, 진단 및 공급자 관리를 위한 명령을 제공합니다. 진입점: `bin/agentproxy.mjs`.

```bash
agentproxy                              # 서버 시작 (기본 포트 20128)
agentproxy setup                        # 대화형 설정 마법사
agentproxy doctor                       # 구성, DB, 포트, 런타임 확인
agentproxy providers list               # 구성된 공급자 연결
agentproxy providers test-all           # 모든 활성 연결 테스트
agentproxy reset-password               # 관리자 비밀번호 재설정
agentproxy logs                         # 요청 로그 스트리밍
agentproxy health                       # 상세 건강 상태 (회로 차단기, 캐시, 메모리)
agentproxy --version                    # 버전 출력
agentproxy --help                       # 모든 명령 보기
```
...
```bash
agentproxy setup                        # 대화형 설정 마법사
agentproxy setup --non-interactive      # CI/자동화 모드 (환경 변수 + 플래그 읽기)
agentproxy setup --password '<value>'   # 관리자 비밀번호 직접 설정
...
| Var                 | Purpose                                                          |
| ------------------- | ---------------------------------------------------------------- |
| `AGENTPROXY_API_KEY` | 공급자 API 키 (`--api-key`에 Commander `.env()`를 통해 바인딩됨) |
| `DATA_DIR`          | AgentProxy 데이터 디렉토리 재정의                                 |

모든 다른 비대화형 입력은 환경 변수가 아닌 플래그로 전달됩니다:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(위의 `agentproxy setup` 옵션 참조).

### 진단

```bash
agentproxy doctor                       # 구성, DB, 포트, 런타임, 메모리, 생존 확인
agentproxy doctor --json                # 기계 판독 가능한 JSON
agentproxy doctor --no-liveness         # HTTP 건강 프로브 건너뛰기
agentproxy doctor --host 0.0.0.0        # 생존 호스트 재정의
agentproxy doctor --liveness-url <url>  # 전체 건강 엔드포인트 URL 재정의
```
...
```bash
agentproxy providers available                       # AgentProxy 공급자 카탈로그
agentproxy providers available --search openai       # ID/이름/별칭/카테고리로 카탈로그 필터링
agentproxy providers available --category api-key    # 카테고리로 필터링 (api-key, oauth, free, ...)
agentproxy providers available --json                # 기계 판독 가능한 JSON

agentproxy providers list                            # 구성된 공급자 연결
agentproxy providers list --json

agentproxy providers test <id|name>                  # 구성된 연결 하나 테스트
agentproxy providers test-all                        # 모든 활성 연결 테스트
agentproxy providers validate                        # 로컬 전용 구조 검증
agentproxy providers add <provider> --credential-env PROVIDER_KEY
agentproxy providers import ./providers.json --dry-run --json
agentproxy providers auth <provider>                 # 기존 OAuth 흐름
...
단지 수정된 존재/형태를 보고합니다. `providers available`은 AgentProxy 카탈로그를 읽습니다;
...
```bash
agentproxy reset-password                # 관리자 비밀번호 재설정 (또한: agentproxy-reset-password)
agentproxy reset-encrypted-columns       # 경고 표시 + 암호화된 자격 증명 재설정에 대한 드라이 런
agentproxy reset-encrypted-columns --force  # SQLite에서 암호화된 자격 증명을 실제로 null로 설정
```

### 자격 증명 내보내기 (⚠ 주의해서 다루기)

```bash
agentproxy auth export                                 # 경고 표시 + 확인 게이트 — DB 접근 없음
agentproxy auth export --force                          # 모든 연결의 복호화된 자격 증명을 stdout에 JSON으로 내보내기
agentproxy auth export --force --id <id>                 # 일치하는 연결만 내보내기
agentproxy auth export --force --format env               # AGENTPROXY_<PROVIDER>_<FIELD>=<value> 형식으로 출력
agentproxy auth export --force --out creds.json           # 파일에 쓰기 (0600 권한으로 생성)
```
...
이들은 다른 경우가 명시되지 않는 한 실행 중인 AgentProxy 서버를 가정합니다:

```bash
agentproxy status                       # 종합적인 런타임 상태
agentproxy logs                         # 요청 로그 스트리밍 (--json, --search, --follow)
agentproxy config show                  # 현재 구성 표시

agentproxy provider list                # 사용 가능한 공급자 목록 (providers list의 별칭)
agentproxy provider add                 # 도구에 AgentProxy를 공급자로 등록
agentproxy keys add | list | remove     # API 키 관리
agentproxy models [provider]            # 모델 목록 (--json, --search)
agentproxy combo list | switch | create | delete

agentproxy backup                       # 구성 + DB 스냅샷
agentproxy restore                      # 이전 스냅샷에서 복원

agentproxy health                       # 상세 건강 상태 (회로 차단기, 캐시, 메모리)
agentproxy quota                        # 공급자 쿼타 사용량
agentproxy cache                        # 캐시 상태
agentproxy cache clear                  # 의미론적 + 서명 캐시 지우기

agentproxy mcp status | restart         # MCP 서버 상태 / 재시작
agentproxy a2a status | card            # A2A 서버 상태 / 에이전트 카드

agentproxy tunnel list | create | stop  # 터널 관리 (cloudflare/tailscale/ngrok)
agentproxy env show | get <k> | set <k> <v>  # 환경 변수 검사 / 설정 (임시)

agentproxy test                         # 공급자 연결성 스모크 테스트
agentproxy update                       # 업데이트 확인
agentproxy completion                   # 셸 완성 생성
```
...
붙여넣기 준비 완료 예제와 토큰화된 AgentProxy URL:
...
| 오류                                     | 원인                      | 수정 방법                                     |
| ---------------------------------------- | ------------------------- | --------------------------------------------- |
| `Connection refused`                     | AgentProxy가 실행되지 않음 | `agentproxy serve`                             |
