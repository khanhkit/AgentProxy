...
AgentProxy는 코딩 CLI(Codex, Claude Code, OpenCode, Cline 등)를 AgentProxy의 백엔드로 사용하도록 구성하는 `setup-*` 명령어 모음을 제공합니다. 이를 통해 도구는 **하나**의 엔드포인트와 통신하고 AgentProxy는 자동으로 적절한 제공자로 라우팅합니다. 각 명령어는 실행 중인 AgentProxy(로컬 또는 원격)에서 **실시간** 모델 카탈로그를 읽고 **당신의** 머신에 도구의 구성 파일을 작성합니다. API 키는 도구가 지원하는 곳에서 환경 변수로 참조됩니다. 도구 로컬 환경 파일을 지속하는 명령어는 아래에 명시되어 있습니다.

또한, `agentproxy run <target>`이라는 일반 실행기가 있어, `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` 또는 `gemini`를 적절한 환경 변수를 주입하여 구성 파일을 작성하지 않고 실행할 수 있습니다. 타겟과 그 별칭은 정식 매니페스트 `bin/cli/cli-manifest.mjs`에서 가져오며(`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), `agentproxy completion`은 동일한 매니페스트에서 파생된 타겟 단어를 제공합니다. 구식 도구별 실행기인 `agentproxy launch`(Claude Code)와 `agentproxy launch-codex`(Codex)도 여전히 사용 가능합니다.
...
- [원격 모드](./REMOTE-MODE.md) — 노트북에서 원격 AgentProxy(VPS / Tailnet) 제어
...
모든 명령어는 **활성 컨텍스트**( `agentproxy connect`로 설정, [원격 모드](./REMOTE-MODE.md) 참조) 또는 명시적 `--remote <url> --api-key <key>` 플래그를 존중합니다. 아래의 "로컬 vs 원격"은 플래그가 없을 경우 `http://localhost:20128`을 대상으로 하며, `--remote`(또는 활성 원격 컨텍스트)가 설정되면 해당 서버에서 카탈로그를 가져와 로컬에 구성을 작성합니다.

| 명령어                     | 도구                         | 작성하는 내용                                                                                                                         | 주요 플래그                                                                                                                                | 로컬 vs 원격 |
| -------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------ |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — 호환 가능한 텍스트 모델당 하나의 프로필 (`codex --profile <name>`)                                    | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | 둘 다        |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — 일치하는 모델당 하나의 프로필 (`CLAUDE_CONFIG_DIR`)                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | 둘 다        |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — 모든 카탈로그 모델을 가진 `agentproxy` 제공자 (`opencode -m agentproxy/<model>`)                   | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | 둘 다        |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI 모드) + VS Code 확장 설정 출력                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | 둘 다        |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + 존재할 경우 `kilocode.*`를 VS Code `settings.json`에 병합                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | 둘 다        |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` 모델, 키는 `${{ secrets.AGENTPROXY_API_KEY }}`를 통해 제공                              | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | 둘 다        |
| `agentproxy setup-cursor`   | Cursor                       | 없음 — 앱 내 단계 출력 (Cursor 구성은 불투명한 SQLite)                                                                                | `--remote` `--api-key` `--only` `--port`                                                                                                   | 둘 다        |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (문서 가져오기) + VS Code `settings.json`이 존재할 경우 `roo-cline.autoImportSettingsPath` 설정      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | 둘 다        |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` 제공자, 키는 `$AGENTPROXY_API_KEY`를 통해 제공                                          | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | 둘 다        |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + 환경 레시피 출력                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | 둘 다        |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + 환경 레시피 출력                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | 둘 다        |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` 배열 + `~/.qwen/.env`의 `AGENTPROXY_API_KEY`                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | 둘 다        |
| `agentproxy run <target>`   | 런타임 실행 (일반)           | 없음 — 적절한 환경과 인수로 `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini`를 생성; Qwen과 Gemini는 임시 격리된 홈을 사용 | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | 둘 다        |
| `agentproxy launch`         | Claude Code                  | 없음 — `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`이 주입된 `claude`를 생성                                                           | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | 둘 다        |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | 없음 — `-c` 플래그를 통해 `agentproxy` 제공자가 주입된 `codex`를 생성                                                                  | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | 둘 다        |

플래그에 대한 주의 사항(명령어 소스에서 확인됨):

- `--remote <url>` — 원격 AgentProxy에서 카탈로그를 가져옵니다( `--port` 및 활성 컨텍스트를 무시합니다). `--api-key <key>`는 해당 서버에 대한 자격 증명을 제공합니다(기본값은 `AGENTPROXY_API_KEY` 환경 변수 또는 활성 컨텍스트의 토큰입니다).
...
- `--model <id>`는 `agentproxy run`에서 매니페스트의 타겟별 연결을 따릅니다(`bin/cli/cli-manifest.mjs`): **aider**는 `--model openai/<id>`를 받고, **opencode**는 `--model agentproxy/<id>`를 받습니다(접두사는 ID가 이미 포함되어 있지 않을 때만 추가됨); **qwen**과 **gemini**는 ID를 그대로 받으며; **claude**는 `ANTHROPIC_MODEL`을 통해, **goose**는 `GOOSE_MODEL`을 통해, **codex**는 `-c model_providers.agentproxy.*` 인수를 통해 받습니다. **Qwen은 유일하게 `--model`을 강제로 요구하는 실행 타겟입니다** — `agentproxy run qwen` 없이 실행하면 `2`와 함께 명시적 오류가 발생합니다.
- `--port <port>` — 로컬 AgentProxy 포트(기본값 `20128`, `--remote`가 설정되면 무시됨). 모든 `setup-*` 및 두 실행기에서 사용 가능합니다.
- `agentproxy run` 종료 코드: 자식 CLI의 종료 코드는 그대로 전파됩니다; `2` = 잘못된 인수(지원되지 않는 타겟, 필수 `--model` 누락, 컨테이너 보호); `127` = 타겟 바이너리가 `PATH`에 없음; `130`/`143`/`129`는 `SIGINT`/`SIGTERM`/`SIGHUP`에 의해 실행이 종료됨; `1` = 기타 런타임 실행 실패.
...
> 더 풍부한 플러그인 통합도 있으며 — `agentproxy setup opencode` — `@agentproxy/opencode-plugin`을 설치합니다. 이들은 서로 다른 명령어이며, 위의 테이블은 `setup-opencode`를 문서화합니다.

---

## 로컬 사용

`localhost:20128`에서 AgentProxy가 실행 중인 경우, 도구에 대한 설정 명령을 실행하기만 하면 됩니다. 카탈로그는 로컬 서버에서 가져옵니다.
...
export AGENTPROXY_API_KEY=sk-...  # {env:AGENTPROXY_API_KEY}를 통해 참조, 디스크에는 저장되지 않음
...
```bash
agentproxy launch                 # Claude Code → 로컬 AgentProxy
agentproxy launch-codex           # Codex CLI → 로컬 AgentProxy
...
모든 설정 명령을 `--remote` + `--api-key`와 함께 원격 AgentProxy에 지정합니다. 카탈로그는 원격에서 가져오며, 구성은 로컬 머신에 작성됩니다.

```bash
# 원격 VPS에 대한 OpenCode, glm/kimi 모델만 유지
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # 먼저 AGENTPROXY_API_KEY를 내보내세요
...
```bash
agentproxy connect 192.168.0.15        # 범위가 지정된 토큰을 발급하고 컨텍스트를 저장합니다
agentproxy setup-codex                 # ← 이제 원격 카탈로그를 사용합니다
agentproxy setup-opencode              # ← 동일
agentproxy launch                      # ← 원격에 대한 Claude Code
```
...
AgentProxy는 `/v1`에서 OpenAI 인터페이스를 노출하고, 루트에서 Anthropic 인터페이스를, `/v1beta`에서 네이티브 Gemini 인터페이스를 제공합니다. 각 통합은 도구가 기대하는 형식에 맞게 연결됩니다 (명령 소스에서 확인됨):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | `/v1` 포함      | 예                                                     |
...
`agentproxy update`로 업데이트할 때 (확인 후 또는 `--apply`와 함께),
AgentProxy는 `--include=optional`을 포함하여 설치를 실행합니다:

```bash
npm install -g agentproxy@latest --include=optional
```

이것은 `agentproxy update`에 전달하는 플래그가 **아닙니다** — 항상
...
# [DRY RUN] 실행할 것: npm install -g agentproxy@latest --include=optional
```

다른 `agentproxy update` 플래그 (소스에서 확인됨): `--check` (구버전일 경우 1로 종료), `--apply` (프롬프트 없이 설치), `--changelog`, `--no-backup`,
`--yes`.
...
(및 `:streamGenerateContent?alt=sse`)를 발행합니다 — 이는 AgentProxy의 네이티브
Gemini 인터페이스 (`/v1beta`)와 정확히 일치합니다. `agentproxy run gemini`는 이를 자동으로 연결합니다:

- `GOOGLE_GEMINI_BASE_URL` → 활성 AgentProxy 기본 URL (루트, `/v1` 없음);
- `GEMINI_API_KEY` → 해결된 AgentProxy 자격 증명 (옵션/환경/컨텍스트);
- **임시 격리된 `GEMINI_CLI_HOME`**의 `.gemini/settings.json`이
  `gemini-api-key` 인증을 선택하므로 저장된 Google OAuth 세션 (Code Assist)
  가 AgentProxy 지시의 실행을 덮어쓰지 않도록 합니다 — 종료 후 제거됩니다;
...
REAL 바이너리를 REAL AgentProxy 서버에 대해 검증하기 위해,
옵트인 하네스가 존재합니다
...
선택 사항: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"`는 스윕을 제한합니다;
`AGENTPROXY_SMOKE_TIMEOUT_MS`는 120초의 각 대상 타임아웃을 재정의합니다.
...
- [Codex CLI 구성](./CODEX-CLI-CONFIGURATION.md) — 일회성 `[model_providers.agentproxy]` 기본 설정
