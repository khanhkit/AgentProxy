...
title: "CLI Integratsioonid – suuna ükskõik milline programmeerimise CLI AgentProxy'i"
version: 3.8.50
...
AgentProxy tarnib komplekti `setup-*` käsklusi, mis konfigureerivad programmeerimise CLI (Codex, Claude Code, OpenCode, Cline, …) kasutama AgentProxy'i oma taustajõuna – nii räägib tööriist **ühe** lõpp-punktiga ja AgentProxy suunab päringu õigele pakkujale koos automaatse varundusega. Iga käsk loeb **reaalajas** mudelikataloogi töötavast AgentProxy'ist (kohalikust või kaugest) ja kirjutab tööriisti enda konfiguratsioonifaili **teie** masinalle. API võtit viidatakse keskkonnamuutujaga, kus tööriist seda toetab. Käsklused, mis salvestavad tööriistikohaliku keskkonnapoolfääli, on all märgitud.

Samuti on olemas üldine käivitaja — `agentproxy run <sihtmärk>` — mis käivitab `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` või `gemini` õigete keskkonnamuutujatega, ilma ühtegi konfiguratsiooni kirjutamata. Sihtmärgid ja nende aliased pärinevad kanoonilisest manifesist `bin/cli/cli-manifest.mjs` (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`) ja `agentproxy completion` pakub samu manifesist tulenevaid sihtmärgi sõnu. Traditsioonilised tööriistipõhised käivitajad — `agentproxy launch` (Claude Code) ja `agentproxy launch-codex` (Codex) — on endiselt saadaval.
...
- [Kaugrežiim](./REMOTE-MODE.md) — juhi kaug AgentProxy'i (VPS / Tailnet) oma sülearvutist
...
Iga käsk austab **aktiivset konteksti** (määratud `agentproxy connect`-iga, vt [Kaugrežiim](./REMOTE-MODE.md)) või selgesõnalisi `--remote <url> --api-key <võti> lippe. "Kohalik vs kaug" all tähendab: ilma lippudeta on sihtmärgiks `http://localhost:20128`; `--remote` (või aktiivne kaugkontekst) puhul hangib see sellelt serverilt kataloogi ja kirjutab konfiguratsiooni kohalikult.

| Käsk                       | Tööriist                  | Mida kirjutab                                                                                                                                                                              | Põhilipud                                                                                                                                  | Kohalik vs kaug |
| -------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI          | `~/.codex/<nimi>.config.toml` — üks profiil iga ühilduva tekstimudeli jaoks (`codex --profile <nimi>`)                                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Mõlemad         |
| `agentproxy setup-claude`   | Claude Code               | `~/.claude/profiles/<nimi>/settings.json` — üks profiil iga sobiva mudeli jaoks (`CLAUDE_CONFIG_DIR`)                                                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Mõlemad         |
| `agentproxy setup-opencode` | OpenCode (openai-ühilduv) | `~/.config/opencode/opencode.json` — `agentproxy` pakkuja iga kataloogimudeliga (`opencode -m agentproxy/<mudel>`)                                                                           | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Mõlemad         |
| `agentproxy setup-cline`    | Cline                     | `~/.cline/data/{globalState,secrets}.json` (CLI režiim) + prindib VS Code laienduse seaded                                                                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Mõlemad         |
| `agentproxy setup-kilo`     | Kilo Code                 | `~/.local/share/kilo/auth.json` (CLI) + liidab `kilocode.*` VS Code `settings.json`-iga, kui see on olemas                                                                                 | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Mõlemad         |
| `agentproxy setup-continue` | Continue / `cn` CLI       | `~/.continue/config.yaml` — `provider: openai` mudelid, võti läbi `${{ secrets.AGENTPROXY_API_KEY }}`                                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Mõlemad         |
| `agentproxy setup-cursor`   | Cursor                    | Mitte midagi — prindib rakendusesiseste sammud (Cursori konfiguratsioon on läbipaistmatu SQLite)                                                                                           | `--remote` `--api-key` `--only` `--port`                                                                                                   | Mõlemad         |
| `agentproxy setup-roo`      | Roo Code                  | `~/.agentproxy/roo-settings.json` (impordidokument) + määrab `roo-cline.autoImportSettingsPath`, kui VS Code `settings.json` on olemas                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Mõlemad         |
| `agentproxy setup-crush`    | Crush                     | `~/.config/crush/crush.json` — `openai-compat` pakkuja, võti läbi `$AGENTPROXY_API_KEY`                                                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Mõlemad         |
| `agentproxy setup-goose`    | Goose                     | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + prindib keskkonna retsepti                                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Mõlemad         |
| `agentproxy setup-aider`    | Aider                     | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + prindib keskkonna retsepti                                                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Mõlemad         |
| `agentproxy setup-qwen`     | Qwen Code                 | `~/.qwen/settings.json` — V4 `modelProviders.openai` massiiv + `AGENTPROXY_API_KEY` `~/.qwen/.env` failis                                                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Mõlemad         |
| `agentproxy setup-5dive`    | 5dive (agendi laevastik)  | Mitte midagi `$HOME` all — kirjutab 5dive **autentimisprofiili** (`/var/lib/5dive/auth-profiles/<nimi>/`) läbi `5dive agent auth set`; ainult root, töötab laevastiku hostil               | `--remote` `--api-key` `--model` `--auth-profile` `--agent` `--byo-provider` `--fivedive-bin` `--no-sudo` `--yes` `--dry-run` `--port`     | Mõlemad         |
| `agentproxy run <sihtmärk>` | Käivitamine (üldine)      | Mitte midagi — käivitab `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` õigete keskkonnamuutujate ja argumentidega; Qwen ja Gemini kasutavad ajutist isoleeritud kodukataloogi | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Mõlemad         |
| `agentproxy launch`         | Claude Code               | Mitte midagi — käivitab `claude` süstitud `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`-iga                                                                                                  | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Mõlemad         |
| `agentproxy launch-codex`   | OpenAI Codex CLI          | Mitte midagi — käivitab `codex` süstitud `agentproxy` pakkujaga läbi `-c` lippe                                                                                                             | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Mõlemad         |

Märkmed lippude kohta (kontrollitud käskluskoodist):

- `--remote <url>` — hangib kataloogi kaug AgentProxy'ist (tühistab `--port` ja aktiivse konteksti). `--api-key <võti` annab selle serveri volikirja (vaikimisi `AGENTPROXY_API_KEY` keskkonnamuutuja või aktiivse konteksti pilet).
...
- `--model <id>` käsul `agentproxy run` järgib manifesti sihtmärgispetsiifilist juhtmestikku (`bin/cli/cli-manifest.mjs`): **aider** saab `--model openai/<id>` ja **opencode** `--model agentproxy/<id>` (prefiks lisatakse ainult siis, kui ID-l seda juba ei ole); **qwen** ja **gemini** saavad ID sõna-sõnalt; **claude** saab selle läbi `ANTHROPIC_MODEL`, **goose** läbi `GOOSE_MODEL` ja **codex** läbi `-c model_providers.agentproxy.*` argumente. **Qwen on ainus run sihtmärk, mis nõuab karmilt `--model`** — `agentproxy run qwen` ilma selleta väljub koodiga `2` ja selge veateatega.
- `--port <port>` — kohalik AgentProxy port (vaikimisi `20128`, ignoreeritakse kui `--remote` on määratud). Olemas kõigil `setup-*` ja mõlemal käivitajal.
- `agentproxy run` väljumiskoodid: lapsprotsessi CLI enda väljumiskood edastatakse sõna-sõnalt; `2` = vale argumendid (toetamata sihtmärk, puuduv nõutav `--model`, konteinerikaitse); `127` = sihtmärgi binaar ei asu `PATH-is`; `130`/`143`/`129` kui käivitus lõpetatakse `SIGINT`/`SIGTERM`/`SIGHUP`-iga; `1` = muu käivitamise tõrge.
...
> Samuti on olemas rikkalikum pistikprogrammi integratsioon — `agentproxy setup opencode` — mis
> installib `@agentproxy/opencode-plugin`. Need on erinevad käsud; ülalolev tabel
> kirjeldab `setup-opencode`.

## Kohalik kasutamine

Kui AgentProxy töötab aadressil `localhost:20128`, käivita lihtsalt oma tööriista seadistamise käsk. Kataloog tuuakse kohalt kohalikult serverist.
...
export AGENTPROXY_API_KEY=sk-...  # viidatakse kaudu {env:AGENTPROXY_API_KEY}, ei kunagi kettale
...
```bash
agentproxy launch                 # Claude Code → kohalik AgentProxy
agentproxy launch-codex           # Codex CLI → kohalik AgentProxy
...
Suuna iga seadmiskäsk kaug AgentProxy'ile kasutades `--remote` + `--api-key`. Kataloog tuuakse kaugserverist; konfiguratsioon kirjutatakse kohalikule masinale.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # ekspordi AGENTPROXY_API_KEY kõigepealt
...
```bash
agentproxy connect 192.168.0.15        # loob piiratud ulatusega tokeni, salvestab konteksti
agentproxy setup-codex                 # ← nüüd kasutab kaugkataloogi
agentproxy setup-opencode              # ← sama
agentproxy launch                      # ← Claude Code kaugvastu
```
...
[5dive](https://5dive.ai) käivitab pikka aega töötavate programmeerivate agentide parve, kus igaüks on omaenda Unix kasutaja all systemd üksus. See ei ole ise programmeeriv CLI, seega ei ole `agentproxy`'l midagi käivitada — `5dive` on **ainult konfigureeritav** sihtmärk.
...
Mõlemad vormid kirjutavad ühe 5dive **autentimisprofiili** ja iga selle profiiliga seotud `claude` istekoht seejärel suhtleb AgentProxy'iga. Kolm asja on selle sihtmärgi jaoks erilised:
...
Profiili suunamine AgentProxy **kombinatsioonile** üksiku mudeli asemel on see, mis saavutab pakkujate ümberlülituse: kui esmane lõpp-punkt läks töö ajal kindlalt alla kirjeldatud juhtumis [#11578](https://github.com/khanhkit/AgentProxy/issues/11578), lõpetas agent ülejäänud sammud varunduslõpp-punktiga ega avaldanud katkestust kunagi.

## Baasi URL-i kokkulepped (millised tööriistad soovivad `/v1`)

AgentProxy pakub OpenAI pinda kataloogis `/v1`, Anthropic pinda juurkataloogis ja emakeelset Gemini pinda kataloogis `/v1beta`. Iga integratsioon on ühendatud vastavalt selle tööriisti oodatud vormingule (kontrollitud lähtekoodis):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | `/v1`-ga        | Jah                                         |
...
Teised `agentproxy update` lipud (kontrollitud lähtekoodis): `--check` ( väljub koodiga 1, kui on vananenud), `--apply` (installib ilma küsimata), `--changelog`, `--no-backup`, `--yes`.

---

## Google Gemini CLI läbi `agentproxy run gemini`

Leping kontrollitud `@google/gemini-cli` 0.50.0 vastu: CLI järgib `GOOGLE_GEMINI_BASE_URL` ja teeb selle vastu `POST /v1beta/models/<model>:generateContent` (ja `:streamGenerateContent?alt=sse`) — täpselt AgentProxy emakeelne Gemini pind (`/v1beta`). `agentproxy run gemini` ühendab selle automaatselt:

- `GOOGLE_GEMINI_BASE_URL` → aktiivne AgentProxy baasi URL (juurkataloog, ilma `/v1`);
- `GEMINI_API_KEY` → lahendatud AgentProxy mandaat (valik/keskkond/kontekst);
- **Ajutine isoleeritud `GEMINI_CLI_HOME`**, mille `.gemini/settings.json` valib `gemini-api-key` autentimise, nii et salvestatud Google OAuth seanss (Code Assist) ei käivitu kunagi AgentProxy suunatud käivitamise üle — eemaldatakse pärast väljumist;
...
AgentProxy serveri vastu on valikuline seadistus olemas
...
Valikuline: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` piirab pühkimist;
`AGENTPROXY_SMOKE_TIMEOUT_MS` katab iga sihtmärgi jaoks 120-sekundilise ajaülempiiri.
...
- [Codex CLI konfiguratsioon](./CODEX-CLI-CONFIGURATION.md) — korduvkasutatav `[model_providers.agentproxy]` põhiseadistus
