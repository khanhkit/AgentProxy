...
![Päringu töövoog (/v1/chat/completions)](../diagrams/exported/request-pipeline.svg)
...
```mermaid
flowchart LR
    subgraph Clients[Arendajate kliendid]
        C1[Claude Code]
        C2[Codex CLI]
        C3[OpenClaw / Droid / Cline / Continue / Roo]
        C4[Kohandatud OpenAI-ühilduvad kliendid]
    ...
    subgraph Router[AgentProxy kohalik protsess]
        ...
        CORE[SSE + tõlketuum\nopen-sse + src/sse]
        DB[(storage.sqlite)]
        UDB[(kasutustabelid + logiartefaktid)]
    end

    subgraph Upstreams[Ülemvoolu pakkujad]
        P1[OAuth pakkujad\nClaude/Codex/Gemini/Qoder/GitHub/Kiro/Cursor/Antigravity]
        P2[API-võtme pakkujad\nOpenAI/Anthropic/OpenRouter/GLM/Kimi/MiniMax\nDeepSeek/Groq/xAI/Mistral/Perplexity\nTogether/Fireworks/Cerebras/Cohere/NVIDIA]
        P3[Ühilduvad sõlmed\nOpenAI-ühilduv / Anthropic-ühilduv]
    end

    subgraph Cloud[Valikuline pilvesünkroonimine]
        CLOUD[Pilvesünkroonimise lõpp-punkt\nNEXT_PUBLIC_CLOUD_URL]
    end
...
sequenceDiagram
    ...
    alt Combo mudel
        Chat->>Chat: itereeri kombo mudelite üle (handleComboChat)
    end

    Chat->>Auth: getProviderCredentials(provider)
    Auth-->>Chat: aktiivne konto + tokenid/API-võti

    Chat->>Core: handleChatCore(body, modelInfo, credentials)
    Core->>Core: tuvasta lähtevorming
    Core->>Core: teisenda päring sihtvormingusse
    Core->>Exec: execute(provider, transformedBody)
    Exec->>Prov: ülemine (upstream) API-kõne
    Prov-->>Exec: SSE/JSON vastus
    Exec-->>Core: vastus + metaandmed

    alt 401/403
        Core->>Exec: refreshCredentials()
        Exec-->>Core: uuendatud tokenid
        Core->>Exec: korda päring
    end

    Core->>Stream: teisenda/normaliseeri voog kliendi vormingusse
    Stream-->>Client: SSE tükid / JSON vastus

    Stream->>Usage: eralda kasutusandmed + salvesta ajalugu/logi
```
...
sequenceDiagram
    ...
    participant ProvAuth as Provideri autentimisserver
    participant DB as localDb
    participant Test as /api/providers/[id]/test
    participant Exec as Provideri täitja (Executor)

    UI->>OAuth: GET authorize või device-code
    OAuth->>ProvAuth: loo auth/device voog
    ProvAuth-->>OAuth: auth URL või device koodi payload
    OAuth-->>UI: voo andmed

    UI->>OAuth: POST exchange või poll
    OAuth->>ProvAuth: tokeni vahetus/päring
    ProvAuth-->>OAuth: juurdepääsu/värskendustokenid
    OAuth->>DB: createProviderConnection(oauth andmed)
    OAuth-->>UI: õnnestumine + ühenduse id

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: valideeri autentimisandmed / valikuline värskendamine
    Exec-->>Test: kehtiv või värskendatud tokeni info
    Test->>DB: värskenda staatust/tokeneid/vigu
    Test-->>UI: valideerimise tulemus
```
...
sequenceDiagram
    ...
    participant DB as localDb
    participant Cloud as Väline pilvesünkroonimine
    participant Claude as ~/.claude/settings.json

    UI->>Sync: POST action=enable
    Sync->>DB: määra cloudEnabled=true
    Sync->>DB: kontrolli, et API võti eksisteerib
    Sync->>Cloud: POST /sync/{machineId} (providerid/aliased/kombod/võtmed)
    Cloud-->>Sync: sünkroonimise tulemus
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: lubatud + verifitseerimise staatus

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: kaugandmed
    Sync->>DB: värskenda uuemaid kohalikke tokeneid/staatust
    Sync-->>UI: sünkroonitud

    UI->>Sync: POST action=disable
    Sync->>DB: määra cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: lülita ANTHROPIC_BASE_URL tagasi kohalikuks (kui vajalik)
    Sync-->>UI: keelatud
```
...
```mermaid
flowchart LR
    subgraph LocalHost[Arendaja hosti]
        CLI[CLI tööriistad]
    ...
    subgraph ContainerOrProcess[AgentProxy töötlusaegne keskkond]
        ...
        UsageDB[(kasutustabelid + logiartefaktid)]
    end

    subgraph External[Välised teenused]
        Providers[AI teenusepakkujad]
        SyncCloud[Pilvesünkroonimisteenus]
    end
