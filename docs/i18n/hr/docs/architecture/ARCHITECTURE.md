...
AgentProxy je lokalni AI usmjerivač (routing gateway) i nadzorna ploča izgrađena na Next.js.
...
![3-slojni model otpornosti](../diagrams/exported/resilience-3layers.svg)
...
AgentProxy može instalirati, nadzirati i preusmjeravati na lokalno pokrenute procese AI alata
...
- Graditelj metapodataka odgovora AgentProxya: `src/domain/agentproxyResponseMeta.ts`
...
- datoteka: `${DATA_DIR}/storage.sqlite` (ili `$XDG_CONFIG_HOME/agentproxy/storage.sqlite` kada je postavljeno, u suprotnom `~/.agentproxy/storage.sqlite`)
...
```mermaid
flowchart TD
    A[Dolazni naziv modela] --> B{Je li naziv kombinacije?}
    B -- Da --> C[Učitaj slijed modela kombinacije]
    B -- Ne --> D[Put jednog modela]

    C --> E[Pokušaj model N]
    E --> F[Razriješi providera/model]
    D --> F

    F --> G[Odaberi vjerodajnice računa]
    G --> H{Vjerodajnice dostupne?}
    H -- Ne --> I[Vrati provider nedostupan]
    H -- Da --> J[Izvrši zahtjev]

    J --> K{Uspjeh?}
    K -- Da --> L[Vrati odgovor]
    K -- Ne --> M{Greška prihvatljiva za fallback?}

    M -- Ne --> N[Vrati grešku]
    M -- Da --> O[Označi račun nedostupnim tijekom hlađenja]
    O --> P{Postoji drugi račun za providera?}
    P -- Da --> G
    P -- Ne --> Q{U kombinaciji sa sljedećim modelom?}
    Q -- Da --> E
    Q -- Ne --> R[Vrati sve nedostupno]
```
...
sequenceDiagram
    ...
    UI->>OAuth: GET authorize ili device-code
    OAuth->>ProvAuth: kreiranje auth/device toka
    ProvAuth-->>OAuth: auth URL ili payload device koda
    OAuth-->>UI: podaci toka

    UI->>OAuth: POST exchange ili poll
    OAuth->>ProvAuth: token exchange/poll
    ProvAuth-->>OAuth: access/refresh tokeni
    OAuth->>DB: createProviderConnection(oauth podaci)
    OAuth-->>UI: uspjeh + id veze

    UI->>Test: POST /api/providers/[id]/test
    Test->>Exec: validacija vjerodajnica / opcionalna obnova
    Exec-->>Test: valjan ili obnovljen podatak o tokenu
    Test->>DB: ažuriranje statusa/tokena/grešaka
    Test-->>UI: rezultat validacije
```
...
sequenceDiagram
    ...
    Sync->>DB: postavi cloudEnabled=true
    Sync->>DB: osiguraj postojanje API ključa
    Sync->>Cloud: POST /sync/{machineId} (provideri/aliasi/kombinacije/ključevi)
    Cloud-->>Sync: rezultat sinkronizacije
    Sync->>Cloud: GET /{machineId}/v1/verify
    Sync-->>UI: omogućeno + status provjere

    UI->>Sync: POST action=sync
    Sync->>Cloud: POST /sync/{machineId}
    Cloud-->>Sync: udaljeni podaci
    Sync->>DB: ažuriraj novije lokalne tokene/status
    Sync-->>UI: sinkronizirano

    UI->>Sync: POST action=disable
    Sync->>DB: postavi cloudEnabled=false
    Sync->>Cloud: DELETE /sync/{machineId}
    Sync->>Claude: vrati ANTHROPIC_BASE_URL na lokalno (ako je potrebno)
    Sync-->>UI: isključeno
```
...
> AgentProxy v3.8.0. Za kanonski i kontinuirano ažurirani popis pogledajte
...
- konačni odgovor klijentu koji vraća AgentProxy; streamani odgovori pohranjuju se u istom sažetom obliku
...
1. `usageDb` i `localDb` dijele istu politiku osnovnog direktorija (`DATA_DIR` -> `XDG_CONFIG_HOME/agentproxy` -> `~/.agentproxy`) uz migraciju zastarjelih datoteka.
...
5. Direktorij `open-sse/` objavljen je kao **npm workspace paket** `@agentproxy/open-sse`. Izvorni kod ga uvozi putem `@agentproxy/open-sse/...` (razrješava ga Next.js `transpilePackages`). Putanje datoteka u ovom dokumentu i dalje koriste naziv direktorija `open-sse/` za konzistentnost.
...
- Izgradnja Docker slike: `docker build -t agentproxy .`
