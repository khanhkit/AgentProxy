...
AgentProxy leverer en familie af `setup-*` kommandoer, der konfigurerer en kodnings-CLI (Codex, Claude Code, OpenCode, Cline, …) til at bruge AgentProxy som sin backend — så værktøjet kommunikerer med **én** endpoint, og AgentProxy ruter til den rette udbyder med automatisk tilbagefald. Hver kommando læser den **live** modelkatalog fra en kørende AgentProxy (lokal eller fjern) og skriver værktøjets egen konfigurationsfil på **din** maskine. API-nøglen refereres af en miljøvariabel, hvor værktøjet understøtter det. Kommandoer, der bevarer en værktøjslokal miljøfil, er noteret nedenfor.

Der er også en generisk launcher — `agentproxy run <target>` — der starter `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` eller `gemini` med den rette miljøvariabel injiceret, uden at skrive nogen konfiguration overhovedet. Mål og deres aliaser kommer fra det kanoniske manifest `bin/cli/cli-manifest.mjs`
(`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`,
`open-code`, `qwen-code`, `gemini-cli`), og `agentproxy completion` tilbyder de samme manifest-afledte målord. De ældre per-værktøj launchers —
`agentproxy launch` (Claude Code) og `agentproxy launch-codex` (Codex) — forbliver tilgængelige.
...
- [Fjernmode](./REMOTE-MODE.md) — styre en fjern AgentProxy (VPS / Tailnet) fra din bærbare computer
...
Hver kommando respekterer den **aktive kontekst** (sat med `agentproxy connect`, se
...
| -------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — én profil pr. kompatibel tekstmodel (`codex --profile <name>`)                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Begge          |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — én profil pr. matchet model (`CLAUDE_CONFIG_DIR`)                                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Begge          |
| `agentproxy setup-opencode` | OpenCode (openai-kompatibel) | `~/.config/opencode/opencode.json` — `agentproxy` udbyder med hver katalogmodel (`opencode -m agentproxy/<model>`)                                                   | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Begge          |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printer VS Code udvidelsesindstillinger                                                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Begge          |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + fusionerer `kilocode.*` ind i VS Code `settings.json`, hvis til stede                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Begge          |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` modeller, nøgle via `${{ secrets.AGENTPROXY_API_KEY }}`                                                              | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Begge          |
| `agentproxy setup-cursor`   | Cursor                       | Intet — printer de trin i appen (Cursor konfiguration er uklar SQLite)                                                                                             | `--remote` `--api-key` `--only` `--port`                                                                                                   | Begge          |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (importdokument) + sætter `roo-cline.autoImportSettingsPath`, hvis en VS Code `settings.json` eksisterer                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Begge          |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-kompat` udbyder, nøgle via `$AGENTPROXY_API_KEY`                                                                             | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Begge          |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + printer miljøopskrift                                                               | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Begge          |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + printer miljøopskrift                                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Begge          |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` array + `AGENTPROXY_API_KEY` i `~/.qwen/.env`                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Begge          |
| `agentproxy run <target>`   | Runtime launch (generisk)    | Intet — starter `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` med det rette miljø og argumenter; Qwen og Gemini bruger et midlertidigt isoleret hjem | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Begge          |
| `agentproxy launch`         | Claude Code                  | Intet — starter `claude` med `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injiceret                                                                                 | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Begge          |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | Intet — starter `codex` med `agentproxy` udbyderen injiceret via `-c` flag                                                                                          | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Begge          |

Bemærkninger om flag (verificeret i kommandoens kilde):

- `--remote <url>` — hent kataloget fra en fjern AgentProxy (overskriver `--port`
  og den aktive kontekst). `--api-key <key>` leverer legitimationsoplysningen for den
  server (standard til `AGENTPROXY_API_KEY` miljøvariabel, eller den aktive kontexts token).
...
- `--model <id>` på `agentproxy run` følger manifestets per-mål wiring
  (`bin/cli/cli-manifest.mjs`): **aider** modtager `--model openai/<id>` og
  **opencode** `--model agentproxy/<id>` (præfikset tilføjes kun, når id'et
  ...
  **codex** via `-c model_providers.agentproxy.*` args. **Qwen er det eneste kørsel
  mål, der hårdt kræver `--model`** — `agentproxy run qwen` uden det afslutter
  `2` med en eksplicit fejl.
- `--port <port>` — lokal AgentProxy port (standard `20128`, ignoreres når `--remote`
  er sat). Tilstede på alle `setup-*` og begge launchers.
- `agentproxy run` exit-koder: barnets CLI's egen exit-kode videreføres
...
> Der er også en rigere plugin-integration — `agentproxy setup opencode` — som
> installerer `@agentproxy/opencode-plugin`. De er forskellige kommandoer; tabellen
...
Med AgentProxy kørende på `localhost:20128`, skal du blot køre opsætningskommandoen for dit værktøj. Kataloget hentes fra den lokale server.
...
export AGENTPROXY_API_KEY=sk-...  # refereret via {env:AGENTPROXY_API_KEY}, aldrig på disk
...
Peg enhver opsætningskommando mod en fjern AgentProxy med `--remote` + `--api-key`. Kataloget hentes fra den fjerne; konfigurationen skrives på din lokale maskine.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # eksportér AGENTPROXY_API_KEY først
...
```bash
agentproxy connect 192.168.0.15        # opretter en scoped token, gemmer konteksten
agentproxy setup-codex                 # ← bruger nu det fjerne katalog
agentproxy setup-opencode              # ← samme
agentproxy launch                      # ← Claude Code mod den fjerne
```
...
AgentProxy eksponerer OpenAI-overfladen ved `/v1`, den Anthropic-overflade ved roden, og en native Gemini-overflade ved `/v1beta`. Hver integration er tilsluttet den form, som dens værktøj forventer (verificeret i kommandoens kilde):
...
Når du opdaterer med `agentproxy update` (efter bekræftelse eller med `--apply`),
kører AgentProxy installationen med `--include=optional` indbygget:

```bash
npm install -g agentproxy@latest --include=optional
```

Dette er **ikke** en flag, du sender til `agentproxy update` — det anvendes altid af
...
# [DRY RUN] Ville køre: npm install -g agentproxy@latest --include=optional
```

Andre `agentproxy update` flag (verificeret i kildekoden): `--check` (afslut 1 hvis
...
(og `:streamGenerateContent?alt=sse`) imod det — præcist AgentProxys native
Gemini-overflade (`/v1beta`). `agentproxy run gemini` forbinder det automatisk:

- `GOOGLE_GEMINI_BASE_URL` → den aktive AgentProxy base URL (rod, ingen `/v1`);
- `GEMINI_API_KEY` → den løste AgentProxy legitimationsoplysning (mulighed/miljø/kontekst);
- en **midlertidig isoleret `GEMINI_CLI_HOME`** hvis `.gemini/settings.json`
  ...
  aldrig overskriver den AgentProxy-styrede lancering — fjernet efter exit;
...
AgentProxy server, findes der en opt-in ramme ved
...
Valgfrit: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` begrænser sweepet;
`AGENTPROXY_SMOKE_TIMEOUT_MS` overskriver timeout på 120s pr. mål.
...
- [Codex CLI konfiguration](./CODEX-CLI-CONFIGURATION.md) — den engangs `[model_providers.agentproxy]` grundopsætning
