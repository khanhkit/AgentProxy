...
AgentProxy ఒక కోడింగ్ CLI (Codex, Claude Code, OpenCode, Cline, …) ని AgentProxy ను బ్యాక్‌ఎండ్‌గా ఉపయోగించడానికి కాంఫిగర్ చేసే `setup-*` కమాండ్ల కుటుంబాన్ని అందిస్తుంది — కాబట్టి ఈ టూల్ **ఒక** ఎండ్‌పాయింట్‌తో మాట్లాడుతుంది మరియు AgentProxy సరైన ప్రొవైడర్‌కు ఆటో-ఫాల్బ్యాక్‌తో మార్గం చూపిస్తుంది. ప్రతి కమాండ్ ఒక నడుస్తున్న AgentProxy (స్థానిక లేదా దూర) నుండి **ప్రస్తుత** మోడల్ కాటలాగ్‌ను చదువుతుంది మరియు **మీ** యంత్రంలో టూల్ యొక్క స్వంత కాంఫిగరేషన్ ఫైల్‌ను రాస్తుంది. API కీ టూల్ దానిని మద్దతు ఇచ్చే చోట ఎక్కడైనా ఒక ఎన్విరాన్‌మెంట్ వేరియబుల్ ద్వారా సూచించబడుతుంది. టూల్-స్థానిక ఎన్విరాన్‌మెంట్ ఫైల్‌ను నిల్వ చేసే కమాండ్లు క్రింద పేర్కొనబడ్డాయి.

అంతేకాకుండా ఒక సాధారణ లాంచర్ ఉంది — `agentproxy run <target>` — ఇది సరైన ఎన్విరాన్‌మెంట్ ఇంజెక్ట్ చేయబడిన `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` లేదా `gemini` ని స్పాన్ చేస్తుంది, ఏ కాంఫిగరేషన్‌ను కూడా రాయకుండా. లక్ష్యాలు మరియు వాటి అలియాస్లు కెనానికల్ మానిఫెస్ట్ `bin/cli/cli-manifest.mjs` నుండి వస్తాయి (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), మరియు `agentproxy completion` అదే మానిఫెస్ట్-ఉత్పన్న లక్ష్య పదాలను అందిస్తుంది. పాత పర్-టూల్ లాంచర్లు — `agentproxy launch` (Claude Code) మరియు `agentproxy launch-codex` (Codex) — అందుబాటులో ఉన్నాయి.
...
- [దూర మోడ్](./REMOTE-MODE.md) — మీ లాప్‌టాప్ నుండి దూర AgentProxy (VPS / Tailnet) ని నడపండి
...
ప్రతి కమాండ్ **చురుకైన సందర్భం** ( `agentproxy connect` తో సెట్ చేయబడింది, చూడండి [దూర మోడ్](./REMOTE-MODE.md)) లేదా స్పష్టమైన `--remote <url> --api-key <key>` ఫ్లాగ్‌లను గౌరవిస్తుంది. "స్థానిక vs దూర" క్రింద అర్థం: ఎలాంటి ఫ్లాగ్‌లతో ఇది `http://localhost:20128` ను లక్ష్యంగా చేస్తుంది; `--remote` (లేదా చురుకైన దూర సందర్భం) తో అది ఆ సర్వర్ నుండి కాటలాగ్‌ను పొందుతుంది మరియు స్థానికంగా కాంఫిగరేషన్‌ను రాస్తుంది.

| కమాండ్                     | టూల్                         | ఇది ఏమి రాస్తుంది                                                                                                                                                                            | కీలక ఫ్లాగ్‌లు                                                                                                                             | స్థానిక vs దూర |
| -------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — ప్రతి అనుకూల టెక్స్ట్ మోడల్‌కు ఒక ప్రొఫైల్ (`codex --profile <name>`)                                                                                        | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | రెండూ          |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — ప్రతి సరిపోయే మోడల్‌కు ఒక ప్రొఫైల్ (`CLAUDE_CONFIG_DIR`)                                                                                         | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | రెండూ          |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — ప్రతి కాటలాగ్ మోడల్‌తో `agentproxy` ప్రొవైడర్ (`opencode -m agentproxy/<model>`)                                                                          | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | రెండూ          |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI మోడ్) + VS కోడ్ విస్తరణ సెట్టింగ్‌లను ముద్రిస్తుంది                                                                                          | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | రెండూ          |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + ఉంటే `kilocode.*` ను VS కోడ్ `settings.json` లో విలీనం చేస్తుంది                                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | రెండూ          |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` మోడల్‌లు, కీ `${{ secrets.AGENTPROXY_API_KEY }}` ద్వారా                                                                                        | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | రెండూ          |
| `agentproxy setup-cursor`   | Cursor                       | ఏమీ కాదు — యాప్‌లో దశలను ముద్రిస్తుంది (Cursor కాంఫిగరేషన్ అంధకమైన SQLite)                                                                                                                   | `--remote` `--api-key` `--only` `--port`                                                                                                   | రెండూ          |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (ఆమోద డాక్యుమెంట్) + ఒక VS కోడ్ `settings.json` ఉంటే `roo-cline.autoImportSettingsPath` ను సెట్ చేస్తుంది                                                   | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | రెండూ          |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` ప్రొవైడర్, కీ `$AGENTPROXY_API_KEY` ద్వారా                                                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | రెండూ          |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + ఎన్విరాన్‌మెంట్ రెసిపీని ముద్రిస్తుంది                                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | రెండూ          |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + ఎన్విరాన్‌మెంట్ రెసిపీని ముద్రిస్తుంది                                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | రెండూ          |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` అరిజ్ + `AGENTPROXY_API_KEY` `~/.qwen/.env` లో                                                                                           | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | రెండూ          |
| `agentproxy run <target>`   | రన్‌టైమ్ లాంచ్ (సాధారణ)      | ఏమీ కాదు — సరైన ఎన్విరాన్‌మెంట్ మరియు ఆర్గ్స్‌తో `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` ని స్పాన్ చేస్తుంది; Qwen మరియు Gemini తాత్కాలిక ఇసోలేటెడ్ హోమ్‌ను ఉపయోగిస్తాయి | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | రెండూ          |
| `agentproxy launch`         | Claude Code                  | ఏమీ కాదు — `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` ఇంజెక్ట్ చేయబడిన `claude` ని స్పాన్ చేస్తుంది                                                                                         | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | రెండూ          |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | ఏమీ కాదు — `-c` ఫ్లాగ్‌ల ద్వారా ఇంజెక్ట్ చేయబడిన `agentproxy` ప్రొవైడర్‌తో `codex` ని స్పాన్ చేస్తుంది                                                                                        | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | రెండూ          |

ఫ్లాగ్‌లపై గమనికలు (కమాండ్ మూలంలో ధృవీకరించబడింది):

- `--remote <url>` — దూర AgentProxy నుండి కాటలాగ్‌ను పొందండి ( `--port` మరియు చురుకైన సందర్భాన్ని ఓవర్‌రైడ్ చేస్తుంది). `--api-key <key>` ఆ సర్వర్ కోసం క్రెడెన్షియల్‌ను అందిస్తుంది (డిఫాల్ట్‌గా `AGENTPROXY_API_KEY` ఎన్విరాన్‌మెంట్ వేరియబుల్ లేదా చురుకైన సందర్భం యొక్క టోకెన్).
...
- `--model <id>` `agentproxy run` పై మానిఫెస్ట్ యొక్క పర్-టార్గెట్ వైరింగ్‌ను అనుసరిస్తుంది (`bin/cli/cli-manifest.mjs`): **aider** `--model openai/<id>` మరియు **opencode** `--model agentproxy/<id>` (id ఇప్పటికే దానిని కలిగి లేకపోతే ప్రిఫిక్స్ జోడించబడుతుంది); **qwen** మరియు **gemini** idని వర్బాటిమ్‌గా పొందుతాయి; **claude** దానిని `ANTHROPIC_MODEL` ద్వారా పొందుతుంది, **goose** `GOOSE_MODEL` ద్వారా, మరియు **codex** `-c model_providers.agentproxy.*` ఆర్గ్స్ ద్వారా. **Qwen మాత్రమే `--model` ను కఠినంగా అవసరం చేస్తుంది** — `agentproxy run qwen` లేకుండా అది స్పష్టమైన పొరపాటుతో `2` ను ఎగుమతి చేస్తుంది.
- `--port <port>` — స్థానిక AgentProxy పోర్ట్ (డిఫాల్ట్ `20128`, `--remote` సెట్ చేసినప్పుడు పరిగణనలోకి తీసుకోబడదు). అన్ని `setup-*` మరియు రెండు లాంచర్లపై అందుబాటులో ఉంది.
- `agentproxy run` ఎగుమతి కోడ్స్: పిల్ల CLI యొక్క స్వంత ఎగుమతి కోడ్ ఖచ్చితంగా ప్రాప్యత చేయబడుతుంది; `2` = చెల్లని ఆర్గ్‌లు (మద్దతు ఇవ్వని లక్ష్యం, అవసరమైన `--model` మిస్సింగ్, కంటైనర్ గార్డ్); `127` = లక్ష్య బైనరీ `PATH` లో లేదు; `130`/`143`/`129` లాంచ్ `SIGINT`/`SIGTERM`/`SIGHUP` ద్వారా ముగిసినప్పుడు; `1` = ఇతర రన్‌టైమ్ లాంచ్ విఫలం.
...
> ఒక సమృద్ధి కలిగిన ప్లగిన్ ఇంటిగ్రేషన్ కూడా ఉంది — `agentproxy setup opencode` — ఇది `@agentproxy/opencode-plugin` ను ఇన్‌స్టాల్ చేస్తుంది. ఇవి వేరు వేరు కమాండ్లు; పై పట్టిక `setup-opencode` ను డాక్యుమెంట్ చేస్తుంది.

---

## స్థానిక వినియోగం

`localhost:20128` వద్ద AgentProxy నడుస్తున్నప్పుడు, మీ టూల్ కోసం సెటప్ ఆదేశాన్ని నడపండి. కాటలాగ్ స్థానిక సర్వర్ నుండి పొందబడుతుంది.
...
export AGENTPROXY_API_KEY=sk-...  # {env:AGENTPROXY_API_KEY} ద్వారా సూచించబడింది, డిస్క్‌పై ఎప్పుడూ కాదు
...
```bash
agentproxy launch                 # Claude Code → స్థానిక AgentProxy
agentproxy launch-codex           # Codex CLI → స్థానిక AgentProxy
...
ఏ సెటప్ ఆదేశాన్ని `--remote` + `--api-key` తో దూర AgentProxy కు సంకేతం చేయండి. కాటలాగ్ దూరం నుండి పొందబడుతుంది; కాన్ఫిగరేషన్ మీ స్థానిక యంత్రంపై రాయబడుతుంది.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # మొదట AGENTPROXY_API_KEY ను ఎగుమతి చేయండి
...
```bash
agentproxy connect 192.168.0.15        # స్కోప్డ్ టోకెన్‌ను సృష్టిస్తుంది, సందర్భాన్ని నిల్వ చేస్తుంది
agentproxy setup-codex                 # ← ఇప్పుడు దూర కాటలాగ్‌ను ఉపయోగిస్తుంది
agentproxy setup-opencode              # ← అదే
agentproxy launch                      # ← Claude Code దూరానికి
```
...
AgentProxy OpenAI ఉపరితలాన్ని `/v1` వద్ద, Anthropic ఉపరితలాన్ని మూలంలో, మరియు ఒక స్వదేశీ Gemini ఉపరితలాన్ని `/v1beta` వద్ద అందిస్తుంది. ప్రతి ఇంటిగ్రేషన్ దాని టూల్ ఆశించే రూపానికి అనుసంధానించబడింది (ఆదేశ మూలంలో ధృవీకరించబడింది):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | `/v1` తో           | అవును                                                |
...
మీరు `agentproxy update` తో నవీకరించినప్పుడు (మరియు నిర్ధారించిన తర్వాత, లేదా `--apply` తో),
AgentProxy `--include=optional` తో ఇన్‌స్టాల్‌ను నడుపుతుంది:

```bash
npm install -g agentproxy@latest --include=optional
```

ఇది `agentproxy update` కు మీరు అందించే ఒక ఫ్లాగ్ **కాదు** — ఇది ఎప్పుడూ
...
ఇతర `agentproxy update` ఫ్లాగ్‌లు (మూలంలో నిర్ధారించబడినవి): `--check` (పాతదిగా ఉంటే 1తో బయటకు రండి), `--apply` (ప్రాంప్ట్ లేకుండా ఇన్‌స్టాల్ చేయండి), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI ద్వారా `agentproxy run gemini`
...
(మరియు `:streamGenerateContent?alt=sse`) కు వ్యతిరేకంగా ఇస్తుంది — ఇది AgentProxy యొక్క స్థానిక
Gemini ఉపరితలానికి ( `/v1beta`). `agentproxy run gemini` దాన్ని ఆటోమేటిక్‌గా కేబుల్ చేస్తుంది:

- `GOOGLE_GEMINI_BASE_URL` → క్రియాశీల AgentProxy బేస్ URL (రూట్, `/v1` లేదు);
- `GEMINI_API_KEY` → పరిష్కరించిన AgentProxy క్రెడెన్షియల్ (ఐచ్ఛికం/పర్యావరణం/సందర్భం);
- ఒక **తాత్కాలిక వేరుపరచబడిన `GEMINI_CLI_HOME`** దీని `.gemini/settings.json`
  ...
  AgentProxy-నిర్దేశిత ప్రారంభాన్ని ఎప్పుడూ మించినది కాదు — నిష్క్రమణ తర్వాత తొలగించబడుతుంది;
...
నిజమైన AgentProxy సర్వర్‌కు నిజమైన బైనరీలను ధృవీకరించడానికి,
...
ఐచ్ఛికం: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"`
స్రవంతిని పరిమితం చేస్తుంది; `AGENTPROXY_SMOKE_TIMEOUT_MS`
...
- [Codex CLI కాన్ఫిగరేషన్](./CODEX-CLI-CONFIGURATION.md) — ఒకసారి `[model_providers.agentproxy]` ప్రాథమిక సెటప్
