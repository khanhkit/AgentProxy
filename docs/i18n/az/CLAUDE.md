...
**AgentProxy** — birləşdirilmiş AI proxy/router. Bir uç nöqtə, 329 LLM təminatçısı, avtomatik geri dönmə.
...
AgentProxy-un üç əlaqəli, lakin fərqli müvəqqəti uğursuzluq mexanizmi var. Marşrut davranışını düzəldərkən onların
...
- **İdxallar**: xarici → daxili (`@/`, `@agentproxy/open-sse`) → nisbət
...
- **Yol aliasları**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard eyni portda)
- **Məlumat qovluğu**: `DATA_DIR` env var, varsayılan `~/.agentproxy/`
