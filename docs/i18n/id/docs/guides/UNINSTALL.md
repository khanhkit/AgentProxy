# AgentProxy — Panduan Mencopot Pemasangan (Bahasa Indonesia)
...
Panduan ini menjelaskan cara mencopot pemasangan AgentProxy dari sistem Anda secara bersih.

---

## Mencopot Pemasangan dengan Cepat (v3.6.2+)

AgentProxy menyediakan dua skrip bawaan untuk penghapusan yang bersih:
...
Perintah ini menghapus aplikasi AgentProxy tetapi **mempertahankan** basis data, konfigurasi, kunci API, dan pengaturan penyedia Anda di `~/.agentproxy/`. Gunakan ini jika Anda berencana memasang ulang nanti dan ingin menyimpan pengaturan yang ada.
...
- Berkas cadangan
- Berkas log
- Semua berkas di direktori `~/.agentproxy/`
...
- Buka `Settings → Apps → AgentProxy → Uninstall`
- Atau jalankan uninstaller NSIS dari direktori instalasi

**macOS:**

- Seret `AgentProxy.app` dari `/Applications` ke Trash
- Hapus data: `rm -rf ~/Library/Application Support/agentproxy`

**Linux:**

- Hapus berkas AppImage
- Hapus data: `rm -rf ~/.agentproxy`
...
AgentProxy menyimpan data di lokasi-lokasi berikut secara default:
