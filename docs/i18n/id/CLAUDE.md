...
**AgentProxy** — proxy/router AI terpadu. Satu endpoint, 329 penyedia LLM, auto-fallback.
...
AgentProxy memiliki tiga mekanisme kegagalan sementara yang terkait tetapi berbeda. Jaga agar ruang lingkup mereka terpisah saat melakukan debug perilaku routing. Lihat
...
- **Impor**: eksternal → internal (`@/`, `@agentproxy/open-sse`) → relatif
...
- **Alias jalur**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Port default**: 20128 (API + dashboard di port yang sama)
- **Direktori data**: variabel env `DATA_DIR`, default ke `~/.agentproxy/`
