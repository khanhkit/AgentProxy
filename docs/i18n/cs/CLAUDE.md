...
**AgentProxy** — jednotný AI proxy/router. Jeden koncový bod, 329 poskytovatelů LLM, automatické zálohování.
...
AgentProxy má tři související, ale odlišné mechanismy dočasného selhání. Udržujte jejich rozsah oddělený při ladění chování routování. Viz
...
- **Importy**: externí → interní (`@/`, `@agentproxy/open-sse`) → relativní
...
- **Cestovní aliasy**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Výchozí port**: 20128 (API + dashboard na stejném portu)
- **Adresář dat**: `DATA_DIR` env var, výchozí hodnota `~/.agentproxy/`
