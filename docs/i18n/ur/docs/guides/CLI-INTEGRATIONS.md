...
AgentProxy ایک خاندان کے `setup-*` کمانڈز فراہم کرتا ہے جو ایک کوڈنگ
CLI (Codex, Claude Code, OpenCode, Cline, …) کو AgentProxy کو اس کے بیک اینڈ کے طور پر استعمال کرنے کے لیے ترتیب دیتا ہے — تاکہ
یہ ٹول **ایک** اینڈپوائنٹ سے بات کرتا ہے اور AgentProxy صحیح فراہم کنندہ کی طرف راستہ بناتا ہے
خودکار فیل بیک کے ساتھ۔ ہر کمانڈ ایک چلتے ہوئے
AgentProxy (مقامی یا دور) سے **زندہ** ماڈل کی کیٹلاگ پڑھتا ہے اور ٹول کی اپنی کنفیگریشن فائل **آپ کے**
...
ایک عمومی لانچر بھی موجود ہے — `agentproxy run <target>` — جو
...
`open-code`, `qwen-code`, `gemini-cli`) سے آتے ہیں، اور `agentproxy completion` پیش کرتا ہے
اسی مینیفیسٹ سے ماخوذ ہدف کے الفاظ۔ وراثتی فی ٹول لانچر —
`agentproxy launch` (Claude Code) اور `agentproxy launch-codex` (Codex) — دستیاب رہتے ہیں۔
...
- [دور کا طریقہ](./REMOTE-MODE.md) — اپنے لیپ ٹاپ سے ایک دور AgentProxy (VPS / Tailnet) کو چلائیں
...
ہر کمانڈ **فعال سیاق و سباق** کی عزت کرتا ہے (جو `agentproxy connect` کے ساتھ سیٹ کیا جاتا ہے، دیکھیں
...
| -------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI             | `~/.codex/<name>.config.toml` — ہر ہم آہنگ ٹیکسٹ ماڈل کے لیے ایک پروفائل (`codex --profile <name>`)                                                                  | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | دونوں             |
| `agentproxy setup-claude`   | Claude Code                  | `~/.claude/profiles/<name>/settings.json` — ہر ملتے جلتے ماڈل کے لیے ایک پروفائل (`CLAUDE_CONFIG_DIR`)                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | دونوں             |
| `agentproxy setup-opencode` | OpenCode (openai-compatible) | `~/.config/opencode/opencode.json` — `agentproxy` فراہم کنندہ کے ساتھ ہر کیٹلاگ ماڈل (`opencode -m agentproxy/<model>`)                                                | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | دونوں             |
| `agentproxy setup-cline`    | Cline                        | `~/.cline/data/{globalState,secrets}.json` (CLI موڈ) + VS Code توسیع کی ترتیبات چھاپتا ہے                                                                            | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | دونوں             |
| `agentproxy setup-kilo`     | Kilo Code                    | `~/.local/share/kilo/auth.json` (CLI) + اگر موجود ہو تو `kilocode.*` کو VS Code `settings.json` میں ضم کرتا ہے                                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | دونوں             |
| `agentproxy setup-continue` | Continue / `cn` CLI          | `~/.continue/config.yaml` — `provider: openai` ماڈلز، کلید `${{ secrets.AGENTPROXY_API_KEY }}` کے ذریعے                                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | دونوں             |
| `agentproxy setup-cursor`   | Cursor                       | کچھ نہیں — ایپ میں مراحل چھاپتا ہے (Cursor کی ترتیب اوپیک SQLite ہے)                                                                                                 | `--remote` `--api-key` `--only` `--port`                                                                                                   | دونوں             |
| `agentproxy setup-roo`      | Roo Code                     | `~/.agentproxy/roo-settings.json` (درآمدی دستاویز) + اگر VS Code `settings.json` موجود ہو تو `roo-cline.autoImportSettingsPath` کو سیٹ کرتا ہے                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | دونوں             |
| `agentproxy setup-crush`    | Crush                        | `~/.config/crush/crush.json` — `openai-compat` فراہم کنندہ، کلید `$AGENTPROXY_API_KEY` کے ذریعے                                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | دونوں             |
| `agentproxy setup-goose`    | Goose                        | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + ماحول کی ترکیب چھاپتا ہے                                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | دونوں             |
| `agentproxy setup-aider`    | Aider                        | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + ماحول کی ترکیب چھاپتا ہے                                                                            | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | دونوں             |
| `agentproxy setup-qwen`     | Qwen Code                    | `~/.qwen/settings.json` — V4 `modelProviders.openai` کی صف + `AGENTPROXY_API_KEY` `~/.qwen/.env` میں                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | دونوں             |
| `agentproxy run <target>`   | رن ٹائم لانچ (جنرل)          | کچھ نہیں — `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` کو صحیح ماحول اور دلائل کے ساتھ شروع کریں؛ Qwen اور Gemini ایک عارضی الگ گھر استعمال کرتے ہیں | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | دونوں             |
| `agentproxy launch`         | Claude Code                  | کچھ نہیں — `claude` کو `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` کے ساتھ شروع کرتا ہے                                                                              | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | دونوں             |
| `agentproxy launch-codex`   | OpenAI Codex CLI             | کچھ نہیں — `codex` کو `agentproxy` فراہم کنندہ کے ساتھ `-c` جھنڈوں کے ذریعے شروع کرتا ہے                                                                              | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | دونوں             |

جھنڈوں پر نوٹس (کمانڈ کے ماخذ میں تصدیق شدہ):

- `--remote <url>` — دور AgentProxy سے کیٹلاگ حاصل کریں (یہ `--port`
  اور فعال سیاق و سباق کو اوور رائیڈ کرتا ہے)۔ `--api-key <key>` اس سرور کے لیے سند فراہم کرتا ہے
  (جو کہ `AGENTPROXY_API_KEY` ماحولیاتی متغیر، یا فعال سیاق و سباق کے ٹوکن کی ڈیفالٹ ہے)۔
...
- `--model <id>` پر `agentproxy run` مینیفیسٹ کے فی ہدف وائرنگ کی پیروی کرتا ہے
  (`bin/cli/cli-manifest.mjs`): **aider** کو `--model openai/<id>` ملتا ہے اور
  **opencode** کو `--model agentproxy/<id>` (پری فکس صرف اس وقت شامل کیا جاتا ہے جب ID
  ...
  **codex** کو `-c model_providers.agentproxy.*` دلائل کے ذریعے۔ **Qwen واحد رن
  ہدف ہے جس کی سختی سے `--model` کی ضرورت ہوتی ہے** — `agentproxy run qwen` اس کے بغیر
  `2` کے ساتھ ایک واضح غلطی کے ساتھ ختم ہوتا ہے۔
- `--port <port>` — مقامی AgentProxy پورٹ (ڈیفالٹ `20128`، جب `--remote`
  سیٹ ہو تو نظر انداز کیا جاتا ہے)۔ تمام `setup-*` اور دونوں لانچروں پر موجود ہے۔
- `agentproxy run` کے خارج ہونے کے کوڈ: بچے CLI کا اپنا خارج ہونے کا کوڈ
...
> ایک امیر پلگ ان انضمام بھی موجود ہے — `agentproxy setup opencode` — جو
> `@agentproxy/opencode-plugin` کو انسٹال کرتا ہے۔ یہ مختلف کمانڈز ہیں؛ اوپر کی جدول
...
جب AgentProxy `localhost:20128` پر چل رہا ہو، تو اپنے ٹول کے لیے سیٹ اپ کمانڈ چلائیں۔ کیٹلاگ مقامی سرور سے حاصل کیا جاتا ہے۔
...
export AGENTPROXY_API_KEY=sk-...  # {env:AGENTPROXY_API_KEY} کے ذریعے حوالہ دیا گیا، کبھی بھی ڈسک پر نہیں
...
```bash
agentproxy launch                 # Claude Code → مقامی AgentProxy
agentproxy launch-codex           # Codex CLI → مقامی AgentProxy
...
کسی بھی سیٹ اپ کمانڈ کو دور دراز AgentProxy پر `--remote` + `--api-key` کے ساتھ نشانہ بنائیں۔ کیٹلاگ دور دراز سے حاصل کیا جاتا ہے؛ کنفیگ آپ کی مقامی مشین پر لکھی جاتی ہے۔
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # پہلے AGENTPROXY_API_KEY کو برآمد کریں
...
```bash
agentproxy connect 192.168.0.15        # ایک مخصوص ٹوکن بناتا ہے، سیاق کو محفوظ کرتا ہے
agentproxy setup-codex                 # ← اب دور دراز کیٹلاگ استعمال کرتا ہے
agentproxy setup-opencode              # ← یہی
agentproxy launch                      # ← Claude Code دور دراز کے خلاف
```
...
AgentProxy اوپن اے آئی کی سطح کو `/v1` پر، اینتھروپک کی سطح کو جڑ پر، اور ایک مقامی جیمینی سطح کو `/v1beta` پر ظاہر کرتا ہے۔ ہر انضمام اس شکل میں جڑا ہوا ہے جس کی اس کا ٹول توقع کرتا ہے (کمانڈ کے ماخذ میں تصدیق شدہ):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | `/v1` کے ساتھ       | جی ہاں                                             |
...
جب آپ `agentproxy update` کے ساتھ اپ ڈیٹ کرتے ہیں (تصدیق کرنے کے بعد، یا `--apply` کے ساتھ)،
AgentProxy انسٹالیشن کو `--include=optional` کے ساتھ چلتا ہے:

```bash
npm install -g agentproxy@latest --include=optional
```

یہ **نہیں** ہے ایک پرچم جو آپ `agentproxy update` کو دیتے ہیں — یہ ہمیشہ اپ ڈیٹر کی طرف سے لاگو ہوتا ہے۔ یہ اس بات کی ضمانت دیتا ہے کہ `optionalDependencies` (`better-sqlite3`, `keytar`,
...
دیگر `agentproxy update` پرچم (ماخذ میں تصدیق شدہ): `--check` (اگر پرانا ہو تو 1 پر نکلیں)، `--apply` (بغیر پوچھے انسٹال کریں)، `--changelog`, `--no-backup`,
`--yes`۔

---

## Google Gemini CLI کے ذریعے `agentproxy run gemini`
...
(اور `:streamGenerateContent?alt=sse`) جاری کرتا ہے — بالکل AgentProxy کی مقامی
Gemini سطح (`/v1beta`)۔ `agentproxy run gemini` یہ خود بخود جوڑتا ہے:

- `GOOGLE_GEMINI_BASE_URL` → فعال AgentProxy بیس URL (جڑ، کوئی `/v1` نہیں)؛
- `GEMINI_API_KEY` → حل شدہ AgentProxy سند (آپشن/env/context)؛
- ایک **عارضی الگ `GEMINI_CLI_HOME`** جس کا `.gemini/settings.json`
  ...
  کبھی بھی AgentProxy کی ہدایت کردہ لانچ کو اووررائیڈ نہ کرے — باہر نکلنے کے بعد ہٹا دیا جاتا ہے؛
...
`tests/unit/cli/run-execution.test.ts`)۔ حقیقی AgentProxy سرور کے خلاف حقیقی
...
اختیاری: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` صفائی کو محدود کرتا ہے؛
`AGENTPROXY_SMOKE_TIMEOUT_MS` ہر ہدف کے لیے 120 سیکنڈ کے ٹائم آؤٹ کو اووررائیڈ کرتا ہے۔
...
- [Codex CLI ترتیب](./CODEX-CLI-CONFIGURATION.md) — ایک بار کی `[model_providers.agentproxy]` بنیادی سیٹ اپ
