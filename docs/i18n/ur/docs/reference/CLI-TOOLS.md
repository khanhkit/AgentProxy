...
AgentProxy تین اقسام کے CLI ٹولز کے ساتھ مربوط ہے جو تین مخصوص ڈیش بورڈ صفحات پر پھیلے ہوئے ہیں:

| صفحہ           | راستہ                   | تصور                                                                                            | تعداد         |
| -------------- | ----------------------- | ----------------------------------------------------------------------------------------------- | ------------- |
| **CLI Code's** | `/dashboard/cli-code`   | کوڈنگ کے ٹولز جنہیں آپ AgentProxy کی طرف اشارہ کرتے ہیں (کلائنٹ → CLI → AgentProxy → فراہم کنندہ) | 26            |
| **CLI Agents** | `/dashboard/cli-agents` | خود مختار ایجنٹس جنہیں آپ AgentProxy کی طرف اشارہ کرتے ہیں (اسی بہاؤ، وسیع دائرہ)                | 8             |
| **ACP Agents** | `/dashboard/acp-agents` | CLIs جو AgentProxy stdio/ACP کے ذریعے بیک اینڈ کے طور پر پیدا کرتا ہے (معکوس بہاؤ)               | دیکھیں رجسٹری |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
           │
           ▼  (سب AgentProxy کی طرف اشارہ کرتے ہیں)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (AgentProxy صحیح فراہم کنندہ کی طرف راستہ بناتا ہے)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agents (معکوس پیداوار کا بہاؤ):
    کلائنٹ کی درخواست → AgentProxy → stdio/ACP کے ذریعے CLI پیدا کرتا ہے → جواب
```
...
آپ کو ہر ٹول کی ترتیب ہاتھ سے لکھنے کی ضرورت نہیں ہے۔ AgentProxy ایک `setup-*`
کمانڈ فراہم کرتا ہے جو ایک چلتے ہوئے
AgentProxy (مقامی یا دور) سے **زندہ** ماڈل کی کیٹلاگ پڑھتا ہے اور آپ کے مشین پر ٹول کی اپنی ترتیب لکھتا ہے:
...
ہر ایک `--remote <url> --api-key <key>` قبول کرتا ہے (ایک مقامی ٹول کو دور AgentProxy کے خلاف ترتیب دینا)، `--dry-run` (لکھے بغیر پیش نظارہ)، اور `--port`۔ ماڈل خودکار دریافت نہ کرنے والے ٹولز (Cline، Kilo، Roo، Goose، Aider، Qwen) `--model <id>` لیتے ہیں (اور غیر تعاملاتی چلانے کے لیے `--yes`)۔ CLI کو صحیح ماحول کے ساتھ شروع کرنے کے لیے اور بالکل کوئی ترتیب نہ لکھنے کے لیے، عمومی
`agentproxy run <target>` لانچر استعمال کریں (claude، codex، aider، goose، opencode، qwen،
gemini — ہدف اور عرفیات `bin/cli/cli-manifest.mjs` سے آتی ہیں)؛ ماضی کے
ہر ٹول کے لانچر `agentproxy launch` (Claude Code) اور `agentproxy launch-codex`
(Codex) دستیاب رہتے ہیں۔ Gemini CLI صرف لانچ کے لیے ہے: یہ ایک `agentproxy run`
...
ایک `setup-*` کمانڈ جو AgentProxy کنٹینر کے اندر چلائی جاتی ہے، کنٹینر کے اپنے ہوم میں لکھتی ہے، جسے کوئی میزبان CLI نہیں پڑھتا اور جو کنٹینر کے ساتھ غائب ہو جاتا ہے۔ AgentProxy اس کا پتہ لگاتا ہے اور لکھنے کے بجائے ہدایات کے ساتھ `2` کے ساتھ باہر نکلتا ہے۔ آگے بڑھنے کے دو سپورٹ شدہ طریقے — CLI کو میزبان پر انسٹال کریں اور
`agentproxy connect` کنٹینر سے، یا کنفیگریشن ڈائریکٹریز کو بائنڈ ماؤنٹ کریں اور `CLI_CONFIG_HOME` سیٹ کریں (کمپوز `host` پروفائل)۔ ہر `setup-*` کمانڈ، ساتھ ہی `agentproxy configure` اور `agentproxy config set`، قبول کرتا ہے
`--allow-container-write` جب کنٹینر کے اپنے CLIs کی ترتیب دینا آپ کا اصل مطلب تھا؛ `AGENTPROXY_ALLOW_CONTAINER_CONFIG_WRITE=true` سرور کے لیے یہی کرتا ہے۔ دیکھیں
...
| **قابل ترتیب** | `agentproxy configure <cli>` کے ذریعہ سپورٹ کیا گیا (سیٹ اپ نسخہ موجود ہے)          | `bin/cli/cli-manifest.mjs` (`configure: true`)                   |
| **قابل آغاز**  | `agentproxy run <target>` کے ذریعہ سپورٹ کیا گیا (env/args انجیکشن کی وضاحت کی گئی) | `bin/cli/cli-manifest.mjs` (`run: true`)                         |
...
یہ صفحہ (جو `/dashboard/agents` سے نام تبدیل کیا گیا ہے) CLIs کو دکھاتا ہے جو AgentProxy **پیدا** کر سکتا ہے بطور بیک اینڈ ایگزیکیوشن انجن stdio/ACP پروٹوکول کے ذریعے۔ کیٹلاگ کو علیحدہ طور پر `src/lib/acp/registry.ts` میں برقرار رکھا جاتا ہے اور یہ `CLI_TOOLS` کے برابر **نہیں** ہے۔
...
### مرحلہ 1 — AgentProxy API کلید حاصل کریں
...
# Google Gemini CLI (لانچ کرنے کے لیے `agentproxy run gemini` → /v1beta surface)
...
```bash
# AgentProxy یونیورسل اینڈپوائنٹ
...
کی کلید `AGENTPROXY_API_KEY` ماحولیاتی متغیر (`env_key`) میں رہتی ہے، کبھی بھی
فائل کے اندر نہیں:
...
> `opencode run "your prompt" --model agentproxy/claude-sonnet-4-5-thinking --variant high` کا استعمال کریں
...
یا AgentProxy ڈیش بورڈ کا استعمال کریں → **CLI Tools → Cline → Apply Config**۔
...
یا AgentProxy ڈیش بورڈ کا استعمال کریں → **CLI Tools → KiloCode → Apply Config**۔
...
اسے اس وقت استعمال کریں جب VS Code Insiders کو حسب ضرورت اینڈپوائنٹ ماڈلز کے لیے ترتیب دیا گیا ہو اور آپ چاہتے ہیں کہ AgentProxy بغیر کسی حسب ضرورت ہیڈر فیلڈ کے کام کرے۔
...
**ٹوکنیزڈ AgentProxy ایلیاس کا استعمال کرتے ہوئے مثال:**
...
- `sk-your-agentproxy-key` کو AgentProxy میں بنائی گئی API کلید سے تبدیل کریں۔
...
# CLI اپنی خود کی توثیق استعمال کرتا ہے — Kiro CLI کے لیے AgentProxy کی ضرورت نہیں ہے۔
# دوسرے ٹولز کے لیے AgentProxy کے ساتھ kiro-cli کا استعمال کریں۔
kiro-cli status
```

**Kiro IDE** ڈیسک ٹاپ ایپ کے لیے، AgentProxy کے ذریعے فراہم کردہ MITM اینڈپوائنٹ کا استعمال کریں
جو `/dashboard/cli-tools → Kiro` کے تحت ہے۔

---

## 10. داخلی AgentProxy CLI

`agentproxy` بائنری سرور کی زندگی کے چکر، سیٹ اپ، تشخیص، اور فراہم کنندہ کے انتظام کے لئے کمانڈز فراہم کرتا ہے۔ داخلہ نقطہ: `bin/agentproxy.mjs`۔

```bash
agentproxy                              # سرور شروع کریں (ڈیفالٹ پورٹ 20128)
agentproxy setup                        # انٹرایکٹو سیٹ اپ وزرڈ
agentproxy doctor                       # کنفیگ، ڈی بی، پورٹس، رن ٹائم چیک کریں
agentproxy providers list               # کنفیگر کردہ فراہم کنندہ کنکشن
agentproxy providers test-all           # ہر فعال کنکشن کا ٹیسٹ کریں
agentproxy reset-password               # ایڈمن پاس ورڈ ری سیٹ کریں
agentproxy logs                         # درخواست کے لاگ اسٹریم کریں
agentproxy health                       # تفصیلی صحت (بریکرز، کیش، میموری)
agentproxy --version                    # ورژن پرنٹ کریں
agentproxy --help                       # تمام کمانڈز دکھائیں
```
...
```bash
agentproxy setup                        # انٹرایکٹو سیٹ اپ وزرڈ
agentproxy setup --non-interactive      # CI/خودکار موڈ (ماحولیاتی متغیرات + فلیگ پڑھتا ہے)
agentproxy setup --password '<value>'   # براہ راست ایڈمن پاس ورڈ سیٹ کریں
...
| Var                 | مقصد                                                                 |
| ------------------- | -------------------------------------------------------------------- |
| `AGENTPROXY_API_KEY` | فراہم کنندہ API کلید (کمانڈر `.env()` کے ذریعے `--api-key` سے منسلک) |
| `DATA_DIR`          | AgentProxy ڈیٹا ڈائریکٹری کو اوور رائیڈ کریں                          |

تمام دیگر غیر انٹرایکٹو ان پٹس کو فلیگ کے طور پر پاس کیا جاتا ہے، ماحولیاتی متغیرات کے طور پر نہیں:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(اوپر `agentproxy setup` کے اختیارات دیکھیں)۔

### تشخیص

```bash
agentproxy doctor                       # کنفیگ، ڈی بی، پورٹس، رن ٹائم، میموری، زندہ ہونے کی جانچ کریں
agentproxy doctor --json                # مشین کے قابل پڑھنے والا JSON
agentproxy doctor --no-liveness         # HTTP صحت کی جانچ چھوڑ دیں
agentproxy doctor --host 0.0.0.0        # زندہ ہونے والے میزبان کو اوور رائیڈ کریں
agentproxy doctor --liveness-url <url>  # مکمل صحت کے اینڈ پوائنٹ کا URL اوور رائیڈ کریں
```
...
```bash
agentproxy providers available                       # AgentProxy فراہم کنندہ کی کیٹلاگ
agentproxy providers available --search openai       # کیٹلاگ کو id/name/alias/category کے ذریعے فلٹر کریں
agentproxy providers available --category api-key    # زمرے کے ذریعے فلٹر کریں (api-key, oauth, free, ...)
agentproxy providers available --json                # مشین کے قابل پڑھنے والا JSON

agentproxy providers list                            # کنفیگر کردہ فراہم کنندہ کنکشن
agentproxy providers list --json

agentproxy providers test <id|name>                  # ایک کنفیگر کردہ کنکشن کا ٹیسٹ کریں
agentproxy providers test-all                        # ہر فعال کنکشن کا ٹیسٹ کریں
agentproxy providers validate                        # مقامی طور پر صرف ساختی توثیق
agentproxy providers add <provider> --credential-env PROVIDER_KEY
agentproxy providers import ./providers.json --dry-run --json
agentproxy providers auth <provider>                 # موجودہ OAuth بہاؤ
agentproxy providers edit <id|name> --default-model <model>
agentproxy providers remove <id|name> --yes
```

`providers add/import/auth/edit/remove` API-first ہیں اور اس لئے فعال مقامی یا دور دراز کے سیاق و سباق کے خلاف کام کرتے ہیں۔ اسناد کی ان پٹ کو `--credential-stdin` یا `--credential-env` کا استعمال کرنا چاہئے؛ `--dry-run --json` صرف ریڈیکٹڈ موجودگی/شکل کی رپورٹ کرتا ہے۔ `providers available` AgentProxy کی کیٹلاگ کو پڑھتا ہے؛ `providers list/test/test-all/validate` اپنی مقامی SQLite کی خصوصیات کو برقرار رکھتے ہیں اور سرور کے چلنے کی ضرورت نہیں ہوتی۔

### بحالی اور ری سیٹ

```bash
agentproxy reset-password                # ایڈمن پاس ورڈ ری سیٹ کریں (اسی طرح: agentproxy-reset-password)
agentproxy reset-encrypted-columns       # انکرپٹ کردہ اسناد کے ری سیٹ کے لئے انتباہ + ڈرائی رن دکھائیں
agentproxy reset-encrypted-columns --force  # SQLite میں انکرپٹ کردہ اسناد کو واقعی نل کریں
```

### اسناد کا برآمد (⚠ احتیاط سے ہینڈل کریں)

```bash
agentproxy auth export                                 # انتباہ + تصدیق کا گیٹ — کوئی DB رسائی نہیں
agentproxy auth export --force                          # تمام کنکشنز کی انکرپٹ کردہ اسناد کو stdout پر JSON کے طور پر برآمد کریں
agentproxy auth export --force --id <id>                 # صرف ملنے والے کنکشن کو برآمد کریں
agentproxy auth export --force --format env               # AGENTPROXY_<PROVIDER>_<FIELD>=<value> لائنیں جاری کریں
agentproxy auth export --force --out creds.json           # ایک فائل میں لکھیں (0600 اجازتوں کے ساتھ بنائی گئی)
```
...
یہ ایک چلتے ہوئے AgentProxy سرور کو فرض کرتے ہیں، جب تک کہ دوسری صورت میں نوٹ نہ کیا جائے:

```bash
agentproxy status                       # جامع رن ٹائم کی حیثیت
agentproxy logs                         # درخواست کے لاگ اسٹریم کریں (--json, --search, --follow)
agentproxy config show                  # موجودہ کنفیگریشن دکھائیں

agentproxy provider list                # دستیاب فراہم کنندگان کی فہرست (providers list کا عرف)
agentproxy provider add                 # ایک ٹول پر فراہم کنندہ کے طور پر AgentProxy کو رجسٹر کریں
agentproxy keys add | list | remove     # API کیز کا انتظام کریں
agentproxy models [provider]            # ماڈلز کی فہرست (--json, --search)
agentproxy combo list | switch | create | delete

agentproxy backup                       # کنفیگ + ڈی بی کا اسنیپ شاٹ
agentproxy restore                      # پچھلے اسنیپ شاٹ سے بحال کریں

agentproxy health                       # تفصیلی صحت (بریکرز، کیش، میموری)
agentproxy quota                        # فراہم کنندہ کی کوٹہ کا استعمال
agentproxy cache                        # کیش کی حیثیت
agentproxy cache clear                  # معنوی + دستخط کیش کو صاف کریں

agentproxy mcp status | restart         # MCP سرور کی حیثیت / دوبارہ شروع کریں
agentproxy a2a status | card            # A2A سرور کی حیثیت / ایجنٹ کارڈ

agentproxy tunnel list | create | stop  # سرنگوں کا انتظام کریں (cloudflare/tailscale/ngrok)
agentproxy env show | get <k> | set <k> <v>  # ماحولیاتی متغیرات کا معائنہ کریں / سیٹ کریں (عارضی)

agentproxy test                         # فراہم کنندہ کی کنیکٹیویٹی اسموک ٹیسٹ
agentproxy update                       # اپ ڈیٹس کے لئے چیک کریں
agentproxy completion                   # شیل مکمل کرنے کے لئے تیار کریں
```
...
پیسٹ کرنے کے لئے تیار مثالیں ایک ٹوکنائزڈ AgentProxy URL کے ساتھ:
...
| خرابی                                        | وجہ                     | حل                                                                |
| -------------------------------------------- | ----------------------- | ----------------------------------------------------------------- |
| `Connection refused`                         | AgentProxy چل نہیں رہا   | `agentproxy serve`                                                 |
