...
Az AgentProxy egy sor `setup-*` parancsot kínál, amelyek egy kódoló CLI-t (Codex, Claude Code, OpenCode, Cline, …) konfigurálnak, hogy az AgentProxy-ot használja háttérként — így az eszköz **egy** végponthoz kapcsolódik, és az AgentProxy a megfelelő szolgáltatóhoz irányít automatikus visszaeséssel. Minden parancs a **valós idejű** modell katalógust olvassa egy futó AgentProxy-ból (helyi vagy távoli), és a saját konfigurációs fájlját írja a **te** gépedre. Az API kulcsot egy környezeti változó hivatkozza, ahol az eszköz támogatja azt. Az alábbiakban a helyi környezeti fájlt megőrző parancsok találhatók.

Van egy általános indító is — `agentproxy run <target>` — amely elindítja a `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` vagy `gemini` eszközöket a megfelelő környezettel, anélkül, hogy bármilyen konfigurációt írna. A célok és azok aliasai a kanonikus manifestből származnak `bin/cli/cli-manifest.mjs` (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), és az `agentproxy completion` ugyanazokat a manifestből származó cél szavakat kínálja. A régi, eszközspecifikus indítók — `agentproxy launch` (Claude Code) és `agentproxy launch-codex` (Codex) — továbbra is elérhetők.
...
- [Távoli Mód](./REMOTE-MODE.md) — vezérelj egy távoli AgentProxy-ot (VPS / Tailnet) a laptopodról
...
Minden parancs tiszteletben tartja az **aktív kontextust** (amelyet az `agentproxy connect`-tel állítanak be, lásd [Távoli Mód](./REMOTE-MODE.md)) vagy az explicit `--remote <url> --api-key <key>` zászlókat. Az alábbi "Helyi vs távoli" azt jelenti: zászlók nélkül a `http://localhost:20128` címet célozza meg; `--remote` (vagy egy aktív távoli kontextus) esetén a katalógust onnan szerzi be, és helyben írja a konfigurációt.

| Parancs                    | Eszköz                         | Amit ír                                                                                                                                                                                               | Kulcs zászlók                                                                                                                              | Helyi vs távoli |
| -------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI               | `~/.codex/<name>.config.toml` — egy profil minden kompatibilis szövegmintához (`codex --profile <name>`)                                                                                              | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Mindkettő       |
| `agentproxy setup-claude`   | Claude Code                    | `~/.claude/profiles/<name>/settings.json` — egy profil minden egyező modellhez (`CLAUDE_CONFIG_DIR`)                                                                                                  | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Mindkettő       |
| `agentproxy setup-opencode` | OpenCode (openai-kompatibilis) | `~/.config/opencode/opencode.json` — `agentproxy` szolgáltató minden katalógus modellel (`opencode -m agentproxy/<model>`)                                                                              | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Mindkettő       |
| `agentproxy setup-cline`    | Cline                          | `~/.cline/data/{globalState,secrets}.json` (CLI mód) + nyomtatja a VS Code kiterjesztés beállításait                                                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Mindkettő       |
| `agentproxy setup-kilo`     | Kilo Code                      | `~/.local/share/kilo/auth.json` (CLI) + egyesíti a `kilocode.*` fájlokat a VS Code `settings.json`-ba, ha létezik                                                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Mindkettő       |
| `agentproxy setup-continue` | Continue / `cn` CLI            | `~/.continue/config.yaml` — `provider: openai` modellek, kulcs a `${{ secrets.AGENTPROXY_API_KEY }}` által                                                                                             | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Mindkettő       |
| `agentproxy setup-cursor`   | Cursor                         | Semmi — nyomtatja az alkalmazáson belüli lépéseket (Cursor konfigurációja átláthatatlan SQLite)                                                                                                       | `--remote` `--api-key` `--only` `--port`                                                                                                   | Mindkettő       |
| `agentproxy setup-roo`      | Roo Code                       | `~/.agentproxy/roo-settings.json` (import doc) + beállítja a `roo-cline.autoImportSettingsPath`-t, ha létezik egy VS Code `settings.json` fájl                                                         | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Mindkettő       |
| `agentproxy setup-crush`    | Crush                          | `~/.config/crush/crush.json` — `openai-compat` szolgáltató, kulcs a `$AGENTPROXY_API_KEY` által                                                                                                        | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Mindkettő       |
| `agentproxy setup-goose`    | Goose                          | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + nyomtatja a környezeti receptet                                                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Mindkettő       |
| `agentproxy setup-aider`    | Aider                          | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + nyomtatja a környezeti receptet                                                                                                      | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Mindkettő       |
| `agentproxy setup-qwen`     | Qwen Code                      | `~/.qwen/settings.json` — V4 `modelProviders.openai` tömb + `AGENTPROXY_API_KEY` a `~/.qwen/.env` fájlban                                                                                              | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Mindkettő       |
| `agentproxy run <target>`   | Futási indítás (általános)     | Semmi — elindítja a `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` eszközöket a megfelelő környezettel és argumentumokkal; a Qwen és a Gemini ideiglenes, elszigetelt otthont használnak | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Mindkettő       |
| `agentproxy launch`         | Claude Code                    | Semmi — elindítja a `claude`-t az `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` injektálásával                                                                                                          | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Mindkettő       |
| `agentproxy launch-codex`   | OpenAI Codex CLI               | Semmi — elindítja a `codex`-t az `agentproxy` szolgáltató injektálásával `-c` zászlók segítségével                                                                                                     | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Mindkettő       |

Zászlók megjegyzései (ellenőrizve a parancs forrásában):

- `--remote <url>` — a katalógust egy távoli AgentProxy-ból szerzi be (felülírja a `--port`-ot és az aktív kontextust). A `--api-key <key>` biztosítja a hitelesítő adatokat a szerverhez (alapértelmezés szerint az `AGENTPROXY_API_KEY` környezeti változót, vagy az aktív kontextus tokenjét használja).
...
- A `--model <id>` az `agentproxy run` parancsnál követi a manifest per-cél vezetékezését (`bin/cli/cli-manifest.mjs`): **aider** a `--model openai/<id>`-t, **opencode** a `--model agentproxy/<id>`-t kap (a prefix csak akkor kerül hozzáadásra, ha az azonosító nem tartalmazza azt); **qwen** és **gemini** az azonosítót szó szerint kapja; **claude** az `ANTHROPIC_MODEL`-on keresztül, **goose** a `GOOSE_MODEL`-on keresztül, és **codex** a `-c model_providers.agentproxy.*` argumentumokon keresztül. **A Qwen az egyetlen futási cél, amely kifejezetten megköveteli a `--model`-t** — az `agentproxy run qwen` nélküle `2`-t ad vissza egy explicit hibával.
- `--port <port>` — helyi AgentProxy port (alapértelmezett `20128`, figyelmen kívül hagyva, ha a `--remote` be van állítva). Minden `setup-*` és mindkét indító esetén jelen van.
- Az `agentproxy run` kilépési kódok: a gyermek CLI saját kilépési kódja verbatim módon propagálódik; `2` = érvénytelen argumentumok (támogatott cél hiánya, kötelező `--model` hiánya, konténer őr); `127` = a cél bináris nem található a `PATH`-ban; `130`/`143`/`129` amikor a launch-t a `SIGINT`/`SIGTERM`/`SIGHUP` zárja le; `1` = egyéb futási indítási hiba.
...
> Van egy gazdagabb plugin integráció is — `agentproxy setup opencode` — amely
> telepíti az `@agentproxy/opencode-plugin`-t. Ezek különböző parancsok; a fenti táblázat a `setup-opencode`-t dokumentálja.

---

## Helyi használat

Az AgentProxy `localhost:20128` címen fut, csak futtasd a beállító parancsot az eszközödhöz. A katalógus a helyi szerverről kerül lekérésre.
...
export AGENTPROXY_API_KEY=sk-...  # hivatkozva {env:AGENTPROXY_API_KEY}, soha nem lemezen
...
```bash
agentproxy launch                 # Claude Code → helyi AgentProxy
agentproxy launch-codex           # Codex CLI → helyi AgentProxy
...
agentproxy run aider --model glm/glm-5.2 -- --message "válasz OK"
agentproxy run goose --model glm/glm-5.2
agentproxy run opencode --model glm/glm-5.2 -- run "válasz OK"
agentproxy run qwen --model glm/glm-5.2 -- -p "válasz OK"
agentproxy run gemini --model glm/glm-5.2 -- --skip-trust -p "válasz OK"

# Explicit parancs útvonal: átad minden, ami a -- után jön
agentproxy run claude -- --print-system-prompt "ellenőrizd ezt a diffet"
```
...
Bármely beállító parancsot irányíts egy távoli AgentProxy-ra `--remote` + `--api-key` használatával. A katalógus a távoli szerverről kerül lekérésre; a konfiguráció a helyi gépeden kerül írásra.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # először exportáld az AGENTPROXY_API_KEY-t
...
```bash
agentproxy connect 192.168.0.15        # létrehoz egy hatókörös tokent, tárolja a kontextust
agentproxy setup-codex                 # ← most a távoli katalógust használja
agentproxy setup-opencode              # ← ugyanaz
agentproxy launch                      # ← Claude Code a távoli ellen
```
...
Az AgentProxy az OpenAI felületet a `/v1`-en, az Anthropic felületet a gyökérnél, és egy natív Gemini felületet a `/v1beta`-n kínál. Minden integráció a formátumhoz van kötve, amit az eszköz elvár (ellenőrizve a parancs forrásában):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | `/v1`-el       | Igen                                               |
...
Amikor frissítesz az `agentproxy update` paranccsal (miután megerősítetted, vagy a `--apply` használatával),
az AgentProxy a frissítést `--include=optional` opcióval futtatja:

```bash
npm install -g agentproxy@latest --include=optional
```

Ez **nem** egy olyan zászló, amelyet az `agentproxy update` parancshoz adsz — ez mindig alkalmazásra kerül a
...
# [DRY RUN] Futna: npm install -g agentproxy@latest --include=optional
```

Más `agentproxy update` zászlók (forrásban ellenőrizve): `--check` (1-es kilépés, ha
elavult), `--apply` (telepítés kérdés nélkül), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI az `agentproxy run gemini` segítségével
...
(és `:streamGenerateContent?alt=sse`) kéréseket küld rá — pontosan az AgentProxy natív
Gemini felületének (`/v1beta`) megfelelően. Az `agentproxy run gemini` ezt automatikusan összeköti:

- `GOOGLE_GEMINI_BASE_URL` → az aktív AgentProxy alap URL (gyökér, nincs `/v1`);
- `GEMINI_API_KEY` → a megoldott AgentProxy hitelesítő (opció/env/környezet);
- egy **ideiglenes elszigetelt `GEMINI_CLI_HOME`**, amelynek `.gemini/settings.json`
  ...
  soha nem írja felül az AgentProxy által irányított indítást — a kilépés után eltávolítva;
...
AgentProxy szerverrel, egy opcionális keretrendszer létezik a
...
Opcionális: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` korlátozza a tesztelést;
`AGENTPROXY_SMOKE_TIMEOUT_MS` felülírja a 120 másodperces célonkénti időkorlátot.
...
- [Codex CLI konfiguráció](./CODEX-CLI-CONFIGURATION.md) — az egyszeri `[model_providers.agentproxy]` alapbeállítás
