...
**AgentProxy** — egységes AI proxy/router. Egy végpont, 329 LLM szolgáltató, automatikus visszaesés.
...
Az AgentProxy három kapcsolódó, de különálló ideiglenes hiba mechanizmust tartalmaz. Tartsd a hatókörüket külön, amikor a routing viselkedést hibakeresed. Lásd a
...
- **Importok**: külső → belső (`@/`, `@agentproxy/open-sse`) → relatív
...
- **Útvonal aliasok**: `@/*` → `src/`, `@agentproxy/open-sse` → `open-sse/`, `@agentproxy/open-sse/*` → `open-sse/*`
- **Alapértelmezett port**: 20128 (API + dashboard ugyanazon a porton)
- **Adatkönyvtár**: `DATA_DIR` környezeti változó, alapértelmezett: `~/.agentproxy/`
