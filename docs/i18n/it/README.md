# 🚀 AgentProxy — Il Gateway AI Gratuito
...
<img src="../../screenshots/MainAgentProxy.png" alt="Dashboard di AgentProxy" width="820"/>
...
> Combinare manualmente i tier gratuiti è scomodo: decine di SDK, decine di rate limit e nessuna idea chiara di quanta capacità sia davvero disponibile. AgentProxy aggrega i tier gratuiti **documentati** di **42 pool di provider / 495 modelli** in un unico numero trasparente e lo mostra in tempo reale nella dashboard (`/dashboard/free-tiers`).

<img src="../../diagrams/free-tier-budget.svg" width="100%" alt="Scheda del budget dei tier gratuiti di AgentProxy: ~1,51 miliardi di token gratuiti al mese in modo continuativo, fino a ~2,13 miliardi nel primo mese includendo i crediti di registrazione, calcolati sui tier gratuiti documentati di 42 pool di provider / 495 modelli dietro un unico endpoint. Calcolo trasparente con deduplicazione dei pool: ogni pool condiviso viene conteggiato una sola volta (contare ogni rate limit 24/7 darebbe ~10 miliardi, dato non pubblicato); 15 provider sono segnalati per i ToS così puoi decidere. Barra del budget dei pool gratuiti conteggiabili con griglia per modello, crediti una tantum del primo mese e provider permanentemente gratuiti senza limite di token pubblicato, mostrati separatamente per non gonfiare il valore principale. Utilizzo e residuo in tempo reale su /dashboard/free-tiers."/>
...
⭐ Metti una stella alla repo se AGENTPROXY ti ha aiutato a risparmiare e a lavorare meglio.
...
**Domande, suggerimenti sui provider, roadmap e supporto → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://github.com/khanhkit/AgentProxy/discussions) · WhatsApp [🌍 Global](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 Brasil](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
<table>
  ...
  <tr>
    ...
    <td align="center"><a href="#-why-agentproxy">🤔 Perché AgentProxy</a></td>
    <td align="center"><a href="#-what-sets-agentproxy-apart">🏆 Cosa lo distingue</a></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td align="center"><a href="#-where-agentproxy-runs--anywhere">🖥️ Dove funziona</a></td>
    <td align="center"><a href="#-private--local-first">🔒 Privacy</a></td>
  </tr>
  <tr>
    <td align="right"><b>👀 Guarda</b></td>
    <td align="center"><a href="#-agentproxy-in-action">🎬 In azione</a></td>
  ...
  <tr>
    ...
    <td align="center"><a href="#-support-agentproxy">💚 Supporta / Dona</a></td>
...
<img src="../../diagrams/works-zero-config.svg" width="100%" alt="Funziona subito dopo l'installazione — configurazione zero. Tre passaggi: 1. Installa — npm i -g agentproxy, il server parte su localhost:20128. 2. Punta il tuo strumento a http://localhost:20128/v1 — qualunque strumento compatibile con OpenAI (Claude Code, Cursor, Cline). 3. Risponde — usa il modello auto per una risposta immediata, senza API key, registrazione o configurazione. OpenCode Free è già collegato alla combo auto, quindi una nuova installazione risponde immediatamente."/>
...
<sub>Preferisci uno specifico backend gratuito? Chiama direttamente `oc/…` (OpenCode Free). Poi passa a `auto` e lascia che sia AgentProxy a scegliere.</sub>
...
<img src="../../diagrams/promise-pillars.svg" width="100%" alt="La promessa — Un endpoint. 349 provider. Non smettere mai di creare: AgentProxy sceglie quello più economico che funziona. Sei pilastri: non raggiungere mai i limiti (fallback automatico tra 349 provider in millisecondi, zero downtime) · risparmia fino al 95% dei token (compressione combinata RTK + Caveman del 15–95%, ~89% in media nelle sessioni ricche di tool) · $0 per iniziare (oltre 90 tier gratuiti, 56 gratis per sempre, senza carta) · ogni strumento funziona (33 agenti di coding con una sola configurazione) · un endpoint (OpenAI ↔ Claude ↔ Gemini ↔ Responses API su /v1) · pronto per la produzione (circuit breaker, TLS stealth, MCP con 110 tool, A2A, memoria, guardrail, eval — oltre 25.000 test)."/>
...
> **Vuoi diventare un Open Source Friend?** Queste sono le aziende che sostengono l'open source e aiutano AgentProxy a continuare a crescere — e dichiariamo pubblicamente dove viene usato ogni token che ci forniscono. Contatto: [diegosouza.pw@outlook.com](mailto:diegosouza.pw@outlook.com)
...
<table>
  ...
  <tr>
    ...
    <td>
      Grazie a <b>Cheaper Inference</b>, un Open Source Friend di AgentProxy, per il sostegno al progetto! Cheaper Inference è un gateway ordinato per costo che rivende 42 modelli di frontiera — Claude, GPT-5.x, Gemini, Kimi K3, GLM, DeepSeek, Grok e MiniMax — dietro un unico endpoint compatibile con OpenAI, instradando ogni richiesta verso il provider idoneo più economico senza mai addebitare più del prezzo di listino del produttore del modello.
      <br/><br/>
      <b>Supporto di prima classe in AgentProxy:</b> Chat Completions, endpoint nativo <code>/v1/responses</code>, vision, tool calling e 3 modelli immagine (<code>grok-imagine</code>, <code>nano-banana-pro</code>, <code>nano-banana-2</code>, raggiungibili come <code>cheaperinference/&lt;model&gt;</code>). <a href="https://cheaperinference.com/"><b>Ottieni una API key →</b></a>
    </td>
  </tr>
</table>

<sub>I link contrassegnati con <code>aff=agentproxy</code> sono link partner. Finanziano il progetto senza costi aggiuntivi per te.</sub>
...
<sub><i>Questa sezione contiene soltanto codici referral/coupon. Le partnership sponsorizzate sono riportate sopra in <b>🤝 Supportato dai nostri amici dell'Open Source</b>. AgentProxy non ha sponsorizzazioni o partnership con i provider elencati qui: sono coupon pubblici utilizzabili da chiunque.</i></sub>
...
<table>
  <tr>
    ...
    <td>
      <sub><b><a href="https://agentrouter.org/register?aff=70LM">AgentRouter</a></b> — registrazione affiliata · <b>$100 di crediti gratuiti</b> alla registrazione (server gratuito, aspettati una latenza maggiore — ideale per test, non per produzione). Supporto di prima classe in AgentProxy dalla <b>v3.8.50</b>: Chat Completions, formato wire compatibile con Anthropic e percorso compatibile con OpenAI. I modelli disponibili includono <code>claude-opus-4-8</code>, <code>claude-opus-5</code>, <code>gpt-5.6-sol</code> e altri. <b><a href="https://agentrouter.org/register?aff=70LM">Ottieni i tuoi $100 →</a></b></sub>
      <br/><br/>
      <sub>⚠️ <i>Link affiliato — AgentProxy non ha sponsorizzazioni o partnership con questo provider.</i></sub>
    </td>
  </tr>
</table>

<sub>Conosci un altro provider con un generoso coupon gratuito di registrazione utile agli utenti AgentProxy? Apri una issue e lo aggiungeremo qui.</sub>
...
> Una **combo** è una catena di modelli tra cui AgentProxy instrada le richieste **automaticamente**. La quota finisce, un provider fallisce o i costi aumentano: la combo passa silenziosamente al modello successivo. **È questo che rende AgentProxy resistente ai guasti.** 🛡️

### ⚡ Zero-config — usa semplicemente `auto`

Non devi creare nessuna combo. Imposta il modello su `auto` (o una sua variante) e AgentProxy costruisce una combo virtuale a partire dai provider collegati, assegnando i punteggi in tempo reale:
...
<a id="-what-sets-agentproxy-apart"></a>

## 🏆 Cosa distingue AgentProxy

</div>

<img src="../../diagrams/comparison-table.svg" width="100%" alt="Cosa distingue AgentProxy — tabella di confronto con 9router, OpenRouter, CLIProxyAPI e LiteLLM su 13 capacità. AgentProxy: 349 provider, oltre 90 provider gratuiti integrati, 19 strategie di routing, compressione token con 12 motori, server MCP integrato con 110 tool, protocollo agenti A2A, memoria persistente, guardrail, cloud agent, TLS fingerprint stealth, Desktop/Termux/PWA, 42 locale UI i18n, self-hosting 100% MIT. AgentProxy è l'unico a includere l'intero insieme; i concorrenti mostrano combinazioni di supporto completo, parziale e assente. Verificato sulla documentazione di ciascun progetto."/>

<sub>📊 Metodologia completa e dettaglio per funzionalità rispetto a 9router, OpenRouter, CLIProxyAPI e LiteLLM → [`docs/comparison/AGENTPROXY_VS_ALTERNATIVES.md`](../../comparison/AGENTPROXY_VS_ALTERNATIVES.md)</sub>

<br/>

<a id="-support-agentproxy"></a>

## 💚 Supporta AgentProxy

AgentProxy è distribuito con licenza MIT e mantenuto apertamente. Se ti fa risparmiare tempo o denaro, ecco come aiutarlo a restare indipendente — scegli ciò che preferisci. Le sponsorizzazioni non influenzano mai la priorità del routing: acquistano visibilità, non posizionamento.

<table>
  <tr><td nowrap>⭐ <b>Metti una stella alla repo</b></td><td>Gratis — aiuta davvero la visibilità</td><td><a href="https://github.com/khanhkit/AgentProxy">Dai una stella a AgentProxy</a></td></tr>
...
<img src="../../assets/pix-qr.png" width="140" align="right" alt="Codice QR PIX di AgentProxy"/>
...
🐛 Hai trovato un bug o vuoi lasciare un feedback? Apri una [Discussion](https://github.com/khanhkit/AgentProxy/discussions).
...
Radar è opt-in e usa soltanto richieste GET. Il client AgentProxy non carica prompt, traffico, configurazione dei provider, telemetria d'uso o lo stato locale di chiusura degli annunci. Dettagli sui requisiti e sul catalogo corrente su **[radar.agentproxy.online/planos](https://radar.agentproxy.example.com/planos)**.
...
- **🤖 Configurazione CLI/agente con un comando** — `setup-*` configura oltre 12 strumenti di coding; `agentproxy run` avvia 7 CLI (Claude Code, Codex, Aider, Goose, OpenCode, Qwen Code, Gemini CLI) senza scrivere configurazioni; `agentproxy configure` è un selettore interattivo provider+modello con preferiti per contesto. → [Integrazioni CLI](../../guides/CLI-INTEGRATIONS.md)
- **🛰️ Modalità remota** — controlla un AgentProxy remoto con token scoped (`connect` / `contexts` / `tokens`) + helper OAuth `antigravity` per installazioni VPS. → [Modalità remota](../../guides/REMOTE-MODE.md)
...
- **💸 Telemetria dei costi ovunque** — header di costo/utilizzo `X-AgentProxy-*` su ogni endpoint, header del risparmio su cache HIT, quote di spesa USD per chiave. → [Riferimento API](../../reference/API_REFERENCE.md)
- **🧠 Memoria sotto il tuo controllo** — disattivata per impostazione predefinita, quantizzazione vettoriale int8 opt-in + decadimento tipizzato, `x-agentproxy-no-memory` per-request. → [Memoria](../../frameworks/MEMORY.md)
...
- **📡 Trasparenza del routing** — ogni risposta include un header `X-AgentProxy-Decision` con strategia/provider/latenza che l'ha servita; una nuova strategia combo `cache-optimized` + il fattore `cacheAffinity` di Auto-Combo riportano le richieste ripetute alla connessione che possiede il prefisso in cache; un endpoint read-only `/v1/auto-combo/{channel}/candidates` espone il pool di candidati live di un canale `auto/*`. → [Auto-Combo](../../routing/AUTO-COMBO.md)
...
<sub>📖 Configurazione per ciascuno dei 34 strumenti (26 CLI Code + 8 CLI Agents) → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 Plugin OpenCode → [`@agentproxy/opencode-provider`](https://www.npmjs.com/package/@agentproxy/opencode-provider)</sub>

</div>

<br/>

**Avvia qualsiasi CLI supportata tramite AgentProxy con un solo comando** — senza scrivere file di configurazione,
...
agentproxy configure codex          # also: claude opencode qwen aider goose cline continue kilo
```

Ogni comando rispetta il contesto remoto attivo (`agentproxy connect <host>`); `--dry-run`
...
<a id="-where-agentproxy-runs--anywhere"></a>
<div align="center">

## 🖥️ Dove gira AgentProxy — ovunque
...
<table>
  ...
  <tr><td align="left" nowrap>📦 <b>npm (globale)</b></td><td align="left" nowrap><code>npm install -g agentproxy</code></td><td align="left">Un comando, qualsiasi OS</td></tr>
  ...
  <tr><td align="left" nowrap>📱 <b>Android (Termux)</b></td><td align="left" nowrap><code>pkg install nodejs && npx -y agentproxy</code></td><td align="left">Gira <b>sul tuo telefono</b>, 24/7, senza root</td></tr>
  ...
  <tr><td align="left" nowrap>🧩 <b>Plugin OpenCode</b></td><td align="left" nowrap><code>@agentproxy/opencode-provider</code></td><td align="left">Integrazione nativa con OpenCode</td></tr>
  <tr><td align="left" nowrap>🤖 <b>VS Code Copilot Chat</b></td><td align="left" nowrap>installa l'estensione <b>OmniCopilot</b></td><td align="left">Tutti i modelli AgentProxy nel selettore nativo di Copilot Chat — Stable e Insiders</td></tr>
...
### 🧩 Novità: AgentProxy dentro il Copilot Chat nativo di VS Code

</div>

> Nessuna nuova barra laterale, nessuna nuova UI di chat — ogni modello servito da AgentProxy compare direttamente nel
...
al tuo server AgentProxy (predefinito `localhost:20128`), poi apri Copilot Chat → selettore modelli
...
Dall'editor: apri la vista **Extensions**, cerca **"AgentProxy"**, fai clic su **Install**
...
> Oltre al server, AgentProxy è una **console completa da riga di comando** con **oltre 80 comandi**, più protocolli agent aperti che permettono a un agent AI di gestirlo **autonomamente**.

### ⌨️ Una vera CLI (non solo `start`)

```bash
agentproxy               # serve gateway + dashboard (port 20128)
agentproxy chat          # interactive TUI chat client (slash: /model /combo /skill /memory)
agentproxy setup         # guided first-run wizard
agentproxy doctor        # diagnose providers, ports, native deps
```

### 🛰️ Modalità remota — esegui qui la CLI, AgentProxy su un VPS

AgentProxy gira su un server? Gestiscilo dal laptop con la **stessa CLI**. Accedi una volta
con un token di accesso con scope; da quel momento ogni comando punta all'istanza remota.

```bash
agentproxy connect 192.168.0.15            # password → scoped token, saved as a context
agentproxy models list                     # ← runs against the REMOTE server
agentproxy configure codex                 # ← picks a remote model, writes a local Codex profile
agentproxy tokens create --name ci --scope read   # mint narrower tokens for other machines
agentproxy contexts use default            # ← switch back to the local server
```
...
<img src="../../diagrams/cli-terminal.svg" width="50%" alt="Demo animata del terminale con la CLI AgentProxy — agentproxy providers list, agentproxy combo list, agentproxy health — che scorre gli oltre 80 comandi disponibili: providers · oauth · keys · combo · nodes · models · cache · compression · cost · usage · quota · health · resilience · telemetry · logs · audit · mcp · a2a · cloud · memory · skills · eval · tunnel · backup · sync · webhooks · policy · pricing · translator · simulate …"/>

</div>

### 🤝 Collega un agent — e controllerà AgentProxy stesso

Esponi AgentProxy tramite **MCP**, **A2A**, una **REST API**, **webhook** o una **CLI remota** — qualsiasi agent compatibile (o il tuo codice) ottiene accesso al gateway: routing, provider, combo, cache, compressione, memoria — in autonomia. Gli endpoint HTTP qui sotto sono serviti su `http://localhost:20128`.

<table>
  <tr><th align="left">Interfaccia</th><th align="left">Endpoint / comando</th><th align="left">A cosa serve</th></tr>
  <tr><td align="left" nowrap>🧰 <b>MCP (stdio)</b></td><td align="left" nowrap><code>agentproxy --mcp</code></td><td align="left">Collegamento a Claude Desktop, Cursor e qualsiasi client MCP</td></tr>
  ...
  <tr><td align="left" nowrap>🛰️ <b>CLI remota</b></td><td align="left" nowrap><code>agentproxy connect <host></code></td><td align="left">Gestisci un'istanza remota con token di accesso con scope</td></tr>
</table>

```bash
# Give Claude Code the full AgentProxy toolset over MCP:
...
<img src="../../diagrams/compression-pipeline.svg" width="100%" alt="Pipeline di compressione AgentProxy: una richiesta client da 10.000 token attraversa 12 motori in cascata — Session-Dedup, CCR, Lite, RTK, Responses Tool Output, Headroom, Relevance, Caveman, Aggressive, LLMLingua-2, Ultra, OmniGlyph — e raggiunge il provider con circa 1.080 token, con un risparmio fino al 95%. Codice, URL e JSON sono sempre preservati byte per byte."/>
...
> **Perché usare molti token quando ne bastano pochi?** Ogni richiesta attraversa la pipeline di compressione di AgentProxy **in modo trasparente** — senza modifiche al client. Ora è una **stack di 12 motori componibili** eseguiti in ordine e combinabili per ciascun routing combo — basati anche su idee di [RTK](https://github.com/rtk-ai/rtk), [Caveman](https://github.com/JuliusBrussee/caveman) (⭐ 90K+), [LLMLingua-2](https://github.com/microsoft/LLMLingua) e [Troglodita](https://github.com/leninejunior/troglodita) (PT-BR).
...
- **🎛️ Dove viene decisa la compressione** _(precedenza, alta → bassa)_ — header per richiesta `x-agentproxy-compression` › override del routing combo › profilo nominato attivo › adaptive / auto-trigger › impostazione predefinita del pannello › off. Il piano applicato viene restituito nell'header di risposta `X-AgentProxy-Compression: <mode>; source=<source>`.
...
Dovresti vedere elencati i modelli collegati. 🎉 Tutto qui — inizia a programmare: AgentProxy instrada automaticamente le richieste ed esegue il fallback quando serve.

Se il tuo client non può inviare header personalizzati, AgentProxy espone anche alias di compatibilità con token incorporato:
...
> **Canale Docker pre-release:** `khanhkit/agentproxy:next` e
> `khanhkit/agentproxy:next-web` seguono l'attuale branch `release/v*` predefinito.
...
Sono supportati `bun install` standard e l'installazione globale (`bun install -g agentproxy`) tramite rilevamento del runtime Bun:

- **`bun:sqlite` integrato**: AgentProxy usa il driver integrato `bun:sqlite` quando gira con Bun, con fallback a `better-sqlite3` su Node.js o a `sql.js`.
...
- **Dockerfile Bun dedicato**: `Dockerfile.bun` multi-stage per deployment di produzione nativi Bun (`docker build -f Dockerfile.bun -t agentproxy:bun .`).
...
```bash
AGENTPROXY_SKIP_POSTINSTALL=1 npm install -g agentproxy   # CI=1 also skips it
```
...
<a id="-agentproxy-in-action"></a>
<div align="center">

# 🎬 AgentProxy in azione
...
> 🎬 **Hai realizzato un video su AgentProxy?** Apri una [issue](https://github.com/khanhkit/AgentProxy/issues/new) o una [discussion](https://github.com/khanhkit/AgentProxy/discussions) con il link — lo metteremo in evidenza qui.
...
| 🌍 **Sito web**                              | [agentproxy.online](https://agentproxy.example.com)                                                                                                 |
| 📦 **Codice sorgente**                       | [github.com/khanhkit/AgentProxy](https://github.com/khanhkit/AgentProxy)                                                               |
| 🐛 **Segnala un bug**                        | [apri una issue](https://github.com/khanhkit/AgentProxy/issues) — allega l'output di `npm run system-info`                                |
...
| 💚 **Sostieni il progetto**                  | [Modi per sostenere ↑](#-support-agentproxy) · [GitHub Sponsors](https://github.com/sponsors/diegosouzapw)                                    |
...
<table>
  ...
  <tr><td nowrap><b>Link</b></td><td><a href="https://agentproxy.example.com">Sito web</a> · <a href="https://www.npmjs.com/package/agentproxy">npm</a> · <a href="https://github.com/khanhkit/AgentProxy/pkgs/container/agentproxy">Docker Hub</a></td></tr>
</table>
...
<table>
  ...
  <tr><td nowrap><b><a href="../../guides/REMOTE-MODE.md">Modalità remota</a></b></td><td>Gestisci un AgentProxy remoto (VPS) dalla CLI del laptop tramite token di accesso con scope</td></tr>
  <tr><td nowrap><b><a href="../../guides/CLAUDE-CODE-CONFIGURATION.md">Configurazione Claude Code</a></b></td><td>Collega Claude Code a AgentProxy (locale/remoto) con <code>launch</code> + profili per modello</td></tr>
...
<table>
  ...
  <tr><td nowrap><b><a href="../../guides/TERMUX_GUIDE.md">Guida Termux</a></b></td><td>Esegui AgentProxy su Android tramite Termux</td></tr>
...
> AgentProxy è plasmato da una community open source appassionata. Queste persone hanno apportato contributi eccezionali che incidono direttamente su qualità, stabilità e diffusione del progetto. **Grazie.**
...
> 🙏 Funzionalità, bug fix e miglioramenti infrastrutturali di questi contributor sono una **parte fondamentale** di ciò che rende AgentProxy affidabile e ricco di funzionalità. Ogni pull request, ogni caso di test e ogni file di traduzione i18n conta. L'open source è costruito da persone come loro.
...
Un grazie di cuore alle persone che finanziano AgentProxy di tasca propria — ogni contributo aiuta a mantenere il progetto gratuito, indipendente e in evoluzione.
...
<b><a href="https://github.com/sponsors/diegosouzapw">💖 Diventa sponsor →</a></b> — ogni contributo mantiene AgentProxy gratuito e indipendente.
...
<a href="https://www.star-history.com/?repos=diegosouzapw%2FAgentProxy&type=date&legend=top-left">
 <picture>
   ...
   <img alt="Grafico storico delle stelle" src="https://api.star-history.com/chart?repos=khanhkit/AgentProxy&type=date&legend=top-left&sealed_token=XP_ycEjv7s31p1edvhsMOXry51OWYsUjDRWjflSG7jQKRpO9hPGg7i_EHvwhI6QtrARTMH-YGjJhi8sumRYflEJD0DPlH_MMHjizhBYCX8fbHFrHEiNvVA" />
 </picture>
</a>
...
AgentProxy è costruito sulle spalle di giganti. È nato come fork di **[9router](https://github.com/decolua/9router)** e come port TypeScript del progetto Go **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** — da lì, ogni sottosistema qui sotto è stato ispirato da un progetto open source arrivato prima. Ognuno ha influenzato una parte concreta di AgentProxy. Questo è il nostro ringraziamento a tutti loro. 🙏
...
<table>
  <tr><th align="left">Progetto</th><th align="center">⭐</th><th align="left">Come ha ispirato AgentProxy</th></tr>
...
**[⬆ Torna all'inizio](#-agentproxy)** · Realizzato con ❤️ per la community AI open source.

<sub>AgentProxy v3.8.49 · Node ≥22.22.2 · Licenza MIT · <a href="https://agentproxy.example.com">agentproxy.online</a></sub>
