...
AgentProxy include una serie di comandi `setup-*` che configurano una CLI di codifica
(Codex, Claude Code, OpenCode, Cline, …) per utilizzare AgentProxy come backend — in modo che
lo strumento comunichi con **un** endpoint e AgentProxy instradi verso il fornitore giusto con
fallback automatico. Ogni comando legge il catalogo del modello **live** da un AgentProxy in esecuzione
...
C'è anche un launcher generico — `agentproxy run <target>` — che avvia
...
`open-code`, `qwen-code`, `gemini-cli`), e `agentproxy completion` offre le
stesse parole chiave derivate dal manifesto. I launcher legacy per strumento —
`agentproxy launch` (Claude Code) e `agentproxy launch-codex` (Codex) — rimangono
disponibili.
...
- [Modalità Remota](./REMOTE-MODE.md) — controlla un AgentProxy remoto (VPS / Tailnet) dal tuo laptop
...
Ogni comando onora il **contesto attivo** (impostato con `agentproxy connect`, vedi
...
| -------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI                  | `~/.codex/<name>.config.toml` — un profilo per ogni modello di testo compatibile (`codex --profile <name>`)                                                       | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | Entrambi         |
| `agentproxy setup-claude`   | Claude Code                       | `~/.claude/profiles/<name>/settings.json` — un profilo per ogni modello corrispondente (`CLAUDE_CONFIG_DIR`)                                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | Entrambi         |
| `agentproxy setup-opencode` | OpenCode (compatibile con openai) | `~/.config/opencode/opencode.json` — fornitore `agentproxy` con ogni modello del catalogo (`opencode -m agentproxy/<model>`)                                        | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | Entrambi         |
| `agentproxy setup-cline`    | Cline                             | `~/.cline/data/{globalState,secrets}.json` (modalità CLI) + stampa le impostazioni dell'estensione VS Code                                                        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | Entrambi         |
| `agentproxy setup-kilo`     | Kilo Code                         | `~/.local/share/kilo/auth.json` (CLI) + unisce `kilocode.*` in `settings.json` di VS Code se presente                                                             | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | Entrambi         |
| `agentproxy setup-continue` | Continue / `cn` CLI               | `~/.continue/config.yaml` — modelli `provider: openai`, chiave tramite `${{ secrets.AGENTPROXY_API_KEY }}`                                                         | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Entrambi         |
| `agentproxy setup-cursor`   | Cursor                            | Niente — stampa i passaggi in-app (la configurazione di Cursor è opaca SQLite)                                                                                    | `--remote` `--api-key` `--only` `--port`                                                                                                   | Entrambi         |
| `agentproxy setup-roo`      | Roo Code                          | `~/.agentproxy/roo-settings.json` (import doc) + imposta `roo-cline.autoImportSettingsPath` se esiste un `settings.json` di VS Code                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | Entrambi         |
| `agentproxy setup-crush`    | Crush                             | `~/.config/crush/crush.json` — fornitore `openai-compat`, chiave tramite `$AGENTPROXY_API_KEY`                                                                     | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | Entrambi         |
| `agentproxy setup-goose`    | Goose                             | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`) + stampa la ricetta dell'ambiente                                                    | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Entrambi         |
| `agentproxy setup-aider`    | Aider                             | `~/.aider.conf.yml` (`openai-api-base` + `model: openai/<id>`) + stampa la ricetta dell'ambiente                                                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | Entrambi         |
| `agentproxy setup-qwen`     | Qwen Code                         | `~/.qwen/settings.json` — array `V4 modelProviders.openai` + `AGENTPROXY_API_KEY` in `~/.qwen/.env`                                                                | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | Entrambi         |
| `agentproxy run <target>`   | Lancio runtime (generico)         | Niente — avvia `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini` con l'ambiente e gli argomenti giusti; Qwen e Gemini usano una home isolata temporanea | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | Entrambi         |
| `agentproxy launch`         | Claude Code                       | Niente — avvia `claude` con `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` iniettati                                                                                 | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | Entrambi         |
| `agentproxy launch-codex`   | OpenAI Codex CLI                  | Niente — avvia `codex` con il fornitore `agentproxy` iniettato tramite flag `-c`                                                                                   | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | Entrambi         |

Note sui flag (verificati nella sorgente del comando):

- `--remote <url>` — recupera il catalogo da un AgentProxy remoto (sovrascrive `--port`
  e il contesto attivo). `--api-key <key>` fornisce la credenziale per quel
  server (di default alla variabile d'ambiente `AGENTPROXY_API_KEY`, o al token del contesto attivo).
...
- `--model <id>` su `agentproxy run` segue il wiring per target del manifesto
  (`bin/cli/cli-manifest.mjs`): **aider** riceve `--model openai/<id>` e
  **opencode** `--model agentproxy/<id>` (il prefisso viene aggiunto solo quando l'id
  ...
  **codex** tramite argomenti `-c model_providers.agentproxy.*`. **Qwen è l'unico target di esecuzione
  che richiede assolutamente `--model`** — `agentproxy run qwen` senza di esso esce
  `2` con un errore esplicito.
- `--port <port>` — porta locale di AgentProxy (default `20128`, ignorato quando `--remote`
  è impostato). Presente su tutti i comandi `setup-*` e su entrambi i launcher.
- Codici di uscita di `agentproxy run`: il codice di uscita della CLI figlia è propagato
...
> C'è anche un'integrazione plugin più ricca — `agentproxy setup opencode` — che
> installa `@agentproxy/opencode-plugin`. Sono comandi diversi; la tabella
...
Con AgentProxy in esecuzione su `localhost:20128`, esegui semplicemente il comando di configurazione per il tuo strumento. Il catalogo viene recuperato dal server locale.
...
export AGENTPROXY_API_KEY=sk-...  # referenziato tramite {env:AGENTPROXY_API_KEY}, mai su disco
...
```bash
agentproxy launch                 # Claude Code → AgentProxy locale
agentproxy launch-codex           # Codex CLI → AgentProxy locale
...
Punta qualsiasi comando di configurazione a un AgentProxy remoto con `--remote` + `--api-key`. Il catalogo viene recuperato da remoto; la configurazione viene scritta sulla tua macchina locale.
...
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # esporta prima AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # genera un token limitato, memorizza il contesto
agentproxy setup-codex                 # ← ora utilizza il catalogo remoto
agentproxy setup-opencode              # ← stesso
agentproxy launch                      # ← Claude Code contro il remoto
```
...
AgentProxy espone la superficie OpenAI a `/v1`, la superficie Anthropic alla radice, e una superficie nativa Gemini a `/v1beta`. Ogni integrazione è collegata alla forma che il suo strumento si aspetta (verificato nella sorgente del comando):
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | con `/v1`           | Sì                                           |
...
Quando aggiorni con `agentproxy update` (dopo aver confermato, o con `--apply`),
AgentProxy esegue l'installazione con `--include=optional` integrato:

```bash
npm install -g agentproxy@latest --include=optional
```

Questo **non** è un flag che passi a `agentproxy update` — viene sempre applicato dall'
...
# [DRY RUN] Eseguirebbe: npm install -g agentproxy@latest --include=optional
```

Altri flag di `agentproxy update` (verificati nel sorgente): `--check` (esci con 1 se
obsoleto), `--apply` (installa senza chiedere), `--changelog`, `--no-backup`,
`--yes`.

---

## Google Gemini CLI tramite `agentproxy run gemini`
...
Gemini di AgentProxy (`/v1beta`). `agentproxy run gemini` collega automaticamente:

- `GOOGLE_GEMINI_BASE_URL` → l'URL base attivo di AgentProxy (root, senza `/v1`);
- `GEMINI_API_KEY` → le credenziali risolte di AgentProxy (opzione/env/context);
- un **`GEMINI_CLI_HOME`** isolato temporaneamente il cui `.gemini/settings.json`
  ...
  non sovrascrive mai il lancio diretto da AgentProxy — rimosso dopo l'uscita;
...
di AgentProxy, esiste un harness opt-in in
...
Opzionale: `AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` restringe la verifica;
`AGENTPROXY_SMOKE_TIMEOUT_MS` sovrascrive il timeout di 120s per target.
...
- [Configurazione di Codex CLI](./CODEX-CLI-CONFIGURATION.md) — la configurazione di base `[model_providers.agentproxy]` una tantum
