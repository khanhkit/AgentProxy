...
title: "CLI 整合 — 將任何編碼 CLI 指向 AgentProxy"
version: 3.8.50
...
AgentProxy 提供一系列 `setup-*` 命令，用於配置編碼 CLI（Codex、Claude Code、OpenCode、Cline 等）以使用 AgentProxy 作為其後端 — 這樣工具只需與 **一個** 端點通信，AgentProxy 會自動將請求路由到正確的提供者並進行自動回退。每個命令都從運行中的 AgentProxy（本地或遠程）讀取 **實時** 模型目錄，並在 **你的** 機器上寫入工具自己的配置文件。API 密鑰在工具支持的地方通過環境變量引用。持久化工具本地環境文件的命令如下所示。

還有一個通用啟動器 — `agentproxy run <target>` — 它會啟動 `claude`、`codex`、`aider`、`goose`、`opencode`、`qwen` 或 `gemini`，並注入正確的環境，而無需寫入任何配置。目標及其別名來自於標準清單 `bin/cli/cli-manifest.mjs`（`claude-code|cc|anthropic`、`codex-cli|openai-codex|openai`、`goose-cli`、`open-code`、`qwen-code`、`gemini-cli`），而 `agentproxy completion` 提供相同的基於清單的目標詞。舊版每個工具的啟動器 — `agentproxy launch`（Claude Code）和 `agentproxy launch-codex`（Codex） — 仍然可用。
...
- [遠程模式](./REMOTE-MODE.md) — 從你的筆記本電腦驅動遠程 AgentProxy（VPS / Tailnet）
...
每個命令都遵循 **活動上下文**（通過 `agentproxy connect` 設置，請參見 [遠程模式](./REMOTE-MODE.md)）或明確的 `--remote <url> --api-key <key>` 標誌。下面的 "本地與遠程" 意味著：不帶標誌時，它的目標是 `http://localhost:20128`；帶有 `--remote`（或活動的遠程上下文）時，它從該服務器獲取目錄並在本地寫入配置。

| 命令                       | 工具                    | 寫入內容                                                                                                                           | 主要標誌                                                                                                                                   | 本地與遠程 |
| -------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| `agentproxy setup-codex`    | OpenAI Codex CLI        | `~/.codex/<name>.config.toml` — 每個兼容文本模型的一個配置文件（`codex --profile <name>`）                                         | `--remote` `--api-key` `--only` `--dry-run` `--port` `--codex-home`                                                                        | 兩者       |
| `agentproxy setup-claude`   | Claude Code             | `~/.claude/profiles/<name>/settings.json` — 每個匹配模型的一個配置文件（`CLAUDE_CONFIG_DIR`）                                      | `--remote` `--api-key` `--only` `--dry-run` `--port` `--claude-home`                                                                       | 兩者       |
| `agentproxy setup-opencode` | OpenCode（兼容 openai） | `~/.config/opencode/opencode.json` — 包含每個目錄模型的 `agentproxy` 提供者（`opencode -m agentproxy/<model>`）                      | `--remote` `--api-key` `--only` `--model` `--dry-run` `--port`                                                                             | 兩者       |
| `agentproxy setup-cline`    | Cline                   | `~/.cline/data/{globalState,secrets}.json`（CLI 模式） + 打印 VS Code 擴展設置                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--cline-dir`                                                                | 兩者       |
| `agentproxy setup-kilo`     | Kilo Code               | `~/.local/share/kilo/auth.json`（CLI） + 如果存在，將 `kilocode.*` 合併到 VS Code 的 `settings.json`                               | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--auth-path` `--vscode-settings`                                            | 兩者       |
| `agentproxy setup-continue` | Continue / `cn` CLI     | `~/.continue/config.yaml` — `provider: openai` 模型，密鑰通過 `${{ secrets.AGENTPROXY_API_KEY }}`                                   | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | 兩者       |
| `agentproxy setup-cursor`   | Cursor                  | 無 — 打印應用內步驟（Cursor 配置是模糊的 SQLite）                                                                                  | `--remote` `--api-key` `--only` `--port`                                                                                                   | 兩者       |
| `agentproxy setup-roo`      | Roo Code                | `~/.agentproxy/roo-settings.json`（導入文件） + 如果存在 VS Code 的 `settings.json`，設置 `roo-cline.autoImportSettingsPath`        | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--import-path` `--vscode-settings`                                          | 兩者       |
| `agentproxy setup-crush`    | Crush                   | `~/.config/crush/crush.json` — `openai-compat` 提供者，密鑰通過 `$AGENTPROXY_API_KEY`                                               | `--remote` `--api-key` `--only` `--dry-run` `--port` `--config-path`                                                                       | 兩者       |
| `agentproxy setup-goose`    | Goose                   | `~/.config/goose/config.yaml`（`GOOSE_PROVIDER`/`OPENAI_HOST`/`GOOSE_MODEL`） + 打印環境配方                                       | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | 兩者       |
| `agentproxy setup-aider`    | Aider                   | `~/.aider.conf.yml`（`openai-api-base` + `model: openai/<id>`） + 打印環境配方                                                     | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path`                                                              | 兩者       |
| `agentproxy setup-qwen`     | Qwen Code               | `~/.qwen/settings.json` — V4 `modelProviders.openai` 陣列 + `AGENTPROXY_API_KEY` 在 `~/.qwen/.env`                                  | `--remote` `--api-key` `--model` `--yes` `--dry-run` `--port` `--config-path` `--env-path`                                                 | 兩者       |
| `agentproxy run <target>`   | 運行時啟動（通用）      | 無 — 啟動 `claude`/`codex`/`aider`/`goose`/`opencode`/`qwen`/`gemini`，並帶有正確的環境和參數；Qwen 和 Gemini 使用臨時隔離的主目錄 | `--remote` `--base-url` `--context` `--provider` `--model` `--api-key` `--api-key-env` `--dry-run` `--json` `--port` `--profile` `--token` | 兩者       |
| `agentproxy launch`         | Claude Code             | 無 — 啟動 `claude`，並注入 `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`                                                             | `--remote` `--api-key` `--token` `--profile` `--port`                                                                                      | 兩者       |
| `agentproxy launch-codex`   | OpenAI Codex CLI        | 無 — 啟動 `codex`，並通過 `-c` 標誌注入 `agentproxy` 提供者                                                                         | `--remote` `--api-key` `--profile` (`-p`) `--port`                                                                                         | 兩者       |

有關標誌的說明（在命令源中已驗證）：

- `--remote <url>` — 從遠程 AgentProxy 獲取目錄（覆蓋 `--port` 和活動上下文）。`--api-key <key>` 提供該服務器的憑據（預設為 `AGENTPROXY_API_KEY` 環境變量，或活動上下文的令牌）。
...
- `--model <id>` 在 `agentproxy run` 上遵循清單的每個目標接線（`bin/cli/cli-manifest.mjs`）：**aider** 接收 `--model openai/<id>`，**opencode** 接收 `--model agentproxy/<id>`（前綴僅在 ID 不包含時添加）；**qwen** 和 **gemini** 直接接收 ID；**claude** 通過 `ANTHROPIC_MODEL` 獲得，**goose** 通過 `GOOSE_MODEL` 獲得，**codex** 通過 `-c model_providers.agentproxy.*` 參數獲得。**Qwen 是唯一一個強制要求 `--model` 的運行目標** — `agentproxy run qwen` 如果沒有它將以明確錯誤退出 `2`。
- `--port <port>` — 本地 AgentProxy 端口（預設為 `20128`，設置 `--remote` 時忽略）。在所有 `setup-*` 和兩個啟動器上均存在。
- `agentproxy run` 退出代碼：子 CLI 的自身退出代碼被逐字傳遞；`2` = 無效參數（不支持的目標，缺少必需的 `--model`，容器保護）；`127` = 目標二進制文件不在 `PATH` 中；`130`/`143`/`129` 當啟動被 `SIGINT`/`SIGTERM`/`SIGHUP` 終止時；`1` = 其他運行時啟動失敗。
...
> 還有一個更豐富的插件整合 — `agentproxy setup opencode` — 它安裝 `@agentproxy/opencode-plugin`。這是不同的命令；上面的表格記錄了 `setup-opencode`。

---

## 本地使用

在 `localhost:20128` 上運行 AgentProxy，只需為您的工具運行設置命令。目錄是從本地服務器獲取的。
...
export AGENTPROXY_API_KEY=sk-...  # 通過 {env:AGENTPROXY_API_KEY} 引用，永遠不會寫入磁碟
...
```bash
agentproxy launch                 # Claude Code → 本地 AgentProxy
agentproxy launch-codex           # Codex CLI → 本地 AgentProxy
...
將任何設置命令指向遠程 AgentProxy，使用 `--remote` + `--api-key`。目錄是從遠程獲取的；配置寫入您的本地機器。

```bash
# OpenCode 對遠程 VPS，僅保留 glm/kimi 模型
agentproxy setup-opencode --remote http://192.168.0.15:20128 --api-key oma_live_xxx \
  --only glm,kimi
opencode -m agentproxy/glm/glm-5.2 "..."   # 首先導出 AGENTPROXY_API_KEY
...
```bash
agentproxy connect 192.168.0.15        # 創建一個範圍令牌，存儲上下文
agentproxy setup-codex                 # ← 現在使用遠程目錄
agentproxy setup-opencode              # ← 同上
agentproxy launch                      # ← Claude Code 對遠程
```
...
AgentProxy 在 `/v1` 上公開 OpenAI 接口，在根目錄上公開 Anthropic 接口，並在 `/v1beta` 上公開原生 Gemini 接口。每個集成都連接到其工具所期望的形式（在命令源中驗證）：
...
| `setup-codex`, `launch-codex` (`model_providers.agentproxy.base_url`)       | 帶 `/v1`       | 是                                       |
...
當你使用 `agentproxy update` 更新時（在確認後，或使用 `--apply`），
AgentProxy 會自動執行帶有 `--include=optional` 的安裝：

```bash
npm install -g agentproxy@latest --include=optional
```

這**不是**你傳遞給 `agentproxy update` 的標誌 — 它始終由更新器應用。這保證了 `optionalDependencies`（`better-sqlite3`、`keytar`、`tls-client`、LLMLingua SLM 堆疊）在更新過程中存活，即使你的 npm 配置設置了 `omit=optional`，這樣會默默地刪除原生 SQLite 驅動程序和 OS-keyring 綁定。要預覽確切的命令而不應用：

```bash
agentproxy update --dry-run
# [DRY RUN] 會運行： npm install -g agentproxy@latest --include=optional
```

其他 `agentproxy update` 標誌（在源代碼中驗證）： `--check`（如果過時則退出 1）、`--apply`（無提示安裝）、`--changelog`、`--no-backup`、`--yes`。

---

## 通過 `agentproxy run gemini` 使用 Google Gemini CLI

合約已針對 `@google/gemini-cli` 0.50.0 進行驗證：該 CLI 尊重
`GOOGLE_GEMINI_BASE_URL` 並對其發出 `POST /v1beta/models/<model>:generateContent`
（和 `:streamGenerateContent?alt=sse`）— 完全符合 AgentProxy 的原生
Gemini 接口（`/v1beta`）。`agentproxy run gemini` 自動連接這些：

- `GOOGLE_GEMINI_BASE_URL` → 當前的 AgentProxy 基本 URL（根，不帶 `/v1`）；
- `GEMINI_API_KEY` → 解決的 AgentProxy 憑證（選項/環境/上下文）；
- 一個**臨時隔離的 `GEMINI_CLI_HOME`**，其 `.gemini/settings.json`
  選擇 `gemini-api-key` 認證，因此存儲的 Google OAuth 會話（代碼助手）
  永遠不會覆蓋 AgentProxy 指導的啟動 — 退出後刪除；
...
可選：`AGENTPROXY_SMOKE_TARGETS="codex,opencode,qwen"` 限制掃描；
`AGENTPROXY_SMOKE_TIMEOUT_MS` 覆蓋每個目標的 120 秒超時。
...
- [Codex CLI 配置](./CODEX-CLI-CONFIGURATION.md) — 一次性的 `[model_providers.agentproxy]` 基本設置
