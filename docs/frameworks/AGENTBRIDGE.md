...
AgentBridge is AgentProxy's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through AgentProxy's unified routing engine. It supports **10 IDE agents** — Antigravity, Kiro, GitHub Copilot, GHE Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making AgentProxy the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Reroute any agent to any provider**: Copilot talking to OpenAI? Redirect it to Anthropic Claude, Gemini, or any of AgentProxy's 352 providers.
...
- **Apply AgentProxy resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.
...
> into the same `agentproxy-mitm.crt` trust-store slot the old leaf used
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to AgentProxy's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — AgentProxy never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "AgentProxy AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
2. Check AgentProxy server logs: `APP_LOG_LEVEL=debug` in `.env`
3. Verify `AGENTPROXY_BASE_URL` points to the correct router endpoint (default: `http://127.0.0.1:20128`)
