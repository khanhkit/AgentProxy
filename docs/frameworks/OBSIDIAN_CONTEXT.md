...
AgentProxy connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points AgentProxy at a
...
per call (per-API-key first, then global). Tools that hit the AgentProxy **sync server**
(the four `obsidian_sync_*` tools) additionally require the sync auth token configured
in AgentProxy settings.
...
| `obsidian_sync_status`       | AgentProxy sync server status: running, vault name, port, uptime, last sync.          |
...
`open-sse/mcp-server/server.ts`, gated on `AGENTPROXY_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `AGENTPROXY_MCP_SCOPES` or the API key's scope context. See
