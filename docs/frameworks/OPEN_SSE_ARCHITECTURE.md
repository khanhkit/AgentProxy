...
> **TL;DR**: `open-sse/` is the core streaming engine that powers every LLM request in AgentProxy. It contains ~1,400 files implementing the request pipeline, executors, services, MCP server, and translation layer. This guide explains how the pieces fit together.
...
`open-sse/` is a **standalone workspace** in the AgentProxy monorepo for several reasons:

1. **Reusability** — `open-sse` is published as `@agentproxy/open-sse` on npm, so other projects can use it independently
2. **Clean boundaries** — the streaming engine is decoupled from the AgentProxy-specific UI/DB layer
...
The Responses API is OpenAI's new format with **stateful conversations** (`previous_response_id`). When a client sends a Responses request, AgentProxy:
