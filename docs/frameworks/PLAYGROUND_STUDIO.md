...
from the current `PlaygroundState`. API key placeholder is always `$AGENTPROXY_API_KEY` (D11).
...
| Export code shows `MISSING_API_KEY`    | Placeholder not inserted      | `codeExport.ts` always uses `API_KEY_PLACEHOLDER = "$AGENTPROXY_API_KEY"`        |
