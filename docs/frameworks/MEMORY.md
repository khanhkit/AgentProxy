...
AgentProxy provides persistent conversational memory keyed by API key (and
...
> A client can opt a single request out with the `x-agentproxy-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embed the query, search the
  collection filtered by `kind = "agentproxy_memory"` and optionally by
...
`qdrantApiKey`, `qdrantCollection` default `"agentproxy_memory"`,
...
- `agentproxy_memory_clear` — `{apiKeyId, type?, olderThan?}` → lists matching
...
- For hard deletion, use `DELETE /api/memory/[id]` or `agentproxy_memory_clear`.
...
AgentProxy's memory engine supports **four embedding sources** (`src/lib/memory/embedding/`). Each has different trade-offs in **latency, cost, model quality, and setup complexity**.
