...
AgentProxy can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through AgentProxy (with retry, timeout, and error classification)
...
> AgentProxy to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
AgentProxy saves the token and immediately runs a 1-result test search; if Notion
...
`open-sse/mcp-server/server.ts` only when `AGENTPROXY_MCP_ENFORCE_SCOPES=true`; the
caller's allowed scopes come from `AGENTPROXY_MCP_SCOPES` (comma-separated) or the
