...
> (typed registry — code lands with [#5868](https://github.com/khanhkit/AgentProxy/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the AgentProxy Node process (the native TS pipeline).
   - `supervised` — a local child process AgentProxy installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint AgentProxy dispatches to but does **not** manage
...
[#5868](https://github.com/khanhkit/AgentProxy/pull/5868)) declares every engine once, with its
...
[#5817](https://github.com/khanhkit/AgentProxy/pull/5817); until it merges,
