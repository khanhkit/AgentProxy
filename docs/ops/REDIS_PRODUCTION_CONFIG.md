...
Redis is an **optional, soft dependency** in AgentProxy — the application degrades gracefully (in-memory
...
All three workloads share one namespace prefix so AgentProxy can co-exist with other apps on a
...
| `REDIS_KEY_PREFIX` env var | `agentproxy:` (default) | `rateLimiter.ts`, `redisQuotaStore.ts`, `.env.example` |
...
AgentProxy shares a Redis instance with whatever else runs on the host. Without a namespace,
...
Set `REDIS_KEY_PREFIX` to a non-empty string to prefix **every** AgentProxy key:

```bash
# .env — all AgentProxy keys become agentproxy:rl:*, agentproxy:auth:*, agentproxy:quota:*
REDIS_KEY_PREFIX=agentproxy:
```

- **Default:** `agentproxy:` (applied when `REDIS_KEY_PREFIX` is unset or blank).
...
# Persistence (optional — AgentProxy is crash‑safe without it)
