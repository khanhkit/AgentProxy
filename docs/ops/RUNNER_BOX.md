...
| runners   | **6 listeners**: 4 AgentProxy (1 `omni-build` + 1 `omni-release`-only + 2 `omni-light`) + OmniHeuris + OmniMind | all share the memory above; `agentproxy-113-3/-4/-7/-8` are disabled (`systemctl enable --now` brings one back)                                |

## Install the janitor (one-time, on the box)

```bash
scp scripts/ops/runner-janitor.sh root@192.168.0.113:/opt/agentproxy-ops/runner-janitor.sh
ssh root@192.168.0.113 'chmod +x /opt/agentproxy-ops/runner-janitor.sh; apt-get install -y lsof'
# cron (root): every 30 min, log to /var/log/runner-janitor.log
*/30 * * * * MAX_ACTIVE_RUNNERS=6 /opt/agentproxy-ops/runner-janitor.sh >> /var/log/runner-janitor.log 2>&1
```
...
What it does every run: sweeps our own leftovers (`runner-*`, `agentproxy-*`,
...
agentproxy/other breakdown). Exit 1 = attention needed; read the log.
...
CPU with no job attached. Every AgentProxy unit carries a drop-in
(`/etc/systemd/system/actions.runner.diegosouzapw-AgentProxy.<name>.service.d/10-killmode.conf`)
...
- **Heavy-build ceiling: ONE at a time — enforced by label (since 2026-08-29).** Every job
  ...
  **`agentproxy-113-5`** carries that label (added through the runners API — no
  ...
  must be on `agentproxy-113-5` (`docker info` is the first step of the publish
  ...
  hosted-runner OOM). `agentproxy-113-6` keeps `omni-release` only. Heavy builds from
  ...
  `gh api -X POST repos/<repo>/actions/runners/<id of agentproxy-113-6>/labels -f 'labels[]=omni-build'`.
- **Light pool: `omni-light` (2026-08-29, #11965).** `agentproxy-113` and `agentproxy-113-2` carry
...
- **Fewer listeners on purpose.** Four AgentProxy units were disabled on 2026-08-29 — with only
