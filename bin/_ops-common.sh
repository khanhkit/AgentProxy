# bin/_ops-common.sh — shared helpers for the AgentProxy ops runbook scripts.
#
...
ops_set_data_dir() {
  AGENTPROXY_DATA_DIR="$1"
  AGENTPROXY_SQLITE="${AGENTPROXY_DATA_DIR}/storage.sqlite"
  AGENTPROXY_BACKUPS_DIR="${DB_BACKUPS_DIR:-${AGENTPROXY_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.agentproxy}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$AGENTPROXY_BACKUPS_DIR/$id" \
    "$AGENTPROXY_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$AGENTPROXY_BACKUPS_DIR" ]; then
    for cand in "$AGENTPROXY_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $AGENTPROXY_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
