 ...
 *
 * Problem (#7791): when AgentProxy is installed via `npm i -g agentproxy`, the
 * package files live under `node_modules/agentproxy/`. tsx's tsconfig-path
 * resolution does not apply there, so specifiers like `@/shared/utils/featureFlags`
 * (declared in tsconfig.json `paths` as `@/* → ./src/*`) or
 * `@agentproxy/open-sse/services/usage` fail with `ERR_MODULE_NOT_FOUND`.
 ...
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@agentproxy/open-sse/"`)
 ...
 *             prefix *without* a trailing slash (e.g. `@agentproxy/open-sse` →
...
export const ALIAS_MAP = [
  ...
  { prefix: "@agentproxy/open-sse/", target: "open-sse", exact: false },
  { prefix: "@agentproxy/open-sse", target: "open-sse", exact: true },
];
 ...
 *   "@/..."                     → <root>/src/...
 *   "@agentproxy/open-sse/..."   → <root>/open-sse/...
 *   "@agentproxy/open-sse"       → <root>/open-sse/index.*
 *
...
export function resolveAlias(specifier, root) {
  ...
  // Exact match (e.g. `@agentproxy/open-sse` with no trailing path) →
