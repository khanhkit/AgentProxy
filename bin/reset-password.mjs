 ...
 *   agentproxy reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | agentproxy reset-password
 *   printf 'NewPass123' | agentproxy reset-password --password-stdin
 *
 * Resets the admin password for AgentProxy.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure AgentProxy has been started at least once.`);
  ...
  console.log("   Restart AgentProxy for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/agentproxy.mjs
    // routes `agentproxy reset-password` here) terminates cleanly instead of
