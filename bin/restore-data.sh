#!/usr/bin/env bash
# bin/restore-data.sh — restore the AgentProxy SQLite data volume from a snapshot
...
Stop AgentProxy before running, and restart it afterwards.
EOF
}
...
ops_log "restore source: $snap → $AGENTPROXY_DATA_DIR"
ops_confirm "Overwrite storage.sqlite at $AGENTPROXY_DATA_DIR from $snap?" || ops_die "aborted"

# Pre-restore safety copy of the live data.
if [ -f "$AGENTPROXY_SQLITE" ]; then
  safety="$AGENTPROXY_BACKUPS_DIR/pre-restore_$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$safety"
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$AGENTPROXY_SQLITE" "VACUUM INTO '$safety/storage.sqlite'" \
      || cp -a "$AGENTPROXY_SQLITE" "$safety/storage.sqlite"
  else
    cp -a "$AGENTPROXY_SQLITE" "$safety/storage.sqlite"
  fi
...
mkdir -p "$AGENTPROXY_DATA_DIR"
# Drop stale WAL/SHM so the restored DB is authoritative, then copy in.
rm -f "$AGENTPROXY_SQLITE" "${AGENTPROXY_SQLITE}-wal" "${AGENTPROXY_SQLITE}-shm"
cp -a "$snap/storage.sqlite" "$AGENTPROXY_SQLITE"

# Restore sibling DBs captured in the snapshot.
for f in "$snap"/*.sqlite; do
  [ -e "$f" ] || continue
  [ "$(basename "$f")" = "storage.sqlite" ] && continue
  cp -a "$f" "$AGENTPROXY_DATA_DIR/"
done

ops_log "restore complete — restart AgentProxy to pick up the restored data"
