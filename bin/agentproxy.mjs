...
// ahead of expensive self-heal hooks), adapted to AgentProxy's Commander CLI where the
...
// #7791: tsx's tsconfig-path resolution does not apply when AgentProxy is
// installed globally (files live under node_modules/agentproxy/), so bare
...
function loadEnvFile() {
  ...
  if (process.env.AGENTPROXY_CLI_SKIP_REPO_ENV !== "1") {
...
// Generate STORAGE_ENCRYPTION_KEY if not set (persisted to ~/.agentproxy/.env)
...
// key or write ~/.agentproxy/.env — running a read-only command should never
// mutate the data dir.
if (shouldProvisionStorageKey(process.argv)) {
  ...
  if (!process.env.STORAGE_ENCRYPTION_KEY) {
    ...
    // Writing only to ~/.agentproxy (the container home, not a volume) silently lost the key on
    // container recreation, leaving the persisted encrypted DB undecryptable (regression of #1622).
    const dataDir = process.env.DATA_DIR || join(homedir(), ".agentproxy");
    const envPath = join(dataDir, ".env");
...
// Apply --lang before Commander parses (program descriptions call t() during setup)
{
  ...
  const langEnv = process.env.AGENTPROXY_LANG;
...
process.on("exit", () => {
  ...
  if (process.env.AGENTPROXY_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g agentproxy` or `agentproxy update --apply`",
    });
  }
});
