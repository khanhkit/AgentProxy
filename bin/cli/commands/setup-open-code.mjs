/**
 * agentproxy setup opencode — Wire the bundled @agentproxy/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g agentproxy` ships the plugin
 * inside the agentproxy package (`@agentproxy/opencode-plugin/dist/`) but
 ...
 *   3. Copies the built plugin into `<opencode>/plugins/agentproxy/`.
 *   4. Creates or updates `opencode.json` with a single `plugin` entry
 *      pointing at the local copy (so OC ≥1.15 picks it up).
 *   5. Optionally runs `opencode auth login --provider agentproxy`
...
// We walk up from this file to find the agentproxy package root. The script
// lives at `<agentproxy>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@agentproxy/opencode-plugin/
// (see root package.json `files`: ["@agentproxy/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.AGENTPROXY_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@agentproxy", "opencode-plugin");
...
/**
 * Locate the bundled @agentproxy/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published agentproxy tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @agentproxy/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means agentproxy was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling agentproxy (npm install -g agentproxy) ` +
        `or run \`cd @agentproxy/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@agentproxy/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/agentproxy/`. We
 ...
 * resolve the peer deps from the agentproxy package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "agentproxy");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`agentproxy setup opencode\`.`
      );
    }
  }
  ...
  const entry = [
    `./plugins/agentproxy/dist/index.js`,
    {
      providerId,
      baseURL,
  ...
  // drop a legacy `opencode-agentproxy-auth` entry if present — that
  // package is the obsolete predecessor of @agentproxy/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-agentproxy-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-agentproxy-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * The bundled @agentproxy/opencode-plugin registers its provider under
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="agentproxy"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "agentproxy";
  ...
  printHeading("AgentProxy → OpenCode Plugin Setup");
...
/**
 * Register the `agentproxy setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `agentproxy setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @agentproxy/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: agentproxy)",
      "agentproxy"
    )
    .option(
      "--base-url <url>",
      "AgentProxy base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote AgentProxy URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
