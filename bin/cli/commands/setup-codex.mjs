/**
 * agentproxy setup-codex — Remote-aware Codex CLI profile generator.
 *
 * Connects to a running AgentProxy instance (local or remote VPS), fetches the
 ...
 *   agentproxy setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  printHeading(`AgentProxy → Codex CLI profile generator`);
...
export function registerSetupCodex(program) {
  program
    ...
    .option(
      "--remote <url>",
      "Remote AgentProxy URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "AgentProxy API key for the remote instance (defaults to AGENTPROXY_API_KEY env var)"
    )
