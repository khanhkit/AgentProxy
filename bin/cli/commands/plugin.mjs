...
export function registerPlugin(program) {
  const plugin = program
    .command("plugin")
    .description(t("plugin.description") || "Manage CLI plugins (agentproxy-cmd-*)");
  ...
  plugin
    ...
    .action(async (opts, cmd) => {
      ...
      if (plugins.length === 0) {
        ...
        process.stdout.write(`Install: agentproxy plugin install <name>\n`);
      }
    });
  ...
  plugin
    ...
    .action(async (name, opts) => {
      ...
      const pkgName = isLocal ? name : `agentproxy-cmd-${name}`;

      if (!opts.yes) {
        process.stderr.write(
          `⚠ WARNING: Plugins run with the same privileges as agentproxy CLI.\n` +
  ...
  plugin
    ...
    .action(async (name, opts) => {
      const pkgName = name.startsWith("agentproxy-cmd-") ? name : `agentproxy-cmd-${name}`;
      if (!opts.yes) {
  ...
  plugin
    ...
    .action(async (name, opts, cmd) => {
      const plugins = await discoverPlugins();
      const p = plugins.find((x) => x.name === name || x.name === `agentproxy-cmd-${name}`);
      if (!p) {
  ...
  plugin
    ...
    .action(async (query) => {
      const q = query ? `agentproxy-cmd-${query}` : "agentproxy-cmd";
      ...
      try {
        ...
        if (rows.length === 0) {
          process.stdout.write(`No plugins found for '${query || "agentproxy-cmd"}'.\n`);
        } else {
          rows.forEach((r) =>
  ...
  plugin
    ...
    .action(async (name) => {
      ...
      const dir = join(process.cwd(), `agentproxy-cmd-${safeName}`);
      if (existsSync(dir)) {
      ...
      writeFileSync(
        ...
        JSON.stringify(
          {
            name: `agentproxy-cmd-${safeName}`,
            version: "0.1.0",
            type: "module",
            main: "index.mjs",
            description: `AgentProxy CLI plugin: ${safeName}`,
      ...
      writeFileSync(
        join(dir, "README.md"),
        `# agentproxy-cmd-${safeName}\n\nAn AgentProxy CLI plugin.\n\n## Install\n\n\`\`\`bash\nagentproxy plugin install ${safeName}\n\`\`\`\n`
      );
      process.stdout.write(`✓ Scaffolded: ${dir}\n`);
      process.stdout.write(`  Run: cd ${dir} && agentproxy plugin install .\n`);
    });
}
