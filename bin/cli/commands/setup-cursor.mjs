/**
 * agentproxy setup-cursor — guide Cursor to use AgentProxy.
 *
...
export function buildCursorInstructions({ apiBase, models }) {
  const lines = [
    ...
    "  3. Set the OpenAI API Key to your AgentProxy key (AGENTPROXY_API_KEY)",
...
export async function runSetupCursorCommand(opts = {}) {
  ...
  printHeading("AgentProxy → Cursor");
  ...
  if (await isContainerRuntime()) {
    printInfo(
      "Note: this ran inside a container, so the base URL above is the container's own view. " +
        "Use the address the host reaches AgentProxy on (e.g. the published port) in Cursor's settings."
    );
  }
  return 0;
}
...
export function registerSetupCursor(program) {
  program
    ...
    .description(
      "Print the steps to point Cursor at AgentProxy (chat panel; Cursor config is not file-writable)"
    )
