/**
 * agentproxy setup-continue — configure Continue (continue.dev) for AgentProxy.
 *
 ...
 * `apiKey: ${{ secrets.AGENTPROXY_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.AGENTPROXY_API_KEY }}";
...
/**
 * Merge AgentProxy models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "AgentProxy Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("AgentProxy → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} AgentProxy model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} AgentProxy models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export AGENTPROXY_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'AGENTPROXY_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the AgentProxy model catalog"
    )
