/**
 * agentproxy setup-claude — Remote-aware Claude Code profile generator.
 *
 ...
 * fetches the live /v1/models catalog from a (possibly remote) AgentProxy and
 ...
 *
 * Launch a profile with:  agentproxy launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `agentproxy
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  try {
    ...
    printInfo(
      "Make sure AgentProxy is running and the --remote URL is correct.\n" +
        "You may also need --api-key if AgentProxy requires authentication."
    );
    return 1;
  }
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log("  agentproxy launch --profile <name>     # e.g. agentproxy launch --profile glm52");
    console.log(
...
export function registerSetupClaude(program) {
  program
    ...
    .description(
      "Fetch the live model catalog from AgentProxy (local or remote VPS) and generate " +
