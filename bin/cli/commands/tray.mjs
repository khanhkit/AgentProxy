...
export function registerTray(program) {
  ...
  cmd
    ...
    .action(() => {
      process.stderr.write(
        "The tray is managed by `agentproxy serve --tray`. Start the server with --tray to enable it.\n"
      );
    });
  ...
  cmd
    .command("quit")
    .description(t("tray.quit") || "Quit AgentProxy via tray")
    .action(async () => {
      const { default: pidUtils } = await import("../utils/pid.mjs").catch(() => ({
        default: null,
      }));
      process.stderr.write("Use `agentproxy stop` to stop the server.\n");
      process.exit(0);
    });
}
