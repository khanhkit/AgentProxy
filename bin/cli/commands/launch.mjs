...
/**
 * Build a clean child env for Claude Code pointed at AgentProxy.
 *
...
export function buildClaudeEnv(baseEnv, baseUrlOrPort, authToken, opts = {}) {
  ...
  // from stopping at its local login gate before it ever contacts AgentProxy (an
  ...
  env.ANTHROPIC_AUTH_TOKEN = (authToken && String(authToken).trim()) || "agentproxy-no-auth";
...
/**
 * Resolve the AgentProxy base URL + auth for launch, honouring (in order):
...
export function resolveLaunchTarget(opts = {}) {
  ...
  if (!authToken) authToken = process.env.ANTHROPIC_AUTH_TOKEN ?? process.env.AGENTPROXY_API_KEY;
...
export function registerLaunch(program) {
  program
    .command("launch")
    .description(
      t("launch.description") || "Launch Claude Code pointed at AgentProxy (local or remote)"
    )
    .option("--port <port>", t("serve.port") || "Proxy port", "20128")
    .option("--remote <url>", "Remote AgentProxy base URL (overrides --port and the active context)")
    .option(
      "--profile <name>",
    ...
    .option("--api-key <key>", "Alias for --token (AgentProxy access token / API key)")
