...
/**
 * `agentproxy configure <cli>` — interactive provider+model picker that writes a
 * local CLI config pointed at the ACTIVE AgentProxy context (local or remote).
 *
 * The model catalog comes from the active context's GET /v1/models, so when you
 * are in remote mode (`agentproxy connect ...`) you pick from the remote server's
 ...
 * `AGENTPROXY_API_KEY`, but the older setup modules receive plain options and
...
export function resolveConfigureTargetOptions(opts = {}) {
  const resolved = { ...opts };
  const ambientKey = process.env.AGENTPROXY_API_KEY || "";
...
function preferenceContextName(opts = {}) {
  if (opts.context || process.env.AGENTPROXY_CONTEXT) {
    return String(opts.context || process.env.AGENTPROXY_CONTEXT);
  }
  try {
...
function buildCodexProfile(modelId, ctx) {
  const lines = [
    ...
    `# ${modelId} — generated by 'agentproxy configure codex'`,
    `model                          = "${modelId}"`,
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  const guard = await guardHostConfigTarget(codexHome, {
    toolLabel: "Codex",
    hostCommand: "agentproxy configure codex",
  ...
  printInfo("Prereq: ~/.codex/config.toml must define the [model_providers.agentproxy] block");
