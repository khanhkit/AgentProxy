...
export function registerOneProxy(program) {
  ...
  op.command("status").action(async (opts, cmd) => {
    const data = await mcpCall("agentproxy_oneproxy_stats", {});
  ...
  op.command("fetch")
    ...
    .action(async (opts, cmd) => {
      const data = await mcpCall("agentproxy_oneproxy_fetch", {
        count: opts.count,
        type: opts.type,
      });
  ...
  op.command("rotate")
    ...
    .action(async (opts, cmd) => {
      const data = await mcpCall("agentproxy_oneproxy_rotate", {
        provider: opts.provider,
