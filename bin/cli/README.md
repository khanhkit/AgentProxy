# bin/cli — AgentProxy CLI internals

This directory contains the CLI runtime, helpers, and commands for the `agentproxy` binary.
...
├── sqlite.mjs              ← openAgentProxyDb() — DB bootstrap
...
All HTTP calls to the AgentProxy server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `AGENTPROXY_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `AGENTPROXY_API_KEY`)
...
Locale detection order: `AGENTPROXY_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `AGENTPROXY_LANG` environment variable
...
```bash
agentproxy config lang set pt-BR       # saves to ~/.agentproxy/.env
agentproxy config lang list            # show all 42 available locales
agentproxy config lang get             # show currently active locale
```

**One-time override:**

```bash
agentproxy --lang de providers list    # run in German, not persisted
AGENTPROXY_LANG=ja agentproxy status    # same effect via env
```
