...
export function getBaseUrl(opts = {}) {
  ...
  const envUrl = process.env.AGENTPROXY_BASE_URL;
  if (envUrl) return stripTrailingSlash(envUrl);

  // Resolve from the active context (canonical store + legacy profile fallback).
  // This is what makes "remote mode" work: `agentproxy contexts use <remote>`
...
export async function buildHeaders(opts) {
  ...
  //
  // The active context's scoped token MUST win over the ambient AGENTPROXY_API_KEY:
  // `agentproxy connect <remote>` saves the context's token, but users keep
  // AGENTPROXY_API_KEY in their shell. The global `--api-key` option is bound to
  // that env var (.env("AGENTPROXY_API_KEY")), so commands that spread
  ...
  const ambientKey = process.env.AGENTPROXY_API_KEY || null;
  const explicitKey = opts.apiKey && opts.apiKey !== ambientKey ? opts.apiKey : null;
  let auth = explicitKey;
  if (!auth) {
    try {
      const ctx = await resolveActiveContextAsync(opts.context ?? process.env.AGENTPROXY_CONTEXT);
  ...
  if (!isLoopbackUrl(destinationUrl)) {
    ...
    const cliToken = opts.cliToken ?? process.env.AGENTPROXY_CLI_TOKEN ?? (await getCliToken());
...
export async function apiFetch(path, opts = {}) {
  ...
  const timeout =
    opts.timeout ?? (Number.parseInt(process.env.AGENTPROXY_HTTP_TIMEOUT_MS || "", 10) || 30000);
  const maxAttempts = opts.retry === false ? 1 : (opts.retryMax ?? RETRY_DEFAULTS.maxAttempts);
  const verbose = opts.verbose ?? process.env.AGENTPROXY_VERBOSE === "1";
