 ...
 *
 * Purely informational invocations must NOT create `~/.agentproxy/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `agentproxy --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `agentproxy` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `agentproxy` runs the default `serve` command → must provision.
