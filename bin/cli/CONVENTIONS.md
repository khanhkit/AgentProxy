# AgentProxy CLI — Internal Conventions
...
```
agentproxy keys add openai sk-xxx
agentproxy combo switch fastest
agentproxy memory search "react hooks"
```

**Not allowed**:

```
agentproxy --add-key openai sk-xxx     # ❌ flag-as-verb
agentproxy add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `AGENTPROXY_BASE_URL` env or `~/.agentproxy/config.json`
  (active profile).
- Injects `Authorization: Bearer ${AGENTPROXY_API_KEY}` when available.
- Injects `x-agentproxy-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `AGENTPROXY_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `AGENTPROXY_LANG` to `~/.agentproxy/.env`.
