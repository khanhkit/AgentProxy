...
// prebuilt addon, etc.), every `agentproxy doctor` DB check reported a false FAIL
  ...
  if (message.includes("NODE_MODULE_VERSION") || message.includes("ERR_DLOPEN_FAILED")) {
    return new Error(
      `better-sqlite3 native binding is incompatible with this runtime. ` +
        `Run \`${rebuildCmd}\` in the AgentProxy project and try again. ` +
        `Or run: agentproxy runtime repair  ` +
...
export async function openAgentProxyDb() {
...
export async function readDatabaseHealth(dbPath) {
  return withReadonlySqlite(dbPath, (db) => {
    ...
    const appliedMigrationVersions = hasMigrationTable
      ? db
          .prepare("SELECT version FROM _agentproxy_migrations")
          .all()
          .map((row) => row.version)
      : [];
