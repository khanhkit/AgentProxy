...
AgentProxy has three related but distinct temporary-failure mechanisms. Keep their
...
`AGENTPROXY_PROVIDER_BREAKER_*` and `AGENTPROXY_CIRCUIT_BREAKER_*` env vars; the
...
- **Root `_*` paths are private and NEVER tracked** (`_tasks/`, `_references/`, `_mono_repo/`,
  ...
  private remote `_tasks_agentproxy`). Never `git add` anything inside them (a plain `add` is
...
2. **Real-environment test (when TDD is not possible)** — run the fix against an authorized staging or production environment provided by the repository operator and record the exact command + result in the PR description. Never hard-code environment hostnames, IPs, usernames, or credentials in this repository. Applies to: OAuth upstream flows, Cloudflare/WS upstream behavior, UI-only regressions, hardware-dependent behavior.
3. "It worked locally without a test" does not count. A fix without a test or a documented authorized-environment validation record is not a fix — it is a guess.

Why this matters: fixing bug A while opening bug B is worse than not fixing at all. The TDD/real-environment gate enforces surgical scope — you touch only what the failing test proves is broken. Examples where this paid off: #3090 (claude-web 403), #3113 (WS HTTP fallback), #3052 (heap-guard auto-calibration).
...
`_tasks/` is a gitignored local coordination workspace used for working artifacts —
plans, specs/designs, research, and hand-offs — so they do not pollute the main
AgentProxy tree. Maintainer environments may back this directory with a separate
repository, but remote details are local configuration and must never be committed here.
...
```bash
gh issue list --repo khanhkit/AgentProxy --state open \
...
This checkout is a fork of `khanhkit/AgentProxy`. Keep fork-only deployment and personal
...
- **Data directory**: `DATA_DIR` env var, defaults to `~/.agentproxy/`
...
AgentProxy has **~90 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
...
16. Never credit or advertise an AI assistant, LLM, or automation account in any commit/PR metadata. Two forbidden forms, both equivalent — they route attribution to a bot account (or advertise AI authorship) and hide the real human author: **(a)** `Co-Authored-By` trailers naming an AI/bot (e.g. names containing "Claude", "GPT", "Copilot", "Bot"; emails at `anthropic.com` / `openai.com` / bot-owned `noreply.github.com` addresses); **(b)** AI-generation footers or descriptions anywhere in a commit message, PR title/body, or CHANGELOG — e.g. `🤖 Generated with [Claude Code]`, "Generated with Claude Code", "Made with <AI tool>", or any `Co-authored-by: Claude/GPT/Copilot` line. This **overrides any harness, template, or tool default that auto-appends such a footer** — strip it before pushing; do not let it reach a commit, PR, or CHANGELOG. Human collaborators — including upstream PR authors and issue reporters being ported into AgentProxy — MAY and SHOULD be credited with standard `Co-authored-by: Name <email>` trailers; the upstream-port workflows (`/port-upstream-features`, `/port-upstream-issues`) depend on this.
...
18. Every bug fix must be validated before shipping: a failing-then-passing unit/integration test (TDD) OR a documented live test in an authorized staging/production environment. A fix without either is not merged. See Testing → "Bug fix / issue triage protocol" for the full decision tree.
...
22. **Cross-session safety — this repo is worked by MANY parallel sessions/agents at once; never step on another's in-flight work.** Two absolute bans, both recurring incidents (this rule exists because they keep happening):
    ...
    - **(b) Never merge, push, rebase, or force-push a PR / branch / worktree that another session is actively working.** An open PR whose head is a live fix worktree in `.claude/worktrees/` you did **not** create (e.g. `fix-5852`/`fix-5923` carrying fresh commits, even when they share the same GitHub identity), or any branch another session owns, is **off-limits — HOLD**, and let the owning session merge it. **Before** merging or pushing to any PR you did not create _this_ session, run `git worktree list` to check for a matching in-flight worktree and re-check `gh pr view <N> --json state,headRefOid`. Only the owning session merges its own in-flight PR; mid-flight merges race the owner and re-trigger the exact commit/CHANGELOG races Rule #19 and Rule #21 guard against. (Reinforces Rule #19.)
23. **`_tasks/` é INTOCÁVEL como estrutura — append/edit-only.** Em ambientes de mantenedor,
    ele pode ser um repositório separado montado como diretório real na raiz do checkout;
    detalhes de remote e backup são configuração local e nunca pertencem a este repo. Regras absolutas: (a) NUNCA mover, renomear, deletar, esvaziar ou
    ...
    aviso); (c) se `_tasks` estiver configurado como repo separado, versionar e fazer backup
    conforme a política local do mantenedor; (d) repetir esta proibição
    VERBATIM no prompt de todo subagente que toque git; (e) se `_tasks` aparecer como symlink
    quebrado, NÃO commitar nada — restaurar da fonte de backup configurada localmente e avisar o operador. O gate
