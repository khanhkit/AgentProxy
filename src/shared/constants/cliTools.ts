...
import { getClaudeCodeDefaultModels } from "@agentproxy/open-sse/config/providerRegistry";
...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  claude: {
    ...
    color: "#D97757",
    description: "Anthropic Claude Code CLI — ANTHROPIC_BASE_URL points to AgentProxy",
  ...
  codex: {
    ...
    color: "#10A37F",
    description: "OpenAI Codex CLI — OpenAI-compatible base URL targets AgentProxy",
  ...
  opencode: {
    ...
    notes: [
      ...
      {
        type: "warning",
        text: 'Thinking variant example: opencode run "implement this feature" --model agentproxy/claude-sonnet-4-5-thinking --variant high',
      },
    ],
    guideSteps: [
  ...
  hermes: {
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "Use the JSON block below as the OpenAI-compatible provider definition for AgentProxy.",
      },
    ],
    codeBlock: {
      language: "json",
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Qwen Code CLI — current V4 OpenAI-compatible model provider via AgentProxy",
    ...
    notes: [
      {
        type: "info",
        text: "AgentProxy is registered under modelProviders.openai using Qwen Code's current bare-array V4 format.",
      },
      {
        type: "info",
        text: "The API key is stored only as AGENTPROXY_API_KEY in ~/.qwen/.env, leaving your existing provider credentials untouched.",
      },
    ],
    guideSteps: [
...
aider --openai-api-base "{{baseUrl}}" --model "{{model}}"`,
   ...
   * AgentProxy's OpenAI-compatible base URL like codex/forge. Installed via
...
OPENAI_API_KEY: "{{apiKey}}"`,
  ...
  omp: {
    ...
    description: "Oh My Pi terminal coding agent via AgentProxy",
    configType: "custom",
    category: "agent",
    vendor: "OSS",
    acpSpawnable: true,
    ...
    notes: [
      {
        type: "info",
        text: "Oh My Pi reads custom OpenAI-compatible providers from ~/.omp/agent/models.yml. AgentProxy adds itself as a provider with auto-discovery — models appear automatically in omp's /model menu.",
      },
      {
        type: "warning",
  ...
  letta: {
    ...
    notes: [
      {
        type: "info",
        text: "Letta CLI uses pi-ai which sends OpenAI-compatible requests. AgentProxy configures it as an OpenAI provider with custom base URL.",
      },
      {
        type: "info",
        text: "CLI (Local Mode): AgentProxy auto-configures ~/.letta/lc-local-backend/providers/auth.json. Use 'letta --info' to check if local mode is enabled.",
      },
      {
        type: "warning",
  ...
  /** ★ 5dive agent fleets (khanhkit/AgentProxy#11578) — 2026-08-28 */
  "5dive": {
    id: "5dive",
    name: "5dive",
    icon: "hub",
    color: "#7C3AED",
    description:
      "5dive — self-hosted fleet of long-running coding agents; one auth profile points every claude seat at AgentProxy",
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "sudo agentproxy configure 5dive --model <id> --auth-profile agentproxy",
      },
    ],
    notes: [
      {
