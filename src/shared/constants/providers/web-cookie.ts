...
export const WEB_COOKIE_PROVIDERS = {
  ...
  "chatgpt-web-codex": {
    ...
    website: "https://chatgpt.com",
    authHint:
      "Paste the full ChatGPT Cookie header. AgentProxy verifies it in an isolated headless browser profile.",
    subscriptionRisk: true,
    riskNoticeVariant: "webCookie",
    toolCalling: "native",
  },
  ...
  lmarena: {
    ...
    authHint:
      "Paste the full Cookie header from arena.ai (DevTools → Network → request → Cookie). Include arena-auth-prod-v1.0/.1… and cf_clearance/__cf_bm when present. AgentProxy uses Chrome TLS impersonation; if Arena still 403s, set providerSpecificData.recaptchaV3Token from a live browser session.",
    riskNoticeVariant: "webCookie",
  },
  "yuanbao-web": {
    id: "yuanbao-web",
  ...
  "zai-web": {
    ...
    riskNoticeVariant: "webCookie",
    authHint:
      'Copy the "token" value from chat.z.ai → DevTools → Application → Local Storage. Do not copy cookies; AgentProxy handles the per-request CAPTCHA through its browser transport.',
  },
  promptql: {
    id: "promptql",
    serviceKinds: ["llm"],
  ...
  maxai: {
    ...
    // (a bearer access token + a long-lived refresh token that AgentProxy refreshes
    ...
    toolCalling: "emulated",
    authHint:
      "Sign in once (email code or browser) to mint a MaxAI access token. AgentProxy signs each request, routes it through residential egress, and refreshes the token browserlessly, so a connection stays valid for about a year without re-login.",
  },
  uc: {
    id: "uc",
    serviceKinds: ["llm"],
    ...
    // — a durable Clerk credential from which AgentProxy mints a fresh short-lived
    ...
    toolCalling: "emulated",
    authHint:
      "Sign in once with an email code to bootstrap a UC (uncensored.com) subscription session. AgentProxy mints a fresh short-lived token per request browserlessly, so the connection renews on its own; you only re-run the email login about once a month when the subscription session rolls over.",
  },
};
