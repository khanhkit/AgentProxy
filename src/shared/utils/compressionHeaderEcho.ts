 ...
 *
 * When a request carries `x-agentproxy-compression`, docs promise the response echoes
 * `X-AgentProxy-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-agentproxy-compression";
const COMPRESSION_RESPONSE_HEADER = AGENTPROXY_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-AgentProxy-Compression: <mode>; source=request-header`
 * when the request supplied `x-agentproxy-compression` and the inner pipeline did not
