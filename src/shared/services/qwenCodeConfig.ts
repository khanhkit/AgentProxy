...
export const QWEN_CODE_ENV_KEY = "AGENTPROXY_API_KEY";
...
/**
 * Identifies only entries owned by AgentProxy. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "agentproxy") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (AgentProxy)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (AgentProxy)`,
...
/**
 * Merge one AgentProxy model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "agentproxy") delete next.selectedProvider;

  return next;
};

export const findAgentProxyQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasAgentProxyQwenCodeConfig = (settings: unknown): boolean =>
  findAgentProxyQwenCodeModel(settings) !== undefined;

/** Remove only AgentProxy-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?AGENTPROXY_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_agentproxy"))}`);
  return `${lines.join("\n")}\n`;
};
