/**
 * Build the AgentProxy `customModels` entries for Factory Droid's `settings.json`.
 *
 * Ported from upstream PR decolua/9router#618 (author Anurag Saxena) — multi-model
 * support for the Factory Droid CLI tool. Adapted to AgentProxy branding
 * (`custom:AgentProxy-<i>`) and extracted as a pure helper so the route-handler
...
/** True when a `customModels` entry was written by AgentProxy (any index). */
export function isAgentProxyCustomModel(entry: { id?: unknown } | null | undefined): boolean {
  return typeof entry?.id === "string" && entry.id.startsWith("custom:AgentProxy");
}
