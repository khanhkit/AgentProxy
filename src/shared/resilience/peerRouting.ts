...
export const AGENTPROXY_PEER_TRACE_HEADER = "X-AgentProxy-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  AGENTPROXY_INSTANCE_ID?: string;
  AGENTPROXY_PEER_URLS?: string;
  AGENTPROXY_PEER_MAX_HOPS?: string;
};
...
function getInstanceId(env: PeerEnvironment): string | null {
  const value = env.AGENTPROXY_INSTANCE_ID?.trim() ?? "";
  return INSTANCE_ID_PATTERN.test(value) ? value : null;
}

function getMaxPeerHops(env: PeerEnvironment): number {
  const parsed = Number.parseInt(env.AGENTPROXY_PEER_MAX_HOPS ?? "", 10);
...
export function isConfiguredAgentProxyPeer(
  targetUrl: string,
  ...
  return (env.AGENTPROXY_PEER_URLS ?? "")
    .split(",")
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, AGENTPROXY_PEER_TRACE_HEADER));
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted AgentProxy URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredAgentProxyPeer(targetUrl, env)) return false;

  const trace = parsePeerTrace(readHeader(clientHeaders, AGENTPROXY_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = AGENTPROXY_PEER_TRACE_HEADER.toLowerCase();
  for (const key of Object.keys(outgoingHeaders)) {
    if (key.toLowerCase() === traceHeaderLower) delete outgoingHeaders[key];
  }
  outgoingHeaders[AGENTPROXY_PEER_TRACE_HEADER] = trace.join(",");
  return true;
}
