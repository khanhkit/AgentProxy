...
import {
  ...
  tokensPerSecond,
} from "@agentproxy/open-sse/utils/generationThroughput";
...
export function getAgentProxyTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatAgentProxyCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-AgentProxy-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-AgentProxy-Provider` header already applies)
 ...
 */
export function buildAgentProxyDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildAgentProxyResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-AgentProxy-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-AgentProxy-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
  /**
  ...
  const tokens = getAgentProxyTokenCounts(usage);
  const headers: Record<string, string> = {
    [AGENTPROXY_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [AGENTPROXY_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [AGENTPROXY_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatAgentProxyCost(costUsd)),
    [AGENTPROXY_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [AGENTPROXY_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [AGENTPROXY_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[AGENTPROXY_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[AGENTPROXY_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[AGENTPROXY_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[AGENTPROXY_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatAgentProxyCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[AGENTPROXY_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  const decisionValue = buildAgentProxyDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[AGENTPROXY_RESPONSE_HEADERS.decision] = decisionValue;
  }
  ...
  if (tps != null) {
    headers[AGENTPROXY_RESPONSE_HEADERS.tokensPerSecond] = toHeaderValue(tps.toFixed(3));
  }

  return headers;
}

export function buildAgentProxySseMetadataComment(
  options: Parameters<typeof buildAgentProxyResponseMetaHeaders>[0]
): string {
  const headers = buildAgentProxyResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-AgentProxy-* response meta headers.
 ...
 */
export function attachAgentProxyMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildAgentProxyResponseMetaHeaders>[0]
): void {
  const built = buildAgentProxyResponseMetaHeaders(meta);
...
/**
 * Attach the X-AgentProxy-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachAgentProxyMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildAgentProxyResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachAgentProxyMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachAgentProxyMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
