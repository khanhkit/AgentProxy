...
const LINUX_CERT_NAME = "agentproxy-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "AgentProxy MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.AGENTPROXY_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] AGENTPROXY_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
 ...
 * cert-path-agnostic and keep writing to the same `agentproxy-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `agentproxy-tproxy-ca.crt` slot
...
// They install / remove the AgentProxy MITM root CA from the OS trust store and
