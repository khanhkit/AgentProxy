 ...
 * trust store under a DEDICATED slot (`agentproxy-tproxy-ca.crt`), separate from
 * the static MITM cert (`agentproxy-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "agentproxy-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.AGENTPROXY_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] AGENTPROXY_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
