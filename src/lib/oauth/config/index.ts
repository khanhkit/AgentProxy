 ...
 * with the running AgentProxy server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.AGENTPROXY_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.AGENTPROXY_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.AGENTPROXY_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
