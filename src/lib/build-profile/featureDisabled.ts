/**
 * Helper used by the `AGENTPROXY_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (AGENTPROXY_BUILD_PROFILE=minimal). ` +
        `Install the full agentproxy artifact instead of agentproxy-secure if you need this feature.`
    );
