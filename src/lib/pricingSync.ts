...
// ─── Provider mapping: LiteLLM provider → AgentProxy aliases ─────
...
/**
 * Transform LiteLLM raw data → AgentProxy PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (AgentProxy format).
 ...
 */
export function transformToAgentProxy(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to AgentProxy providers
    const litellmProvider = info.litellm_provider || "";
    const AgentProxyProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (AgentProxyProviders) {
      for (const provider of AgentProxyProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToAgentProxy(raw);
