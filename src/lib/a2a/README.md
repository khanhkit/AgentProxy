# AgentProxy A2A Server

> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use AgentProxy as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes AgentProxy as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through AgentProxy intelligent pipeline",
...
Routes prompts through AgentProxy's intelligent pipeline with full observability.
...
Discovers AgentProxy agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use AgentProxy A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to AgentProxy, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via AgentProxy A2A
    ...
    # Step 2: Review the code via AgentProxy A2A (different model)
...
Multiple agents share quota through AgentProxy, using the quota skill to coordinate.
...
If no API key is configured on the server (`AGENTPROXY_API_KEY` is empty), authentication is bypassed.
