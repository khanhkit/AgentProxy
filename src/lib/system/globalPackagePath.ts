...
/**
 * Resolve the real install directory of the globally-installed `agentproxy` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/agentproxy/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/agentproxy`, not `/agentproxy/app`. We probe the real
 ...
 * global install.
 */
export async function resolveGlobalAgentProxyPath(
  ...
  const packageRoot = path.join(globalRoot, "agentproxy");
