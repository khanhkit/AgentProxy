/**
 * AgentProxy Copilot — Tool definitions
 *
 * Tools the copilot can execute to configure AgentProxy on behalf of the user,
...
// ── Helper: check if agentproxy CLI is available ──────────────────────────────

function getAgentProxyCliPath(): string | null {
  try {
    const result = execSync("which agentproxy 2>/dev/null || command -v agentproxy 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the AgentProxy codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within AgentProxy.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runAgentProxyCli",
    description:
      "Execute an 'agentproxy' CLI command to configure or query the AgentProxy app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: agentproxy list-keys, agentproxy switch-combo [id], agentproxy set-budget 10, agentproxy set-strategy [id] priority, agentproxy health, agentproxy mcp (starts MCP server), agentproxy db-health, agentproxy reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'agentproxy'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getAgentProxyCliPath();
      if (!cliPath) return "agentproxy CLI not found in PATH. Install AgentProxy first.";
