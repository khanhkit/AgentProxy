/**
 * AgentProxy Copilot — Chat Engine
 *
...
function getKnowledgeResponse(query: string): string | null {
  ...
  if (
    ...
    return `## AgentProxy Architecture
...
Use \`createCombo\` tool or \`runAgentProxyCli\` to create them.`;
  }
...
AgentProxy supports 212+ providers across categories:
...
I can use CodeGraph to explore the AgentProxy codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## AgentProxy Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`agentproxy health\`
- "CLI list-combos" → ejecuta \`agentproxy list-combos\`
- "CLI set-budget 10" → ejecuta \`agentproxy set-budget 10\`

### Conocimiento
- "Cómo funciona AgentProxy?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with AgentProxy.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runAgentProxyCli") {
    ...
    return {
      message: result,
      toolCalls: [{ name: "runAgentProxyCli", args: intent.args, result }],
    };
  }
