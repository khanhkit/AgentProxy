/**
 * Cloudflare Worker source emitter for the AgentProxy proxy relay.
 *
 ...
 *
 * The AgentProxy variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----AgentProxyCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// AgentProxy Cloudflare Worker proxy relay — generated at deploy time.
