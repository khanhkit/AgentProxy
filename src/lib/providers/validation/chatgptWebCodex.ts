...
import { inspectBrowserLoginCapabilities } from "@agentproxy/open-sse/vendor/codex-chatgpt-web/browser-login.ts";
import { decodeChatGptWebCodexSecrets } from "@agentproxy/open-sse/executors/chatgpt-web-codex/credentials.ts";
import {
  connectionRuntimePaths,
  ensureConnectionStorageState,
  ensureConnectionStorageStateFromCredential,
} from "@agentproxy/open-sse/executors/chatgpt-web-codex/storageState.ts";
...
export async function validateChatGptWebCodexProvider({
  ...
  try {
    ...
    const { detectChromeExecutable } =
      await import("@agentproxy/open-sse/executors/chatgpt-web-codex.ts");
