...
import { splitCodexReasoningSuffix } from "@agentproxy/open-sse/executors/codex/reasoningSuffix.ts";
import { isModelSelectable } from "@agentproxy/open-sse/services/modelLifecycle.ts";
import { getCodexUsage } from "@agentproxy/open-sse/services/usage/codex.ts";
import { throttleQuotaFetch } from "@agentproxy/open-sse/services/quotaFetchThrottle.ts";
...
async function loadQuotaAutoPingExecutor(provider: string): Promise<BaseExecutor> {
  ...
  try {
    codexExecutorPromise ??= import("@agentproxy/open-sse/executors/codex.ts").then(
...
export async function resolveQuotaAutoPingModel(
  ...
  const { getProviderModels } = await import("@agentproxy/open-sse/config/providerModels.ts");
