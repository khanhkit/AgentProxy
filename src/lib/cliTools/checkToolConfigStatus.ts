...
import { hasAgentProxyQwenCodeConfig } from "@/shared/services/qwenCodeConfig";
import {
...
/**
 * Check if a tool has AgentProxy configured by reading its config file directly.
...
export async function checkToolConfigStatus(
  ...
  try {
    ...
    if (toolId === "grok-build") {
      ...
      return settings.default === "agentproxy" &&
    ...
    if (toolId === "hermes") {
      ...
      const hasAgentProxy =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasAgentProxy ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores AgentProxy config differently
    switch (toolId) {
      case "claude":
        return (config?.env as Record<string, unknown>)?.ANTHROPIC_BASE_URL
          ...
          : "not_configured";
      case "qwen":
        return hasAgentProxyQwenCodeConfig(config) ? "configured" : "not_configured";
      case "droid":
      case "openclaw":
      case "cline":
      case "kilo": {
        // Generic check: look for AgentProxy-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("agentproxy") ||
          configStr.includes("sk_agentproxy") ||
