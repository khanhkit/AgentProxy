...
const LABEL = "com.agentproxy.browser-login";
...
export function sessionKey(sessionId: string): string {
  return `agentproxy-browser-login-${sessionId.replace(/[^a-zA-Z0-9]/g, "").slice(0, 20)}`;
}
...
export async function startSession(connectionId: string): Promise<VncSession> {
  ...
  try {
    ...
    if (result.code !== 0) {
      ...
      const buildHint =
        VNC_CONFIG.image === "agentproxy-vnc-chromium:local" &&
        /pull access denied|unable to find image|not found/i.test(message)
          ? " Build it with: docker build -t agentproxy-vnc-chromium:local docker/vnc-browser/chromium"
          : "";
