...
import { clearProviderFailure, clearModelLock } from "@agentproxy/open-sse/services/accountFallback";
import { resolveProviderAlias } from "@agentproxy/open-sse/services/model";
...
export async function buildProviderHealthAutopilotReport(
  ...
  const [{ getAllCircuitBreakerStatuses }, { getAllModelLockouts }, quotaMonitor] =
    await Promise.all([
      ...
      import("@agentproxy/open-sse/services/quotaMonitor.ts").catch(() => null),
    ]);
