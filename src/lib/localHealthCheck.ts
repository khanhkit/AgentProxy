...
function getLHCState() {
  if (!globalThis.__agentproxyLocalHC) {
    globalThis.__agentproxyLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__agentproxyLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("AGENTPROXY_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
