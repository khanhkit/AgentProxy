...
export interface OwnerOnlyPermissionRuntime {
...
function assertSafeParentDirectory(filePath: string, platform: NodeJS.Platform): void {
...
function runWindowsAclCommand(
  ...
  if (result.error || result.status !== 0) {
    const detail = asText(result.stderr).trim() || result.error?.message || `exit ${result.status}`;
    throw new Error(`Failed to apply owner-only Windows ACL via ${command}: ${detail}`);
  }
  return asText(result.stdout);
}

function resolveWindowsUserSid(spawnSync: SpawnSyncLike): string {
...
/**
 * Repair an existing secret-bearing file so only the current OS user retains
 * access. Windows uses icacls and fails closed; POSIX uses mode 0600.
 */
export function applyOwnerOnlyPermissionsSync(
  filePath: string,
  runtime: OwnerOnlyPermissionRuntime = {}
): void {
  assertRegularFile(filePath);
  const platform = runtime.platform ?? process.platform;
  if (platform === "win32") {
    const spawnSync = runtime.spawnSync ?? (nodeSpawnSync as unknown as SpawnSyncLike);
...
export function repairOwnerOnlyFileSync(
  filePath: string,
  runtime: OwnerOnlyPermissionRuntime = {}
): void {
  const platform = runtime.platform ?? process.platform;
...
/**
 * Write secret-bearing local state without exposing the new contents under a
 * permissive mode/ACL. Permissions are hardened before truncation/write.
 */
export function writeOwnerOnlyFileSync(
  filePath: string,
  data: string | NodeJS.ArrayBufferView,
  options: { encoding?: BufferEncoding; runtime?: OwnerOnlyPermissionRuntime } = {}
): void {
  ...
  try {
    if (platform === "win32") {
      ...
      fs.fchmodSync(fd, OWNER_ONLY_FILE_MODE);
    }
    ...
    if (!completed && !existed) {
      try {
        ...
        // Best effort cleanup of the empty file created before permission hardening failed.
      }
    }
  }
}
