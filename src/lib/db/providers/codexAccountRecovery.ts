...
function clearScope(data: RecordValue, scope: Scope): void {
  ...
  if (toRecord(data.codexQuotaState).scope === scope) {
    delete data.codexExhaustedWindow;
  }
}
...
/** Atomic, account-scoped release. Credentials and terminal parent state are untouched. */
export async function clearCodexAccountCooldown(id: string, scope: Scope = "codex") {
  return mutate(id, (data) => clearScope(data, scope));
}

/**
 * Persist a fresh provider-limits quota observation and release a scope only
 * when both windows prove headroom. The expected pre-fetch state is compared
 * inside the transaction so a newer concurrent cooldown/error always wins.
 */
export async function syncCodexQuotaObservation(
  id: string,
  usage: RecordValue,
  expected: unknown
) {
