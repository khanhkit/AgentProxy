...
/**
 * Operators who set AGENTPROXY_WAL_TRUNCATE_INTERVAL_MS to a positive value believe a
...
function warnPeriodicTruncateRemoved(env: NodeJS.ProcessEnv): void {
  ...
  console.warn(
    "[DB] AGENTPROXY_WAL_TRUNCATE_INTERVAL_MS is no longer used and has no effect: periodic " +
...
export function getWalPassiveIntervalMs(env: NodeJS.ProcessEnv = process.env): number {
  const rawValue = env.AGENTPROXY_WAL_PASSIVE_INTERVAL_MS;
...
export function getWalGuardMaxBytes(env: NodeJS.ProcessEnv = process.env): number {
  const rawValue = env.AGENTPROXY_WAL_GUARD_MAX_MB;
  if (typeof rawValue === "string" && rawValue.trim().length > 0) {
    const parsed = Number(rawValue);
