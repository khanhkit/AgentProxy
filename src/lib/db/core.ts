...
// AGENTPROXY_BUILDING=1 (set by build-next-isolated.mjs and inherited by every
  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir agentproxy`
    );
  }
}
...
declare global {
  ...
  var __agentproxyDbProbeRestoreCount: number | undefined;
  ...
  var __agentproxyDbOomFailureCount: number | undefined;
}
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.AGENTPROXY_FORCE_DB_HEALTHCHECK === "1") return true;
...
export function getDbInstance(): SqliteDatabase {
  ...
  if (!fs.existsSync(sqliteFile) && probeFailureBackups.length > 0) {
    ...
    if (
      (globalThis.__agentproxyDbProbeRestoreCount =
        (globalThis.__agentproxyDbProbeRestoreCount || 0) + 1) > 3
    ) {
      throw new Error(
  ...
  if (fs.existsSync(sqliteFile)) {
    try {
      ...
      if (
        ...
        if (
          (globalThis.__agentproxyDbOomFailureCount =
            (globalThis.__agentproxyDbOomFailureCount || 0) + 1) > 3
        ) {
          throw new Error(
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _agentproxy_migrations (version, name)
 ...
 * can place it in ~/.agentproxy/runtime/ without blocking a synchronous caller.
 *
