...
export function getOmpCredentials(providerId: string) {
  ...
  if (!Database) return { hasAgentProxy: false, baseUrl: null, apiKey: null };
  ...
  try {
    ...
    if (row?.data) {
      ...
      return { hasAgentProxy: true, baseUrl: parsed.baseUrl || null, apiKey: parsed.apiKey || null };
    }
    return { hasAgentProxy: false, baseUrl: null, apiKey: null };
  } catch {
    return { hasAgentProxy: false, baseUrl: null, apiKey: null };
  }
}
