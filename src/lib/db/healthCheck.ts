...
interface RunDbHealthCheckOptions {
   ...
   * Set via env var `AGENTPROXY_SKIP_DB_HEALTHCHECK=1`.
...
function isInvalidQuotaSnapshot(
  row: QuotaSnapshotRow,
  connectionStmt: PreparedStatement
): boolean {
...
function scanQuotaSnapshots(
  ...
  const firstPage = db.prepare(
    "SELECT CAST(id AS TEXT) AS id, connection_id, created_at FROM quota_snapshots " +
      "WHERE id <= CAST(? AS INTEGER) ORDER BY quota_snapshots.id LIMIT ?"
  );
  const nextPage = db.prepare(
    "SELECT CAST(id AS TEXT) AS id, connection_id, created_at FROM quota_snapshots " +
      "WHERE id > CAST(? AS INTEGER) AND id <= CAST(? AS INTEGER) " +
      "ORDER BY quota_snapshots.id LIMIT ?"
  );
  const connectionStmt = db.prepare(
    "SELECT 1 AS ok FROM provider_connections WHERE id = ? LIMIT 1"
  );
  ...
  while (rows.length > 0) {
    for (const row of rows) {
      if (!isInvalidQuotaSnapshot(row, connectionStmt)) continue;
      issueCount += 1;
      if (deleteByRowId) {
        options.beforeRepair();
...
export function runDbHealthCheck(
  ...
  // Skip entirely when skipIntegrityCheck is set (env AGENTPROXY_SKIP_DB_HEALTHCHECK=1).
