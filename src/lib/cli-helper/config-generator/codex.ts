 ...
 * below matches the documented AgentProxy block
 ...
 *   var named by `env_key` (`AGENTPROXY_API_KEY`), so the generated content is
 ...
 *   `[model_providers.agentproxy]` are set. An existing file that fails TOML
 *   parsing aborts generation instead of clobbering the operator's config.
 */

export const CODEX_MODEL_PROVIDER_ID = "agentproxy";
...
export async function generateCodexConfig(options: {
  ...
  const header =
    "# Generated by AgentProxy. The API key is read from the AGENTPROXY_API_KEY\n" +
