...
export function getRuntimePorts(): RuntimePorts {
  // AGENTPROXY_PORT preserves the user's canonical PORT in wrapped runtimes
  // where Next.js requires process.env.PORT to be the dashboard listener port.
  const basePort = parsePort(process.env.AGENTPROXY_PORT || process.env.PORT, DEFAULT_PORT);
