...
const LOCAL_USAGE_MODEL = "agentproxy/local-usage";
  ...
  try {
    ...
    return {
      preferredConnectionId:
        ...
        readHeader(request, "x-agentproxy-connection")?.trim() ||
        null,
      preferredProvider: url.searchParams.get("provider")?.trim() || null,
    };
  } catch {
    return {
      preferredConnectionId: readHeader(request, "x-agentproxy-connection")?.trim() || null,
