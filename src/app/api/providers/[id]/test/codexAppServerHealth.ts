...
export async function testCodexAppServerConnection(
  ...
  const { resolveAppServerConfig } = await import(
    "@agentproxy/open-sse/executors/codex/appServerConfig.ts"
  );
  ...
  try {
    ...
    try {
      const [{ probeCodexAppServerAuth }, { getCodexAppServerWebsocketTransport }] =
        await Promise.all([
          import("@agentproxy/open-sse/executors/codex/appServerAuthProbe.ts"),
          import("@agentproxy/open-sse/executors/codex.ts"),
        ]);
