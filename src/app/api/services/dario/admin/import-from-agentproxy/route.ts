/**
 * GET/POST /api/services/dario/admin/import-from-agentproxy
 *
 * Imports an existing AgentProxy `claude` provider connection's OAuth tokens
 * directly into Dario's account store, skipping the interactive browser OAuth
 * flow entirely. This works because AgentProxy's native `claude` provider and
 ...
 * PKCE/token-exchange ourselves, since AgentProxy's own decrypt() already
 ...
 * the file we stop+start the AgentProxy-managed supervisor to force a clean
...
function safeAliasFromSource(email: string | null | undefined, connectionId: string): string {
  const base = (email || connectionId || "agentproxy").toLowerCase();
  const cleaned = base.replace(/[^a-z0-9_.-]/g, "-").replace(/^[^a-z0-9]+/, "");
  const alias = cleaned || "agentproxy";
  return `agentproxy-${alias}`.slice(0, 64);
}
