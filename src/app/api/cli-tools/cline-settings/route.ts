...
// Check if AgentProxy is configured as OpenAI-compatible provider
const hasAgentProxyConfig = (globalState: any) => {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasAgentProxy: hasAgentProxyConfig(globalState),
...
// POST - Configure Cline to use AgentProxy as OpenAI-compatible provider
export async function POST(request: Request) {
  ...
  try {
    ...
    secrets.openAiApiKey = apiKey || "sk_agentproxy";
...
// DELETE - Remove AgentProxy OpenAI-compatible provider config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AgentProxy settings removed from Cline",
    });
  } catch (error) {
