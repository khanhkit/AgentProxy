...
/**
 * Render the AgentProxy provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by AgentProxy (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-AgentProxy marker comment.
 */
...
// POST — write AgentProxy settings to Forge config.toml
...
// DELETE — remove Forge AgentProxy config
