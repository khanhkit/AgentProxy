...
// Check if AgentProxy OpenAI-compatible provider is configured
const hasAgentProxyConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["agentproxy"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("agentproxy")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasAgentProxy: hasAgentProxyConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use AgentProxy as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update AgentProxy as openai-compatible provider
...
// DELETE - Remove AgentProxy config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AgentProxy provider
    delete auth["openai-compatible"];
    delete auth["agentproxy"];
    ...
    return NextResponse.json({
      success: true,
      message: "AgentProxy settings removed from Kilo Code",
    });
  } catch (error) {
