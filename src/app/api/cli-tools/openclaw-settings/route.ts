...
// Check if settings has AgentProxy config
const hasAgentProxyConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["agentproxy"];
};
...
// POST - Update AgentProxy settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `agentproxy/${model}`;

    // Update models.providers.agentproxy
    settings.models.providers["agentproxy"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AgentProxy from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["agentproxy"];
    ...
    // Reset agents.defaults.model.primary if it uses agentproxy
    if (settings.agents?.defaults?.model?.primary?.startsWith("agentproxy/")) {
