...
// the `agentproxy setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. AgentProxy is registered under
 * the `agentproxy` provider id as an `openai-compat` provider — same shape
...
const hasAgentProxyConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const agentproxy = providers?.agentproxy as Record<string, unknown> | undefined;
  return (
    !!agentproxy &&
    agentproxy.type === "openai-compat" &&
    typeof agentproxy.base_url === "string" &&
    agentproxy.base_url.length > 0
  );
};
...
// POST — write AgentProxy settings to crush.json (providers.agentproxy)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `AgentProxy: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove AgentProxy provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Crush AgentProxy settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
