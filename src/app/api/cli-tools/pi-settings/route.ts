...
// POST — write AgentProxy settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge AgentProxy settings (pi uses OpenAI-compatible config)
...
// DELETE — remove AgentProxy settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi AgentProxy settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
