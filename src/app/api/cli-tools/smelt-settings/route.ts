...
/**
 * Check if the config file contains AgentProxy settings.
 */
const hasAgentProxyConfig = (settings: Record<string, unknown> | null): boolean => {
  ...
  return (
    ...
    settings._managedBy === "agentproxy"
  );
};
...
// POST — write AgentProxy settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge AgentProxy settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove AgentProxy settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AgentProxy-managed fields
    ...
    return NextResponse.json({ success: true, message: "Smelt AgentProxy settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
