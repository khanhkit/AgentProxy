...
/**
 * Render the AgentProxy config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by AgentProxy (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
...
const hasAgentProxyConfig = (content: string | null): boolean => {
  ...
  return content.includes("managed by AgentProxy");
};
...
// POST — write AgentProxy settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove AgentProxy CodeWhale config (primary + legacy, if present)
