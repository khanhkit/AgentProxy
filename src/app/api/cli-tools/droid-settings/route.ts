...
// Check if settings has AgentProxy customModels.
// Multi-model entries are stored as `custom:AgentProxy-0`, `custom:AgentProxy-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasAgentProxyConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isAgentProxyCustomModel);
};
...
// POST - Update AgentProxy customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing AgentProxy config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isAgentProxyCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend AgentProxy entries (one per requested model)
...
// DELETE - Remove AgentProxy customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AgentProxy customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isAgentProxyCustomModel(m));
