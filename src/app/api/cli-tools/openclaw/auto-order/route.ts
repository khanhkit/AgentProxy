...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [healthRes, combosRes] = await Promise.allSettled([
      fetch(`${AGENTPROXY_BASE_URL}/api/monitoring/health`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${AGENTPROXY_BASE_URL}/api/combos`, { signal: AbortSignal.timeout(5000) }),
    ]);
