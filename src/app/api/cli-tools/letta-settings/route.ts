...
const getBackupPath = () =>
  path.join(getLocalBackendDir(), "providers", "auth.json.agentproxy-backup");
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-AgentProxy endpoint
    let lmstudioConflict = false;
    if (provider && !isAgentProxyUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasAgentProxy: hasAgentProxyConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply AgentProxy as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-AgentProxy provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isAgentProxyUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isAgentProxyUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-AgentProxy conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-agentproxy provider if present
    if (authFile.providers?.["lc-agentproxy"]) {
      delete authFile.providers["lc-agentproxy"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a AgentProxy model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json(
...
// ── DELETE - Remove AgentProxy configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "AgentProxy config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "AgentProxy config removed. Restart Letta CLI to take effect.";
