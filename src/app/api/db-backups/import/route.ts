 ...
 * via `AGENTPROXY_DB_IMPORT_MAX_MB`. Invalid / out-of-range values fall back to the 100 MB
 * default and are clamped to a 4 GB ceiling.
 */
export function resolveMaxUploadSizeBytes(
  env: NodeJS.ProcessEnv = process.env
): number {
  const raw = env.AGENTPROXY_DB_IMPORT_MAX_MB;
...
// Required tables that must exist in a valid AgentProxy database
...
export async function POST(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          error:
            ...
            `Set AGENTPROXY_DB_IMPORT_MAX_MB to raise it, or VACUUM the database before exporting.`,
        },
        { status: 400 }
      );
    }
    ...
    tmpPath = path.join(os.tmpdir(), `agentproxy-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid AgentProxy database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
