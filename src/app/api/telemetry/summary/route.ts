...
export async function GET(request) {
  try {
    ...
    const { getQuotaMonitorSummary } = await import("@agentproxy/open-sse/services/quotaMonitor.ts");
    const { getActiveSessions } = await import("@agentproxy/open-sse/services/sessionManager.ts");
