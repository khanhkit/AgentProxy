...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const config =
      validation.data as import("@agentproxy/open-sse/services/taskAwareRouter.ts").TaskRoutingConfig;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (actionRequest.action === "detect") {
      const { detectTaskType } = await import("@agentproxy/open-sse/services/taskAwareRouter.ts");
