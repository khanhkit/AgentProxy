 ...
 *
 * Feature request: https://github.com/khanhkit/AgentProxy/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("agentproxy-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("agentproxy-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "agentproxy-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "AgentProxy",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.agentproxy}`,
      models,
    };

    const responsesConfig = {
      name: "AgentProxy-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.agentproxy}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("agentproxy-cli-key-copilot", value);
  };
