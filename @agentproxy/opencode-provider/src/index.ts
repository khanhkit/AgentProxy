/**
 * OpenCode provider plugin for AgentProxy AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * AgentProxy instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any AgentProxy-exposed
 ...
 *     });
 *     // config -> { $schema, provider: { agentproxy: { npm, name, options, models } } }
 *     ```
 *
 ...
 *     const provider = createAgentProxyProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.agentproxy in opencode.json
 *     ```
 *
...
export const AGENTPROXY_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed AgentProxy models. Synced
 * with the Alph4d0g/opencode-agentproxy-auth AGENTPROXY_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-agentproxy-auth, MIT) and extended
 * with Claude Code passthrough models (`cc/` prefix).
 */
export const AGENTPROXY_DEFAULT_OPENCODE_MODELS = [
  "cc/claude-opus-4-8",
  "cc/claude-opus-4-7",
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-agentproxy-auth
 * (https://github.com/Alph4d0g/opencode-agentproxy-auth, MIT).
 */
 ...
 * Matches the context lengths used by AgentProxy's provider registry.
 */
export const AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `AgentProxyProviderOptions.modelCapabilities`.
 */
export const AGENTPROXY_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface AgentProxyProviderOptions {
  /** AgentProxy base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** AgentProxy API key. Required. Use `sk_agentproxy` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"AgentProxy"`. */
  displayName?: string;
   ...
   * `AGENTPROXY_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"agentproxy/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.agentproxy.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to AgentProxy. */
  npm: typeof AGENTPROXY_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"agentproxy/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"agentproxy/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [AGENTPROXY_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@agentproxy/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@agentproxy/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@agentproxy/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.agentproxy` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createAgentProxyProvider(options: AgentProxyProviderOptions): OpenCodeProviderEntry {
  ...
  const modelList =
    options.models && options.models.length > 0
      ? [...options.models]
      : [...AGENTPROXY_DEFAULT_OPENCODE_MODELS];
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = AGENTPROXY_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
  ...
  return {
    npm: AGENTPROXY_PROVIDER_NPM,
    name: options.displayName?.trim() || "AgentProxy",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.agentproxy`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"agentproxy/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildAgentProxyOpenCodeConfig(
  options: AgentProxyProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [AGENTPROXY_PROVIDER_KEY]: createAgentProxyProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${AGENTPROXY_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${AGENTPROXY_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the AgentProxy provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.agentproxy` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildAgentProxyOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [AGENTPROXY_PROVIDER_KEY]: partial.provider[AGENTPROXY_PROVIDER_KEY],
  };
 ...
 */
export const AGENTPROXY_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type AgentProxyMCPScope = (typeof AGENTPROXY_MCP_DEFAULT_SCOPES)[number] | string;

export interface AgentProxyMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** AgentProxy API key forwarded to the MCP server as `AGENTPROXY_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `AGENTPROXY_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `AGENTPROXY_MCP_SCOPES`.
   * When omitted `AGENTPROXY_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: AgentProxyMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.agentproxy` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createAgentProxyMCPEntry({
 *   serverPath: "/home/user/.local/share/agentproxy/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_agentproxy",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.agentproxy
 * ```
 */
export function createAgentProxyMCPEntry(options: AgentProxyMCPOptions): OpenCodeMCPServerEntry {
  ...
  if (options.managementApiKey !== undefined) {
    ...
    if (mgmtKey) env.AGENTPROXY_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.AGENTPROXY_MCP_ENFORCE_SCOPES = "true";
    env.AGENTPROXY_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@agentproxy/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that AgentProxy's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different AgentProxy releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-agentproxy-auth (MIT).
 */
export interface AgentProxyLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running AgentProxy instance.
 *
 ...
 * `display_name`) field variants across AgentProxy versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createAgentProxyProvider` / `buildAgentProxyOpenCodeConfig` instead of
 * relying on `AGENTPROXY_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - AgentProxy base URL (with or without `/v1`).
 * @param apiKey    - AgentProxy API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_agentproxy");
 * const config = buildAgentProxyOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_agentproxy",
 *   models,                    // AgentProxyLiveModel[] — contextLength auto-extracted
...
export async function fetchLiveModels(
  ...
  const models: AgentProxyLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from AgentProxy's /v1/models response.
    // AgentProxy returns context_length in snake_case for both synced
 ...
 */
export type AgentProxyCompressionOverride =
  | ""
  | "off"
  | "lite"
  | "standard"
...
export interface AgentProxyCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: AgentProxyCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running AgentProxy instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as AgentProxyCompressionOverride)
      : "";
...
/**
 * Options for `createAgentProxyComboConfig`.
 * Mirrors the subset of combo fields exposed by the AgentProxy `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface AgentProxyComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for AgentProxy's management API.
 *
 ...
 * @example
 * ```ts
 * const payload = createAgentProxyComboConfig({
 *   name: "claude-primary",
 *   strategy: "priority",
 ...
 * ```
 */
export function createAgentProxyComboConfig(
  options: AgentProxyComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface AgentProxyRoleOverrides {
...
/** Per-role binding used by `createAgentProxyAgentBlock`. */
export interface AgentProxyAgentRole extends AgentProxyRoleOverrides {
  /** AgentProxy model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createAgentProxyAgentBlock`. */
export interface AgentProxyAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, AgentProxyAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends AgentProxyRoleOverrides {
  /** Always emitted as `"agentproxy/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: AgentProxyAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${AGENTPROXY_PROVIDER_KEY}/${modelId}` };
 ...
 * specific AgentProxy model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createAgentProxyAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "agentproxy/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createAgentProxyAgentBlock(
  options: AgentProxyAgentBlockOptions
...
/**
 * Per-mode binding used by `createAgentProxyModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `AgentProxyAgentRole` + `createAgentProxyAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface AgentProxyMode extends AgentProxyAgentRole {}

/**
 * Options for `createAgentProxyModesBlock`.
 *
 * @deprecated See `AgentProxyMode`.
 */
export interface AgentProxyModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, AgentProxyMode>;
}
 ...
 * a specific AgentProxy model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createAgentProxyAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createAgentProxyModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createAgentProxyModesBlock(
  options: AgentProxyModesBlockOptions
...
export default createAgentProxyProvider;
