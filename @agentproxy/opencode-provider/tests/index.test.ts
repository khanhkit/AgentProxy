...
import {
  buildAgentProxyOpenCodeConfig,
  createAgentProxyAgentBlock,
  createAgentProxyComboConfig,
  createAgentProxyMCPEntry,
  createAgentProxyModesBlock,
  createAgentProxyProvider,
  fetchLiveModels,
  listCombos,
  mergeIntoExistingConfig,
  normalizeBaseURL,
  AGENTPROXY_DEFAULT_MODEL_CAPABILITIES,
  AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS,
  AGENTPROXY_DEFAULT_OPENCODE_MODELS,
  AGENTPROXY_MCP_DEFAULT_SCOPES,
  AGENTPROXY_PROVIDER_NPM,
...
test("createAgentProxyProvider validates required fields", () => {
  assert.throws(
    () => createAgentProxyProvider({ baseURL: "", apiKey: "x" } as never),
    /baseURL is required/
  );
  assert.throws(
    () => createAgentProxyProvider({ baseURL: "http://x", apiKey: "" } as never),
    /apiKey is required/
  );
});

test("createAgentProxyProvider produces the OpenCode-compatible shape", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  });

  assert.equal(provider.npm, AGENTPROXY_PROVIDER_NPM);
  assert.equal(provider.name, "AgentProxy");
  assert.equal(provider.options.baseURL, "http://localhost:20128/v1");
  assert.equal(provider.options.apiKey, "sk_agentproxy");
  assert.equal(typeof provider.models, "object");
});

test("createAgentProxyProvider seeds the default model catalog", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  });
  ...
  const defaultIds = [...AGENTPROXY_DEFAULT_OPENCODE_MODELS].sort();
...
test("createAgentProxyProvider honours a custom models list and labels", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
...
test("createAgentProxyProvider deduplicates and trims model ids", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
...
test("createAgentProxyProvider honours displayName override", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
    displayName: "Local AgentProxy",
  });
  assert.equal(provider.name, "Local AgentProxy");
});

test("buildAgentProxyOpenCodeConfig wraps the provider with the OpenCode schema", () => {
  const doc = buildAgentProxyOpenCodeConfig({
    baseURL: "http://localhost:20128/v1",
    apiKey: "sk_agentproxy",
  });

  assert.equal(doc.$schema, OPENCODE_CONFIG_SCHEMA);
  assert.equal(typeof doc.provider.agentproxy, "object");
...
test("buildAgentProxyOpenCodeConfig emits model and small_model prefixed with provider key", () => {
  const doc = buildAgentProxyOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  ...
  assert.equal(doc.model, "agentproxy/claude-sonnet-4-5-thinking");
  assert.equal(doc.small_model, "agentproxy/gemini-3-flash");
});

test("buildAgentProxyOpenCodeConfig omits model and small_model when not supplied", () => {
  const doc = buildAgentProxyOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  });
...
test("buildAgentProxyOpenCodeConfig ignores blank model strings", () => {
  const doc = buildAgentProxyOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
    model: "   ",
...
test("mergeIntoExistingConfig preserves existing provider entries", () => {
  ...
  assert.ok("agentproxy" in (result.provider as Record<string, unknown>));
  assert.deepEqual((result as Record<string, unknown>).keybinds, { submit: "enter" });
});

test("mergeIntoExistingConfig overwrites existing agentproxy entry", () => {
  const existing = {
    provider: {
      agentproxy: {
  ...
  const agentproxy = (result.provider as Record<string, unknown>).agentproxy as { name: string };
  assert.equal(agentproxy.name, "NEW");
});
...
test("mergeIntoExistingConfig writes model and small_model when supplied", () => {
  ...
  assert.equal(result.model, "agentproxy/claude-sonnet-4-5-thinking");
  assert.equal(result.small_model, "agentproxy/gemini-3-flash");
});

test("mergeIntoExistingConfig does not add model keys when not supplied", () => {
  const result = mergeIntoExistingConfig(
    {},
    { baseURL: "http://localhost:20128", apiKey: "sk_agentproxy" }
  );
  assert.ok(!("model" in result));
  assert.ok(!("small_model" in result));
});

test("AGENTPROXY_MCP_DEFAULT_SCOPES contains 7 read-only scopes", () => {
  assert.equal(AGENTPROXY_MCP_DEFAULT_SCOPES.length, 7);
  assert.ok(AGENTPROXY_MCP_DEFAULT_SCOPES.every((s) => s.startsWith("read:")));
});

test("createAgentProxyMCPEntry defaults to tsx runtime", () => {
  const entry = createAgentProxyMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_agentproxy",
  });
  assert.equal(entry.command, "npx");
  assert.deepEqual(entry.args, ["tsx", "/path/to/server.ts"]);
  assert.equal(entry.env.AGENTPROXY_API_KEY, "sk_agentproxy");
  assert.ok(!("AGENTPROXY_MCP_ENFORCE_SCOPES" in entry.env));
  assert.ok(!("AGENTPROXY_MANAGEMENT_API_KEY" in entry.env));
});

test("createAgentProxyMCPEntry uses node runtime when specified", () => {
  const entry = createAgentProxyMCPEntry({
...
test("createAgentProxyMCPEntry sets management key and scopes when supplied", () => {
  const entry = createAgentProxyMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_agentproxy",
  ...
  assert.equal(entry.env.AGENTPROXY_MANAGEMENT_API_KEY, "sk_manage");
  assert.equal(entry.env.AGENTPROXY_MCP_ENFORCE_SCOPES, "true");
  assert.equal(entry.env.AGENTPROXY_MCP_SCOPES, "read:health,read:combos,execute:completions");
});

test("createAgentProxyMCPEntry rejects missing required fields", () => {
  assert.throws(
    () => createAgentProxyMCPEntry({ serverPath: "", apiKey: "x" }),
    /serverPath is required/
  );
  assert.throws(
    () => createAgentProxyMCPEntry({ serverPath: "/p", apiKey: "" }),
    /apiKey is required/
  );
});
...
test("createAgentProxyComboConfig builds minimal payload", () => {
  const payload = createAgentProxyComboConfig({ name: "my-combo", strategy: "priority" });
...
test("createAgentProxyComboConfig includes optional fields when supplied", () => {
...
test("AGENTPROXY_DEFAULT_OPENCODE_MODELS includes cc/ prefixed models", () => {
  const defaults = [...AGENTPROXY_DEFAULT_OPENCODE_MODELS];
...
test("AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS covers every default model id", () => {
  for (const id of AGENTPROXY_DEFAULT_OPENCODE_MODELS) {
    const ctx = AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.ok(
...
test("createAgentProxyProvider emits limit.context on default model entries", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  });
...
test("createAgentProxyProvider omits limit.context for unknown model ids", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
...
test("createAgentProxyProvider reads contextLength from a live model entry for ids absent from the static map", () => {
  // #3298 regression guard: the static AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS
...
test("createAgentProxyProvider: a live model contextLength wins over the static default map", () => {
...
test("createAgentProxyProvider serialises limit.context to JSON", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  });
  ...
  for (const id of AGENTPROXY_DEFAULT_OPENCODE_MODELS) {
    const expectedContext = AGENTPROXY_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.equal(
...
test("AGENTPROXY_DEFAULT_MODEL_CAPABILITIES covers every default model id", () => {
  for (const id of AGENTPROXY_DEFAULT_OPENCODE_MODELS) {
    const caps = AGENTPROXY_DEFAULT_MODEL_CAPABILITIES[id];
...
test("createAgentProxyProvider emits default capability flags inline with the model entry", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
  });
...
test("createAgentProxyProvider modelCapabilities overrides defaults and merges per id", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
    modelCapabilities: {
...
test("createAgentProxyProvider applies capability overrides to non-default model ids", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
    models: ["custom-model"],
...
test("createAgentProxyProvider modelLabels still works when modelCapabilities omits label", () => {
  const provider = createAgentProxyProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_agentproxy",
...
test("createAgentProxyAgentBlock builds provider-prefixed entries per role", () => {
  const block = createAgentProxyAgentBlock({
    roles: {
  ...
  assert.equal(block.build.model, "agentproxy/claude-sonnet-4-5-thinking");
  assert.equal(block.build.temperature, 0.2);
  assert.equal(block.plan.model, "agentproxy/claude-opus-4-5-thinking");
  assert.equal(block.plan.top_p, 0.95);
  assert.equal(block.review.model, "agentproxy/gemini-3-flash");
  assert.equal(block.review.temperature, 0.0);
});

test("createAgentProxyAgentBlock omits optional fields when not supplied", () => {
  const block = createAgentProxyAgentBlock({
...
test("createAgentProxyAgentBlock skips roles with empty modelId", () => {
  const block = createAgentProxyAgentBlock({
    roles: {
...
test("createAgentProxyAgentBlock emits tools as Record<string, boolean> per OC schema", () => {
  const block = createAgentProxyAgentBlock({
    roles: {
      build: {
...
test("createAgentProxyAgentBlock filters invalid tool entries and omits empty maps", () => {
  const block = createAgentProxyAgentBlock({
    roles: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
test("createAgentProxyModesBlock builds provider-prefixed mode entries", () => {
  const block = createAgentProxyModesBlock({
    modes: {
...
test("createAgentProxyModesBlock skips modes with empty modelId", () => {
  const block = createAgentProxyModesBlock({
    modes: {
      build: { modelId: "claude-sonnet-4-5-thinking" },
      plan: { modelId: "" },
...
test("createAgentProxyModesBlock honours numeric overrides limited to OC schema", () => {
  const block = createAgentProxyModesBlock({
    modes: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
// #3419 — soft-deprecation in favour of @agentproxy/opencode-plugin. Guard the
...
test("package is marked deprecated in favour of @agentproxy/opencode-plugin (#3419)", () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const pkg = JSON.parse(readFileSync(join(here, "..", "package.json"), "utf8"));
  assert.match(pkg.description, /DEPRECATED/);
  assert.match(pkg.description, /@agentproxy\/opencode-plugin/);

  const readme = readFileSync(join(here, "..", "README.md"), "utf8");
  assert.match(readme, /Deprecated/i);
  assert.match(readme, /@agentproxy\/opencode-plugin/);
});
