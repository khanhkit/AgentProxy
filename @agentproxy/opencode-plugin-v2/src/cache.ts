...
import type {
  AgentProxyEnrichmentEntry,
  AgentProxyEnrichmentMap,
  AgentProxyProviderConnection,
...
export interface CatalogSnapshot {
  ...
  providers?: AgentProxyProviderConnection[];
...
interface DiskSnapshotV2 {
   ...
   */
  enrichment?: [string, AgentProxyEnrichmentEntry][];
  writtenAt: number;
}
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return join(dir, "plugins", `agentproxy-${providerId}.json`);
}
...
export async function readDiskSnapshot(
  ...
  try {
    ...
    const models = (parsed.models as AgentProxyRawModelEntry[]).filter(
      (entry) => !isStaleSnapshotModel(entry)
    );
    if (stale > 0) {
      logger?.warn(`[agentproxy-v2] dropping ${stale} stale snapshot entries with an unusable api block`);
    }
    if (models.length === 0) return undefined;
    return {
      models,
      combos: parsed.combos as AgentProxyRawCombo[],
      autoCombos: Array.isArray(parsed.autoCombos)
        ? (parsed.autoCombos as AgentProxyRawAutoCombo[])
        : [],
      providers: Array.isArray(parsed.providers)
        ? (parsed.providers as AgentProxyProviderConnection[])
        : [],
      ...
      enrichment: Array.isArray(parsed.enrichment)
        ? new Map(parsed.enrichment as [string, AgentProxyEnrichmentEntry][])
        : undefined,
