...
export const defaultAgentProxyModelsFetcher: AgentProxyModelsFetcher = async (
  ...
  if (!apiKey) throw new Error("[agentproxy-v2] apiKey required to fetch /v1/models");
  if (!baseURL) throw new Error("[agentproxy-v2] baseURL required to fetch /v1/models");
...
function warnDeprecatedPrefixesOnce(prefixes: string[]): void {
  ...
  console.warn(
    "[agentproxy-plugin] [WARN] apiFormat.anthropicPrefixes is deprecated; convert to anthropicModels (full IDs)"
  );
}
 ...
 *      is AgentProxy's responsibility at routing time.
 *   3. `tool_call` → `toolcall` (ModelV2 field name; one word).
 *   4. `attachment` maps from `capabilities.vision` per AgentProxy
 ...
 *      reasoning flag.
 *   6. `last_updated` from AgentProxy has no ModelV2 slot — dropped.
