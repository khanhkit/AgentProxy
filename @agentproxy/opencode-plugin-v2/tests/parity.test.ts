...
interface Fixture {
  models: AgentProxyRawModelEntry[];
  combos: AgentProxyRawCombo[];
}
...
describe("v1-vs-v2 catalog parity", () => {
  it("same fixture models publish the same key set modulo documented exclusions", async () => {
    ...
    const fetcher: AgentProxyModelsFetcher = async () => fixture.models;
    const combosFetcher: AgentProxyCombosFetcher = async () => fixture.combos;

    const recorded = await loadV1Parity();
    assert.equal(recorded.hookId, "opencode-agentproxy", "v1 published under its own provider id");

    const stripX = (k: string): string =>
      k.startsWith("agentproxy/") ? k.slice("agentproxy/".length) : k;
    ...
    const mAlpha = draft.models.get("agentproxy/m-alpha");
    ...
    // (`Combo Fast` -> `agentproxy/combo-fast`) and keys the colliding combo
    // by its raw id (`good-combo` -> `agentproxy/good-combo`, combo wins);
    // v2 publishes combo ids verbatim (`agentproxy/combo-fast`,
    // `agentproxy/good-combo` overwriting the raw model). v1 and v2 therefore
    ...
    // `startsWith("agentproxy/")` filter and is dropped here).
    ...
    try {
      ...
      assert.ok(draft2.models.has("agentproxy/good-combo"), "colliding combo key wins in v2");
    } finally {
    ...
    for (const combo of fixture.combos) {
      const members = (combo.models ?? [])
        ...
        .filter((m): m is AgentProxyRawModelEntry => m !== undefined);
      assert.deepEqual(
        JSON.parse(JSON.stringify(sharedMapCombo(combo, members, "agentproxy", TEST_OPTS.baseURL))),
