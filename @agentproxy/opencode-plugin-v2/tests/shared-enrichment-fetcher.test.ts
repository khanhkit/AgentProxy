...
describe("defaultAgentProxyEnrichmentFetcher", () => {
  it("merges display names and pricing from the two catalog sources", async () => {
    ...
    try {
      const map = await defaultAgentProxyEnrichmentFetcher("https://gw.example.com", "k", 1000);
  ...
  it("returns an empty map without credentials instead of throwing", async () => {
    const map = await defaultAgentProxyEnrichmentFetcher("https://gw.example.com", "", 1000);
