...
describe("defaultAgentProxyAutoCombosFetcher", () => {
  it("404 returns [] with a warn (old gateway without the endpoint stays fail-open)", async () => {
    ...
    try {
      const res = await defaultAgentProxyAutoCombosFetcher("https://gw.example.com", "k", 5000);
  ...
  it("500 throws (with warn + token-hint report) so the caller keeps last-known", async () => {
    ...
    try {
      await assert.rejects(
        defaultAgentProxyAutoCombosFetcher("https://gw.example.com", "k", 5000, undefined, (e, r) =>
          reported.push(`${e} ${r}`)
        ),
        /HTTP 500/
      );
  ...
  it("a network throw rejects instead of resolving empty", async () => {
    ...
    try {
      await assert.rejects(
        defaultAgentProxyAutoCombosFetcher("https://gw.example.com", "k", 20),
        /abort|aborted/i
      );
    } finally {
      globalThis.fetch = origFetch;
...
describe("mapAutoComboToModelV2", () => {
  ...
  it("defaults the model id to the auto variant key (auto, auto/coding)", () => {
    const def = mapAutoComboToModelV2({ id: "whatever" }, "agentproxy", "https://gw.example.com");
