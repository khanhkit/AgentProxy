...
describe("shared leveled logger (v1 parity)", () => {
  ...
  it("lines carry the namespace tag and the uppercased level", () => {
    ...
    assert.match(cap.warns[0], /\[agentproxy-plugin\] \[WARN\] hello/);
    assert.match(cap.errors[0], /\[agentproxy-plugin\] \[ERROR\] boom/);
  });
  ...
  it("child loggers append their tag after the namespace", () => {
    ...
    assert.match(cap.warns[0], /\[agentproxy-plugin\]\[v2\] \[INFO\] hello/);
  });
