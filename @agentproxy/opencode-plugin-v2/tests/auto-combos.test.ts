...
describe("catalog auto combos (v1 parity)", () => {
  it("publishes auto/* ids from the auto-combos fetcher", async () => {
    ...
    try {
      ...
      assert.ok(models.has("agentproxy/auto"), "auto entry must be published");
      assert.ok(models.has("agentproxy/auto/coding"), "auto/coding entry must be published");
      const coding = models.get("agentproxy/auto/coding");
  ...
  it("auto-combos fetch throw stays fail-open (warn, models kept)", async () => {
    ...
    try {
      ...
      assert.ok(models.has("agentproxy/m1"));
    } finally {
      restore();
    }
  ...
  it("hidden auto combos are skipped", async () => {
    ...
    try {
      ...
      assert.ok(!models.has("agentproxy/auto/offline"));
      assert.ok(models.has("agentproxy/auto/fast"));
    } finally {
      restore();
    }
  });
