...
import { defaultAgentProxyModelsFetcher } from "../src/shared/models-map.js";
import { defaultAgentProxyCombosFetcher } from "../src/shared/combos-map.js";
...
describe("shared default fetchers abort on timeout", () => {
  it("models fetcher aborts a hanging request after timeoutMs", async () => {
    ...
    try {
      await assert.rejects(
        defaultAgentProxyModelsFetcher("https://gw.example.com", "k", 20),
        /aborted/i
      );
    } finally {
  ...
  it("combos fetcher aborts a hanging request after timeoutMs", async () => {
    ...
    try {
      await assert.rejects(
        defaultAgentProxyCombosFetcher("https://gw.example.com", "k", 20),
        /aborted/i
      );
    } finally {
      globalThis.fetch = origFetch;
