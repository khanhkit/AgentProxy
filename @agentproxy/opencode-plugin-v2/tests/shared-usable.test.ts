...
import type { AgentProxyEnrichmentMap } from "../src/shared/enrich.js";
...
function enrichmentOf(...pairs: Array<[string, string]>): AgentProxyEnrichmentMap {
...
describe("defaultAgentProxyProvidersFetcher envelopes", () => {
  it("reads the { connections: [...] } envelope", async () => {
    ...
    try {
      const out = await defaultAgentProxyProvidersFetcher("https://gw.example.com/v1/", "k");
  ...
  it("reads a bare array envelope", async () => {
    ...
    try {
      const out = await defaultAgentProxyProvidersFetcher("https://gw.example.com", "k");
      assert.deepEqual(
        out.map((c) => c.provider),
        ["gemini"]
      );
  ...
  it("throws on non-2xx so the caller keeps last-known instead of unfiltering", async () => {
    ...
    try {
      await assert.rejects(
        defaultAgentProxyProvidersFetcher("https://gw.example.com", "k"),
        /HTTP 500/
      );
    } finally {
      globalThis.fetch = origFetch;
  ...
  it("throws on network failure so the caller keeps last-known", async () => {
    ...
    try {
      await assert.rejects(defaultAgentProxyProvidersFetcher("https://gw.example.com", "k"), /down/);
    } finally {
      globalThis.fetch = origFetch;
    }
  });
  ...
  it("soft-fails to [] without baseURL or apiKey", async () => {
    assert.deepEqual(await defaultAgentProxyProvidersFetcher("", "k"), []);
    assert.deepEqual(await defaultAgentProxyProvidersFetcher("https://gw.example.com", ""), []);
  });
});
