...
import { defaultAgentProxyEnrichmentFetcher } from "../src/shared/enrich.js";
...
describe("enrichment source failures are reported", () => {
  it("names each endpoint a gateway refuses, then throws so the caller keeps last-known", async () => {
    ...
    await assert.rejects(
      defaultAgentProxyEnrichmentFetcher("https://gw.example.com", "k", 1000, (endpoint, reason) =>
  ...
  it("reports a transport failure with its cause, then throws", async () => {
    ...
    const seen: string[] = [];
    await assert.rejects(
      defaultAgentProxyEnrichmentFetcher("https://gw.example.com", "k", 1000, (_e, reason) =>
        seen.push(reason)
      ),
      /enrichment (catalog source|sources) failed/
    );
  ...
  it("stays silent when every source answers", async () => {
    ...
    const seen: string[] = [];
    await defaultAgentProxyEnrichmentFetcher("https://gw.example.com", "k", 1000, (e) =>
      seen.push(e)
    );
