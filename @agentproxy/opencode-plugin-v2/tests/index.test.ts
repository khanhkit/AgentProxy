...
describe("plugin-v2 entrypoint", () => {
  it("boot line is silent by default, visible with startupDebug", async () => {
    ...
    try {
      await (plugin as unknown as { setup: (ctx: FakeCtx) => Promise<void> }).setup(
        fakeCtx({ baseURL: "https://gw.example.com", providerId: "agentproxy" }, seen)
      );
    } finally {
    ...
    try {
      await (plugin as unknown as { setup: (ctx: FakeCtx) => Promise<void> }).setup(
        fakeCtx(
          { baseURL: "https://gw.example.com", providerId: "agentproxy", startupDebug: true },
          seen2
        )
      );
    } finally {
    ...
    assert.ok(
      warns2.some((line) => line.includes("init providerId=agentproxy")),
  ...
  it("declares key plus env methods and no oauth in the integration transform", async () => {
    ...
    const ctx: FakeCtx = {
      options: { baseURL: "https://gw.example.com", providerId: "agentproxy" },
      catalog: {
        transform: (cb: (draft: unknown) => unknown) => {
          seen.push({ kind: "catalog" });
  ...
  it("lazy refresh: [m1] then [m1,m2] reloads once; identical runs never reload", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "agentproxy-lazy-"));
    ...
    try {
      ...
      // The optional tier is intentionally detached from the first publish.
      // Wait for that bounded async upgrade instead of assuming a 5 ms timer
      // is enough on every CI host; otherwise its reload can race the next
      // model refresh and make one assertion observe two independent reloads.
      for (let attempt = 0; attempt < 50 && reloads < 1; attempt += 1) {
        await sleep(2);
      }
      assert.equal(reloads, 1, "the first optional-tier upgrade reloads exactly once");
