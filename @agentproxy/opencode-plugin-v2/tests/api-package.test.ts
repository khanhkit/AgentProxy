...
describe("catalog api package (models + combos + auto-combos)", () => {
  it("every published entry carries a non-empty supported api.package", async () => {
    ...
    for (const key of ["agentproxy/gpt-x", "agentproxy/combo-a", "agentproxy/auto"]) {
  ...
  it("auto-combos follow the same anthropic apiFormat rule as models", async () => {
    ...
    assert.equal(apiPackageOf(models.get("agentproxy/anthropic/claude-x")), "@ai-sdk/anthropic");
    assert.equal(apiPackageOf(models.get("agentproxy/auto/coding")), "@ai-sdk/anthropic");
    assert.equal(apiPackageOf(models.get("agentproxy/auto/fast")), "@ai-sdk/openai-compatible");
  });
});
