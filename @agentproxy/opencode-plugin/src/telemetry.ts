/**
 * Map gateway-reported AgentProxy inference telemetry onto the JSON/SSE
 ...
 */
export type AgentProxyInferenceTelemetry = {
  costUsd?: number;
  tokensIn?: number;
  tokensOut?: number;
...
export function parseAgentProxyInferenceTelemetry(headers: Headers): AgentProxyInferenceTelemetry {
  const out: AgentProxyInferenceTelemetry = {};
...
function telemetryFromUsage(usage: Record<string, unknown>): AgentProxyInferenceTelemetry {
  const out: AgentProxyInferenceTelemetry = {};
...
function mergeTelemetry(
  base: AgentProxyInferenceTelemetry,
  extra: AgentProxyInferenceTelemetry,
): AgentProxyInferenceTelemetry {
  return {
    ...base,
...
function attachToUsage(
  ...
  telemetry: AgentProxyInferenceTelemetry,
...
export function attachAgentProxyTelemetryToPayload(
  payload: unknown,
  telemetry: AgentProxyInferenceTelemetry,
): unknown {
...
export function attachAgentProxyTelemetryToSseLine(
  line: string,
  telemetry: AgentProxyInferenceTelemetry,
): string {
  ...
  try {
    ...
    const updated = attachAgentProxyTelemetryToPayload(parsed, telemetry);
    if (updated === parsed) {
      return line;
    }
...
export async function applyAgentProxyInferenceTelemetry(response: Response): Promise<Response> {
  const telemetry = parseAgentProxyInferenceTelemetry(response.headers);
  ...
  const next = attachAgentProxyTelemetryToPayload(parsed, telemetry);
  if (next === parsed) {
    return new Response(text, {
      status: response.status,
      statusText: response.statusText,
    ...
    new TransformStream<Uint8Array, Uint8Array>({
      transform(chunk, controller) {
        ...
        for (const line of lines) {
          controller.enqueue(encoder.encode(`${attachAgentProxyTelemetryToSseLine(line, live)}\n`));
        }
      },
      flush(controller) {
        if (pending.length > 0) {
          controller.enqueue(encoder.encode(attachAgentProxyTelemetryToSseLine(pending, live)));
        }
      },
    }),
  );
