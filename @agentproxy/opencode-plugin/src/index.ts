/**
 * OpenCode plugin for the AgentProxy AI Gateway.
 *
 * Implements the official `@opencode-ai/plugin` Plugin contract (auth +
 * provider + config hooks) to drive a running AgentProxy instance from
 ...
 *                  pass-through (AgentProxy is the source of truth — no
 ...
 *
 *  1. Single-instance (default `providerId: "agentproxy"`):
 *
 *     ```json
 *     {
 ...
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": [
 *         ["@agentproxy/opencode-plugin", { "providerId": "agentproxy" }],
 *         ["@agentproxy/opencode-plugin", { "providerId": "agentproxy-preprod" }]
 *       ]
 *     }
 *     ```
 *
 ...
 *
 * Companion library: `@agentproxy/opencode-provider` (build-time config generator)
 ...
 * @see https://github.com/khanhkit/AgentProxy for the AI Gateway.
 */
...
import { applyAgentProxyInferenceTelemetry } from "./telemetry.js";
 ...
 * injects the same partial shape (see `createAgentProxyConfigHook` deps).
 */
type AgentProxyLoggerSink = {
 ...
 *                     different `providerId` ("agentproxy", "agentproxy-preprod",
 *                     "agentproxy-local"). Maps to `ProviderHook.id` and
 ...
 *  - `baseURL`        Override base URL for this AgentProxy instance. When
 ...
 *                           JSONL file at
 *                           `~/.local/share/opencode/plugins/agentproxy-debug-{providerId}.jsonl`.
 ...
 *                           (covers AgentProxy's canonical Anthropic aliases).
 *
 ...
 * import `AgentProxyPluginOptions` as before; only the source of truth
 * shifted from a hand-written interface to `z.infer<typeof optionsSchema>`.
 */
export type AgentProxyPluginOptions = z.infer<typeof optionsSchema>;
 ...
 */
export const AGENTPROXY_FEATURE_DEFAULTS = {
...
/** Union of the boolean feature-flag keys declared in `AGENTPROXY_FEATURE_DEFAULTS`. */
export type AgentProxyFeatureFlag = keyof typeof AGENTPROXY_FEATURE_DEFAULTS;
...
export function resolveEffectiveFeatureFlags(
  features?: AgentProxyPluginOptions["features"]
): Record<AgentProxyFeatureFlag, boolean> {
  const f: Partial<Record<AgentProxyFeatureFlag, boolean>> = features ?? {};
  const out = {} as Record<AgentProxyFeatureFlag, boolean>;
  for (const key of Object.keys(AGENTPROXY_FEATURE_DEFAULTS) as AgentProxyFeatureFlag[]) {
    const val = f[key];
    out[key] = typeof val === "boolean" ? val : AGENTPROXY_FEATURE_DEFAULTS[key];
  }
  return out;
}

export const AGENTPROXY_PROVIDER_KEY = "agentproxy" as const;
 ...
 */
export function resolveAgentProxyPluginOptions(opts?: AgentProxyPluginOptions): Required<
  Pick<
    AgentProxyPluginOptions,
    "providerId" | "displayName" | "modelCacheTtl" | "autoSyncIntervalMs"
  >
> & {
  /**
   * #6859: the UNPREFIXED provider id ("agentproxy", "agentproxy-preprod", …).
   ...
   * `provider.<id>` block). `agentproxyProviderId` MUST be used everywhere an
   * identifier reaches or represents something AgentProxy's own server parses
   * (model `id` prefix, `ModelV2.providerID`, combo catalog keys in the
   * dynamic provider hook) — AgentProxy's `parseModel()` has no alias for
   * "opencode-<x>", so a prefixed id there is unrecoverable and credential
   * lookup fails with "No credentials for opencode-<x>".
   */
  agentproxyProviderId: string;
} & Pick<AgentProxyPluginOptions, "baseURL" | "managementReadToken" | "features"> {
  const rawProviderId = opts?.providerId ?? AGENTPROXY_PROVIDER_KEY;
  const agentproxyProviderId = trimLeadingOpencodePrefix(rawProviderId);
  // OC 1.17.8+ native-adapter gate rejects providerID not in
  // {openai, anthropic, opencode*}. Silently prefix so existing
  // configs (providerId: "agentproxy") keep working.
  const providerId = rawProviderId.startsWith("opencode-")
    ? rawProviderId
    : `opencode-${rawProviderId}`;
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${AGENTPROXY_PROVIDER_KEY}`
      ? "AgentProxy"
      : `AgentProxy (${providerId})`);
  const modelCacheTtl =
...
export type ResolvedAgentProxyPluginOptions = ReturnType<typeof resolveAgentProxyPluginOptions>;

/**
 * Strip a leading "opencode-" prefix (added only for the OC native-adapter
 * gate — see `resolveAgentProxyPluginOptions`) so the returned id is safe to
 * embed in anything AgentProxy's own server parses. A user-supplied
 * `providerId: "opencode-agentproxy"` (already prefixed) resolves to the same
 * unprefixed "agentproxy" as the default, matching `providerId`'s own
 ...
 * direct factory call) into the validated `AgentProxyPluginOptions` shape.
 *
 ...
 *
 * Validation happens at plugin invocation time (inside `AgentProxyPlugin`),
 ...
 */
export function parseAgentProxyPluginOptions(opts: unknown): AgentProxyPluginOptions {
  ...
  if (!result.success) {
    const errs = result.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid @agentproxy/opencode-plugin options: ${errs}`);
  }
  return result.data;
}

/**
 * Internal coercion shim. Delegates to `parseAgentProxyPluginOptions` to keep
 ...
 */
function coercePluginOptions(opts?: PluginOptions): AgentProxyPluginOptions {
  return parseAgentProxyPluginOptions(opts);
}
 ...
 * Covers AgentProxy's canonical Anthropic aliases: `cc/`, `claude/`,
 ...
 *     the multi-instance bug in opencode-agentproxy-auth@1.2.1 which pinned
 *     `AGENTPROXY_PROVIDER_ID = "agentproxy"` at module scope).
 *   - `methods[0]` is the `api` flavor (no OAuth flow; AgentProxy issues bearer
 ...
 */
export function createAgentProxyAuthHook(opts?: AgentProxyPluginOptions): AuthHook {
  const { providerId, displayName, baseURL, features } = resolveAgentProxyPluginOptions(opts);
        ...
        prompts: [
          {
            type: "text",
            key: "apiKey",
            message: `AgentProxy API key (${providerId})`,
          },
        ],
      },
    ],
    loader: async (getAuth, _provider) => {
      ...
      if (
        ...
        if (wantFetchInterceptor) {
          composedFetch = createAgentProxyFetchInterceptor({
            apiKey,
 ...
 */
export function invalidateAgentProxyFetchCache(
  cache: AgentProxyFetchCache,
  baseURL?: string
): number {
  if (!baseURL) {
    const n = cache.size;
 ...
 */
export async function resolveAgentProxyRuntimeAuth(
  resolved: ResolvedAgentProxyPluginOptions,
  readAuthJson?: AgentProxyReadAuthJson
  ...
  if (resolved.agentproxyProviderId && !lookupKeys.includes(resolved.agentproxyProviderId)) {
    lookupKeys.push(resolved.agentproxyProviderId);
  }
...
/**
 * Force-refresh AgentProxy catalog: clear memory + disk cache, re-fetch /v1/models
 * (and optional management endpoints), and repopulate the shared cache.
 * OpenCode equivalent of Pi `/omni sync`.
 */
export async function forceSyncAgentProxyModels(args: {
  resolved: ResolvedAgentProxyPluginOptions;
  cache: AgentProxyFetchCache;
  readAuthJson?: AgentProxyReadAuthJson;
  fetcher?: AgentProxyModelsFetcher;
  combosFetcher?: AgentProxyCombosFetcher;
  autoCombosFetcher?: AgentProxyAutoCombosFetcher;
  enrichmentFetcher?: AgentProxyEnrichmentFetcher;
  compressionMetaFetcher?: AgentProxyCompressionMetaFetcher;
  providersFetcher?: AgentProxyProvidersFetcher;
  now?: () => number;
  logger?: _Logger;
}): Promise<{
  ok: boolean;
  ...
  const fetcher = args.fetcher ?? defaultAgentProxyModelsFetcher;
  const combosFetcher = args.combosFetcher ?? defaultAgentProxyCombosFetcher;
  const autoCombosFetcher = args.autoCombosFetcher ?? defaultAgentProxyAutoCombosFetcher;
  const enrichmentFetcher = args.enrichmentFetcher ?? defaultAgentProxyEnrichmentFetcher;
  const compressionMetaFetcher =
    args.compressionMetaFetcher ?? defaultAgentProxyCompressionMetaFetcher;
  const providersFetcher = args.providersFetcher ?? defaultAgentProxyProvidersFetcher;
  const logger =
    args.logger ??
    createLogger(
  ...
  const auth = await resolveAgentProxyRuntimeAuth(
    resolved,
  ...
  if (!auth) {
    return {
      ...
      count: 0,
      combos: 0,
      provider: resolved.agentproxyProviderId,
      clearedMemory: 0,
      clearedDisk: false,
      error:
        "No AgentProxy credentials/baseURL available. Run `opencode connect agentproxy` or set plugin baseURL.",
    };
  }

  const clearedMemory = invalidateAgentProxyFetchCache(cache, auth.baseURL);
  // Clear residual entries from prior baseURL history as well.
  const clearedAll = invalidateAgentProxyFetchCache(cache);
  let clearedDisk = false;
  if (wantDiskCache) {
    clearedDisk = await clearDiskSnapshot(resolved.providerId);
    if (resolved.agentproxyProviderId !== resolved.providerId) {
      clearedDisk = (await clearDiskSnapshot(resolved.agentproxyProviderId)) || clearedDisk;
    }
  }
  ...
  try {
    ...
    let rawEnrichment: AgentProxyEnrichmentMap = new Map();
    if (wantEnrichment) {
      try {
    ...
    let rawCompressionCombos: AgentProxyCompressionCombo[] = [];
...
export function createAgentProxySyncModelsTool(args: {
  resolved: ResolvedAgentProxyPluginOptions;
  cache: AgentProxyFetchCache;
  logger?: _Logger;
  ...
  return tool({
    description:
      "Force-refresh the AgentProxy model catalog (OpenCode equivalent of Pi `/omni sync`). " +
    ...
    async execute(toolArgs) {
      const result = await forceSyncAgentProxyModels({ resolved, cache, logger: args.logger });
      const reason = toolArgs.reason ? ` reason=${toolArgs.reason}` : "";
      if (!result.ok) {
        return {
          title: "AgentProxy sync failed",
          output:
            `AgentProxy model sync failed for provider=${result.provider}.${reason}\n` +
            `${result.error ?? "unknown error"}`,
          metadata: result,
        };
      }
      return {
        title: `AgentProxy sync: ${result.count} models`,
        output:
          `AgentProxy models synced.` +
 ...
 */
export function startAgentProxyAutoSync(args: {
  resolved: ResolvedAgentProxyPluginOptions;
  cache: AgentProxyFetchCache;
  intervalMs?: number;
  logger?: _Logger;
  ...
  const tick = () => {
    ...
    if (inFlight) return;
    inFlight = (async () => {
      const result = await forceSyncAgentProxyModels({ resolved, cache, logger });
      if (!result.ok) {
...
export const AgentProxyPlugin: Plugin = async (_input, options) => {
  const resolved = resolveAgentProxyPluginOptions(coercePluginOptions(options));
  ...
  // Each `AgentProxyPlugin(...)` invocation gets its OWN cache via closure,
  // so prod + preprod side-by-side instances do NOT collide.
  const sharedCache: AgentProxyFetchCache = new Map();
  ...
  startAgentProxyAutoSync({ resolved, cache: sharedCache, logger });

  const syncTool = createAgentProxySyncModelsTool({ resolved, cache: sharedCache, logger });
  const bareProviderId = resolved.agentproxyProviderId;
  ...
  const baseConfigHook = createAgentProxyConfigHook(resolved, {
    cache: sharedCache,
    logger,
  ...
  const configWithSyncCommand = async (input: Config) => {
    ...
    if (!cfg.command["omni-sync"]) {
      cfg.command["omni-sync"] = {
        description: "Force-refresh AgentProxy model catalog (like Pi /omni sync)",
        template:
          `Force-refresh the AgentProxy model catalog now using the agentproxy_sync_models tool ` +
    ...
    if (!cfg.command["omni-autosync"]) {
      cfg.command["omni-autosync"] = {
        description: "Show AgentProxy auto-sync / cache status",
        template:
          `Report AgentProxy discovery status for provider ${bareProviderId}: ` +
          `autoSyncIntervalMs=${resolved.autoSyncIntervalMs}, modelCacheTtl=${resolved.modelCacheTtl}. ` +
          `If the user asked to refresh now, call agentproxy_sync_models.`,
      };
    }
  };

  return {
    auth: createAgentProxyAuthHook(resolved),
    provider: createAgentProxyProviderHook(resolved, { cache: sharedCache, logger }),
    config: configWithSyncCommand,
    tool: {
      agentproxy_sync_models: syncTool,
    },
  };
};
 ...
 * constants (AGENTPROXY_PROVIDER_KEY, DEFAULT_MODEL_CACHE_TTL_MS) + types +
 ...
 */
const AgentProxyV1Plugin = {
  id: "@agentproxy/opencode-plugin",
  server: AgentProxyPlugin,
};

export default AgentProxyV1Plugin;
...
/**
 * Raw shape of a `/v1/models` entry from AgentProxy. Captured verbatim from
 ...
 */
export interface AgentProxyRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * AgentProxy instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@agentproxy/opencode-provider`'s
 ...
 */
export type AgentProxyModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AgentProxyRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope AgentProxy
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future AgentProxy build trims the wrapper). Anything that
 * isn't an object with a string `id` is filtered out silently.
 */
export const defaultAgentProxyModelsFetcher: AgentProxyModelsFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 30_000
) => {
  if (!apiKey) throw new Error("@agentproxy/opencode-plugin: apiKey required to fetch /v1/models");
  if (!baseURL) throw new Error("@agentproxy/opencode-plugin: baseURL required to fetch /v1/models");
  ...
  try {
    ...
    if (!res.ok) {
      const err = new Error(
        `@agentproxy/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: AgentProxyRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as AgentProxyRawModelEntry);
      }
    }
    return out;
  } finally {
 ...
 *   5. `thinking` from AgentProxy has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from AgentProxy has no ModelV2 slot — dropped (the
 ...
 *   7. `temperature: true` per AgentProxy convention (OpenAI-compat mode
 ...
 *      Unknown strings (future AgentProxy additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — AgentProxy doesn't tier models alpha/beta on
...
export function mapRawModelToModelV2(
  ...
  // UI offers exactly the tiers AgentProxy vouches for (instead of opencode's
  ...
  return {
    ...
    // `providerId` so credentials resolve as `(agentproxy, model)`.
     ...
     * the caller (`createAgentProxyProviderHook`) overlays
...
/**
 * Raw shape of a single combo entry as returned by AgentProxy's `/api/combos`.
 *
 * Schema established via a live probe against
 * an AgentProxy `/api/combos` endpoint with a management-scoped key
 ...
 * bare array envelope so the plugin keeps working if a future AgentProxy
 * build trims the wrapper (mirrors the same pattern in the sibling
 * `@agentproxy/opencode-provider#listCombos`).
 *
...
export const defaultAgentProxyCombosFetcher: AgentProxyCombosFetcher = async (
  ...
  if (!apiKey) throw new Error("@agentproxy/opencode-plugin: apiKey required to fetch /api/combos");
  if (!baseURL)
    throw new Error("@agentproxy/opencode-plugin: baseURL required to fetch /api/combos");
...
/**
 * Raw shape of an auto combo entry as returned by AgentProxy's
 ...
 */
export interface AgentProxyRawAutoCombo {
  ...
  /** MAX of candidates' context windows, served by newer AgentProxy builds.
 ...
 */
export type AgentProxyAutoCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number,
  logger?: AgentProxyLoggerSink
) => Promise<AgentProxyRawAutoCombo[]>;
 ...
 */
export const defaultAgentProxyAutoCombosFetcher: AgentProxyAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000,
  logger?: AgentProxyLoggerSink
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ? ((body as { combos: unknown[] }).combos as unknown[])
        : [];
    const out: AgentProxyRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as AgentProxyRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
 ...
 * AgentProxy builds). MUST be positive: OpenCode's overflow guard treats
...
export function mapAutoComboToStaticEntry(
  autoCombo: AgentProxyRawAutoCombo
): AgentProxyStaticModelEntry {
...
/**
 * Per-model enrichment overlay derived from AgentProxy's
 ...
 */
export interface AgentProxyEnrichmentEntry {
   ...
   * Populated by `defaultAgentProxyEnrichmentFetcher` from
...
export type AgentProxyEnrichmentMap = Map<string, AgentProxyEnrichmentEntry>;

export type AgentProxyEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
 ...
 * other.
 */
export const defaultAgentProxyEnrichmentFetcher: AgentProxyEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const out: AgentProxyEnrichmentMap = new Map();
    ...
    if (res.ok) {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          // AgentProxy hasn't curated a label for this slot.
          ...
          for (const m of models) {
            ...
            const entry: AgentProxyEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<AgentProxyEnrichmentEntry["pricing"]> = {};
            // AgentProxy `/api/pricing` keys:
            //   input         → cost.input
            //   output        → cost.output
...
async function writeStartupDiagnostics(params: {
  ...
  autoCombos: AgentProxyRawAutoCombo[];
  features?: AgentProxyPluginOptions["features"];
}): Promise<void> {
  const {
    providerId,
  ...
  lines.push(
    `features(effective): ` +
      (Object.keys(effectiveFlags) as AgentProxyFeatureFlag[])
  ...
  try {
    ...
    const diagPath = path.join(diagDir, "plugins", "agentproxy-startup-diagnostics.log");
    let existing = "";
    try {
...
// Re-export from naming.ts — thin wrapper preserving AgentProxyEnrichmentEntry signature
export function shortProviderLabel(
  enrichment: AgentProxyEnrichmentEntry | undefined
): string | undefined {
 ...
 *
 * AgentProxy's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: AgentProxyEnrichmentMap | undefined
): Map<string, string> {
...
): AgentProxyEnrichmentEntry | undefined {
...
export function canonicalDedupSet(
  rawModels: ReadonlyArray<AgentProxyRawModelEntry>,
  canonicalToAlias: Map<string, string>
): Set<string> {
 ...
 *
 * Real example: AgentProxy's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `AgentProxyEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: AgentProxyEnrichmentMap | undefined
): Map<string, AgentProxyEnrichmentEntry> {
  const out = new Map<string, AgentProxyEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: AgentProxyEnrichmentEntry | undefined,
  aliasIndex: Map<string, AgentProxyEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): AgentProxyEnrichmentEntry | undefined {
  if (direct) {
...
export interface AgentProxyCompressionStep {
...
export interface AgentProxyCompressionCombo {
  id: string;
  name?: string;
  description?: string;
  pipeline: AgentProxyCompressionStep[];
  isDefault?: boolean;
}

export type AgentProxyCompressionMetaFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AgentProxyCompressionCombo[]>;
 ...
 */
export const defaultAgentProxyCompressionMetaFetcher: AgentProxyCompressionMetaFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: AgentProxyCompressionCombo[] = [];
  ...
  try {
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { combos?: unknown[] })?.combos)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: AgentProxyCompressionCombo[] = [];
    for (const raw of list) {
      ...
      const steps: AgentProxyCompressionStep[] = [];
      for (const step of pipeline) {
        ...
        const entry: AgentProxyCompressionStep = { engine };
        if (typeof intensity === "string" && intensity.length > 0) {
          entry.intensity = intensity;
        }
        steps.push(entry);
      }
      const combo: AgentProxyCompressionCombo = { id, pipeline: steps };
 ...
 * AgentProxy knows but the plugin doesn't.
 *
 ...
 */
export function formatCompressionPipeline(pipeline: AgentProxyCompressionStep[]): string {
...
// `features.usableOnly` filter. Returns the operator's installed AgentProxy
...
export interface AgentProxyProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type AgentProxyProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AgentProxyProviderConnection[]>;
 ...
 */
export const defaultAgentProxyProvidersFetcher: AgentProxyProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: AgentProxyProviderConnection[] = [];
  if (!baseURL || !apiKey) return empty;
  const root = baseURL.replace(/\/v1\/?$/, "").replace(/\/$/, "");
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ? (body as { data: unknown[] }).data
          : [];
    const out: AgentProxyProviderConnection[] = [];
    for (const raw of list) {
      if (!raw || typeof raw !== "object") continue;
 ...
 * (`claude/sonnet-4`) depending on the AgentProxy deployment's `/v1/models`
 * surface shape.
 *
...
export function usableProviderAliasSet(
  connections: AgentProxyProviderConnection[],
  enrichment: AgentProxyEnrichmentMap | undefined
): {
  aliases: Set<string>;
  ...
  if (enrichment) {
    ...
    // populated by `defaultAgentProxyEnrichmentFetcher`. Every entry carries
...
export function isUsableCombo(
  ...
  // The provider id is folded INTO the full model string by AgentProxy's
 ...
 */
export function passesComboAllowlist(combo: AgentProxyRawCombo, visible?: ModelListFilter): boolean {
 ...
 * (e.g. `agentproxy/MASTER`, `agentproxy/MASTER-LIGHT`), guaranteeing uniqueness
 ...
 *
 * NOTE: the key MUST carry the OWNING provider prefix (`agentproxy/…`), never a
 ...
 * "Unable to determine provider", whereas `agentproxy/MASTER` resolves provider=
 * `agentproxy` and the openai-compatible adapter strips the prefix and sends the
 ...
 * OC user may register prod + preprod AgentProxy side-by-side with distinct
 ...
 *     AgentProxyPlugin instance — sharing the cache means /v1/models +
 *     /api/combos each hit the gateway exactly ONCE per TTL refresh, not
 *     twice.
 */
export interface AgentProxyFetchCacheEntry {
  rawModels: AgentProxyRawModelEntry[];
  rawCombos: AgentProxyRawCombo[];
  rawAutoCombos: AgentProxyRawAutoCombo[];
  /** Display-name + pricing overlay from /api/pricing/models. Empty Map when feature is disabled or fetch failed. */
  rawEnrichment: AgentProxyEnrichmentMap;
  /** Compression combos from /api/context/combos. Empty array when feature is disabled or fetch failed. */
  rawCompressionCombos: AgentProxyCompressionCombo[];
  /** Provider connections from /api/providers. Empty array when feature is disabled or fetch failed. */
  rawConnections: AgentProxyProviderConnection[];
  expiresAt: number;
}

export type AgentProxyFetchCache = Map<string, AgentProxyFetchCacheEntry>;
 ...
 *   - **Combo / model ID collisions: combos win.** AgentProxy treats combos
 ...
 *     `/api/combos` requires a management-scoped key and AgentProxy may
 ...
 *             outside tests today).
 */
export function createAgentProxyProviderHook(
  opts?: AgentProxyPluginOptions,
  deps: {
    fetcher?: AgentProxyModelsFetcher;
    combosFetcher?: AgentProxyCombosFetcher;
    autoCombosFetcher?: AgentProxyAutoCombosFetcher;
  ...
  const resolved = resolveAgentProxyPluginOptions(opts);
  const logger =
    deps.logger ??
    createLogger(
      resolved.features?.startupDebug ? "debug" : (resolved.features?.logLevel ?? "warn")
    );
  const fetcher = deps.fetcher ?? defaultAgentProxyModelsFetcher;
  ...
  const combosFetcher = deps.combosFetcher ?? defaultAgentProxyCombosFetcher;
  const autoCombosFetcher = deps.autoCombosFetcher ?? defaultAgentProxyAutoCombosFetcher;
  const enrichmentFetcher = deps.enrichmentFetcher ?? defaultAgentProxyEnrichmentFetcher;
  const compressionMetaFetcher =
    deps.compressionMetaFetcher ?? defaultAgentProxyCompressionMetaFetcher;
  const providersFetcher = deps.providersFetcher ?? defaultAgentProxyProvidersFetcher;
  ...
  const cache: AgentProxyFetchCache = deps.cache ?? new Map();
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      let rawCombos: AgentProxyRawCombo[];
      let rawAutoCombos: AgentProxyRawAutoCombo[];
      let rawEnrichment: AgentProxyEnrichmentMap;
      let rawCompressionCombos: AgentProxyCompressionCombo[];
      let rawConnections: AgentProxyProviderConnection[];
      if (cached && cached.expiresAt > t) {
        ...
        // management-scoped key and AgentProxy may not have any combos
        ...
        // may not exist yet on older AgentProxy versions).
        rawAutoCombos = [];
        if (wantAutoCombos) {
          try {
        ...
        // the dynamic pipeline fired and how much catalog AgentProxy returned.
      ...
      for (const entry of rawModels) {
        ...
        const model = mapRawModelToModelV2(entry, {
          ...
          providerId: resolved.agentproxyProviderId,
          baseURL,
        ...
        // prefixed id on `model.id` (e.g. `agentproxy/claude-primary`), so we
      ...
      // compression metadata is present). AgentProxy returns at most one
        ...
        for (const combo of pending) {
          ...
          for (const step of memberSteps) {
            ...
            if (stepKind === "combo-ref") {
              ...
              } as unknown as AgentProxyRawModelEntry);
              continue;
            }
          ...
          // Apply enrichment overlay to combos too (AgentProxy's
          ...
          const comboKey = buildComboKey(combo, usedComboKeys, resolved.agentproxyProviderId);
...
// same-origin AgentProxy inference requests only
// ────────────────────────────────────────────────────────────────────────────
 ...
 * a secret bound to both the configured AgentProxy origin and these intended
 * inference endpoints, and MUST NOT leak elsewhere.
 *
 * Ported from Alph4d0g's `opencode-agentproxy-auth@1.2.1` `createFetchInterceptor`
 ...
 *     any caller-supplied Bearer for the same AgentProxy provider.
 *
 ...
 */
export function createAgentProxyFetchInterceptor(config: {
  apiKey: string;
  baseURL: string;
}): typeof fetch {
  ...
  return async (input, init = {}) => {
    ...
    return applyAgentProxyInferenceTelemetry(await fetch(input, { ...init, headers }));
  };
}
 ...
 * validator throws on them, breaking AgentProxy → Gemini chains transparently.
 *
 * Source: behavioural reverse-engineering from Alph4d0g's
 * opencode-agentproxy-auth@1.2.1 (dist/src/plugin.js:517).
 */
 ...
 *              interceptor from `createAgentProxyFetchInterceptor`).
 */
    ...
    try {
      ...
      if (typeof initBody === "string") {
        ...
        if (!geminiStreamingWarningEmitted) {
          ...
          console.warn(
            "[agentproxy-plugin] sanitizeGemini: streaming Request body, skipping schema strip (Gemini may reject)"
          );
        }
        return inner(input, init);
      } else if (
...
// `@agentproxy/opencode-provider/src/index.ts` (`createAgentProxyProvider`,
 ...
 * `OpenCodeModelEntry` exported by `@agentproxy/opencode-provider`. Stripped
 ...
 * when AgentProxy didn't surface a value, NOT emitted as `undefined` — the
 * resulting JSON must be diffable across AgentProxy deployments without
 * `undefined` noise.
 */
/** Modalities accepted by OC's static catalog reader (see `@opencode-ai/sdk`). */
export type AgentProxyModalityKind = "text" | "audio" | "image" | "video" | "pdf";

const STATIC_MODALITY_VALUES: ReadonlySet<AgentProxyModalityKind> = new Set([
  "text",
  "audio",
  "image",
  "video",
...
function normaliseModalities(raw: unknown): AgentProxyModalityKind[] {
  if (!Array.isArray(raw)) return [];
  const out: AgentProxyModalityKind[] = [];
  const seen = new Set<string>();
  for (const v of raw) {
    if (typeof v !== "string") continue;
    const lower = v.toLowerCase() as AgentProxyModalityKind;
...
export interface AgentProxyStaticModelEntry {
   ...
   * model KEY already carries the provider prefix (e.g. `agentproxy/MASTER`),
  ...
  tool_call?: boolean;
  /**
   * Per-million-token cost. Maps from AgentProxy `/api/pricing` shape:
   ...
   * AgentProxy's `input_modalities` / `output_modalities` on `/v1/models`.
   ...
   */
  modalities?: {
    input: AgentProxyModalityKind[];
    output: AgentProxyModalityKind[];
  };
}

/**
 * Static `provider.<id>` block written to `input.provider` by the config hook.
 * Mirrors `OpenCodeProviderEntry` from `@agentproxy/opencode-provider`.
 *
 *   - `npm` is always `"@ai-sdk/openai-compatible"` — AgentProxy exposes an
 ...
 */
export interface AgentProxyStaticProviderEntry {
  npm: "@ai-sdk/openai-compatible";
  name: string;
  options: {
    baseURL: string;
  ...
  models: Record<string, AgentProxyStaticModelEntry>;
}
 ...
 *
 * Mapping rules (per the sibling `createAgentProxyProvider` output spec):
 *
...
export function buildStaticProviderEntry(
  rawModels: AgentProxyRawModelEntry[],
  rawCombos: AgentProxyRawCombo[],
  opts: ReturnType<typeof resolveAgentProxyPluginOptions>,
  baseURL: string,
  apiKey: string,
  enrichment?: AgentProxyEnrichmentMap,
  compressionCombos?: AgentProxyCompressionCombo[],
  connections?: AgentProxyProviderConnection[],
  rawAutoCombos?: AgentProxyRawAutoCombo[],
  logger?: AgentProxyLoggerSink
): AgentProxyStaticProviderEntry {
  const log = logger ?? _logger;
  const models: Record<string, AgentProxyStaticModelEntry> = {};
  ...
  // Build a name-set of every non-hidden combo from `/api/combos`. AgentProxy
  ...
  for (const raw of rawModels) {
    ...
    // model KEY (e.g. `agentproxy/claude-opus-4`) is what OC uses to recover
    // (providerID, modelID) when the user selects a model.
    const entry: AgentProxyStaticModelEntry = { name: displayName };
    ...
    // Modalities — emit when AgentProxy surfaced any. Without this field
    ...
    // AgentProxy field names to OC's static-schema field names.
    const pricing = enrichmentEntry?.pricing;
    if (pricing && (typeof pricing.input === "number" || typeof pricing.output === "number")) {
      const cost: NonNullable<AgentProxyStaticModelEntry["cost"]> = {
  ...
  const resolvedStaticCombosByName = new Map<string, AgentProxyStaticModelEntry>();
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      // provider and the KEY prefix (`agentproxy/<slug>`) is what OC parses.
      const entry: AgentProxyStaticModelEntry = { name: displayName };
      ...
      if (hasMembers) {
        ...
        const intersect = (sets: Set<AgentProxyModalityKind>[]): AgentProxyModalityKind[] => {
      ...
      // `opts.providerId` reaches AgentProxy's server doubled
      // (`opencode-agentproxy/opencode-agentproxy/<slug>`), and `parseModel()`
      // resolves credentials for the nonexistent provider `opencode-agentproxy`
      // instead of `agentproxy`. See #7976.
      const key = buildComboKey(combo, usedComboKeys, opts.agentproxyProviderId).split("/").pop()!;
      models[key] = entry;
 ...
 *
 * Exported as a dependency-injectable function on `createAgentProxyConfigHook`
...
// `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/agentproxy-<providerId>.json`.
...
interface AgentProxyDiskSnapshot {
  v: 2;
  ...
  rawEnrichment: Array<[string, AgentProxyEnrichmentEntry]>;
  rawCompressionCombos: AgentProxyCompressionCombo[];
  rawConnections: AgentProxyProviderConnection[];
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return path.join(dir, "plugins", `agentproxy-${providerId}.json`);
}
...
export type AgentProxyDiskSnapshotWriter = (
  providerId: string,
  entry: Omit<AgentProxyFetchCacheEntry, "expiresAt">,
  identityFingerprint: string
) => Promise<void>;

export type AgentProxyDiskSnapshotReader = (
  providerId: string,
  identityFingerprint: string
) => Promise<(Omit<AgentProxyFetchCacheEntry, "expiresAt"> & { writtenAt?: number }) | undefined>;
...
export const defaultDiskSnapshotWriter: AgentProxyDiskSnapshotWriter = async (
  providerId,
  entry,
  identityFingerprint
) => {
  try {
    ...
    const snapshot: AgentProxyDiskSnapshot = {
      v: 2,
      identityFingerprint,
...
export const defaultDiskSnapshotReader: AgentProxyDiskSnapshotReader = async (
  providerId,
  identityFingerprint
) => {
  try {
    ...
    const parsed = JSON.parse(body) as Partial<AgentProxyDiskSnapshot>;
    if (
      !parsed ||
      parsed.v !== 2 ||
 ...
 * Also used as the default in createAgentProxyConfigHook so that tests
 ...
 * The AgentProxyPlugin function passes the real defaultDiskSnapshotReader
 * explicitly. */
export const noopDiskSnapshotReader: AgentProxyDiskSnapshotReader = async () => undefined;
export const noopDiskSnapshotWriter: AgentProxyDiskSnapshotWriter = async () => {};
...
function debugLogPath(providerId: string): string {
  return join(debugLogDir(), `agentproxy-debug-${providerId}.jsonl`);
}

function debugStatePath(providerId: string): string {
  return join(debugLogDir(), `agentproxy-debug-${providerId}.state.json`);
}
...
export function debugLogSetEnabled(providerId: string, enabled: boolean): void {
  try {
    ...
    console.warn(`[agentproxy-plugin] debugLogSetEnabled failed: ${(err as Error).message}`);
  }
}
...
export function debugLogAppend(entry: DebugLogEntry): void {
  try {
    ...
    console.warn(`[agentproxy-plugin] debugLogAppend failed: ${(err as Error).message}`);
  }
}
...
export function debugLogClear(providerId: string): void {
  try {
    ...
    console.warn(`[agentproxy-plugin] debugLogClear failed: ${(err as Error).message}`);
  }
}
...
export type AgentProxyReadAuthJson = () => Promise<AuthJsonShape | undefined | null>;

export const defaultReadAuthJson: AgentProxyReadAuthJson = async () => {
  const dir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode");
 ...
 *   - `fetcher`          — replaces `defaultAgentProxyModelsFetcher`.
 *   - `combosFetcher`    — replaces `defaultAgentProxyCombosFetcher`.
 *   - `now`              — clock for cache TTL (default `Date.now`).
 *   - `cache`            — shared fetch-result cache (see
 *                          `AgentProxyFetchCache`). Pass the same Map the
 ...
 */
export function createAgentProxyConfigHook(
  opts?: AgentProxyPluginOptions,
  deps: {
    readAuthJson?: AgentProxyReadAuthJson;
    fetcher?: AgentProxyModelsFetcher;
    ...
    compressionMetaFetcher?: AgentProxyCompressionMetaFetcher;
    providersFetcher?: AgentProxyProvidersFetcher;
    diskSnapshotReader?: AgentProxyDiskSnapshotReader;
    diskSnapshotWriter?: AgentProxyDiskSnapshotWriter;
    now?: () => number;
    cache?: AgentProxyFetchCache;
    logger?: {
      error?: (message: string, ...args: unknown[]) => void;
  ...
  return async (input: Config) => {
    ...
    // Try both prefixed (e.g. opencode-agentproxy) and unprefixed (e.g. agentproxy)
    // keys so a user who ran `/connect agentproxy` before the auto-prefix fix
    ...
    if (cached && cached.expiresAt > t) {
      ...
      // ─────────────────────────────────────────────────────────────────────
      let warmSnapshot: Omit<AgentProxyFetchCacheEntry, "expiresAt"> | undefined;
      if (wantDiskCache) {
      ...
      const doRefresh = async (): Promise<void> => {
        ...
        let localRawModels: AgentProxyRawModelEntry[] = [];
        let localRawCombos: AgentProxyRawCombo[] = [];
        let localRawAutoCombos: AgentProxyRawAutoCombo[] = [];
        let localRawEnrichment: AgentProxyEnrichmentMap = new Map();
        let localRawCompressionCombos: AgentProxyCompressionCombo[] = [];
        let localRawConnections: AgentProxyProviderConnection[] = [];
