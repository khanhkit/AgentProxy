 ...
 *
 * Validates that two `AgentProxyPlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@agentproxy/opencode-plugin", {"providerId": "agentproxy-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@agentproxy/opencode-plugin", {"providerId": "agentproxy-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { AgentProxyPlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof AgentProxyPlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await AgentProxyPlugin(fakeInput, {
    providerId: "agentproxy-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await AgentProxyPlugin(fakeInput, {
    providerId: "agentproxy-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-agentproxy-prod");
  assert.equal(b.auth?.provider, "opencode-agentproxy-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await AgentProxyPlugin(fakeInput, { ...opts });
  const second = await AgentProxyPlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => AgentProxyPlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await AgentProxyPlugin(fakeInput, {
