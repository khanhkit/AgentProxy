...
function combo(models: AgentProxyRawCombo["models"]): AgentProxyRawCombo {
...
test("parseAgentProxyPluginOptions: visibleModels string[] → preserved", () => {
...
test("parseAgentProxyPluginOptions: hiddenModels string[] → preserved", () => {
  const r = parseAgentProxyPluginOptions({
...
test("parseAgentProxyPluginOptions: both lists together → preserved", () => {
  const r = parseAgentProxyPluginOptions({
    features: {
...
test("parseAgentProxyPluginOptions: empty string in visibleModels → rejects", () => {
  assert.throws(
    () =>
      parseAgentProxyPluginOptions({
...
test("parseAgentProxyPluginOptions: empty string in hiddenModels → rejects", () => {
  assert.throws(
    () =>
      parseAgentProxyPluginOptions({
...
test("parseAgentProxyPluginOptions: unknown features key still rejects (strict invariant)", () => {
  assert.throws(
    () =>
      parseAgentProxyPluginOptions({
...
const FAKE_RAW_MODELS: AgentProxyRawModelEntry[] = [
...
test("buildStaticProviderEntry: no allowlist → all models emitted", () => {
  const opts = resolveAgentProxyPluginOptions({ features: {} });
...
test("buildStaticProviderEntry: visibleModels filters to only listed IDs", () => {
  const opts = resolveAgentProxyPluginOptions({
