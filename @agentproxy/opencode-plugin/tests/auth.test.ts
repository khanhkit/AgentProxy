 ...
 *
 * Covers the `createAgentProxyAuthHook(opts)` factory and its loader behaviour
...
import { createAgentProxyAuthHook } from "../src/index.js";

test("createAgentProxyAuthHook: default providerId is 'agentproxy'", () => {
  const hook = createAgentProxyAuthHook();
  assert.equal(hook.provider, "opencode-agentproxy");
});

test("createAgentProxyAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createAgentProxyAuthHook({ providerId: "agentproxy-preprod" });
  assert.equal(hook.provider, "opencode-agentproxy-preprod");
});

test("createAgentProxyAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createAgentProxyAuthHook();
  ...
  assert.equal(m.label, "AgentProxy API Key");

  const custom = createAgentProxyAuthHook({ providerId: "agentproxy-preprod" });
  assert.equal(custom.methods[0].label, "AgentProxy (opencode-agentproxy-preprod) API Key");
});

test("createAgentProxyAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("agentproxy"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createAgentProxyAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(
