...
import {
  ...
  type AgentProxyCompressionMetaFetcher,
  ...
  type AgentProxyDiskSnapshotReader,
  type AgentProxyDiskSnapshotWriter,
  type AgentProxyCompressionCombo,
} from "../src/index.js";
...
const AUTO_COMBO: AgentProxyRawAutoCombo = {
  id: "auto",
  name: "Auto",
};

const COMPRESSION_COMBO: AgentProxyCompressionCombo = {
  id: "ctx-combo-1",
  name: "Context Combo",
  pipeline: "gzip",
};

const CONNECTION_CLAUDE: AgentProxyProviderConnection = {
  id: "c1",
  provider: "claude",
  isActive: true,
  testStatus: "active",
...
test("warm-startup: snapshot data used when snapshot is present", async () => {
  ...
  const fetcher = immediateFetcher<AgentProxyModelsFetcher>([MODEL_CLAUDE]);
  const combosFetcher = immediateFetcher<AgentProxyCombosFetcher>([]);
  const autoCombosFetcher = immediateFetcher<AgentProxyAutoCombosFetcher>([]);
  const enrichmentFetcher = immediateFetcher<AgentProxyEnrichmentFetcher>(new Map());
  const compressionMetaFetcher = immediateFetcher<AgentProxyCompressionMetaFetcher>([]);
  const providersFetcher = immediateFetcher<AgentProxyProvidersFetcher>([]);
  const logger = captureWarn();

  const snapshot: Omit<import("../src/index.js").AgentProxyFetchCacheEntry, "expiresAt"> = {
  ...
  const diskSnapshotReader: AgentProxyDiskSnapshotReader = async () => snapshot;
  const diskSnapshotWriter: AgentProxyDiskSnapshotWriter = async () => {};
  ...
  const hasGemini = entry.models["opencode-agentproxy/gemini-3-flash"] !== undefined;
  const hasClaude = entry.models["opencode-agentproxy/claude-sonnet-4-6"] !== undefined;
  assert.ok(
    hasGemini || hasClaude,
...
test("warm-startup: fingerprint mismatch → no warm publish, awaited fetch", async () => {
  ...
  const diskSnapshotReader: AgentProxyDiskSnapshotReader = async () => undefined;
  const diskSnapshotWriter: AgentProxyDiskSnapshotWriter = async () => {};
  ...
  assert.ok(
    entry.models["opencode-agentproxy/claude-sonnet-4-6"],
...
test("warm-startup: parallel refresh updates cache + writes snapshot", async () => {
  const fetcher = immediateFetcher<AgentProxyModelsFetcher>([MODEL_CLAUDE]);
  const combosFetcher = immediateFetcher<AgentProxyCombosFetcher>([COMBO_CLAUDE_TIER]);
  const autoCombosFetcher = immediateFetcher<AgentProxyAutoCombosFetcher>([AUTO_COMBO]);
  const enrichmentFetcher = immediateFetcher<AgentProxyEnrichmentFetcher>(
  ...
  const providersFetcher = immediateFetcher<AgentProxyProvidersFetcher>([CONNECTION_CLAUDE]);
  const logger = captureWarn();
...
test("warm-startup: failed refresh keeps the snapshot, no disk overwrite", async () => {
  const fetcher = throwingFetcher<AgentProxyModelsFetcher>();
  const combosFetcher = throwingFetcher<AgentProxyCombosFetcher>();
  const logger = captureWarn();
  ...
  assert.ok(
    entry.models["opencode-agentproxy/gemini-3-flash"],
...
test("warm-startup: all fetchers start concurrently (parallel fan-out)", async () => {
  ...
  const fetcher = instrumentedFetcher<AgentProxyModelsFetcher>([MODEL_CLAUDE]);
  const combosFetcher = instrumentedFetcher<AgentProxyCombosFetcher>([]);
  const autoCombosFetcher = instrumentedFetcher<AgentProxyAutoCombosFetcher>([]);
  const enrichmentFetcher = instrumentedFetcher<AgentProxyEnrichmentFetcher>(new Map());
  const compressionMetaFetcher = instrumentedFetcher<AgentProxyCompressionMetaFetcher>([]);
  const providersFetcher = instrumentedFetcher<AgentProxyProvidersFetcher>([]);
  const logger = captureWarn();
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", features: { enrichment: true, compressionMetadata: true, usableOnly: true } },
    {
      readAuthJson: authStub(),
      fetcher,
      combosFetcher,
...
test("warm-startup: combos reject → models-only catalog with warn", async () => {
  const fetcher = immediateFetcher<AgentProxyModelsFetcher>([MODEL_CLAUDE]);
  const combosFetcher = throwingFetcher<AgentProxyCombosFetcher>("403 Forbidden");
  const logger = captureWarn();
...
test("warm-startup: enrichment rejects → raw-id catalog with warn", async () => {
  const fetcher = immediateFetcher<AgentProxyModelsFetcher>([MODEL_CLAUDE]);
  const combosFetcher = immediateFetcher<AgentProxyCombosFetcher>([]);
  const enrichmentFetcher = throwingFetcher<AgentProxyEnrichmentFetcher>("ETIMEDOUT");
  const logger = captureWarn();
  ...
  assert.ok(entry, "provider entry published");
  assert.equal(
    entry.models["opencode-agentproxy/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
...
test("warm-startup: providers reject → usableOnly filter disabled with warn", async () => {
  const fetcher = immediateFetcher<AgentProxyModelsFetcher>([MODEL_CLAUDE]);
  const combosFetcher = immediateFetcher<AgentProxyCombosFetcher>([]);
  const providersFetcher = throwingFetcher<AgentProxyProvidersFetcher>("ETIMEDOUT");
  const logger = captureWarn();
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", features: { usableOnly: true } },
    {
      readAuthJson: authStub(),
      fetcher,
      combosFetcher,
...
test("warm-startup: concurrent hook invocations dedupe refresh", async () => {
  ...
  const fetcher: AgentProxyModelsFetcher = async () => {
    fetchCount++;
    await slowResolve;
    return [MODEL_CLAUDE];
  };
...
test("warm-startup: diskCache=false disables warm read, falls through to awaited fetch", async () => {
  ...
  const diskSnapshotReader: AgentProxyDiskSnapshotReader = async () => {
    readerCalled = true;
    return {
      rawModels: [MODEL_GEMINI],
      rawCombos: [],
