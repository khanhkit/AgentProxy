 ...
 *
 * Surfaces tested:
 *   - `parseAgentProxyPluginOptions({ features: ... })`  → schema accept/reject
 *   - `applyEnrichment(model, entry)`                   → mutation semantics
 *   - `formatCompressionPipeline(steps)`                → display formatting
 *   - `createAgentProxyProviderHook` with mocked
 *     `enrichmentFetcher` / `compressionMetaFetcher`    → overlay applied,
 *                                                         off-by-default
 *                                                         gating works.
 *   - `createAgentProxyConfigHook` with `features.mcpAutoEmit:true`
 *                                                       → emits mcp entry
 *                                                       → falls back to
 *                                                         provider apiKey
 *                                                         when mcpToken
...
import {
  ...
  defaultAgentProxyCompressionMetaFetcher,
...
test("parseAgentProxyPluginOptions: empty features object → preserved", () => {
  const r = parseAgentProxyPluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseAgentProxyPluginOptions: all boolean features set → preserved", () => {
  const r = parseAgentProxyPluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseAgentProxyPluginOptions: mcpToken string → preserved", () => {
  const r = parseAgentProxyPluginOptions({
...
test("parseAgentProxyPluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseAgentProxyPluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @agentproxy\/opencode-plugin options/
  );
});

test("parseAgentProxyPluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseAgentProxyPluginOptions({
...
test("parseAgentProxyPluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseAgentProxyPluginOptions({ features: { mcpToken: "" } }),
    /Invalid @agentproxy\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: AgentProxyRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  ...
  const hook = createAgentProxyProviderHook(
    { providerId: "agentproxy", baseURL: "https://or.example.com/v1" },
    {
  ...
  // #6859: dynamic-hook catalog keys use the unprefixed agentproxyProviderId.
  const m = out["agentproxy/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(
    out["agentproxy/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id preserved"
  );
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: AgentProxyCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["agentproxy/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-agentproxy": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-agentproxy"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-agentproxy"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-agentproxy"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-agentproxy": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-agentproxy"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createAgentProxyConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-agentproxy-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-agentproxy-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultAgentProxyEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultAgentProxyEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultAgentProxyEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultAgentProxyEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultAgentProxyCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultAgentProxyCompressionMetaFetcher("", "sk", 100);
...
test("defaultAgentProxyEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultAgentProxyEnrichmentFetcher(
...
test("defaultAgentProxyEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultAgentProxyEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultAgentProxyEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): AgentProxyEnrichmentMap {
  const map: AgentProxyEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: AgentProxyRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as AgentProxyRawModelEntry,
    { id: "claude/claude-opus-4-7" } as AgentProxyRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: AgentProxyRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as AgentProxyRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: AgentProxyRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as AgentProxyRawModelEntry,
    { id: "claude/claude-opus-4-7" } as AgentProxyRawModelEntry,
    { id: "cx/gpt-5.5" } as AgentProxyRawModelEntry,
    { id: "codex/gpt-5.5" } as AgentProxyRawModelEntry,
    { id: "pol/openai-large" } as AgentProxyRawModelEntry,
    { id: "pollinations/openai-large" } as AgentProxyRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
