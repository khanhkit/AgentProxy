...
import {
  ...
  diskSnapshotPath,
  type AgentProxyFetchCacheEntry,
} from "../src/index.js";

function makeEntry(): Omit<AgentProxyFetchCacheEntry, "expiresAt"> {
  return {
    rawModels: [],
    rawCombos: [],
...
test("defaultDiskSnapshotWriter writes an owner-only (no group/other) snapshot", async (t) => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-disk-perms-"));
