 ...
 *
 * Covers `createAgentProxyConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@agentproxy/opencode-provider`).
...
import {
  ...
  type AgentProxyReadAuthJson,
  type AgentProxyStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: AgentProxyRawModelEntry = {
...
const MODEL_GEMINI: AgentProxyRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: AgentProxyRawCombo = {
...
): AgentProxyReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: AgentProxyReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): AgentProxyReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: AgentProxyReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: AgentProxyRawModelEntry[]
): AgentProxyModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AgentProxyModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: AgentProxyRawCombo[]
): AgentProxyCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AgentProxyCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): AgentProxyModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: AgentProxyModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): AgentProxyCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: AgentProxyCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: AgentProxyEnrichmentMap): AgentProxyEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AgentProxyEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): AgentProxyEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: AgentProxyEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createAgentProxyConfigHook(
  ...
  const provider = (input as { provider: Record<string, AgentProxyStaticProviderEntry> }).provider;
  const entry = provider["opencode-agentproxy"];
  assert.ok(entry, "input.provider['opencode-agentproxy'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "AgentProxy");
...
//     prefixed (`opencode-agentproxy`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `agentproxy` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-agentproxy`.
  const readAuthJson = stubReadAuthJson({
    agentproxy: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy" }, // resolves to opencode-agentproxy internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    agentproxy: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, AgentProxyStaticProviderEntry> }).provider[
    "opencode-agentproxy"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  const manual = {
    ...
    name: "Manual AgentProxy",
    options: { baseURL: "http://manual/v1", apiKey: "manual-key" },
    models: { "manual-model": { name: "manual-model" } },
  };
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-agentproxy": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-agentproxy"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-agentproxy-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createAgentProxyConfigHook(
    { providerId: "agentproxy-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createAgentProxyConfigHook(
  ...
  const provider = (input as { provider: Record<string, AgentProxyStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-agentproxy-prod"], "prod block present");
  assert.ok(provider["opencode-agentproxy-preprod"], "preprod block present");
  assert.equal(provider["opencode-agentproxy-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-agentproxy-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-agentproxy-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(
    provider["opencode-agentproxy-preprod"].options.baseURL,
    "https://preprod.example/v1"
  );
  assert.notEqual(
    provider["opencode-agentproxy-prod"],
    provider["opencode-agentproxy-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createAgentProxyConfigHook(
    { providerId: "agentproxy", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createAgentProxyProviderHook(
    { providerId: "agentproxy", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: AgentProxyFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @agentproxy/opencode-provider", () => {
  ...
  assert.equal(block.name, "AgentProxy");
...
test("buildStaticProviderEntry: expected raw auto twin does not warn and auto combo wins", () => {
  ...
  let block: AgentProxyStaticProviderEntry;
  try {
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveAgentProxyPluginOptions({ providerId: "agentproxy" });
  const withDate: AgentProxyRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
...
test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveAgentProxyPluginOptions({ providerId: "agentproxy" });
  const TEXT_ONLY: AgentProxyRawModelEntry = {
    id: "text-only",
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: AgentProxyPlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("AgentProxyPlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await AgentProxyPlugin({} as never, { providerId: "agentproxy" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-agentproxy": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-agentproxy"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, AgentProxyEnrichmentEntry>([
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
function stubProvidersFetcher(payload: AgentProxyProviderConnection[]): AgentProxyProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: AgentProxyProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): AgentProxyProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: AgentProxyProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: AgentProxyRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: AgentProxyRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot AgentProxy hasn't curated a human label for yet. We still
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    { providerId: "agentproxy", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, AgentProxyStaticProviderEntry> }).provider[
    "opencode-agentproxy"
  ];
  assert.equal(
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, AgentProxyStaticProviderEntry> }).provider[
    "opencode-agentproxy"
  ];
  assert.equal(
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveAgentProxyPluginOptions({ providerId: "agentproxy" });
  const rawModels: AgentProxyRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: AgentProxyRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
