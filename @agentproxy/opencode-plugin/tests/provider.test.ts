 ...
 *
 * Covers `createAgentProxyProviderHook(opts, deps)`:
...
const FIXTURE: AgentProxyRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: AgentProxyRawModelEntry[]): AgentProxyModelsFetcher & {
  callCount: () => number;
...
test("createAgentProxyProviderHook: default providerId is 'agentproxy'", () => {
  const hook = createAgentProxyProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-agentproxy");
});

test("createAgentProxyProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createAgentProxyProviderHook(
    { providerId: "agentproxy-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createAgentProxyProviderHook(
    { providerId: "agentproxy-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-agentproxy-preprod");
  assert.equal(b.id, "opencode-agentproxy-local");
});
...
test("models: extracts apiKey from ctx.auth (type=api) and calls fetcher with it", async () => {
  ...
  // ("agentproxy"), not the OC-gate-prefixed hook.id ("opencode-agentproxy") —
  // that prefix must never leak into anything AgentProxy's server parses.
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createAgentProxyProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  // agentproxyProviderId ("agentproxy") — the OC-gate prefix ("opencode-")
  // must stay OC-internal (hook.id / AuthHook.provider) and never leak into
  // anything AgentProxy's own server parses for credential lookup.
  ...
  assert.equal(claude.providerID, "agentproxy");
  ...
  // cost is zeroed (AgentProxy /v1/models has no pricing)
