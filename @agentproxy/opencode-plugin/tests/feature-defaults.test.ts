 ...
 *
 * `AGENTPROXY_FEATURE_DEFAULTS` declares those defaults explicitly and
...
test("AGENTPROXY_FEATURE_DEFAULTS: declares each flag with its documented default", () => {
  for (const key of DEFAULT_ON) {
    assert.equal(AGENTPROXY_FEATURE_DEFAULTS[key], true, `${key} defaults ON`);
  }
  for (const key of DEFAULT_OFF) {
    assert.equal(AGENTPROXY_FEATURE_DEFAULTS[key], false, `${key} defaults OFF`);
  }
});
...
test("resolveEffectiveFeatureFlags: non-boolean sibling keys do not leak into flags", () => {
  ...
  assert.equal(Object.keys(flags).length, Object.keys(AGENTPROXY_FEATURE_DEFAULTS).length);
