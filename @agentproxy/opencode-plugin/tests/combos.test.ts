 ...
 *
 * Covers:
 *   - `defaultAgentProxyCombosFetcher(baseURL, apiKey, timeoutMs?)`
 ...
 *   - `createAgentProxyProviderHook(opts, deps)` extension
...
const MODEL_PRIMARY: AgentProxyRawModelEntry = {
  id: "claude-primary",
  capabilities: {
    tool_calling: true,
    reasoning: true,
...
const MODEL_SECONDARY: AgentProxyRawModelEntry = {
  id: "claude-secondary",
  capabilities: {
    tool_calling: true,
    reasoning: false,
...
const MODEL_NO_TOOLS: AgentProxyRawModelEntry = {
  id: "gemini-3-flash",
...
): AgentProxyModelsFetcher & { callCount: () => number } {
  let n = 0;
...
function failingCombosFetcher(
  err = new Error("boom")
): AgentProxyCombosFetcher & { callCount: () => number } {
  let n = 0;
...
// ────────────────────────────────────────────────────────────────────────────
// defaultAgentProxyCombosFetcher — envelope tolerance + error surfacing
// ────────────────────────────────────────────────────────────────────────────

test("defaultAgentProxyCombosFetcher: parses {combos:[…]} envelope", async () => {
  ...
  try {
    const combos = await defaultAgentProxyCombosFetcher("https://or.example.com", "sk-test");
...
test("defaultAgentProxyCombosFetcher: parses bare array envelope", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const combos = await defaultAgentProxyCombosFetcher("https://or.example.com/v1", "sk-test");
...
test("defaultAgentProxyCombosFetcher: strips trailing /v1 before /api/combos", async () => {
  const originalFetch = globalThis.fetch;
  let observedUrl = "";
  globalThis.fetch = (async (input: unknown) => {
  ...
  try {
    await defaultAgentProxyCombosFetcher("https://or.example.com/v1/", "sk-test");
...
test("defaultAgentProxyCombosFetcher: throws on non-2xx with status code in message", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
  ...
  try {
    await assert.rejects(
      async () => {
        await defaultAgentProxyCombosFetcher("https://or.example.com", "sk-bad");
      },
      (err: unknown) => {
...
test("defaultAgentProxyCombosFetcher: throws when apiKey missing", async () => {
  await assert.rejects(
    async () => defaultAgentProxyCombosFetcher("https://or.example.com", ""),
    /apiKey required/
  );
});

test("defaultAgentProxyCombosFetcher: throws when baseURL missing", async () => {
  await assert.rejects(
    async () => defaultAgentProxyCombosFetcher("", "sk-test"),
    /baseURL required/
  );
});
...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  ...
  assert.equal(m.providerID, "agentproxy-preprod");
...
test("mapComboToModelV2: explicit member temperature=false drops combo temperature=false", () => {
  const tempFalse: AgentProxyRawModelEntry = {
    id: "no-temp",
...
// ────────────────────────────────────────────────────────────────────────────
// createAgentProxyProviderHook — combos merge + collision + soft-fail + cache
// ────────────────────────────────────────────────────────────────────────────
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.ok(out["agentproxy/claude-primary"]);
  assert.ok(out["agentproxy/claude-secondary"]);
  assert.ok(out["agentproxy/gemini-3-flash"]);
  assert.ok(out["agentproxy/claude-tier"]);

  const combo = out["agentproxy/claude-tier"];
  assert.equal(combo.name, "Claude Tier");
  assert.equal(combo.providerID, "agentproxy");
...
test("models(): combo with unknown member ids degrades to all-false LCD posture", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["agentproxy/phantom-combo"]);
  // With zero resolvable members, LCD = all-false (defensive posture).
  assert.equal(out["agentproxy/phantom-combo"].capabilities.toolcall, false);
  assert.equal(out["agentproxy/phantom-combo"].capabilities.reasoning, false);
  assert.equal(out["agentproxy/phantom-combo"].limit.context, 0);
});
...
test("models(): hidden combos are excluded from the map", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["agentproxy/visible"]);
  assert.ok(!out["agentproxy/hidden"], "hidden combo must be omitted");
});
...
test("models(): combo name exactly matches raw model id → raw deleted, raw deleted, no warn", async () => {
  ...
  // the real AgentProxy payload where /v1/models pre-mirrors combos as
  // no-slash raw entries whose ids match /api/combos friendly names.
  const colliderCombo: AgentProxyRawCombo = {
    id: "uuid-collider",
  ...
  assert.ok(out["agentproxy/claude-primary"], "combo surfaces under prefixed key");
  assert.equal(out["agentproxy/claude-primary"].name, "claude-primary");
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  const combos: AgentProxyRawCombo[] = [
    {
      id: "uuid-a",
      name: "Claude",
  ...
  assert.ok(out["agentproxy/claude"], "first combo at prefixed slug");
  assert.ok(out["agentproxy/claude-uuid"], "second combo disambiguated by id prefix");
});
...
test("models(): combos cached + reused within TTL (one combo fetch per TTL window)", async () => {
  ...
  assert.ok(second["agentproxy/claude-tier"]);
});
...
test("models(): nested combo-ref context is the min of nested + raw members", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  const masterLight = out["agentproxy/master-light"];
