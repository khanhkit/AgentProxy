...
const RAW_MODELS: AgentProxyRawModelEntry[] = [
  {
    id: "openai/gpt-test",
...
test("options: managementReadToken is accepted and preserved", () => {
  const parsed = parseAgentProxyPluginOptions({ managementReadToken: MANAGEMENT_READ_TOKEN });
...
test("provider hook: management GET fetchers use managementReadToken while /v1 uses apiKey", async () => {
  ...
  const enrichmentFetcher: AgentProxyEnrichmentFetcher = async (_baseURL, token) => {
    calls.push(["pricing", token]);
    return new Map();
  };
  const compressionMetaFetcher: AgentProxyCompressionMetaFetcher = async (_baseURL, token) => {
    calls.push(["context", token]);
    return [];
  };
  const providersFetcher: AgentProxyProvidersFetcher = async (_baseURL, token) => {
...
test("config hook: managementReadToken stays out of provider inference and MCP config", async () => {
  ...
  const hook = createAgentProxyConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-agentproxy": { type: "api" as const, key: API_KEY },
      }),
      fetcher: async (_baseURL, token) => {
        calls.push(["models", token]);
        return RAW_MODELS;
  ...
  assert.equal(input.provider?.["opencode-agentproxy"]?.options?.apiKey, API_KEY);
  assert.equal(
    input.mcp?.["opencode-agentproxy"]?.headers?.Authorization,
    `Bearer ${API_KEY}`,
...
test("disk cache: snapshot written under management token A is rejected under token B", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "agentproxy-token-snapshot-"));
  ...
  try {
    ...
    const tokenAHook = createAgentProxyConfigHook(
    ...
    const tokenBHook = createAgentProxyConfigHook(
    ...
    assert.deepEqual(
      input.provider?.["opencode-agentproxy"]?.models,
      {},
