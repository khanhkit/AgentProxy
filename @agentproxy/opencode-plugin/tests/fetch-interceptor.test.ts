 ...
 *
 * Covers `createAgentProxyFetchInterceptor` (URL-prefix gating, header merge,
...
import { createAgentProxyAuthHook, createAgentProxyFetchInterceptor } from "../src/index.js";
...
test("createAgentProxyFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAgentProxyFetchInterceptor: path-prefixed baseURL scopes auth to its normalized inference paths", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const prefixedBase = "https://or.example.com/tenant-a/v1";
    const f = createAgentProxyFetchInterceptor({
      apiKey: KEY,
    ...
    const suffixingInterceptor = createAgentProxyFetchInterceptor({
      apiKey: KEY,
...
test("createAgentProxyFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createAgentProxyFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createAgentProxyFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAgentProxyFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAgentProxyFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAgentProxyFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAgentProxyFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAgentProxyFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({
      apiKey: KEY,
...
test("createAgentProxyFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAgentProxyFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createAgentProxyAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createAgentProxyAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createAgentProxyAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
